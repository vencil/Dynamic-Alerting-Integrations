...
---
<!-- 此檔為產生物，來源 agents/skills/vibe-playbook-nav/SKILL.md —— 請改那份 SSOT，再跑 `make agent-adapters`；不要直接編輯這份複本。 -->
...
1. 讀完任務分流表，找到當前任務對應列
2. 讀「必讀」欄位**逐字列出**的範圍——標「§章節」就只讀那幾節，標「全文」就讀整份（表內只有 testing-playbook 與 benchmark-playbook 兩列是全文，因為它們的坑分散在全篇）
