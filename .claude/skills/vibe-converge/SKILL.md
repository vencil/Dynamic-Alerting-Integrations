...
⚠️ oracle 是**整條鏈**的宣告，不屬於任何一輪，所以 **`round` 欄位不要寫**。寫了也不會造幽靈輪次或撐大輪數（只在報告標籤上照樣印出來）。⚠️ **但不是「完全忽略」**：寫了而**不是整數**（例如 `"round":"1"`）會產生一筆 FORMAT violation、rc=1。⚠️ 這一句的前一版寫「會造出一個幽靈輪次」——那描述的是**修掉之前**的行為，在同一顆 commit 內就過期了，由盲審抓出。
...
⚠️ **`kind=oracle-result` 是唯一能讓這條鏈被報告成「完成」的記錄。** 報告**每次**都印一行 `VERDICT: FINISHED` / `NOT FINISHED`——因為在這一行存在之前，**一輪什麼都沒找到的盲審與一條真的跑完 oracle 的鏈，輸出與 exit code 逐字相同**（實測）——那正是被刪掉的 CONVERGED 在做的代換，只是往上一層。`passed` 必須是真的 boolean，`evidence` 不得為空。
