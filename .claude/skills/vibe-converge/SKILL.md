---
name: vibe-converge
description: 多輪修正的收斂協議 —— 開工前先寫一條可跑的 oracle（沒有就不動工）、decidability gate、跨輪交接契約（只有 verified claim / open question / 已打死方向表跨輪）、面積預算、輪數上限 5 輪。⛔ 停止條件**不是**「審到零 finding」。Use when 同一個缺陷進入第 2 輪（含）以後的修正、拿到 review finding 要開始修、或出現「每修一輪就冒出新洞、審不完」的感覺。SKIP for 第一輪實作、單檔 doc-only、以及還沒開始寫 code 的設計階段（用 `vibe-brainstorm`）。
---
...
## 第 0 步之前 — 寫一條 oracle，寫不出來就不動工

**這條鏈憑什麼結束？** 開工前寫一行進帳本，兩個欄位缺一不可：

- `command` — **要跑什麼**（可複製貼上）
- `falsifier` — **哪一個 production 改動會讓它失敗**

寫不出 `falsifier` ⇒ 你手上沒有 oracle，這條鏈唯一的終點就是「reviewer 沒話說了」。那不是交付物的性質，是 reviewer 的性質——**去走第 0 步換受審主體，或者不要動工**。
...
0. **ORACLE-MISSING** — 整條鏈沒有任何一筆 `kind=oracle` ⇒ ⛔ blocking。理由見上一節。
1. **ROUND-CAP** — 輪數上限 **5**。開下一輪的條件是 **oracle 仍然沒過**，不是「reviewer 還有話說」。到達上限 ⇒ 停下來帶**兩個數字**去找 owner：這一輪幾條、其中幾條是**我上一輪修出來的**。
...
```text
{"ts":"<date -u +%FT%TZ>","round":1,"kind":"oracle","command":"<可複製貼上的指令>","falsifier":"<哪個 production 改動會讓它失敗>"}
...
- **每條規則最便宜的轉綠方式**（守衛的失敗訊息如果指名了比較便宜比較壞的修法，它就會被照做的人拆掉，所以這裡先講）：ORACLE-MISSING 可以用一行看起來合理的假 oracle 滿足；ROUND-CAP 可以用「不記錄輪次」滿足。**兩者都是說謊而不是繞過**，而帳本 append-only ⇒ 都會留痕。本協議不防這兩件事。
