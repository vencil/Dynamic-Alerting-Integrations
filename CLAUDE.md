...
- **`vibe-converge`** — 多輪修正的收斂協議（**開工前寫一條可跑的 oracle，寫不出來就不動工**／decidability gate「這題用手上的證據判得出來嗎」／跨輪只傳 verified claim + open question + 已打死方向表／面積預算／**輪數上限 5**；⛔ 停止條件**不是**「審到零 finding」 + `dev/<scope>/ROUNDS.jsonl` 帳本 + `make converge-status`；同一缺陷進入第 2 輪修正、或「每修一輪就冒出新洞」時觸發；TRK-360）
