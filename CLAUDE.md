...
⛔ 這些不是偶發，是**結構性**的。⚠️ 第 1 項取決於**本 session 以什麼身分在操作 GitHub**——看 PR 作者的 `user.type`，別假設。第 2、4 項與身分無關。第 3 項**只有 GraphQL 那半**與身分無關（通道限制），REST 寫入那半**只在 `User` 下量過、`Bot` 未知**；而且兩半的結論都取決於 session 類型（見該項末句）。
...
3. **resolve review thread 可行，兩條路都實測過（`User` 身分；`Bot` 未量）**：MCP `pull_request_read(get_review_comments)` 取 thread 的 GraphQL node id → `resolve_review_thread`；或 REST `GET /repos/{o}/{r}/pulls/{n}/ccr/review_threads` 列出、`POST .../ccr/comments/{comment_id}/resolve`（`/unresolve` 同，可逆）。⛔ **原生 GraphQL 在 Claude Code session 被通道擋掉**，回的是端點指引而非權限錯誤 ⇒ 不是身分問題，換身分重試無意義。⚠️ **上面兩條路與那個通道限制，都只在走 GitHub 代理的 session 成立；本機 CLI（直接用 `gh` 認證）正好相反**——`ccr/review_threads` 回 404，而 `gh api graphql` 的 `reviewThreads` 查詢與 `resolveReviewThread` mutation 都可用（`User`，[#2099](https://github.com/vencil/Dynamic-Alerting-Integrations/pull/2099)／[#2101](https://github.com/vencil/Dynamic-Alerting-Integrations/pull/2101) 實測）。⇒ 先看自己在哪一種 session，一條 404 或一條被擋都不代表 resolve 做不到。
4. **「為什麼 blocked」讀得到**：`GET /branches/main/protection` 回 200，吐出完整的 branch protection 設定（required checks、conversation-resolution、linear-history 等欄位都在裡面）——`User` 身分實測 2026-09-20，`Bot` 未量。⛔ 先前這裡寫的「回 403、與身分無關」不成立，差異原因未查明 ⇒ **先打那支端點，不要預設只能做差分**。讀不到時的退路才是**差分**：比對前後兩個 head 的 check 名單＋結論，相同就代表不是 CI 造成的，再往 review / thread 方向找。
...
⚠️ 編輯 PR body 只會重觸發**自己在 `on.pull_request.types` 宣告了 `edited` 的那些 workflow**，不是全部。[#1830](https://github.com/vencil/Dynamic-Alerting-Integrations/pull/1830) 實測：`commitlint` / `planning-status-sync` / `self-review-pass2` 三支重跑，**主 `CI` 沒有**（`ci.yml` 沒寫 `types:`，預設值不含 `edited`）。⇒ 別預設改 body 就是一輪完整 CI。

⛔ 而「哪些 workflow 吃 `edited`」**不能用 grep 問**：`grep -l 'edited' .github/workflows/*` 會撈到 `ci.yml`，而它的命中**全在英文註解裡**（`a matrix edited to match one side` 之類），沒有一個是 trigger。要問就解析 YAML 的 `on.pull_request.types`（解析失敗要當成量不到、列出來，不可靜默跳過）——這條本身就是本檔反覆在講的那個形狀的活例：**述詞比對的是字串，而問題問的是結構。**
