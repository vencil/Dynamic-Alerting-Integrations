...
3. **resolve review thread 走 REST 的 CCR route；原生 GraphQL 在 Claude Code session 被整個擋掉。** ⛔ 原本寫的「只有 GraphQL（REST 無端點）」與「搆不搆得到取決於身分」**兩個子句都已被實測推翻**。發 GraphQL query 回的是**通道訊息**而非權限錯誤（`GitHub GraphQL is not available from Claude Code sessions`），且該訊息直接指名替代端點 ⇒ 換個身分重試沒有意義。實測（`User` token，在 [#1844](https://github.com/vencil/Dynamic-Alerting-Integrations/pull/1844) 上把同一條 thread resolve→unresolve→resolve 翻一輪）：`GET /repos/{owner}/{repo}/pulls/{n}/ccr/review_threads` 回 **200** 並給出每條 thread 的 `resolved` / `outdated` / `path` / `comment_ids`；`POST .../ccr/comments/{comment_id}/resolve` 與 `/unresolve` 各回 **200**，翻完回查**逐欄位回到原狀**。⚠️ **`Bot` 身分下未量**——寫入端只在 `User` 下驗過，那是「量不到」不是「量了沒事」。
