...
111 auto-run + 13 manual-stage hooks，清單見 [`.pre-commit-config.yaml`](.pre-commit-config.yaml)。
...
⛔ 它們**不能**放回 `.pre-commit-config.yaml`：pre-commit 最多只餵 hook **一個** refspec，於是「同時推 `feat/x` 和 `main`」可能讓 main 對守衛隱形（哪一列被看見隨 push 形狀而變，沒有哪種分支命名是安全的；[#1852](https://github.com/vencil/Dynamic-Alerting-Integrations/issues/1852)）。
