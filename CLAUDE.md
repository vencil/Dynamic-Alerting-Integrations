...
1. **生態系擴展**：新增 Rule Pack（PostgreSQL / Kafka / RabbitMQ），接入模式標準化（每個 pack ~半天）。有具體 DB 需求時優先
2. **PR 回測 Bot**（可選）：CI 閾值變更回測 — PR 修改 threshold 時用歷史 Prometheus 數據驗證告警影響。前提：CI runner 需連到 Prometheus API（企業環境限制待評估）。目前 `baseline_discovery.py` 為手動替代
3. **收斂 review**：同 v1.5.0/v1.6.0 模式
