...
def test_waveform_compile_does_not_return_caller_error_on_a_real_pack(argv, capsys):
    """對照組：真的跑得起來時 rc 不是 2——否則上一格對一支永遠回 2 的實作也成立。"""
    argv("--check", str(_REPO_ROOT / "scripts/tools/dx/waveform_pack_template.yaml"))
    rc = waveform_compile.main()
    capsys.readouterr()
    assert isinstance(rc, int)
    assert rc != 2, "真實 pack 不該被判成『量不到』"
...
def test_check_trk_index_coverage_runs_against_the_real_repo(capsys):
    rc = check_trk_index_coverage.main(["--repo", str(_REPO_ROOT)])
    capsys.readouterr()
    assert isinstance(rc, int)
    assert rc != 2, "真實 repo 讀得到 planning SSOT，不該回『量不到』"
