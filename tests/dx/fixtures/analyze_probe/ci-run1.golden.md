...
⚠️ 相關係數說「哪一個跟著動」，最後一欄說「它最多能推動多少」。一個分量要當成因，兩欄都要成立——這是第二欄存在的**設計理由**。⛔ 它目前**沒有實測背書**：原本佐證它的那組合成資料數字，因為產生它的 harness 從未進 repo、無人能核對，已一併刪除（TRK-374 補回）。⇒ 這張表要兩欄一起讀，但不要把「兩欄一起讀就分得出形狀」當成已驗證。
