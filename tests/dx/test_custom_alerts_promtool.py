...
p99 quantile (the 5 recipe goldens under fixtures/custom_alerts_promtool/).
...
def test_goldens_present():
    ...
    assert len(_GOLDENS) == 6, f"expected 6 goldens (5 recipes + mode_routing), found {[p.name for p in _GOLDENS]}"
