...
# other two divergences has an open ticket of its own behind it:
...
#     — one tree, several enumerators — not this reader's hidden-axis
#     answer. #1589 (the exporter's `pkg/config` enumerator,
#     path-vs-basename) is closed; #1827 IS open on the hidden axis, but
#     its subject is `assemble_config_dir` assembling `.`-prefixed carriers
#     — not a Python reader scanning `.hidden.yaml`, so this disclosure
#     still has to carry itself.
