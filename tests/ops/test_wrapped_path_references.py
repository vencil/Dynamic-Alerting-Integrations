...
def test_no_reference_is_split_across_a_line_break() -> None:
    ...
    assert not offenders, (
        ...
        "⚠️ FIRST DECIDE WHICH ONE YOU HAVE. If the thing named below is NOT a "
        "reference — a directory listing, two unrelated list items, a sentence "
        "whose last word ran into a filename — then there is nothing to reflow "
        "and the report is wrong. That is a defect in THIS GUARD, not in your "
        "change: this module has no per-line exemption on purpose, so raise it "
        "against the guard, and see the FALSE POSITIVES section of this "
        "module's docstring, which names those shapes and what a harmless "
        "rewrite looks like for each — none of which FIXES anything.\n"
        "⛔ If it IS a reference, a reflow is the only fix. Going green any "
        "other way leaves it exactly as invisible as it is now.\n"
        + "\n".join(
