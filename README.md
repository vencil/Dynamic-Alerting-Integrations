...
> **企業級多租戶監控治理平台** v1.4.0 — 配置驅動閾值、租戶零 PromQL、10 個預載規則包 (MariaDB / Redis / MongoDB / Elasticsearch / Oracle / DB2 / ClickHouse / Kubernetes / Platform / Operational)、AST 遷移引擎、三層治理模型、Regex 維度閾值、排程式時間窗口、三態運營模式 (Normal / Silent / Maintenance)、HA 部署。
...
| **低導入成本** | 租戶學 PromQL 花數天、部署 clone repo 手動對齊版號 | OCI Helm chart 一行部署（`helm install oci://...`）。`da-tools` 容器封裝 10 個 CLI，`docker pull` 即用。`scaffold` 互動式產生配置 | `docker run --rm ghcr.io/vencil/da-tools:1.4.0 scaffold` |
...
| **7 種 DB 即插即用** | 每種資料庫的監控規則從零開始寫 → 重複造輪子 | 10 個 Rule Pack 涵蓋 MariaDB / Redis / MongoDB / Elasticsearch / Oracle / DB2 / ClickHouse + K8s + Platform 自監控。Projected Volume `optional: true`——不用的規則包零成本 | `da-tools scaffold --catalog` |
...
- **Projected Volume**：10 個 Rule Pack ConfigMap（含 Platform 自我監控）透過 projected volume 合併掛載至 `/etc/prometheus/rules/`，各團隊獨立維護、零 PR 衝突。
