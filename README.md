...
| 規則評估時間 | 隨租戶線性增長 | 60ms（2 或 102 租戶皆同，[Benchmark](docs/benchmarks.md#2-為什麼能-scale-架構保證-om-向量匹配)） |
...
graph TD
    ...
    subgraph PL["Platform Layer"]
        TE["threshold-exporter ×2 HA<br/>conf.d/ 階層目錄 / Dual-Hash 熱重載<br/>(ADR-017/018, v2.7.0)"]
...
| [`scripts/`](scripts/) | Shell 進入點 + `scripts/tools/{ops,dx,lint}` 下 157 個 Python 工具 | 跑工具、lint、開發者體驗 |
...
所有路徑均支援 [OCI Registry 安裝](components/threshold-exporter/README.md#6-deploy)。
...
| Profile-as-Directory-Default（v2.8.0）| Cluster 共通閾值寫 `_defaults.yaml`（cluster median）, 偏離者寫 `<id>.yaml` 只含 override；禁止 50 份 tenant.yaml 拷貝（GitOps 反模式）| [ADR-019](docs/adr/019-profile-as-directory-default.md) |
