...
> **企業級多租戶監控治理平台** v1.2.0 — 配置驅動閾值、租戶零 PromQL、10 個預載規則包 (MariaDB / Redis / MongoDB / Elasticsearch / Oracle / DB2 / ClickHouse / Kubernetes / Platform / Operational)、AST 遷移引擎、三層治理模型、Regex 維度閾值、排程式時間窗口、三態運營模式 (Normal / Silent / Maintenance)、HA 部署。
...
```bash
# 不需 clone — 直接使用
docker run --rm -it ghcr.io/vencil/da-tools:1.2.0 scaffold --tenant my-app --db mariadb,redis
```
...
helm install threshold-exporter \
  oci://ghcr.io/vencil/charts/threshold-exporter --version 1.2.0 \
...
| 價值 | 機制 | 可驗證性 |
|------|------|----------|
| **一行部署 (One-Command Deploy)** | Helm chart 發佈至 OCI registry，chart 內綁定 image 版本。基礎 chart `tenants: {}` + overlay 分離 | `helm install oci://ghcr.io/vencil/charts/threshold-exporter --version 1.2.0` |
| **可攜帶工具鏈 (Portable Toolchain)** | `da-tools` 容器封裝 9 個 CLI 工具，`docker pull` 即用——不需 clone、不需裝 Python | `docker run --rm ghcr.io/vencil/da-tools:1.2.0 --help` |
...
| **Multi-DB 生態系 (Multi-DB Ecosystem)** | 10 個 Rule Pack 涵蓋 7 種資料庫 + K8s + Platform 自監控 | `da-tools scaffold --catalog` 列出所有支援的 DB 類型 |
...
```bash
# 查看支援的 DB 類型
docker run --rm ghcr.io/vencil/da-tools:1.2.0 scaffold --catalog

# 新租戶：互動式配置產生器（支援 8 種 DB）
docker run --rm -it -v $(pwd)/output:/output ghcr.io/vencil/da-tools:1.2.0 scaffold

# 舊規則自動轉換（AST 引擎）
docker run --rm -v $(pwd):/data ghcr.io/vencil/da-tools:1.2.0 migrate /data/legacy-rules.yml

# Shadow Monitoring 驗證
docker run --rm -e PROMETHEUS_URL=http://prometheus:9090 \
  ghcr.io/vencil/da-tools:1.2.0 validate --mapping /data/prefix-mapping.yaml
```
