...
> **企業級多租戶監控治理平台** v1.8.0 — 配置驅動閾值、租戶零 PromQL、12 個預載規則包 (MariaDB / PostgreSQL / Redis / MongoDB / Elasticsearch / Oracle / DB2 / ClickHouse / Kafka / RabbitMQ / Kubernetes / Platform / Operational)、AST 遷移引擎、三層治理模型、Regex 維度閾值、排程式時間窗口、三態運營模式 (Normal / Silent / Maintenance)、安全護欄（SSRF 防護 + Schema 驗證 + Cardinality 限制）、HA 部署、企業級遷移工具。
...
**✅ 我們的方案：**
12 個獨立 Rule Pack ConfigMap，透過 Projected Volume 掛載。各團隊（DBA、SRE、K8s、Analytics）獨立維護自己的規則包。SHA-256 hash 熱重載 — 不需重啟 Prometheus。目錄模式（`conf.d/`）支援 per-tenant YAML 檔案。
...
| **低導入成本** | 租戶學 PromQL 花數天、部署 clone repo 手動對齊版號 | OCI Helm chart 一行部署（`helm install oci://...`）。`da-tools` 容器封裝 10 個 CLI，`docker pull` 即用。`scaffold` 互動式產生配置 | `docker run --rm ghcr.io/vencil/da-tools:1.8.0 scaffold` |
...
| **12 個 Rule Pack 即插即用** | 每種資料庫的監控規則從零開始寫 → 重複造輪子 | 涵蓋 9 種 DB/MQ（MariaDB / PostgreSQL / Redis / MongoDB / Elasticsearch / Oracle / DB2 / ClickHouse / Kafka / RabbitMQ）+ K8s + Platform 自監控 + Operational。Projected Volume `optional: true`——不用的規則包零成本 | `da-tools scaffold --catalog` |
...
| [規則包目錄](rule-packs/README.md) | 12 個 Rule Pack 規格、結構範本、exporter 連結 | 全體 |
...
12 個 Rule Pack 透過 Kubernetes **Projected Volume** 預載於 Prometheus 中，各自擁有獨立 ConfigMap（`optional: true`），由不同團隊獨立維護：
...
| 工具 | 用途 |
|------|------|
| `onboard_platform.py` | 既有配置反向分析 — Alertmanager / Rule file / scrape config 自動轉換 |
...
│       ├── configmap-rules-*.yaml  # 12 獨立 Rule Pack ConfigMaps (含 platform)
...
- **Projected Volume 隔離**：12 個 Rule Pack ConfigMap 各自獨立掛載（`optional: true`），各團隊獨立維護、零 PR 衝突。刪除任一 ConfigMap 不影響 Prometheus 運行。
