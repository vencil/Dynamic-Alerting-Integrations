...
> **企業級多租戶監控治理平台** v1.9.0 — 配置驅動閾值、租戶零 PromQL、12 個預載規則包、AST 遷移引擎、全鏈遷移自動化（Onboard → Scaffold → Shadow → Auto-Convergence → Cutover → Health Report）、Config Diff Preview、PR 歷史回測 Bot、三態運營模式、安全護欄（SSRF + Schema + Cardinality）、HA 部署。
...
| **低導入成本** | 租戶學 PromQL 花數天、部署 clone repo 手動對齊版號 | OCI Helm chart 一行部署（`helm install oci://...`）。`da-tools` 容器封裝 16 個 CLI，`docker pull` 即用。`scaffold` 互動式產生配置 | `docker run --rm ghcr.io/vencil/da-tools:1.9.0 scaffold` |
