...
> 租戶只寫 YAML、不碰 PromQL —— 連自訂告警都用參數化 recipe 自助產出（v2.9.0 **Custom Alerts**）。新租戶分鐘級導入，變更秒級生效。
...
| **我是主管 / 決策者，想看商業價值與風險** | [核心指標](#核心指標)（成本 / 規模 / 導入時間）· [性能基準](docs/benchmarks.md)（千租戶實證 + soak）· [供應鏈簽章](#客戶導入migration-toolkit-v280)（cosign + SBOM） |
