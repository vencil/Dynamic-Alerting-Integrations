...
> **版本**：1.10.0（獨立版號，與 threshold-exporter 脫鉤）
...
```bash
# 本地建構（見下方「本地建構」章節）
cd components/da-tools/app && ./build.sh 1.10.0

# 或從 registry 拉取（需 CI/CD 已推送）
docker pull ghcr.io/vencil/da-tools:1.10.0

# 查看說明
docker run --rm ghcr.io/vencil/da-tools:1.10.0 --help

# 查看版本
docker run --rm ghcr.io/vencil/da-tools:1.10.0 --version
```
...
`da-tools` 採用**獨立版號**，與平台版本（v1.10.0+）和 threshold-exporter 版號脫鉤：

| 元件 | 版號 | Git Tag | 說明 |
|------|------|---------|------|
| 平台文件 | v1.10.0 | `v1.10.0` | Shadow Monitoring 自動化 + AM GitOps + Blind Spot + Config Diff |
| threshold-exporter | v1.8.0 | `exporter/v1.8.0` | Go binary |
| **da-tools** | **v1.10.0** | **`tools/v1.10.0`** | **Python CLI 工具集（19 命令，新增 cutover / blind-spot / config-diff）** |
