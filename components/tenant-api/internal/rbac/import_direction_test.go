...
// `linters.default` all leave depguard silent while the import graph — which
// the code relies on (middleware.go mirrors handler's error codes BY VALUE
// because of this rule) — is left unguarded.
//
// Parity with depguard, measured on golangci-lint 2.12.2 so this test is not
// the narrower of the two:
//   - deny is a path-segment prefix: `internal/handler/federation` is rejected
//     as well as `internal/handler`.
