...
func TestTenantsLackingPermission_EmptyInput(t *testing.T) {
	...
	if got := tenantsLackingPermission(rbacMgr, nil, principalWithGroups("any"), nil, rbac.PermWrite, nil); len(got) != 0 {
		t.Errorf("nil tenant list: got %v, want empty", got)
	}
	if got := tenantsLackingPermission(rbacMgr, nil, principalWithGroups("any"), []string{}, rbac.PermWrite, nil); len(got) != 0 {
...
func TestTenantsLackingPermission_OpenModeRead(t *testing.T) {
	...
	rbacMgr := newRBACManager(t, "")
	got := tenantsLackingPermission(rbacMgr, nil, principalWithGroups("any"), []string{"db-a", "db-b"}, rbac.PermRead, nil)
	if len(got) != 0 {
...
func TestTenantsLackingPermission_OpenModeWriteRejectsAll(t *testing.T) {
	...
	rbacMgr := newRBACManager(t, "")
	got := tenantsLackingPermission(rbacMgr, nil, principalWithGroups("any"), []string{"db-a", "db-b"}, rbac.PermWrite, nil)
	if len(got) != 2 {
...
`)
	...
	read := tenantsLackingPermission(rbacMgr, nil, p, []string{"db-a", "db-b"}, rbac.PermRead, nil)
	if len(read) != 0 {
		t.Errorf("read check: got %v, want empty", read)
	}
	write := tenantsLackingPermission(rbacMgr, nil, p, []string{"db-a", "db-b"}, rbac.PermWrite, nil)
	if len(write) != 2 {
...
func TestTenantsLackingPermission_PartialAccess(t *testing.T) {
  ...
  - name: dba-team-a
    ...
    permissions: [admin]
`)
	got := tenantsLackingPermission(rbacMgr, nil, principalWithGroups("dba-team-a"), []string{"db-a", "db-b"}, rbac.PermWrite, nil)
...
func TestTenantsLackingPermission_DeduplicatesInput(t *testing.T) {
	...
	got := tenantsLackingPermission(rbacMgr, nil, principalWithGroups("any"),
		[]string{"db-a", "db-a", "db-b", "db-a"}, rbac.PermWrite, nil)
	if len(got) != 2 {
...
func TestTenantsLackingPermission_SkipsEmptyIDs(t *testing.T) {
	...
	rbacMgr := newRBACManager(t, "")
	got := tenantsLackingPermission(rbacMgr, nil, principalWithGroups("any"),
		[]string{"", "db-a", ""}, rbac.PermWrite, nil)
