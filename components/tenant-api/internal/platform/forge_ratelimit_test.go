...
func TestDetectRateLimit(t *testing.T) {
	...
	tests := []struct {
		...
		{"bare 429, no signals → still limited (429 is unambiguous)", http.StatusTooManyRequests,
			hdr(), "", true, 0},
		{"429 + HTTP-date Retry-After (unparsed) → still limited via 429", http.StatusTooManyRequests,
			hdr("Retry-After", "Wed, 21 Oct 2025 07:28:00 GMT"), "", true, 0},
		{"403 + huge Retry-After → clamped to maxRetryAfterSecs (no overflow)", http.StatusForbidden,
