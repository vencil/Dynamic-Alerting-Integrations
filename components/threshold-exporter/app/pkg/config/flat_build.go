...
func BuildFlatConfig(scan *TreeScan, in FlatBuildInput) (FlatBuild, error) {
	...
	for _, name := range scan.Keys {
		...
		if isNestedPlatformFile(name) {
			reportUnparseableNestedPlatformFile(fullPath, data, in.Obs, logger)
			continue
		}
...
func reportUnparseableNestedPlatformFile(fullPath string, data []byte, metrics ScanObserver, logger *log.Logger) {
	var probe any
...
func ReportUnparseableNestedPlatformFile(fullPath string, data []byte, metrics ScanObserver, logger *log.Logger) {
	reportUnparseableNestedPlatformFile(fullPath, data, metrics, logger)
}
func ParsePartialConfig(name, path string, data []byte, metrics ScanObserver, logger *log.Logger) (ThresholdConfig, bool) {
...
func MergePartialConfigs(configs map[string]ThresholdConfig) ThresholdConfig {
	return mergePartialConfigs(configs)
}
func MergePartialInto(merged *ThresholdConfig, partial ThresholdConfig) {
