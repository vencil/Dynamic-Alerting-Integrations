...
func newConfigMetrics() *configMetrics {
	return &configMetrics{
		...
		hierarchyDivergentTenants: prometheus.NewGauge(prometheus.GaugeOpts{
			...
			Help: "Number of tenants currently visible to the hierarchical conf.d scanner (/effective) but ABSENT from the merged config that feeds the collector (#1521). Non-zero means those tenants emit no user_threshold series and their alerts cannot fire — the known cause is a tenant file in a conf.d sub-directory, which only the recursive scanner sees (ADR-016 requires directory depth NOT to affect metric labels). State-coded: re-Set on every config commit, so it returns to 0 once the layout is fixed. Alert: > 0 for 10m = page ops; the accompanying ERROR log names the tenants and their source files.",
		}),
	}
}
