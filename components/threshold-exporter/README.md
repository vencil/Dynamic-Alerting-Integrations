...
| `da_config_parse_failure_total{file_basename}` | Counter | YAML parse 失敗次數（定位壞檔）；每個壞檔**每次掃描計一次**（#1957 起，租戶檔由 walker 計、平面層不重計） |
