...
| [`vibe-converge`](agents/skills/vibe-converge/SKILL.md) | 多輪修正的收斂協議 —— 觸發時先寫一條可跑的 oracle（寫不出來就換受審主體或停手）、decidability gate、跨輪交接契約（只有 verified claim / open question / 已打死方向表跨輪）、面積預算、輪數上限 5 輪。⛔ 停止條件**不是**「審到零 finding」。Use when 同一個缺陷進入第 2 輪（含）以後的修正、拿到 review finding 要開始修、或出現「每修一輪就冒出新洞、審不完」的感覺。SKIP for 第一輪實作、單檔 doc-only、以及還沒開始寫 code 的設計階段（用 `vibe-brainstorm`）。 |
