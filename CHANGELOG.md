...
- **Customer-facing docs codename sweep + `check_codename_leak.py` scope/pattern 擴充（v2.8.0, [issue #462](https://github.com/vencil/Dynamic-Alerting-Integrations/issues/462)）** — 對外文件殘留的 internal planning codenames（`B-1` / `B-4` / `C-8`~`C-12` / `Phase .a~.e` / `Phase B Track A/C` / `PR-1`~`PR-5` / `S#101` / `DEC-X` / `v2.X.0-final` / `v2.X.0-rc1`）對客戶徒增認知門檻 — 一律改 feature name + 版號（如 `C-12 Dangling Defaults Guard` → `Dangling Defaults Guard (v2.8.0)`、`Phase .c C-10 Batch PR Pipeline` → `Migration Batch PR Pipeline`、`v2.7.0-final` → `v2.7.0`）。Lint side 三件套：(a) PATTERNS 加 `DEC-X` / `v\d+\.\d+\.\d+-(final\|rc\d*\|alpha\|beta)` 兩條，明確不加 `Phase N digit`（合法 prose 太多）和 `R0/R1/R2`（CPU register FP）；(b) DEFAULT_SCAN_PATHS 從 T0+component README 擴到 T1（`docs/**` 排除 `internal/`）— customer-facing 全在 CI default scope；(c) `PER_FILE_ALLOWLIST` 新機制收 `docs/adr/020-planning-ssot.md`（該 ADR 主題就是 codename namespace，例子要保留）。22 個對外文件、~213 處替換、PR template 加具體 codename 列舉的 checklist 條目（不是 "no jargon" 模糊話）。Layer 2 self-healing 機制（glossary-driven gate）見 follow-up [issue #TBD] — 本 PR 是 Layer 1 catch-up，承認永遠 lag pattern 的根本不會被 regex 完全解。
