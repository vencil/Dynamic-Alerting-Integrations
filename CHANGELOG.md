...
- **CLAUDE.md 瘦身 + epic #570 收尾淨值核算（TRK-310）**：epic 過程 CLAUDE.md 從 110 行（起點）膨脹到 138（高頻地雷 always-on + 6 skill bullets + 各 pointer）；本項把既有 verbose 段下放/收 pointer——測試注入 Seam table → test-map.md pointer、Skill 優先級宣告 3 bullets → inline、環境層 4 behavior bullets → inline、起手式 blockquote + AI Agent 環境 bullets 合併——收回 **109 行（< 起點 110）**，且 5 條高頻地雷 + 6 skill 清單 + 計數短語零損（always-on 安全不變）。**淨值**：epic 淨**加了** always-on 安全規則 + 3 個新 skill + 矩陣/audit pointer，CLAUDE.md tier-1 行數反而比起點略小；加 plugin prune（marketing/productivity/design 大半，~900-1000 token/turn）→ epic 整體 tier-1 token **net-negative**。epic #570（AI Tooling Hardening）至此全數收尾。
