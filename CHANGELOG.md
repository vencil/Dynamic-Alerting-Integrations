...
- **`make api-docs` Makefile target — OpenAPI spec build pipeline for tenant-api（v2.8.0, Phase .e prep, [closes #226](https://github.com/vencil/Dynamic-Alerting-Integrations/issues/226), TECH-DEBT-021）** — closes a long-standing gap where 18 swag annotations across `components/tenant-api/internal/handler/*` (`@Router` / `@Summary` / `@Param`) were authored but never built into a spec — silent drift between annotations and reality. New target lazy-installs `github.com/swaggo/swag/cmd/swag` inside the Dev Container and runs `swag init -g cmd/server/main.go --parseInternal --parseDependency`, emitting `components/tenant-api/docs/{swagger.json,swagger.yaml,docs.go}` (13 endpoint paths covering groups / tenants / batch / diff / effective / validate / views / me / prs). Generated artefacts committed to git so CI can drift-check via `git diff --exit-code components/tenant-api/docs/` after re-generation in the `Go Tests` job. Added `@title`/`@version`/`@BasePath` general API info to `cmd/server/main.go` (was missing — required by swag CLI). **Unblocks downstream**: schemathesis contract tests (TECH-DEBT-022 candidate), portal OpenAPI client codegen, public API docs hosting. Discovered during testing audit when 18 dead annotations were grep'd but no build pipeline existed. 詳見 [#226](https://github.com/vencil/Dynamic-Alerting-Integrations/issues/226).
