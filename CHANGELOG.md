...
- **MongoDB 存活告警改為 HA-aware 三層分級（#875 / [ADR-026](docs/adr/026-node-maintenance-liveness-suppression.md)）**：修正 HA replica set 正常 failover 切換一台成員就被當 **critical 誤 page** 的客戶痛點。`MongoDBDown`（單一成員 `mongodb_up==0`）由 **critical 降為 warning** 並吃既有維護 opt-out（`user_state_filter{maintenance}`，計畫性 drain 靜音）；新增 **`MongoDBClusterDown`**（`max by(tenant)(mongodb_up)==0` → 整體實例皆掛才 critical，拓撲無關、單實例與 HA 皆正確）與 **`MongoDBNoPrimary`**（replica set 成員可連線但無 PRIMARY → critical，接住 `max(up)==0` 結構性漏掉的「2/3 掛、失 quorum 但仍有 secondary 存活」寫入中斷）。叢集存活類**不提供** config opt-out（計畫性單實例維護改用 Alertmanager Silence）。promtool 六情境驗證（failover 不誤 page／維護靜音／quorum-loss 觸發／短暫選舉去抖）。**範圍**：MongoDB 為 reference 實作（平台目前唯一已開 cluster-state collector 的引擎，`member_state` 與既有 replication-lag 同源）；MariaDB 等其他引擎同款分級**延後**——須先確認叢集拓撲（Galera 有 wsrep quorum／primary-replica 無）並開對應 cluster-state collector（#875 ⛔ 驗證步驟），否則裸 `max(up)==0` 會把 quorum-loss 誤降為 warning。
- **recipe-preview would-fire 預覽服務（#657 PR 2，先上 try-local）**：新增獨立的小型 stdlib HTTP 服務 `components/recipe-preview/`——`POST /preview` 給一條 recipe + 情境值，當場回「會不會觸發」，走平台**同一套** compiler + `promtool`（`_recipe_preview`，不另寫 eval）。**PEP（policy enforcement point）**：服務不自判租戶授權，把呼叫者身分轉發去打 tenant-api 的 `GET /tenants/{id}/access`（#876）做決策，`403`／非 `200`／連不到一律 **fail-closed 拒絕**——RBAC 留在 tenant-api 單一權威。信任邊界比照 tenant-api（須擺認證代理後 + NetworkPolicy，**只轉發兩個身分標頭**防 confused-deputy）；try-local 用 dev-bypass。§6 護欄：並發上限、每租戶滑動視窗限流、`promtool` 2.53.2 SHA-pin（判定與版本綁）+ 啟動記錄版本。17 pytest（驗證／PEP fail-closed／身分標頭隔離／限流／契約透傳 + 經真實 core 的 e2e）。接線進 try-local docker-compose（`:8082`）；首版 `threshold` 型，時間相依型回 `supported:false`，正式環境部署延後（[設計 §9](docs/design/recipe-would-fire-preview.md)）。
