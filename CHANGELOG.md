...
- **K8s 副本就緒度告警 `TenantHAReplicasDegraded`（#875 follow-up，Gemini 對抗審查）**：補上 value-based DB 存活規則（#875 的 `*Down`／`*ClusterDown`／`*NoPrimary`、#869 `TenantExporterAbsent`）結構性看不到的盲區——HA 叢集中**單一 pod「消失」**（series 缺席非 `==0`，存活副本在每條規則都把它遮住）導致 N→N-1 降級（仍服務、但失去備援）卻**零告警**。新增 K8s 層互補規則：db-* 的 HA 工作負載（desired ≥2，StatefulSet 或 Deployment）就緒/可用副本 < 期望數 → **warning**（仍服務故非 page）；`tenant` 由 namespace `label_replace`、吃維護 opt-out + metadata LOJ（比照 node-health），`for:15m` 吸收滾動更新／重排程。單實例（desired<2）刻意不納（1→0＝total-down，已由 #875 ClusterDown／TenantExporterAbsent page）。promtool 五情境（SS/Deploy 降級觸發／健康+單實例靜默／維護 opt-out／跨租戶隔離／for: 去抖）。kube_* 指標經 monitoring-components job 全量採集（無 keep-list），KSM v2.18.0。
