...
- **`check_playwright_coldstart_drift.py` lint — Tier 1 of testing-playbook §LL §11 3-tier feasibility analysis（v2.8.0, lint hardening, S#97）** — closes the loop on PR #186 amend by shipping the Tier 1 mechanical safety net it identified as RECOMMENDED. Continues the codify-while-warm cadence: S#92→S#93 (CI fail → mechanical lint), S#94→S#95→S#96 (CI fail → read-time + mechanical), **S#96→S#97 (codify-then-ship)**. **`scripts/tools/lint/check_playwright_coldstart_drift.py` (~340 LoC)**: per-test-block scan that flags `await expect(page.getByTestId('X')).toBeVisible()` assertions when X matches `*-state-(ready|success|loaded|error|fail)` AND the same `test()` block contains NO preceding input establishment. **Pattern narrowed during dogfood**: original LL §11 spec wrote `*-state-* / *-result-* / *-output-* / *-preview-*` but dogfood immediately caught false positive — `simulate-preview-tenant-id` is an INPUT testid (not state) where `preview` is part of the tool name; pattern tightened to `state-` suffix only with future extension noted. **Input establishment patterns** (any one in the block before the assertion suppresses the warning): `.fill(` / `.click(` / `.dispatchEvent(` / `.selectOption(` / `.setInputFiles(` / `.check(` / `.uncheck(` / `.press(` / `.type(` / `page.goto(<url ?key=value>)` / `page.route(` / `page.evaluate(`. **Two markers** with semantic distinction: `// playwright-coldstart: auto-fire` is **block-level** (one marker exempts the entire test from the check; intent: spec author has read component source and verified default state satisfies render gate); `// playwright-coldstart: ignore` is **per-line** with 3-line lookback for JSDoc (intent: residual edge cases). **🔑 Pre-merge intentional-break dogfood**: removed `await page.route(...)` from `simulate-preview.spec.ts` `auto-simulates on mount` block → lint immediately caught `tests\e2e\simulate-preview.spec.ts:82:23 [coldstart-drift] testid='simulate-preview-state-ready' block@L68` exit=1 → restored. **Live audit clean**: 0 violations across 20 spec files (PR #185 fix already merged). **49 unit tests** all passing across 9 test classes (drift detection × 5 state-suffix parametrized + PR #185 historical pattern; negative cases × 9 input-establishment parametrized + state-empty/loading whitelist + false-positive guard for `*-tenant-id` input testid; markers × 3-line lookback variants; multi-block independence + `test.describe()` not counted + `test.skip/only` recognized; comment safety; edge cases incl. no-trailing-newline; severity matrix; subprocess integration; live-repo audit; regression dogfood). **9th `make lint-extract` text scaffold dogfood** (after PR #154/#162/#166/#169/#170/#173 ×2/#183/#186) — even with scaffold, ~70% remains lint-specific complexity (block tracking via `_TestBlock` dataclass, two-marker semantic distinction, narrow-from-dogfood iteration). **Pre-commit hook `playwright-coldstart-drift-check`** auto-stage FATAL on findings; `validate_all.py` registry +1; tool-map auto-regen **138→139**. **testing-playbook §LL §11 Tier 1 flip**: ⏸️ deferred → 🟢 shipped, with narrowing rationale (`-result/-output/-preview` dropped) + dogfood evidence + audit baseline. Tier 2 (marker-enforced convention, ~250 LoC) and Tier 3 (component AST × spec AST reachability, ~400+ LoC) remain deferred. Cross-refs: PR #185 fix commit `3beb127` (cold-start motivating incident), PR #186 LL §11 codification + 3-tier analysis (the codify), this PR (the ship).
