...
- **Python SAST baseline — bandit + dev-rules Rule #5 code-driven enforcement（issue [#455](https://github.com/vencil/Dynamic-Alerting-Integrations/issues/455)）**：dev-rules.md Rule #5（SAST 7 條安全準則）從 reviewer-convention 升級為 code-driven。新增 `.bandit` config（excludes `tests/` `.venv/` `__pycache__/` `vendor/`；無 global skip — 每處 nosec 必帶 rationale）+ `.github/workflows/security-audit.yaml`（PR + `main` push trigger，scope 限 `scripts/tools/**` + `components/da-tools/**` Python 變更，`bandit -ll -ii` = MEDIUM 嚴重度且 MEDIUM 信心度為基線）。baseline 對 169 個 Python 檔（52,816 LOC）跑出 13 個 medium+ findings、逐個 triage：1 個 HIGH (B324 MD5 in `generate_tenant_fixture.py` fixture key derivation) 加 `usedforsecurity=False` 修正；1 個 MEDIUM (B314 XML parse in `coverage_delta.py` 對 pytest-cov 本機 Cobertura) 加 nosec rationale；11 個 MEDIUM (B310 urlopen scheme audit) 在 `_lib_prometheus.py` 等 6 個 ops 檔逐處加 nosec rationale (`# nosec B310  # operator-supplied internal Prometheus URL` 雙井號慣例避開 bandit prose-as-test-id 警告)；`run_chaos_soak.py` 補上 scheme allowlist。baseline 收斂後 0 findings、0 warnings。**Rollout**：workflow 起手 2 週 `continue-on-error: true`（warn-only soak），若無 false-positive flood 則翻硬 fail；既有 secret-scan workflow rollout 模式參照。bandit 內建覆蓋 Rule #5 中 shell（B602/B603/B605）/ yaml_load（B506）/ hardcoded password（B105/B106 部分）/ eval-exec-pickle（B102/B301/B307）；encoding / chmod 0o777 / stderr routing 三項暫保留 reviewer convention，自定 hook 為 follow-up。詳 [dev-rules.md §5](docs/internal/dev-rules.md#5-sast7-條安全-review-準則)。
