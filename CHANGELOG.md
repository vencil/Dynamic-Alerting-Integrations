...
- **`lint_jsx_babel.py` line-count guard（v2.8.0, lint, issue #152）** — codifies the lesson from PR #150 / S#67: `tenant-manager.jsx` accumulated 3 latent bugs over months because the 1671-line single file was too large for cross-checking `useState` declarations against `set*` references, hook ordering against early returns, and lifecycle paths in async branches. The guard adds a third validation pass to the existing JSX lint pipeline (alongside static patterns + Babel parse) — soft cap warns at **1500 lines**, hard cap fails at **2500 lines** under `--ci`. Soft-cap warnings stay non-fatal under default mode (so commits aren't blocked during the v2.8.0 transition) and become fatal under `--strict` (CI manual stage). Hard-cap violations are always fatal under `--ci` regardless of `--strict`, mirroring how Babel parse errors are handled. Thresholds carry a `# TODO(v2.9.0): revisit` comment — they're conservative starting points (1500 = top-decile signal observing today's 39 interactive JSX tools sit between 200-1200 lines; 2500 = ~67% headroom over today's worst offender so it doesn't insta-fail current reality but blocks the next such offender). 15 new unit tests in `tests/lint/test_lint_jsx_babel_linecount.py` pin the constants + boundary semantics (`>` vs `>=` at both caps + trailing-newline counting matches `wc -l` semantics + issue dict shape downstream `main()` depends on). The guard's actionable error message points to issue #153 (PR-2d file decomposition) so future hits know the pattern to apply.
