...
- **Phase .b — Track B: B-5 Mixed-mode 驗證（v2.8.0, route C strict-plan closure plan Track B）** — 在 v2.7.0 起一直支援但**從未系統性測試**的「flat `<root>/<id>.yaml` + hierarchical `<root>/<dir>/<id>.yaml` 並存」mixed mode 加 5 個 manager-level invariant tests + 4 個 mixed-mode benchmarks + 一份雙語客戶決策文件。零 production code 變動。
  ...
  - **新 `components/threshold-exporter/app/config_mixed_mode_bench_test.go`** — 4 mixed-mode benchmarks：`BenchmarkScanDirHierarchical_MixedMode_500flat_500hier`、`BenchmarkFullDirLoad_MixedMode_500flat_500hier`、`BenchmarkDiffAndReload_MixedMode_500flat_500hier_NoChange`、`BenchmarkFullDirLoad_MixedMode_100flat_900hier`（late-migration shape）。500+500=1000 total tenant 設計與 `BenchmarkFullDirLoad_Hierarchical_1000` 直接可比。Single-shot dev-container 量測（**directional only, NOT statistically valid**）：mixed 比 pure-hierarchical ~1.85-3.0× 慢，**觸發 planning §B-5 ≥10% degradation follow-up threshold**；nightly `bench-record.yaml` 累積 28+ 點 + `analyze_bench_history.py` 跑統計後若數字維持，**mixed mode 確實是「儘快 cutover 完」的場景，不適合長駐**
  ...
  - **不在本 bundle scope（明示 follow-up）**：(a) duplicate-tenant 跨 mode 從 WARN 升 hard error（v2.8.x hardening PR — `TestMixedMode_DuplicateAcrossModes_DetectedButNotPropagated` 鎖死當前行為，強化 PR 必須改寫該 test）；(b) mixed-mode degradation perf profile 與優化（待 nightly bench 數據累積後決定是否值得）；(c) `_metadata.{domain,region,environment}` 從目錄路徑自動推斷（v2.7.0 起的 ADR-017 advisory text，目前只有 migration tool 有，runtime 不做 — 不在 Track B scope）
