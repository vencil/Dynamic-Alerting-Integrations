...
- **da-portal platform-health 硬編色遷移到 design token + 元件拆分（ROI refactor 第五波）**：`platform-health.jsx` 的 26 處硬編 Tailwind 色階（`bg-green-500`/`bg-yellow-500`/`bg-red-500`/`text-gray-*` 等，**不隨 light/dark theme 切換**）換成既有 `--da-color-*` 語意 token（healthy→`success`、degraded/maintenance→`warning`、down→`error`、blue chrome→`accent`、中性→`fg`/`muted`/`card-*`/`tag-*`/`surface-*`），比照 `component-health.jsx`／`threshold-heatmap.jsx` 已遷移的 arbitrary-value pattern（`bg-[color:var(--da-color-*)]`）以維持視覺意圖與跨工具一致；所有用到的 token 皆已定義（過 `check_undefined_tokens.py` gate）。並依 tenant-manager／config-diff 慣例把工具拆成 `platform-health/{components,fixtures}/`（StatusDot／MetricCard／ComponentHealth／TenantOverview／RulePackDistribution／ReloadTimeline + 資料常數，只搬移不改邏輯）。順帶把工具本地 demo 常數 `PLATFORM_DATA` 改名 `PLATFORM_HEALTH_DATA`——原本若沿用舊名並自註冊 `window.__PLATFORM_DATA` 會**遮蔽平台注入的 live rule-pack 資料全域**（threshold-heatmap 等讀取者所依賴），改名徹底避開。補 19 個元件測試。
