...
- **GitOps render 對 email receiver 租戶產出 Alertmanager 自家 parser 拒收的 config（latent；#838 amtool 對抗 pass 揪出）**：`generate_alertmanager_routes.py` 的 `--apply` / `--output-configmap` 對真實 `conf.d/` 的 email 路徑有兩個缺陷——(A) `email_configs[].to` 被輸出成 YAML **list**，但 AM 的 `to` 是 **string** → `cannot unmarshal !!seq into string`；(B) email receiver 只有 `smarthost`、無 `from`，且部署用 base 無 global `smtp_*` → `no global SMTP from set`。兩者皆 latent（手寫的 `configmap-alertmanager.yaml` 無 email receiver 故驗得過），但對 live conf.d 跑 GitOps render 即壞。修法：(A) 在 `build_receiver_config` 把 list `to` coerce 成 AM 慣例的逗號分隔字串（builder 端、與 conf.d 寫法解耦）；(B) `_defaults.yaml` 的 email default 補 per-receiver `from`，並把 `from` 升為 email receiver 的**必填**欄位（Python `RECEIVER_TYPES` + Go `internal/guard/routing.go` lock-step）——如此 tenant override / profile 等任何路徑缺 `from` 時，`--validate` / da-guard 會早一步 WARN+skip，而非只靠 amtool 兜底（同 `smarthost` 已是 per-receiver 必填的假設：部署無 global `smtp_*`）。並封死根因——既有 Python 測試只把組裝後 config 當 dict 驗結構、整類「AM parser 拒收」從未覆蓋 → 新增 `amtool check-config`（`prom/alertmanager` 鏡像）回歸測試（`tests/ops/test_alertmanager_amtool_validity.py`，含 list-`to`「守住守門員」+ 缺-`from` skip negative case）＋ `validate.yaml` CI step（render → amtool 權威驗證）。
