...
- **`check_head_blob_hygiene.py` regression tests + S#74 lesson codify（v2.8.0, follow-up to PR #164）** — PR #164 fixed the `_batch_cat_blobs` Popen deadlock in flight while shipping PR-2c, but did not include tests or codify the agent-discipline lesson that surfaced. This PR closes both gaps. **18 new tests in `tests/lint/test_check_head_blob_hygiene.py`**: 4 deadlock-regression tests (small / large / huge batch + empty input — using a real on-disk git repo fixture, not mocks, since the deadlock is in the real Popen pipe interaction), 2 output-parsing tests (binary-safe content round-trip + missing-path skip), 4 `scan_blob` rule tests (NUL / EOF strict-only / TRUNC mkdocs-yml regression), 8 `_is_skippable` parametrized tests. Tests run in ~5s locally. Critical property: the deadlock-regression tests would HANG (not silently pass) if someone reverts the fix back to the write-then-read-loop pattern — the assertion budgets are explicitly tight. **Trap #57 in `windows-mcp-playbook.md` updated** from "🟡 Mitigated" to "🟢 Resolved" with the second root-cause class (Popen pipe deadlock, orthogonal to the FUSE stale-temp class) documented; identification rules for which root-cause applies to a given symptom (NTFS → 100% Popen; FUSE side + phantom → stale temp; FUSE side + clean phantom → likely Popen). **`testing-playbook.md` v2.8.0 Lessons §4 (NEW)** codifies S#74: subprocess hang requires an SLA prior + escalation policy, not patience. Three-step escalation: (a) `ps -ef | grep <suspect>` to locate the live subprocess, (b) isolate and reproduce the stuck path standalone, (c) instrument the suspect path. Plus a verify-reference extension to S#73: hook scripts authored by others are not exempt — read the critical paths (Popen / pipe / I/O) before assuming "it works".
