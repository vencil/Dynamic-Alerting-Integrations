...
- **tenant-api machine-identity 稽核加 workload-identity mismatch 判定（[#962](https://github.com/vencil/Dynamic-Alerting-Integrations/issues/962) 信任硬化，audit-only）**：承上條稽核基線，KSAResolver 驗過 SA token 後不再只記 raw workload，改依 machine-identity allowlist 分類：**synthetic** 呼叫端（如 threshold-govern 合成固定 `--identity-groups`）宣稱 expected 集合外的 group → `mismatch`（盜用 SA token 的提權訊號）；**relay** 呼叫端（如 recipe-preview 轉發使用者 X-Forwarded-*）採信轉發的 header groups → `verified`（使用者身份合法多變）；allowlist 外的 SA → `unknown_workload`。`tenant_api_identity_audit_total{result}` 新增兩條 series（`mismatch`／`unknown_workload`），mismatch／unknown 走 WARN log（可 alert）。**仍 audit-only**（分類只 label metric/log、不改授權）。allowlist 現為硬編最簡（SA name→spec、namespace-agnostic）；externalize config + namespace 精確釘定 defer 到 enforce PR。（ADR-027 machine-path；呼叫端掛 projected token = PR-1b-ii-b。）
