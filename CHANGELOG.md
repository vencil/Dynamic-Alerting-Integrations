...
- **`dev-rules-enforcement-audit.md` — 27-rule cross-reference + 3 doc-drift fixes（v2.8.0, governance, follow-up to PR #166 self-review）** — manual audit cross-referencing every rule in `dev-rules.md` against the actual pre-commit hook / CI gate that's supposed to enforce it. **Headline finding: 4 doc-drift gaps** where the rule text claims a hook that doesn't exist in the repo: (i) Rule #2 Tenant-Agnostic claims `lint_hardcode_tenant` — **NOT FOUND**, (ii) Rule #5 SAST claims "pre-commit stage 跑 7 條 SAST 規則" — **NO matching lint scripts** (no `check_encoding`/`check_shell_safe`/etc), (iii) Rule #6 推銷語言 claims `check_marketing_language` — **NOT FOUND**, (iv) Rule #1 ConfigMap heredoc — process-only by design but doc didn't explicitly say so. **Categorization** of all 27 rules (12 main + 5 style S1-S5 + 3 process P1-P3 + 7 artifact A1-A7) into ✅ Enforced (11) / 🟡 Partially (4) / 🔴 Doc-drift (4) / ⚫ Process-only (8). For each gap: documented quick-fix (5 min — annotate doc honestly) + real-fix (estimated 30-120 min — write the missing lint). This PR ships the audit doc + applies the 3 quick doc fixes (Rules #2 / #5 / #6 now annotated `⚠️ doc-drift, real lint backlogged`) so dev-rules.md no longer makes false claims. Real lints to be filed as follow-up issues. **Process rule** added to `dev-rules-enforcement-audit.md` §How-to-maintain: "if the lint claim makes it through review, the lint must exist" — captured as a checklist for new dev-rules entries to prevent future drift.
