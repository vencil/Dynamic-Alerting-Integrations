...
- **config hot-reload 失敗補上 metric + 告警（Gemini #1056 外審 disposition 3a；動共用 configwatcher，惠及全 WatchLoop 型 config manager）**：tenant-api 走週期性 WatchLoop 的 config manager（rbac／policy／tenantorg／federation-policy）在熱重載 parse 失敗時**保留 last-good 快照**——SRE 改了 `_rbac.yaml`／`_domain_policy.yaml`／`_tenant_orgs.yaml`／`_federation_policy.yaml` 若含 typo，壞 config 靜默不生效、舊 config 續行，過去唯一痕跡僅一行 WARN log（「緊急時改了卻不生效」難察）。新增 `tenant_api_config_reload_failures_total{component}` counter（configwatcher 宣告 `ReloadFailureRecorder` 介面、handler 實作並於啟動經 instance-method DI 注入各 manager，鏡射既有 scope-would-deny recorder；固定已知 component 集合＝WatchLoop 型 manager，series 自啟動即 0 保穩定形狀）＋平台告警 `TenantApiConfigReloadFailing`（`increase[15m] > 2` for 5m warning——`> 2` 同時免疫單發 mid-write blip 與 pod-restart counter-reset extrapolation 噪音，`{component}` label 隨告警帶出哪個 config 卡住）。groups／views 刻意不納入——它們走寫入後 `Reload()`、error 直接回呼叫端而非靜默 WatchLoop 路徑。pre-merge schema 驗證為 shift-left 互補（另一支 PR）。純 tenant-api Go ＋平台 rules（helm 無新 value）。細節見 [#962](https://github.com/vencil/Dynamic-Alerting-Integrations/issues/962) LD-6 P4a fast-follow。
