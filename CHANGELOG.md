...
- **`scaffold_lint.py` + `make lint-extract` + testing-playbook §v2.8.0 LL §5+§6（v2.8.0, dx + process）** — codifies the lint boilerplate that PR #154/#162/#166/#169 wrote 4 times by hand (closed PR #170 was a 5th attempt; its marketing-language-specific lint was dropped because keyword lists are inherently incomplete for natural-language nuance, but the **shared boilerplate pattern** is real and worth scaffolding). **Scaffold-before-third-instance rule** says 4 hand-rolls is past time. **Five `--kind` templates** matching common lint shapes: `ast` (PR #166 subprocess-timeout shape — Python AST walker for code patterns); `text` (line-by-line keyword/regex with fenced-block + inline-backtick suppression); `yaml` (structural validation with `yaml.safe_load` parse + `_metadata`-aware walker); `meta` (PR #169 dev-rules-enforcement shape — cross-file consistency with trigger-context discrimination); `freshness` (front-matter / age guard with date parsing). Each generated lint includes: `_compute_exit_code` pure helper for severity matrix, `Finding` dataclass with `render()` that handles paths-outside-PROJECT_ROOT (PR #166 amend caught this as a real bug — codified into template defensively), `scan_source(path, source)` per-file scanner placeholder, `_iter_target_files()` / `_resolve_target_paths()` discovery, `main(argv)` argparse + scan + print + exit, **per-line ignore comment with 3-line lookback** (consistent with PR #166 `# subprocess-timeout: ignore` / PR #169 `<!-- enforcement-claim: ignore -->`). **Generated test scaffold** includes `TestComputeExitCode` truth table (always-passing parametrized boilerplate), `TestMain` integration, **`TestLiveRepo` dogfood gate** that scans actual repo before merge. **Per-kind ignore marker convention** auto-derived: `text` uses HTML comment (for docs); `yaml`/`ast`/`meta`/`freshness` use Python-style comment. **Pre-commit hook entry auto-inserted** with FATAL-on-finds severity (mirrors PR #169 / #170 ship-strict-from-day-1 pattern when post-rewrite violation count is 0); idempotent on duplicate hook id. **`make lint-extract NAME=foo_bar KIND=text DESCRIPTION="..." FILES='^docs/.*\\.md$'`** wraps the script with arg validation + usage block. **62 unit tests** (`scripts/tools/dx/scaffold_lint.py` validation: naming pattern × 18 / path derivation × 5 / per-kind ignore marker × 5 / template rendering compiles for all 5 kinds × 2 / hook insertion idempotency × 3 / E2E main() with --dry-run / --force / --no-hook / invalid-name / unknown-kind × 7) all pass in 0.78s. **Idempotent** on re-run (skips existing files unless `--force`); `--dry-run` previews; `--no-hook` opt-out for manual wiring. **Dogfood** — generated a placeholder `check_scaffold_dogfood.py` smoke-test then deleted; all `compile()` checks in tests prove syntactic validity for all 5 kinds. **What this scaffold does NOT do**: write the actual scan logic (placeholder TODO), specific keyword lists / AST visitors / YAML structures (kind-specific TODOs), or auto-update tool-map (existing `tool-map-check` hook does that on next commit). Next lint is now **~15 min instead of 1-2 hr**. **Bundled with**: `testing-playbook.md` §v2.8.0 LL **§5 (Self-review pass 2)** + **§6 (Intentional-break dogfood for regression tests)** — two process patterns from PR #166 amend session (S#77) that are **explicitly NOT codifiable as lint** (meta-discipline) but worth doc-codifying. §5 documents how user-prompted second-pass review caught 3 real bugs + 1 robustness crash that pass 1 missed (`timeout=None`/`0` slipping past structural check / internal helpers without direct tests / `BlobViolation.render()` crash on absolute paths outside PROJECT_ROOT / docstring drift); pattern: pre-merge ship 前最後一輪深層 self-review 應該變 default discipline，"if production regresses, would this test catch?" 自驗必跑. §6 documents the empirical regression-test verification loop (revert fix → run test → confirm fail → restore → re-run pass); **critical finding**: 沒做 intentional-break loop 的 regression test 是 **article-of-faith** — PR #166 原 headline test 用 300×1KB total 不會在 Windows runner 觸發 deadlock；1000×2KB 才可靠 trigger，path count > total bytes 更關鍵. Three-layer regression-detection ladder codified (pytest-timeout marker / soft elapsed assert / inner `communicate(timeout=N)`) — 任一層 reverted 剩兩層仍守.
