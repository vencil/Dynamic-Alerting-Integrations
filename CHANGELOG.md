...
### Benchmark（1000 tenants, Dev Container, `-benchtime=3s -count=6`）

> v2.9.0 baseline 與 4-hr compressed-time readiness soak 數字回填中（`make benchmark-report` + `run_chaos_soak.py` ~240 reloads / ~480 polls；soak-report.md 作 release asset）。平台 rule-pack（含 version-aware 能力 A）維持 O(shapes)、不隨租戶數膨脹；Custom Alerts 向量化消去同指標跨租戶扇出（規則數 = shape 數），但隨自訂告警**種類**線性增長、由 per-tenant cap 封頂（非 rule-pack 的 O(M) 類）。v2.7.0 scale SLO（cold load ~112 ms / steady-state reload ~1.3 ms @ 1000 tenants）維持。
