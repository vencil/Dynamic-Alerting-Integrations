...
- **Customer-facing docs codename sweep + `check_codename_leak.py` scope/pattern 擴充（v2.8.0, [issue #462](https://github.com/vencil/Dynamic-Alerting-Integrations/issues/462)）** — 對外文件殘留的 internal planning codenames（`B-1` / `B-4` / `C-8`~`C-12` / `Phase .a~.e` / `Phase B Track A/C` / `PR-1`~`PR-5` / `S#101` / `DEC-X` / `v2.X.0-final` / `v2.X.0-rc1` / `v2.X.0-preview.N`）對客戶徒增認知門檻 — 一律改 feature name + 版號（如 `C-12 Dangling Defaults Guard` → `Dangling Defaults Guard (v2.8.0)`、`Phase .c C-10 Batch PR Pipeline` → `Migration Batch PR Pipeline`、`v2.7.0-final` → `v2.7.0`）。Lint side 三件套：(a) PATTERNS 加 `DEC-X` / `v\d+\.\d+\.\d+-(final\|rc\d*\|alpha\|beta\|preview\d*)` 兩條，明確不加 `Phase N digit`（合法 prose 太多）和 `R0/R1/R2`（CPU register FP）；(b) DEFAULT_SCAN_PATHS 從 T0+component README 擴到 T1（`docs/**` 排除 `internal/`）— customer-facing 全在 CI default scope；(c) `PER_FILE_ALLOWLIST` 新機制收 `docs/adr/020-planning-ssot.md`（該 ADR 主題就是 codename namespace，例子要保留）。22 個對外文件、~213 處替換、PR template 加具體 codename 列舉的 checklist 條目（不是 "no jargon" 模糊話）。Layer 2 self-healing 機制（glossary-driven gate）見 follow-up [issue #469](https://github.com/vencil/Dynamic-Alerting-Integrations/issues/469) — 本 PR 是 Layer 1 catch-up，承認永遠 lag pattern 的根本不會被 regex 完全解。

- **Planning ID namespace 統一為 TRK（v2.8.1-dx-interim, [issue #379](https://github.com/vencil/Dynamic-Alerting-Integrations/issues/379) chunk 1 of 5）** — 依 [ADR-020 Option C refined hybrid](docs/adr/020-planning-ssot.md) 把 `TECH-DEBT-NNN` / `TD-NN` / `HA-NN` / `REG-NNN` 四個分散 namespace 統一為 `TRK-NNN`。新增 [`docs/internal/planning-id-mapping.md`](docs/internal/planning-id-mapping.md) 含完整對映表 + 三段分區編號政策（TRK-001~099 ← HA / TRK-100~199 ← REG / TRK-200~299 ← TECH-DEBT/TD / TRK-300+ 為新分配）。全 repo 批次替換 ~73 處引用（CHANGELOG / CLAUDE.md / dev-rules / dx-tooling-backlog / testing-playbook / tool-map / ADR-014/015/016 等）；`docs/adr/020-planning-ssot.md` 政策段保留舊 ID 字樣作為 namespace 論述本身。ADR namespace 不動，`S#NNN` sprint ledger 不動，`Trap #N` / `pitfall #N` 不動。Lint `scripts/tools/lint/check_techdebt_drift.py` 因 `known-regressions.md` v2.8.0 Phase .a 已 phantom-delete 為 no-op，後續 chunk 2b 以 `check_planning_status_sync.py` 取代。

- **Test infrastructure — exporter 三個 global-swap antipatterns 拆掉，全部走 `ConfigManager` test-only setter 注入（v2.8.0, PRs #363–#369, ~7 PRs）** — 測試姿態從「solid」推到「robust」：(a) `withIsolatedMetrics` → `freshMetrics` + `m.SetMetrics`；(b) `log.SetOutput` → `m.SetLogger(testLogger)`；(c) WatchLoop / debounce 從 `time.Sleep` 等 ticker 改 `m.SetClock(fakeClock)` + `Advance` + state-poll。前述三類過去因 global state race 必須 serial 的測試現在 `t.Parallel`-eligible，full app pkg `-count=5 -race` 12.1s（previously 20.7s）。TRK-217（WatchLoop time.Sleep flake）以 FakeClock 結構性關閉。AI agent quickref → `docs/internal/test-map.md` §測試注入 Seam + `CLAUDE.md` §測試注入 Seam。**v2.8.0 condensation 階段請以「測試達到的境界」粗顆粒重寫**，不要保留個別 PR 數字。

- **`bench-gate-pr.yaml` 改 single-runner sequential 拓樸 — 撤回 horizontal sharding（v2.8.0, [issue #433](https://github.com/vencil/Dynamic-Alerting-Integrations/issues/433) W2, Gemini round-12 + 機率分析驅動）** — Free-tier GH-hosted runner pool 有 ≥3 種 CPU（AMD EPYC 7763 / 9V74 / Intel Xeon 8370C）。原 v2 sharded design（3 shards × 2 sides = 6 runners）的「全 runner 同 CPU」機率僅 ~1.7%，配合 v4 加上的 INCONCLUSIVE 強制 fail（Trap 11 in user memory feedback，封 silent false-negative），實測 INCONCLUSIVE rate ~98% — gate 等於不可用。v5 改成 **單 runner 內 sequential 跑 base + pr**（checkout merge-base → bench → drop_caches → checkout pr → bench → benchstat），同 VM 同 CPU by construction → INCONCLUSIVE 率 ~0%。Wall time 從 sharded ~4-5 min 退到 ~20 min deterministic（vs Option B 撤 sharding 回 2 runners 的 ~18 min 期望但 11-33 min 範圍），勝在每次都一樣的可預測體驗。`drop_caches` 步驟在兩次 bench 之間 flush OS page cache + dentries + inodes，緩解 ~1-3% 系統性 cache-warmth bias。Codify 進 memory feedback Trap 12 in user memory feedback：「Horizontal sharding on heterogeneous CI runner pool = guaranteed unreliability — count distinct hardware-variant draws, compute P(all agree); free-tier pool 用 sharding 是反模式」。所有先前 round-7~11 的防線（event-scoped override / Ghost Green race 阻擋 / Blanket Immunity Trap 阻擋 / α=0.01 + 5% magnitude floor / `\|\| true` 容忍 happy path / partial-shard 偵測（已 N/A）/ INCONCLUSIVE defense-in-depth）保留。

- **`bench-gate-pr.yaml` + `bench-override-audit.yaml` 二輪 SRE 對抗審查的 governance 加固 — Ghost Green race + Blanket Immunity Trap 雙修正（v2.8.0, [issue #433](https://github.com/vencil/Dynamic-Alerting-Integrations/issues/433) W2, Gemini round-7 + round-8 連抓兩漏）** —
  - **Round-7 (Ghost Green race)**：bench-gate preflight (~5s) 比 bench-override-audit (~20s) 跑得快。Unauthorized 使用者 apply label → preflight 只憑 label 存在性就 mark skip=true 短路綠燈，audit 雖然紅燈但 branch protection 若沒設 audit 必過則 PR 可帶 bypass 入 main。
  - **Round-8 (Blanket Immunity Trap)**：即使加 timeline-based applier 驗證，仍漏 — maintainer T0 同意 6% regression 應用 label → T1 dev push 含 500% O(n²) bug 的新 commit → `synchronize` 事件 → timeline 仍顯示 maintainer 是最後 applier → preflight 認可 skip → 靜默放行災難級 regression。
  - **Unified fix（v4 design）**：preflight 改 event-scoped semantics — 只有 (a) 觸發 event 本身是 `labeled` AND (b) 應用的 label 是我們的 override label AND (c) sender (= labeled event 的 applier) 有 admin/maintain 角色 三條件**同時**滿足才認可 skip。任何其他 event（opened / synchronize / reopened / labeled-of-other-label）一律忽略 label 即使還在 PR 上。Override 變成「per-event, per-commit-state」而非 per-PR。`bench-override-audit.yaml` 對應加 `strip-stale` job 在 `synchronize` event 自動撕掉舊 label + 留 comment 引導 maintainer 重新審核新 commit 後 re-apply。零信任設計：每次 code push 都強制重新評估。

- **`bench-override-audit.yaml` 新工作流 — `override: bench-regress-ok` label 權限稽核（v2.8.0, [issue #433](https://github.com/vencil/Dynamic-Alerting-Integrations/issues/433) W2 governance closure）** — soft enforcement of「maintainer-only override」semantics。GitHub branch-protection 不支援 per-label permission restriction，任何 Triage+ collaborator 都能 apply 任何 label，但 bench-gate-pr 把這個 label 當 CI-skip signal（跳過 4-5 min bench 跑 + regression verdict）→ 如不防護則任何 Triage 使用者可繞過 gate。新 workflow 在 `pull_request_target: [labeled]` 觸發（fork PR 也能寫 → 用 `_target` 取得 write token；同時不 checkout PR head 維持安全），檢查 `github.event.sender.login` 的 effective role（admin / maintain / write / triage / read / none），只允許 **admin** 或 **maintain** 應用 label。其他角色：自動 `gh pr edit --remove-label` + 留 comment 引導 PR contributor 走「文件化 trade-off rationale → ping maintainer」流程 + workflow exit 1 讓 PR check 顯式 fail。Heredoc 內容用 awk 動態 dedent 避免 markdown 縮排 4+ spaces 被當 code block 渲染。Self-bootstrap safe：只有 admin 能 grant admin，所以攻擊者無法用此 workflow 提權。Future tightening note：若改為 org 而非 personal repo，可改 team-membership API 查詢取代「any admin/maintain」blanket allowlist。

- **`bench-gate-pr.yaml` 並行 sharding 加速 + benchstat 閾值雙修正 — wall time 11 min → ~4-5 min（v2.8.0, issue [#433](https://github.com/vencil/Dynamic-Alerting-Integrations/issues/433) W2 後續優化, PR #437）** — 在 PR [#435](https://github.com/vencil/Dynamic-Alerting-Integrations/pull/435) 初版（2 jobs：base + pr parallel）的基礎上，再把 bench 工作分 3 shards × 2 sides = **6 個並行 runner**。分片以 source-grep（`grep -rh "^func Benchmark"`）+ `(NR-1) % SHARDS` modulo partition 完成，~0.1s 列舉成本，**deterministic 且新增 bench 自動分配**不需手動維護 list。Compare job 用顯式 `seq 0..N-1` 迴圈合併 shard artifacts（缺一個立即 hard-fail，避免 partial-shard silent acceptance — 這比 lex 排序問題嚴重得多，是 Gemini round-5 review + 實作時自查雙重抓到）。統計效力**無變動** — 每個 benchmark 仍在單一 runner 跑 COUNT=6 樣本；sharding 切的是 benchmarks 不是 counts。CI 分鐘 +20%（每 shard 各付 ~30s setup-go + cache-restore 獨立 overhead），換 ~55% wall-time 節省。Tail-variance 風險：6 runners 比 2 runners 更易遇 noisy-neighbor 拖尾 — 若 FP rate >10%/2 weeks 則觸發 Larger Runners 升級（per rollout 設計）。`SHARDS_TOTAL` 為 workflow-level env var 方便調整，但 matrix 仍須手動同步（GitHub Actions matrix 不能直接讀 env）。**順帶修掉 PR #435 帶下來的 benchstat threshold bug**：原 `-confidence=0.99` 其實只控制 geomean 顯示區間寬度，**不**是 per-bench 顯著性閾值（那是 `-alpha`）；gate 一直以 default α=0.05 在運作，沒達到宣稱的 99% confidence。改 `-alpha=0.01` 並追加 |Δ|≥5% 幅度門檻（two-tier detection：統計顯著 AND 效應大小），解決 free-tier runner ~1-3% 噪音造成 YAML-only diff 也誤判 regression 的問題（self-test 從 PR #437 第 2 跑 fail 暴露出來，原工作流自己 dogfood 找出自己 bug）。
...
- **`bench-gate-release.yaml` workflow — Phase 2 Tier 2 release-time cumulative drift report（v2.8.0, [issue #433](https://github.com/vencil/Dynamic-Alerting-Integrations/issues/433) W4 closure）** — 補完 #433 兩層 bench-gate 架構：Tier 2 在 `release: published` 事件觸發（platform `v*` 線；exporter/tools/portal/tenant-api/v* 不在 scope），對 prior `v*` tag vs current release tag 跑 single-runner sequential bench（同 Tier 1 v5 拓樸 — checkout prior → bench → drop_caches → clean workspace → checkout current → bench → benchstat）。閾值較 Tier 1 寬鬆（`-alpha=0.05` + `|Δ| ≥ 10%`），原因：Tier 1 已 catch 每 PR 的 perf 變動，Tier 2 只該 flag 「user-visible drift」（10% 以下的累積 ±drift 在 release-to-release 跨多週 timeframe 多半是 runner variance + Go 版本變動 + 依賴升級 noise，不是真的 regression）。Tier 2 **純 informational** — 不阻擋 release，結果寫到 workflow step summary，maintainer 自行讀後在 release notes 文字承認 drift 或開 perf follow-up issue。也支援 `workflow_dispatch` for pre-tag manual review（maintainer 可 `prior_ref` / `current_ref` 自選 refs，在 tag 前先看 drift report）。第一個有意義的比對是 v2.9.0 vs v2.8.0（v2.7.0 創建在 PR #117 attach-baseline workflow 之前，沒可用 baseline；workflow auto-detect "no prior baseline" case 直接 render informational summary 無動作 exit 0）。Auto-discovery of prior tag via `git describe --tags --abbrev=0 --match='v*' --exclude='*/*'` 排除其他 release lines。#433 W4 milestone 至此 close — Tier 1 + Tier 2 + governance 三層完整。

- **`da-tools state-reconcile` 新命令 — 遷移狀態目錄聲明式一致化（v2.8.0, [issue #405](https://github.com/vencil/Dynamic-Alerting-Integrations/issues/405) Category A）** — 取代 [troubleshooting-checklist §schema_version drift / manifest drift](docs/integration/troubleshooting-checklist.md) 兩段手動 jq 流程，統一為「make `.da/` consistent」一句話。掃 `.da/state/*.json` → 驗 `schema_version` 對齊現行版本（未來 1.1/1.2 引入時走 `MIGRATIONS` registry 自動 migrate）→ 從檔案系統重建 `.da/manifest.json`（manifest 是 derived view，state 檔是 SOT）。提供 `--dry-run` 預覽、`--ci` gate（dry-run + 偵測到漂移時 exit 1）、`--json` 結構化輸出。Exit codes：0 一致 / 1 unresolvable drift 或 CI pending / 2 caller error。手動 jq 流程在文件中保留為 airgapped / 工具不可用時的備援。Atomic write (tempfile + os.replace) + LF line endings + manifest self-exclusion 三層 robustness 防 customer GitOps repo corrupt / cross-platform CRLF / phantom-cluster 死循環。

- **`da-tools silencer-drift-check` 新命令 — Alertmanager silence offline 漂移審計（v2.8.0, [issue #405](https://github.com/vencil/Dynamic-Alerting-Integrations/issues/405) Category B）** — 取代 [troubleshooting-checklist §1.3.2](docs/integration/troubleshooting-checklist.md) 的手動 `amtool silence query -o json` + `jq` + `comm` 流程。Offline-first 設計：吃 amtool 抓下來的 JSON 檔（**不**直接連 AM——避免 VPN / Ingress / Auth-proxy 邊界踩雷），對指定 rule pack source（檔或目錄）做 matcher reconciliation。實作完整 AM matcher 4 種組合（isEqual × isRegex；不只 `alertname=` 還含 severity / team 等 label-value drift），absent label 視為空字串（AM 慣例）。輸出 orphan silence 清單（matchers / id / comment / endsAt）。`--ci` 模式 orphan 時 exit 1；`--json` 結構化；`--include-inactive` 切換 expired/pending 是否納入檢查。Exit codes：0 / 1 / 2 一致 with state-reconcile / rule-pack-diff。45 tests 覆蓋 matcher 全 combinations、absent-label semantics、active vs timestamp 切換、empty input / typo path / malformed JSON。Issue #405 整個 umbrella 至此 close（Cat A + B + D ship；Cat C closed as inline workflow）。

- **`da-tools rule-pack-diff` 新命令 — Rule Pack 兩版本機械比對（v2.8.0, [issue #405](https://github.com/vencil/Dynamic-Alerting-Integrations/issues/405) Category D）** — 取代 [staged-adoption-guide §7.3](docs/scenarios/staged-adoption-guide.md) 中「人工對照 CHANGELOG + `git diff` rule pack 目錄」流程。掃兩版 YAML 的 `groups[*].rules[*]`，分類為 added / removed / modified / breaking label schema。Breaking 判定：alert 被移除 / label key 增減 / label value 變動 / alert↔record kind 互換（這些都會讓客戶既有的 AM silencer matcher 與 `_defaults.yaml` disable list 失效 → double-fire alert storm）。`expr` 變化、annotation 變化、`for:` 變化 = informational 不自動判 breaking（語意等價無法自動判定）。`--ci` 模式 breaking 偵測到 exit 1 作為平台 release CI gate。Exit codes：0 無 breaking / 1 `--ci` 偵測到 breaking / 2 caller error。輸入用檔案路徑（不假設 repo 目錄結構），典型用法 `git show vX.Y.Z:rule-packs/<pack>.yaml > /tmp/v.yaml` 兩次後比對。33 tests 覆蓋 alert/record 索引、breaking 分類矩陣、YAML 邊界情況。Issue #405 Category B (silencer-drift-check, upgrade-check) 仍 backlog。

- **Portal ESM build + Vitest unit-test foundation（v2.8.0, [TRK-230a #247](https://github.com/vencil/Dynamic-Alerting-Integrations/issues/247) of TRK-230 Option C sweep）** — first PR of a multi-PR sweep that retires `jsx-loader.html` and migrates all 77 portal JSX/JS files to ES modules. This foundation PR is **pure infrastructure**: `tools/portal/build.mjs` (esbuild, frontmatter strip, manifest-driven), `tools/portal/vitest.config.ts` (jsdom + RTL + frontmatter plugin), Makefile targets (`portal-build` / `portal-build-watch` / `test-portal`), new `Portal Tests` CI job. Manifest starts empty; subsequent PRs (TRK-230b..f) add entries as they migrate. jsx-loader.html stays functional for not-yet-migrated tools — TRK-230z does the final retirement. User explicitly accepted v2.8.0 release delay as the cost of this architectural cleanup; see project memory `project_portal_vitest_choice.md`.

### Added

- **`bench-gate-pr.yaml` workflow — Phase 2 Tier 1 (PR-time perf regression gate)（v2.8.0, [issue #433](https://github.com/vencil/Dynamic-Alerting-Integrations/issues/433) W2 deliverable）** — supersedes the structurally-unattainable window-based Phase 2 from #67 (closed). New design splits gating into two tiers: **Tier 1 (this PR)** runs benchstat -confidence=0.99 on `merge-base` vs PR HEAD, blocking PRs that introduce statistically-significant perf regression — attributes blame to the PR's own diff, not to ancestors' accumulated drift (Scapegoat Trap defense). **Tier 2 (release-time)** ships as a separate follow-up in #433 W4. Override mechanism uses GitHub PR label `override: bench-regress-ok` (branch-protection-restricted to maintainers, not a PR-body string) to avoid Broken Window decay; workflow triggers on `[opened, synchronize, reopened, labeled]` so applying the override re-runs CI (Label Trigger Trap defense). `actions/checkout` with `fetch-depth: 0` + explicit `git merge-base` for correct base-commit identification (do NOT use `origin/main` HEAD which would mix in unrelated commits). Hardware floor starts on `ubuntu-latest-2-cores`; conditional escalation trigger documented (Tier 1 FP rate > 10% / 2 weeks → mandatory Larger Runners upgrade). Path-scoped to threshold-exporter changes — doc-only PRs skip the gate (CI cost preservation). Five SRE adversarial-review gotchas codified in workflow comments + issue #433 body.

### Fixed

- **`da-tools` entrypoint local-dev dispatch — every subcommand was failing outside Docker（v2.8.0 closure E-4 audit follow-up, [PR #451](https://github.com/vencil/Dynamic-Alerting-Integrations/pull/451)）** — `python3 components/da-tools/app/entrypoint.py <subcommand>` previously failed with `Error: Tool script not found` for **every** subcommand when run from a local repo checkout. Root cause: `entrypoint.py` resolved `script_path = TOOLS_DIR/<script>`, a layout produced only when `build.sh` flattens scripts into the Docker image — local checkout has scripts at `scripts/tools/{ops,dx,}/`. Fix: `_resolve_script_path()` keeps `TOOLS_DIR` lookup as primary (Docker image path unchanged) then walks up to find the repo root and searches the canonical source dirs in order. `_find_repo_root()` handles both regular checkouts (`.git/` dir) and git worktrees (`.git` file). Error message now lists every path searched, bilingual ZH/EN. Initial framing during E-4 audit was "v2.8.0's 3 new tools (#422 / #424 / #431) broke entrypoint"; verification showed all existing subcommands (`check-alert` / `scaffold` / `baseline` / etc.) had the same break since entrypoint was introduced. Docker image build flow unaffected (primary path unchanged). +59 LoC tests covering fallback resolution / worktree-style `.git` / negative path / Docker primary precedence; 31 tests pass.

- **`da-tools` cp950 console crash on legacy Windows + `_lib_compat.py` consolidation（v2.8.0, [PR #432](https://github.com/vencil/Dynamic-Alerting-Integrations/pull/432)）** — fix `UnicodeEncodeError` crash when rendering Unicode characters (`✓ ⚠️ ➕ ≤ →`) to stdout on legacy Windows console codecs (cp950 / cp936 / cp1252). Originally surfaced on `analyze_bench_history.py` during [issue #67](https://github.com/vencil/Dynamic-Alerting-Integrations/issues/67) hands-on review; identified as the 4th instance of the same bug class (state-reconcile / rule-pack-diff / silencer-drift-check already had local in-tool fixes from each PR's round-2 self-review). Consolidated to a single shared helper `try_utf8_stdout()` in new `scripts/tools/_lib_compat.py` (stdlib-only by contract; no third-party imports). Helper invokes `sys.stdout.reconfigure(encoding="utf-8", errors="replace")` inside a try/except — modern terminals (UTF-8 Linux / macOS / Windows Terminal / Docker Alpine bundle) keep emoji rendering; legacy consoles substitute "?" instead of crashing. Note: 20+ other ops/dx tools also emit emoji in stdout but were not migrated in this PR — they're rarely invoked outside the Docker image (UTF-8 by default); apply the helper proactively when next touching them.
...
- **`/api/v1/me` JSON wire shape: nil-vs-empty-array drift（v2.8.0, [TRK-228 #242](https://github.com/vencil/Dynamic-Alerting-Integrations/issues/242)）** — schemathesis contract test (TRK-222) caught two `Response violates schema` failures on `GET /api/v1/me`: when caller had no group memberships, `groups` and `accessible_tenants` serialised as `null` even though the OpenAPI spec declared both as `array`. Root cause: `rbac.RequestGroups(r)` returns `nil` when the `X-Forwarded-Groups` header is absent, and `accessibleTenants` was only populated by appending to a nil slice. Fix: in `internal/handler/me.go`, normalise `groups` to `[]string{}` when nil and explicitly initialise `resp.AccessibleTenants = []string{}` so the JSON encoder emits `[]` not `null`. Added wire-level regression test `TestMe_EmptyGroupsRendersAsArray` that asserts on the encoded body string (`"groups":[]`), not the Go struct field, because the failure mode is encoding-side. **CI integration enabled in same PR**: `make contract-test` now runs in `Go Tests (1.26)` after the swag drift check (`CONTRACT_MAX_EXAMPLES=5`, ~10–20s). Two known warnings remain (input-side `pattern` missing on path params + `example` values for IDs) — non-blocking and tracked as follow-up.

- **`check_head_blob_hygiene.py` regression tests + hang-localization milestones + S#74 lesson codify（v2.8.0, follow-up to PR #164）** — PR #164 fixed the `_batch_cat_blobs` Popen deadlock in flight while shipping PR-2c, but did not include tests or codify the agent-discipline lesson that surfaced. This PR closes those gaps **and** adds the "**仍待**：progress output" item that Trap #57 had been carrying since v2.8.0 Phase .a. **20 new tests in `tests/lint/test_check_head_blob_hygiene.py`**: 3 deadlock-regression (small batch sanity / large batch 1000×2KB regression-catcher / empty input — using a real on-disk git repo fixture, not mocks, since the deadlock is in the real Popen pipe interaction), 2 output-parsing, 4 `scan_blob` rule (NUL / EOF strict-only / TRUNC mkdocs-yml), **3 hang-localization milestone tests** (default-mode "Reading N..." + "batch read complete" + per-100 scan progress; --verbose per-file scan progress), 8 `_is_skippable` parametrized. Tests run in ~6.4s locally. Three layers of regression defense: (1) `@pytest.mark.timeout(N)` per-test pytest-level fast-fail (added `pytest-timeout` to CI deps), (2) explicit elapsed-time soft-budget assertion, (3) inner `communicate(timeout=60)` in the function under test. **Empirical dogfood verified the regression test catches the regression** by intentionally reverting `_batch_cat_blobs` back to the write-then-read-loop pattern, observing the headline test timeout at 120s (pytest-timeout firing), then restoring — without this loop the test would have been an article-of-faith. Notable empirical finding: 300×1KB total output does NOT reliably trigger the deadlock on Windows (pipe buffer happens to absorb it); reliable trigger threshold is ~1000+ paths, captured in the headline test parameters. **Hang-localization milestones now baked into the hook itself**: prints `Reading N HEAD blob(s)...` → `✓ batch read complete: N loaded in Xs` → `...scanned I/N` every 100 in default mode → final summary. Three observation points let an operator pin down "is it stuck before the batch read, in the batch read, or in the per-blob scan" without re-running with `--verbose` — closes the diagnostic-gap that Trap #57 had been waiting on since 2026-04-23. **Trap #57 in `windows-mcp-playbook.md`** updated from "🟡 Mitigated" to "🟢 Resolved": (a) second root-cause class (Popen pipe deadlock, orthogonal to FUSE stale-temp class) documented with identification rules (NTFS → 100% Popen; FUSE + phantom → stale temp; FUSE + clean phantom → likely Popen), (b) hang-localization milestones documented as the diagnostic path. **`testing-playbook.md` v2.8.0 Lessons §4 (NEW)** codifies S#74: subprocess hang requires an SLA prior + escalation policy, not patience. Three-step escalation: (a) `ps -ef | grep <suspect>` to locate the live subprocess, (b) isolate and reproduce the stuck path standalone, (c) instrument the suspect path. Plus a verify-reference extension to S#73: hook scripts authored by others are not exempt — read the critical paths (Popen / pipe / I/O) before assuming "it works".

- **`check_head_blob_hygiene.py` Popen deadlock above ~150 paths（v2.8.0, lint, discovered while shipping PR-2c）** — the `_batch_cat_blobs` helper used a single-threaded write-then-read-loop pattern over `git cat-file --batch --buffer`'s stdin/stdout pipes. With `--buffer` git accumulates ALL stdout in memory until stdin EOF then flushes at once; once total output exceeded the OS pipe buffer (~64KB on Windows) git blocked on stdout write while the script was still reading from a different path's body bytes — classic single-thread Popen pipe deadlock. Symptom: pre-commit hook **`HEAD blob hygiene (NUL bytes / truncated structured files)`** hung indefinitely on every commit (the repo passed 1040 tracked files with ~2.18MB of cat-file output, well over the deadlock threshold). Fix: replace manual write-then-read with `proc.communicate(input=request, timeout=60)`, which uses internal threads to drain stdout/stderr while writing stdin — sidestepping the deadlock without changing semantics (we already buffered all output in memory for the parse loop anyway). Output parsing rewritten as in-memory byte-buffer walk over the bytes returned by `communicate()`. After fix: `--ci` run completes in **0.57s for 1040 blobs** (was indefinite hang). 60s timeout + try/except on `OSError` and `TimeoutExpired` ensures the script can never hang the commit again — empty-result fallback to per-path `_cat_blob` loop preserved. Net regression unblock: every commit was previously hanging on this hook for sufficiently large repos; this restores the hook to its intended fast-path. Discovered while shipping PR-2c — included here rather than as a separate PR because the bug was actively blocking the commit.
...
- **Mixed-mode benchmarks 加入 nightly bench-record cron（v2.8.0, Phase B Track B follow-up, [issue #128](https://github.com/vencil/Dynamic-Alerting-Integrations/issues/128) prerequisite）** — Track B 期間 Phase B PR 落地 4 個 mixed-mode benchmarks（`BenchmarkScanDirHierarchical_MixedMode_500flat_500hier` / `BenchmarkFullDirLoad_MixedMode_500flat_500hier` / `BenchmarkDiffAndReload_MixedMode_500flat_500hier_NoChange` / `BenchmarkFullDirLoad_MixedMode_100flat_900hier`），但 Phase 1 baseline 的 `make benchmark-report` 用 `-bench='_1000(_|$)'` 過濾，4 個 mixed-mode 名稱結尾為 `_500hier` / `_900hier` / `_NoChange` 全 miss。結果 nightly `bench-record.yaml` workflow 從未真正量過 mixed mode — issue #128 等的「authoritative numbers ... 28+ data points」start clock 從未起跑。
  ...
  - **時間預算**：nightly cron 一輪預估從 ~10-15 min ramp 到 ~12-18 min（4 個 mixed-mode 的 fixture-setup + 各 count=6 × benchtime=3s ≈ 18-72s extra），仍在 timeout 30 min 之內
  - **wall-clock 計時**：本 PR landed 之後第一次 nightly 為「Day 1」；累積至 28+ 點數據（~28 天 wall-clock，target ~2026-05-29）後可由 #128 acceptance 跑 `analyze_bench_history.py` 統計判讀，決定 mixed mode 是否需 perf optimization PR
  - **本 PR 不關閉 #128**：本 PR 是 #128 的 prerequisite —「啟動 wall-clock 累積」；真正關閉 #128 需要等 28 天後拿到統計判讀。但 issue body 可在 PR merge 後 update 加上「nightly 已從 PR #XXX 啟動，data accumulating from {date}」
  - 同步更新 `docs/internal/github-release-playbook.md` 對應「13 benches × count=6」描述為「17 benches × count=6（包含 4 mixed-mode 自 issue #128 加入後）」+ runner 時間預估同步
...
- **tenant-api 11-PR refactor sweep — complete (waves 1-5 of 5)（v2.8.0, Phase .e prep）** — pre-v2.8.0 quality lift via small, independently-shippable PR bundles. **Wave 1 [#215](https://github.com/vencil/Dynamic-Alerting-Integrations/pull/215)** (PR-1..3, merged): README rewrite v2.7.0 → v2.8.0 (new sections: Capabilities matrix / Operational concerns / Observability / resource-grouped endpoint tables) + `/ready` bug fix (was always 200 OK regardless of `configDir` state — readinessProbe couldn't drain a pod whose ConfigMap mount failed; now `os.Stat`s the dir and returns 503 if missing/non-directory) + dead-code purge (`handler.ListTasks` never wired; `github.PRInfo` BC alias from v2.6.0 had no callers; `view.go` defensive validation block superseded by tightened struct tags) + `gitops.Writer` dedup (extracted `commitFileChange` helper shared by `Write` and `writeSpecialFile`; ~30 LoC duplicate code removed; log format unified to `commit_tag=<id-or-entity>`) + generic `platform.PollingTracker` for github+gitlab (was 142×2 LoC ~95% identical; now 20 LoC each + 181 LoC platform helper; ~244 LoC duplication eliminated; `Sync()` and `SyncInterval()` now exported for test drive). **Wave 2 [#216](https://github.com/vencil/Dynamic-Alerting-Integrations/pull/216)** (PR-4..6): `handler.Deps` struct collapses 8-arg constructors to receiver methods (21 dependency-injected handlers migrated; `BatchTenants(writer, configDir, rbacMgr, policyMgr, taskMgr, writeMode, prClient, prTracker)` → `(d *Deps).BatchTenants()`; `WriteMode` + `IsPRMode` moved from `tenant_put.go` to `deps.go`; tests use inline struct literals for explicit dependency surface) + `main.go` PR-mode bootstrap extracted to `cmd/server/wire.go::wirePRBackend` (348→298 LoC; main() now reads as a clean wiring shape) + shared `Deps.createPRAndRegister` helper unifies PutTenant/BatchTenants tracker registration. **Behavior delta (intentional consistency improvement)**: BatchTenants per-tenant tracker entries now preserve `CreatedAt` from the PR/MR API response (was zero-valued; single-tenant path already had this), so `/api/v1/prs` consumers see consistent fields across single-vs-batch paths. Test count 322 → 323 (+2 Ready edge cases, -1 dead-code `TestListTasks`); waves 1+2 are pure refactor with 0 logic changes apart from the documented `/ready` and `CreatedAt` fixes. `go vet ./...` clean throughout. **Wave 3** (PR-7..8): generic `handler.filterByRBAC[T]` collapses 3 near-identical permission filters (`filterAccessibleMembers` / `filterAccessiblePRs` / `filterTaskResults`) into a single typed-extractor helper (group / PR / task wrappers shrink to one-liners) + new `internal/configwatcher.Watcher[T]` package collapses 4 near-identical hot-reload managers (rbac / policy / groups / views) — each Manager now embeds `*Watcher[Config]` and inherits Get / Reload / WatchLoop, contributing only its domain-specific check methods (HasPermission / CheckWrite / ListGroups / ListViews). Per-manager LoC: rbac 330→295 / policy 227→191 / groups 190→142 / views 183→136. Test count 323 → 330 (+7 configwatcher unit tests). Test surface: `m := &Manager{}; m.value.Store(&Config{...})` pattern replaced with new `NewForTest(&Config{...})` per-package helpers (rbac + policy expose them; tests use embedded-Watcher's exported `Override` / `LastHash` for the dedup-hash assertions). Also extends `validate.yaml` path filter to include `components/tenant-api/**` in wave 2 — closes the post-merge required-checks gap that surfaced after the wave-1 squash, so future tenant-api-only PRs satisfy main's branch protection without artificial cross-touch into `docs/**`. **Wave 4** (PR-9..10): unified error envelope across all handlers — single canonical shape `{error, code, request_id, violations?, retry_after_s?, help?, action?, ...extra}` produced via `handler.ErrorResponse` + `writeErrorEnvelope`; six pre-PR-9 shapes (`writeJSONError` / `writeValidationErrors` / `writePolicyViolation` / RateLimit middleware inline / pending-PR check inline / task-not-found inline) all funnel through one helper. Additive — every pre-existing key preserved; new keys (`code`, `request_id`) appear additionally. Plus full `slog` migration: 22 `log.Printf` call sites across 9 packages (configwatcher / groups / views / policy / gitops / platform / github / gitlab / cmd-server) → structured `slog.Info`/`Warn`/`Error` with attributes; new `handler.SlogRequestLogger` chi middleware emits one JSON line per request with `request_id` matching the `X-Request-ID` echo header (5xx → WARN, else INFO). `TA_LOG_LEVEL` env var controls verbosity. Test count 330 → 339 (+7 envelope tests + 2 SlogRequestLogger tests). **Wave 5** (PR-11): rate limiter polish — `tenant_api_rate_limit_rejections_total` counter (rejection visibility for SLO dashboards) + `tenant_api_rate_limit_active_callers` gauge + 5-min `bucket sweeper` goroutine evicting buckets aged out of the rolling window (closes the unbounded-map theoretical concern noted in PR-1's design comments). Configurable HTTP server timeouts via `TA_READ_TIMEOUT` / `TA_WRITE_TIMEOUT` / `TA_IDLE_TIMEOUT` env vars (defaults preserved 15s / 30s / 60s) — fixes PR-mode batch operations brushing up against the previously-hardcoded 30s WriteTimeout in customer deployments. **Test count 339 → 343** (+4 PR-11 unit tests). **Sweep total**: 11 PRs across 5 waves, ~750 net LoC reduction across cross-package duplication eliminated, +49 unit tests added (291 → 343), zero behavior regressions; the only intentional behavior deltas are documented as additive consistency improvements (`/ready` 503 on missing config_dir; batch tracker entries preserve `CreatedAt`; error envelope adds `code` + `request_id`).

- **Phase .d SSOT 語言策略 closure — ZH primary policy locked, ZH→EN migration NOT pursued（v2.8.0, S#101）** — reverses the v2.5.0-era recommendation in `ssot-language-evaluation.md` §7.4 (Plan C: switch to EN SSOT). Previously deferred to v2.8.1 (planning §6) but never executed; this PR explicitly closes the loop with policy lock instead of letting it fade. **Audit (testing-playbook §LL §12a 4-question, with Q4 NEW)**: (Q1) spec written v2.5.0 era — mixed; (Q2) ZH-primary bilingual hooks already running for multiple version cycles — strong yes; (Q3) migration would add path used only by hypothetical international contributors who don't exist — strong yes; **(Q4 NEW: spec premise validation)** — original premise was "open-source community SSOT should be EN" with implicit "international contributors will appear"; user explicitly rejected this in 2026-05-02 review ("不討論現在還沒真實出現的英文主體客戶，現在客戶主體均是中文") — premise has no evidence support, **strong fail**. 4-question audit fully fails → policy reverse. **Changes (~150 LoC doc-only, zero code change, zero CI risk)**: (1) `CLAUDE.md §語言策略` rewritten — policy locked + 3 trigger conditions for re-evaluation; (2) `dev-rules.md §9b` rewritten from "v2.7.0 起開始遷移" to "ZH primary as official policy + dormant tools description"; (3) `check_bilingual_structure.py` + `check_bilingual_content.py` docstrings drop "（SSOT 目標）" labeling, mark EN-mode as dormant; (4) `ssot-language-evaluation.md` frontmatter `status: decided → superseded` + banner explaining reversal; (5) `ssot-migration-pilot-report.md` frontmatter `status: completed → completed-pilot-only` + banner clarifying execution-phase cancelled while pilot tools remain valid; (6) `migrate_ssot_language.py` module docstring marked DORMANT with reactivation conditions; (7) `testing-playbook §LL §12a` extends from 3 to **4 questions** (Q4 NEW: premise validation) with Phase .d as worked example. **Phase 1 pilot tools preserved as future-option** (zero deletion): `migrate_ssot_language.py` + `check_bilingual_*.py` dual-mode lint + 45 bilingual lint tests + pre-commit `.zh.md` triggers. **Trigger conditions explicitly codified** in CLAUDE.md / dev-rules / planning §6: (1) ≥3 non-Chinese-native contributor PR/issue, (2) customer RFP requires EN SSOT, (3) maintainer pivots to international-positioning project. **Process honest correction**: this is the THIRD application of LL §12a discipline (after S#99 §C-4 closure and S#100 C-6 ZH-customer-base default-to-ship); the LL §12a 3-question audit was insufficient for Phase .d because the discriminating signal was Q4 (premise validation), not Q1-Q3 alone. Codifying Q4 means future planning evaluations explicitly check whether the spec's STATED premise has evidence, not just whether external picture changed. Cross-refs: `ssot-language-evaluation.md` (status: superseded), `ssot-migration-pilot-report.md` (execution cancelled), `dev-rules.md §9b` (current policy), `testing-playbook §LL §12a Q4` (the audit discipline that fell out of this).
...
- **testing-playbook §v2.8.0 LL §8 codify + planning §12.5 hygiene（v2.8.0, doc, S#89）** — codifies the PR #178 lesson where `Self-Review-Pass-2:` trailer claimed "Wiring triple synced" but actually three sites (script docstring header / tool-map zh+en descriptions / `validate_all.py` registry) all stale at the original commit. **LL §5 case (i) 連續第 4 PR 重蹈覆轍** (#168 → #171 → #172 → #178). Adds **§8 "Pass 2 wiring-triple — claim vs verify"** with concrete verification commands for the Python-lint case (since PR #172's PR template Pass 1 check #4 was JSX-component-shaped, didn't translate to Python). Three concrete verification steps: (1) `head -3 scripts/tools/lint/check_<name>.py` to inspect docstring header, (2) `grep -n "<lint_name>" scripts/tools/validate_all.py` to verify registry description matches new scope, (3) `python3 scripts/tools/dx/generate_tool_map.py --check` (zh + en) to detect docstring↔tool-map drift. Also documents a v2.9.0 mechanical-safety-net candidate: lint that detects "scope changed (functional code added) but docstring header unchanged" via staged-diff heuristic. **Bundles two planning §12.5 hygiene closures**: line 742 PR #38 working-dir-cleanup TODO (close as non-actionable — `_*` files are gitignored, no PR ever required) and line 821 Process #3 estimation calibration (close as personal-note-never-to-PR; categorical mismatch with §12.5 actionable list). **Self-aware caveat**: per PR #172 lesson "doc-only codification is insufficient" — adding §8 doesn't mechanically prevent #5 occurrence. The mechanical safety net candidate (staged-diff heuristic) remains a v2.9.0 follow-up. 詳見 PR #178 review thread + commit `6a8c53b` audit.

- **CLAUDE.md slim-down: 110 → 88 lines (-20%)（v2.8.0, context budget, S#87）** — closes planning §12.5 line 738 deferred Phase .e CLAUDE.md slim-down item. CLAUDE.md auto-injects every turn (2nd biggest agent context pressure source after planning.md). Compressed 5 sections by removing duplication-with-codified-hooks and duplication-with-skills, while preserving every load-bearing rule verbatim. **Removed**: telemetry / debug command list (§Agent 起手式 — vibe-workflow skill covers); SSOT migration tooling list (§語言策略 — pilot-report covers); pre-commit bash code-fence (§Pre-commit — common knowledge); doc/tool counts 118/130/39 (§文件/工具 — drift constantly, available via doc-map / tool-map); benchmark Phase 1 narrative (§Release — github-release-playbook.md covers). **Preserved verbatim**: 4 dev-rules Top hot rules (#12 / #11 / #4 / #2), 7 Makefile must-know commands, 3 local skills, 9 architecture concepts list, 6 AI Agent environment bullets, main-path/escape-hatch design principle. Cross-reference scan confirms no other doc references the removed sections by line/heading; README pointers remain generic and accurate.

### Added

- **`make api-docs` Makefile target — OpenAPI spec build pipeline for tenant-api（v2.8.0, Phase .e prep, [closes #226](https://github.com/vencil/Dynamic-Alerting-Integrations/issues/226), TRK-221）** — closes a long-standing gap where 18 swag annotations across `components/tenant-api/internal/handler/*` (`@Router` / `@Summary` / `@Param`) were authored but never built into a spec — silent drift between annotations and reality. New target lazy-installs `github.com/swaggo/swag/cmd/swag` inside the Dev Container and runs `swag init -g cmd/server/main.go --parseInternal --parseDependency`, emitting `components/tenant-api/docs/{swagger.json,swagger.yaml,docs.go}` (13 endpoint paths covering groups / tenants / batch / diff / effective / validate / views / me / prs). Generated artefacts committed to git so CI can drift-check via `git diff --exit-code components/tenant-api/docs/` after re-generation in the `Go Tests` job. Added `@title`/`@version`/`@BasePath` general API info to `cmd/server/main.go` (was missing — required by swag CLI). **Unblocks downstream**: schemathesis contract tests (TRK-222 candidate), portal OpenAPI client codegen, public API docs hosting. Discovered during testing audit when 18 dead annotations were grep'd but no build pipeline existed. 詳見 [#226](https://github.com/vencil/Dynamic-Alerting-Integrations/issues/226).

- **da-portal 5-PR refactor sweep — `_common/` shared library + ErrorBoundary safety net + portal-shared decomposition + operator-wizard split + release hygiene（v2.8.0, Phase .e prep）** — coordinated 5-PR landing that lifts da-portal code quality before v2.8.0 release, sequenced so each PR is independently shippable. **PR-1 [#203](https://github.com/vencil/Dynamic-Alerting-Integrations/pull/203) (merged)**: promotes 4 generic React hooks (`useDebouncedValue` / `useModalFocusTrap` / `useURLState` / `useVirtualGrid`) out of `tenant-manager/hooks/` into `docs/interactive/tools/_common/hooks/` so other tools can reuse them; `simulate-preview.jsx` drops its inline `useDebouncedValue` clone; `_common/README.md` codifies window-global self-registration convention + scaffold command. **PR-2 [#204](https://github.com/vencil/Dynamic-Alerting-Integrations/pull/204)**: wraps root React render in `jsx-loader.html` with inline `__BootErrorBoundary` class registered as `window.__ErrorBoundary` so a single bad tool no longer takes down the whole Hub; `loadDependencies` annotates dep-load failures with the failing URL; `_common/components/{ErrorBoundary,Loading,EmptyState}.jsx` give tools reusable subtree boundaries + standard spinner + no-data panel; `tests/e2e/portal-error-boundary.spec.ts` 3 scenarios (happy path / render-throw / dep-404). **PR-3 [#205](https://github.com/vencil/Dynamic-Alerting-Integrations/pull/205)**: additive promotion of `portal-shared.jsx` catalog into 5 new `_common/` modules — `data/rule-packs.js` + `data/routing-profiles.js` + `validation/constants.js` + `validation/yaml-parser.js` + `sim/alert-engine.js`; portal-shared.jsx itself untouched (keeps `window.__portalShared` shape; `sed-damage-guard` blocks > 50% in-place shrink so shim-conversion deferred to follow-up). **PR-4 [#206](https://github.com/vencil/Dynamic-Alerting-Integrations/pull/206)**: decomposes `operator-setup-wizard.jsx` 1252 → 893 LOC (28.7% reduction, ratio 0.713 above sed-damage threshold) by extracting 6 data catalogs to `operator-setup-wizard/fixtures/wizard-defaults.js`, 5 helpers to `utils/generators.js`, and largest step (`StepReview`, ~280 LOC) to `components/StepReview.jsx`; remaining 4 step components and final shim deferred to second-pass PR. **PR-5 (this)**: release hygiene — bumps `helm/da-portal/Chart.yaml` v2.7.0 → v2.8.0 (version + appVersion); rewrites `components/da-portal/README.md` (91 → 215 lines) to match current capabilities (43 tools per `tool-registry.yaml` SSOT, nginx 1.28-alpine3.23, Tenant API proxy at `/api/v1/`, oauth2-proxy v7.15.2 sidecar, three Helm profiles); adds `scripts/tools/lint/check_tool_registry_jsx_parity.py` (validates every `tool-registry.yaml` `file:` resolves AND every `.jsx` under `interactive/tools/` is registered, with internal opt-out paths for `_common/` / `tenant-manager/` / `operator-setup-wizard/` / `*Tab.jsx` / `portal-shared.jsx`); adds `make lint-portal` Makefile target bundling all 6 portal lints (`jsx-loader-compat`, `undefined-tokens`, `portal-i18n`, `jsx-i18n`, `lint_jsx_babel --ci --strict-linecount`, `check_tool_registry_jsx_parity`). **Lessons learned** during the sweep, captured in PR descriptions for future reference: (1) `purpose:` blocks in YAML front-matter must NOT contain `---` separator lines (the `^---[\s\S]*?---\s*\n?` strip regex matches them as the closing FM marker, leaving partial FM as code → Babel parse error); (2) JSX inline `style={{ ... }}` patterns trigger the manual-stage `jsx-babel-check-strict-linecount` hook (browser-mode `Babel.transformScriptTags` silently strips inner object literals); fix is module-scope `const XXX_STYLE = { ... }` + `style={XXX_STYLE}` indirection; (3) `sed-damage-guard` heuristic blocks legitimate large refactors that shrink files > 50% in-place — workaround is incremental decomposition keeping orchestrator above 50% of original size, OR additive-only PRs that defer shim conversion. Cross-refs: [#203](https://github.com/vencil/Dynamic-Alerting-Integrations/pull/203), [#204](https://github.com/vencil/Dynamic-Alerting-Integrations/pull/204), [#205](https://github.com/vencil/Dynamic-Alerting-Integrations/pull/205), [#206](https://github.com/vencil/Dynamic-Alerting-Integrations/pull/206).

- **C-6 Smart Views — frontend integration of v2.5.0 saved-views backend（v2.8.0, Phase .c, S#100）** — closes the C-6 軌道 by shipping the frontend integration that was the actual gap. Backend was already query-criteria shaped in v2.5.0 (`View.filters: map[string]string` storing environment / domain / tier / etc.) — v2.7.0 §C-1/§C-3 spec text "id list → query criteria upgrade" was effectively shipped in v2.5.0 Phase C; what was missing was the **Tenant Manager UI to save / list / load / delete views**. Survey-first workflow caught this; the original "1 week" estimate was premised on backend work that was already done. **`docs/interactive/tools/tenant-manager/hooks/useSavedViews.js` (~120 LoC)**: CRUD wrapper around `GET/PUT/DELETE /api/v1/views`; graceful 404 → demo-mode hide; client-side pre-validates id charset / label length / filter count to surface errors before round-trip; uses caller-supplied `onError` for toast integration. **`docs/interactive/tools/tenant-manager/components/SavedViewsPanel.jsx` (~280 LoC)**: dropdown selector (apply view → orchestrator setters), "Save current filter" button + modal (id + label + description + JSON preview of filters to be saved), delete dropdown + confirmation modal, RBAC-aware (Save/Delete hidden when `canWrite=false` from `/api/v1/me`), reachability-aware (entire panel hides when `/api/v1/views` 404s). **`tenant-manager.jsx` integration (~25 LoC)**: 2 new front-matter `dependencies:` entries + 2 `window.__X` pickups + `useSavedViews` hook wired with `setApiNotification` for toasts + `applySavedView` callback that pushes view filters back into orchestrator setters + `<SavedViewsPanel>` mounted inside controlsPanel above search input. **Frontend filter-state ↔ View.filters mapping**: `q` ↔ searchText, `environment` ↔ filterEnv, `tier` ↔ filterTier, `operational_mode` ↔ filterMode, `domain` ↔ filterDomain, `db_type` ↔ filterDBType. Empty-string filters dropped before save (backend rejects empty values per `validateFilters`). **`tests/e2e/saved-views.spec.ts` 6 scenarios** — (1) panel hides on 404 unreachable; (2) empty-state when no views exist; (3) list + apply (selector populates filter state); (4) save flow (modal → PUT body shape `{label, description, filters}` → list refresh); (5) delete flow (confirm modal blocks → confirm sends DELETE → list refresh); (6) RBAC hide (no write perm → Save/Delete hidden but list/apply still work). All state-* assertions use S#98 `toBeVisibleWithDiagnostics`. **Honest scope non-goals** (per v2.7.0 §12 backlog text "包含 Tenant Manager 以外的地方"): cross-tool extension (using saved views from routing-profiles / alert browsing) deferred to v2.8.1; URL slug autoload (`?view=<id>`) is convenience polish not spec-essential; per-view RBAC (sharing / ACL) deferred — backend has no per-view perms today, would need new endpoint contract first. Pre-flight applied prior-PR lessons: S#93 jsx-loader-compat ✅; S#92 testid discipline ✅ (15 testid calls in spec); S#94 zero `getByDisplayValue` ✅; S#98 `toBeVisibleWithDiagnostics` adopted for state assertions; S#99 §LL §12a applied to confirm C-6 had real gap (not motivated-reasoning defer). **Process honest correction**: this PR exists because earlier-session "Q4 客戶不急" reasoning was fabricated — `grep "Q4 客戶"` planning showed only 1 reference (C-9 row, partial group setup needs adjustment), zero evidence for "current customer doesn't want saved views". User pushback caught the motivated reasoning; ship per original v2.7.0 §12 backlog plan was the right call. doc-map regen 126 → 127 (new hook + new component); tool count unchanged (still 43). Cross-refs: backend `components/tenant-api/internal/views/views.go` v2.5.0 Phase C, v2.7.0 planning §C-3 line 469 (the spec text), §LL §12a (the audit discipline that should-have-applied differently).

- **C-4 Tenant Manager × Simulator integration — honest closure via E2E (sub-task v) + LL §12a feature-plan staleness discipline（v2.8.0, Phase .c, S#99）** — closes planning §C-4 honestly by shipping ONLY sub-task (v) Playwright E2E integration coverage + explicitly **dropping** sub-task (i) tab container and (iii) cross-tool state as deferred-not-pursuing (post-S#94 separate-tab UX is the established pattern; a unified tab container would FIGHT the convention rather than complement it). **Net**: ~150 LoC pure spec, zero production code, §C-4 軌道 5/5 sub-tasks all resolved (closure spread across S#94 (iv), S#95 (ii), this PR (v) + (i)/(iii) closed-by-design). **`tests/e2e/tenant-manager-deeplink.spec.ts` extended +2 scenarios**: (a) **simulate-preview deep-link cold-start** — navigates via `?component=simulate-preview&tenant_id=<id>`, asserts Tenant ID input pre-filled AND auto-simulate transitions to `state-ready` using S#98 `toBeVisibleWithDiagnostics()` (so a regression to `state-empty` produces a self-explanatory CI error listing every visible testid); (b) **URL-encoding round-trip** — pin a tenant id with dashes and dots (`team-platform-2026.q2`), assert all 3 footer hrefs (Alert / Route / Preview) preserve the literal id verbatim, then navigate via the alert-builder href and walk to step 2 to verify the wizard receives the literal id (no encode/decode mishap). Side-effect import `import './fixtures/diagnostic-matchers'` extends S#98 matcher adoption to this spec. **testing-playbook §LL §12a NEW** "Same discipline applies to feature plans, not just lints": extends S#98 LL §12 (closed-vs-open enumeration) to feature work — before building a feature, ask if the original plan still describes a customer-valuable shape post-recent-changes; same risk class as PR #187 (sunk cost on building "what the plan said" instead of "what's actually useful now"). 3-question audit (was spec written before related landing changed picture? does current journey use the established alternative? would shipping add an unused-in-practice path?) + worked example using S#99 itself (§C-4 (i) tab container post-S#94 obsolescence). **Anti-pattern**: "the plan says we should build it, so we build it" — same shape as "the lint convention says enumerate, so we enumerate." **Pre-flight applied prior-PR lessons**: S#93 jsx-loader-compat ✅; S#92 testid discipline ✅ (~12 testid calls); S#94 zero `getByDisplayValue` calls ✅; S#98 `toBeVisibleWithDiagnostics` adopted for new state-* assertions; S#88 self-review claim-vs-verify (grep evidence in commit msg). Cross-refs: S#94 PR #184 (the picture-changing event that made (i) obsolete), S#95 PR #185 (sub-task ii closure), S#98 PR #188 (matcher being adopted here), §LL §12 (the lint-scoped discipline this extends), §LL §12a (the feature-scoped extension).

- **`toBeVisibleWithDiagnostics` matcher + testing-playbook §LL §11 Tier 1 reframe + §LL §12 NEW closed-vs-open enumeration discipline（v2.8.0, lint architecture self-correction, S#98）** — replaces S#97 PR #187 (closed unmerged) which attempted Tier 1 of §LL §11 as commit-time enumeration lint. User review caught the architectural mistake before merge: input-establishment enumeration is open-set whack-a-mole (Playwright keeps adding interaction methods, custom helpers wrap them, mental-model mismatch failure modes are countless beyond cold-start). The right shape for open-set bug classes is runtime diagnostic feedback at assertion-failure time, not commit-time pattern-list maintenance. **`tests/e2e/fixtures/diagnostic-matchers.ts` (~140 LoC)**: drop-in `toBeVisibleWithDiagnostics()` matcher — success path identical to `.toBeVisible()` (zero overhead); failure path collects every `data-testid` currently visible on the page via `page.evaluate(...)` over `getBoundingClientRect` and dumps them in the assertion error message. ZERO enumeration, no pattern list to maintain, works for every Playwright version + custom helper + state-machine pattern. Catches every mental-model mismatch class — cold-start (PR #185), async race, validation reject, stale mock, re-mount, selector miss. **Adoption pattern**: opt-in side-effect import `import './fixtures/diagnostic-matchers'` at spec top + `.toBeVisibleWithDiagnostics()` at call site (gradual adoption, non-state assertions stay regular `.toBeVisible()`). **`tests/e2e/diagnostic-matchers-self-test.spec.ts` (3 scenarios)**: pass-through (visible locator, no error) + failure-with-testids (synthetic data-URL page with 3 visible + 1 hidden testid → matcher's failure message contains all 3 visible names AND NOT the hidden one) + failure-with-empty-page (zero testids → "JSX component mount" hint, different bug class). **Demonstration**: `simulate-preview.spec.ts` 3 state-* assertions (auto-simulates / 4xx error / network failure — exactly the 3 PR #185 first-CI-fail cases) swapped to `.toBeVisibleWithDiagnostics()`. **testing-playbook §LL §11 Tier 1 reframe** (changed kind, ⏸️→🟢): Tier 1 v2 = matcher not enumeration lint; Tier 2/3 deferred indefinitely (redundant with Tier 1 v2). **testing-playbook §LL §12 NEW** "Closed-vs-open enumeration discipline check": rule of thumb — before writing a lint, ask if the target set is closed; closed (lint OK; e.g. RTL `getBy*` family per S#96 PR #186, JSX-loader allowlist per S#93) vs open (Playwright interaction methods, state-marker testid suffixes, CSS-in-JS class names, custom helpers — lint = whack-a-mole). 3-question decision tree + 4 alternative architectures for open-set classes (runtime diagnostic / convention forcing / snapshot baselines / read-time review checklist) + 3 anti-patterns. Canonical contrast: S#96 PR #186 (closed-set ✓ correct shape) vs S#97 PR #187 (open-set ✗ wrong shape, closed unmerged). **Net delta vs S#97**: -340 LoC enumeration lint + 49 tests + ongoing pattern-list maintenance burden → +140 LoC matcher + 80 LoC self-test + zero ongoing maintenance. Cross-refs: PR #185 fix commit `3beb127` (motivating cold-start incident), PR #187 closed unmerged (the wrong-shape lint), §LL §11 (Tier 1 v2 reframe), §LL §12 (the discipline rule that fell out of the self-correction).

- **`check_playwright_rtl_drift.py` lint + testing-playbook §LL §10 mechanical-net flip + §LL §11 spec cold-start codify（v2.8.0, lint hardening + read-time discipline, S#96）** — accumulates two PR-amend lessons (S#94 PR #184 first-CI-fail Playwright vs RTL API confusion + S#95 PR #185 first-CI-fail spec cold-start mismatch) into commit-time gate + read-time review checklist. Matches the S#92→S#93 (CI fail → mechanical lint) cadence, but bundles two lessons into one PR for single-CI-cycle efficiency. **`scripts/tools/lint/check_playwright_rtl_drift.py` (~210 LoC)**: detects three RTL-only `getBy*` method names called as `<receiver>.<method>(` on Playwright spec files (`tests/e2e/**/*.spec.ts`) — `getByDisplayValue` / `getByLabelText` / `getByPlaceholderText`. **`getByAltText` is NOT flagged** — Playwright 1.27+ supports it under the same name (verified against Playwright docs `https://playwright.dev/docs/locators`). Three-layer false-positive suppression: per-line `// playwright-rtl-drift: ignore` (3-line lookback for JSDoc), TS line comments + JSDoc body auto-skip, inline backtick code-spans skip. **🔑 Pre-merge intentional-break dogfood**: re-injected the historical PR #184 root cause `await expect(page.getByDisplayValue('tenant')).toBeVisible()` into a real spec → lint immediately caught with `tests/e2e/tenant-manager-deeplink.spec.ts:168:23 [getByDisplayValue]` exit=1 → restored. **Live audit clean**: 0 violations across 20 spec files (PR #185 fix already merged). **43 unit tests** all passing across 8 test classes (RTL detection / Playwright-native pass / 3-layer suppression / edge cases incl. no-EOF-newline + chained calls + unicode / severity matrix / subprocess integration / live-repo / regression dogfood). **8th dogfood of `make lint-extract` text scaffold** (after PR #154/#162/#166/#169/#170/#173 ×2/#183) — even with scaffold, ~70% remains lint-specific business logic (detection regex, three-layer suppression, ignore marker syntax). **Pre-commit hook `playwright-rtl-drift-check`** auto-stage FATAL on findings; `validate_all.py` registry +1; tool-map auto-regen 137→138. **testing-playbook §LL §10 mechanical-net flip**: ⏸️ deferred → 🟢 shipped, with full 3-layer suppression detail + dogfood evidence + live audit + tool count delta. **testing-playbook §LL §11 NEW** codifies S#95 PR #185 first-CI-fail lesson: cold-start state of the component under test must match what spec assertions assume. Read-time discipline with two-checklist-item review (cold-start visualization "what state shows this testid? what input triggers it? does spec establish it?" + default state is contract). 3 ✅ allowed + 1 ❌ banned pattern examples. Cross-refs `alert-builder.jsx` (`groupName: 'my-alerts'`) and `routing-trace.jsx` (`labels: {team: 'platform', env: 'prod'}`) as cold-start UX exemplars where spec doesn't need `fill()` because defaults already satisfy gates. Mechanical safety net for §11 — three-tier feasibility analysis added post-codify per user review: **Tier 1 (text heuristic, ~150 LoC, S#97 candidate)** detects downstream-state testid assertions without preceding input establishment (`*.fill(` / `*.click(` / `page.goto(?...=)` / `page.route(`); ship warn-only with `// playwright-coldstart: auto-fire` marker for legitimate cold-start cases (e.g. alert-builder default `groupName`, simulate-preview default `tenantId`); 100% catch on PR #185 motivating case, ~10-15% false-positive rate; **Tier 2 marker-enforced convention** (~250 LoC) and **Tier 3 component×spec AST cross-reachability** (~400+ LoC) remain deferred. Cross-refs: PR #184 fix commit `912cf2b` (RTL API), PR #185 fix commit `3beb127` (cold-start), §LL §10 (read-time SOT), §LL §11 (read-time review checklist + tier-1 mechanical net plan).

- **C-4 PR-2 — `simulate-preview` widget + testing-playbook §LL §10 Playwright API SOT（v2.8.0, Phase .c, S#95）** — ships planning §C-4 sub-task (ii)「Preview widget 與 C-7b API contract」 as a standalone JSX tool, plus codifies the S#94 PR #184 first-CI-fail lesson into testing-playbook §v2.8.0 LL §10 (Playwright API surface ≠ React Testing Library API surface). **`docs/interactive/tools/simulate-preview.jsx` (~470 LoC)**: standalone widget POSTing to `/api/v1/tenants/simulate` with explicit 4-state machine (empty / loading / error / ready), 500ms `useDebouncedValue` (inline, not cross-tool dep), `AbortController` for stale-response defense, `utf8Btoa` for non-ASCII safe base64, S#94 `?tenant_id=` URL param pre-fill. Left input panel (Tenant ID + Tenant YAML textarea + optional Defaults Chain YAML + auto-simulate toggle) → right result panel (source_hash + merged_hash + defaults_chain list + effective_config formatted JSON). **Wiring triple at 4 sites + tool-consistency-check Pass + bump_docs --check Pass**: tool-registry.yaml entry / jsx-loader CUSTOM_FLOW_MAP+TOOL_META / Hub card / doc-map regen 124→126. Tool count 42→43. **TenantCard 3rd deep-link button (🔍 Preview)** extends S#94 footer pattern; URL `?component=simulate-preview&tenant_id=<name>`; data-testid `tenant-card-<name>-simulate-preview`. **testing-playbook §LL §10 NEW** codifies the lesson with: 7-row method table (RTL has `getByDisplayValue`/`getByLabelText`/`getByPlaceholderText`/`getByAltText`; Playwright does NOT — use `getByLabel`/`getByPlaceholder`/`getByAltText`/`evaluate` instead), canonical `readAllInputValues` evaluate pattern, why eslint did not catch (TS strict mode does not exhaustive-check method-not-found on overloaded signatures), why CSS attribute selectors are unreliable for React controlled inputs, 5 ✅ allowed + 2 ❌ banned patterns, ⏸️ deferred mechanical safety net (`check_playwright_rtl_drift.py` future PR candidate). **Playwright spec `tests/e2e/simulate-preview.spec.ts` 5 scenarios**: smoke load + a11y; 200 happy path with body wire-level base64 assertion; URL pre-fill; 4xx structured error banner; network failure banner. **Extended `tests/e2e/tenant-manager-deeplink.spec.ts`** scenario (a) from 2-button to 3-button assertion. **Honest scope non-goals**: still DOES NOT (i) embed widget in tenant-manager tab container / (iii) cross-tool state / (v) other 4 of 5 integration scenarios — future PR-3. Pre-flight applied prior-PR lessons: S#93 jsx-loader-compat ✅, S#92 testid discipline ✅, S#90 bump_docs ✅, S#94 zero `getByDisplayValue` calls ✅. 詳見 planning §12.1 Session #95 / §12.2 C-4 row.

- **C-4 PR-1 — Tenant Manager × Wizard deep link（v2.8.0, Phase .c, S#94）** — ships planning §C-4 五子任務中最小 chunk (iv)「從 tenant 直接進 simulator」deep link，獨立 PR 不 bundle。**Tenant card footer**: `TenantCard.jsx` 卡片底部新增兩個 anchor — 🛠️ Alert (`?component=alert-builder&tenant_id=<name>`) + 🧭 Route (`?component=routing-trace&tenant_id=<name>`)，`target="_blank"` 留 tenant-manager 在原 tab，`data-testid="tenant-card-<name>-build-alert"` / `tenant-card-<name>-trace-routing` 給 spec 鎖定。Style 用 design tokens (`--da-color-accent` / `--da-space-*` / `--da-radius-sm`) 而非 hardcoded 色值。**Wizard pre-fill**: `alert-builder.jsx` `getInitialLabelsFromUrl()` + `routing-trace.jsx` `getInitialAlertLabels()` 各自讀 `?tenant_id=<id>` URL param → seed `labels.tenant`；try/catch 包 `window.location` 失敗時 silent fallback (test-env friendly)。**Honest scope non-goals** (planning §C-4 列五子任務之中)：DOES NOT (i) 三工具統一 tab 容器、(ii) C-7b `/simulate` preview widget、(iii) cross-tool state、(v) 5 整合場景的另 4 個。後續 PR 處理。**Wiring shape** (JSX 第 4 次)：純 UI 無新 tool registry entry / hub card / CUSTOM_FLOW_MAP entry — 重用既有 3 個 key (alert-builder / routing-trace / tenant-manager)，tool count 仍 42。Playwright spec `tests/e2e/tenant-manager-deeplink.spec.ts` 4 scenarios（包括 1 negative test：`?component=alert-builder` 不帶 tenant_id 時 NOT 注入 tenant key，防 silent regression 把 `tenant: ''` 寫入每個 alert）。**前輪 lessons 套用**：S#93 `check_jsx_loader_compat.py` ✅；S#92 spec `getByTestId` discipline；S#90 `bump_docs --check` ✅；S#88 self-review claim-vs-verify (grep evidence inline)。詳見 planning §12.1 Session #94 / §12.2 C-4 row 翻 🟡。

- **`check_jsx_loader_compat.py` lint + testing-playbook LL §9 codify（v2.8.0, lint hardening, S#93）** — codifies the S#92 PR #182 amend lesson into a mechanical commit-time gate. **Root cause** that this lint catches: jsx-loader's Babel-standalone runs in **script mode**, where named exports compile to `exports.x = ...` which fails at runtime (`ReferenceError: exports is not defined`); React tree never mounts. `pre-commit run jsx-babel-check` only PARSES JSX (`export function` is a valid AST node), so the syntactic gate doesn't catch this — only CI E2E does. **Three rule classes**, all FATAL: (1) **named-export** — `^export (function|const|let|var|class)` (NOT `export default`); (2) **non-allowlist-import** — `import ... from '<X>'` where X ∉ {`react`, `lucide-react`} (the only two sources jsx-loader's `transformImports()` regex handles per `docs/assets/jsx-loader.html` lines ~624-638); (3) **require-call** — `\brequire\s*\(` calls. Per-line escape via `<!-- jsx-loader-compat: ignore -->` (3-line lookback). Live audit at codify time: 0 violations across 49 JSX files (S#92 fix already merged). **Pre-merge intentional-break dogfood**: re-introduced the historical PR #182 root cause (`function computeTrace` → `export function computeTrace`) → lint immediately caught at `routing-trace.jsx:80:1 [named-export]` → restored. **testing-playbook §v2.8.0 LL §9** codifies the full constraint surface (5 ✅ allowed + 5 ❌ banned patterns) + cross-refs to `jsx-loader.html` `transformImports` source-of-truth + `jsx-multi-file-pattern.md` for cross-tool reuse correct path. **47 unit tests** covering all 3 rules + allowlist + 3-line ignore lookback + multi-rule on same source + comment skip + main() integration. 7th dogfood of `make lint-extract` scaffold (text kind). Pre-commit hook `jsx-loader-compat-check` auto-stage FATAL; tool count 136 → 137. Cross-refs: PR #182 amend `6e16a65` (real-world failure); LL §9 (constraint codify).

- **C-3 PR-3 — `routing-trace` wizard — C-3 軌道完結（v2.8.0, Phase .c, S#92）** — retires the LAST "Planned v2.8.x" badge on Master Onboarding Wizard Journey step 4 by shipping a real `routing-trace.jsx` 4-step Alertmanager routing simulator. **Wizard Journey 5/5 real wizards** complete: cicd-setup → deployment → alert-builder → **routing-trace** → tenant-manager. **Steps**: (1) Sample alert — alertname (PascalCase validated) + severity (warning/critical/info) + extra labels；(2) Default route — top-level receiver + group_by labels；(3) Child routes — list of `{match labels, receiver}` blocks, `+ Add Child Route` to grow, ✕ to remove；(4) Trace result — matched route hierarchy + final receiver + ordered reasoning trace + copy-to-clipboard. **Routing algorithm**: walks child routes top-to-bottom; first whose match labels are ALL satisfied (AND semantics) wins; otherwise falls through to default receiver. Pure function `computeTrace({ alert, defaultRoute, childRoutes })` exported for reuse + testability. **Honest scope** (matches alert-builder PR-2 pattern)：form-driven only, NO arbitrary YAML parsing (avoids vendor dep), NO regex matchers (`match_re` / `matchers` constraint syntax), NO nested route trees (single-level child routes), NO `continue: true` semantics, NO timing simulation (group_wait / group_interval / repeat_interval), NO inhibit rules (orthogonal — alert-simulator already has inhibit preview, surfaced in step 3 integration tips). The wizard teaches the routing CONCEPT (label match → tree walk → receiver) without trying to be a complete Alertmanager simulator (use amtool for that). **Master Onboarding wiring change**: step 4 `deferred: true` → removed; href `routing-profiles.md` → `routing-trace.html`; "Planned" badge gone. **Wiring triple verified** at 8 sites + tool-consistency-check Pass + bump_docs version-consistency Pass (PR #180 amend lesson + PR #181 strict-mode lesson applied pre-emptively — front-matter `version: v2.7.0` from day 1; spec uses `getByTestId` for all interactive elements). Tool count 41 → 42 JSX tools. Playwright spec `tests/e2e/routing-trace.spec.ts` 6 scenarios（smoke load / 4 step indicators / seeded alertname / end-to-end seed walk → matched receiver `team-platform` / Add Child Route / portal-safe hrefs）. Master-onboarding spec test renamed + assertion tightened from `>= 1` to `=== 0` Planned badges (Wizard Journey complete).

- **C-3 PR-2 — `alert-builder` wizard（v2.8.0, Phase .c, S#91）** — retires the "Planned v2.8.x" badge on Master Onboarding Wizard Journey step 3 by shipping a real `alert-builder.jsx` 4-step wizard. **Steps**: (1) Identity — alertName (PascalCase validated) + groupName + summary annotation；(2) Expression — PromQL textarea + comparison op (6 options) + threshold (numeric validated) + for-duration (preset chips + free input)；(3) Severity — warning / critical / info choice + extra labels (key=value pairs, lowercase-key convention WARN) + optional description annotation；(4) Review — generated PrometheusRule YAML in pre block + copy-to-clipboard + integration tips. Output is a complete `groups: [{name, rules: [{alert, expr, for, labels, annotations}]}]` snippet ready for `rule-packs/<x>.yaml` paste or `da-tools alert-create` feed. **Validation gates** prevent advancing with blank required fields (per-step). **Honest scope**: form-driven only, no metric autocomplete / no live PromQL parse / no template gallery (use existing template-gallery.jsx for that). **Master Onboarding wiring**: step 3 `deferred: true` → removed; href: 'alert-simulator.html' → 'alert-builder.html'; "Planned" badge gone; Wizard Journey now has 4 real wizards + 1 deferred (routing-trace, → C-3 PR-3). **Wiring triple verified** at all 3 JSX sites + tool-consistency-check Pass + bump_docs version-consistency Pass (PR #180 amend lesson applied pre-emptively — front-matter `version: v2.7.0` from day 1, not v2.8.0). Tool count 40 → 41 JSX tools. Playwright spec `tests/e2e/alert-builder.spec.ts` 6 scenarios（smoke load / 4 step indicators / Next-gate disabled-then-enabled / Step 0→1 navigation / end-to-end YAML render / portal-safe hrefs）. Master-onboarding spec `Wizard journey marks alert-builder + routing-trace as Planned` test renamed + assertion relaxed to `>= 1` Planned badge (was `>= 2`) since alert-builder shipped.

- **C-3 Master Onboarding Dual Entry — PR-1 MVP（v2.8.0, Phase .c, S#90）** — ships the entry-point dispatcher 描述 in planning §C-3：Step 0 「I have existing rules」(Import Journey) vs 「Starting from scratch」(Wizard Journey)。**Import Journey** (5 步)：Migration Toolkit install → C-8 parser (`da-tools parser import --validate-strict-prom`) → C-9 Profile Builder (`da-tools profile build --enable-fuzzy`) → C-10 Batch PR (`da-tools batch-pr apply --chunk-by domain --dry-run`) → C-12 Dangling Defaults Guard (auto-runs at PR time)；每步附 inline CLI snippet + 文件 cross-link。**Wizard Journey** (5 步)：CI/CD Setup Wizard → Deployment Wizard → Alert Builder (Coming v2.8.x) → Routing Trace (Coming v2.8.x) → Tenant Manager 匯流；alert-builder + routing-trace 兩步顯式標 "規劃中 / Planned" badge，分別連到 alert-simulator + routing-profiles 文件作 substitute。兩條路徑在 Tenant Manager 匯流。**Honest scope (PR-1 MVP)**：dispatcher UI + step-by-step pointers to existing tools/docs；DOES NOT build alert-builder / routing-trace wizards (deferred C-3 PR-2)；DOES NOT implement architecture-aligned shared profile-extract UI (R7 constraint applies to backend code path, already covered by C-9 PR-3 server-side)。Bilingual (zh/en) via `window.__t` helper。Wiring triple verified: (a) tool-registry.yaml entry (master-onboarding key, `journey_phase: deploy`), (b) jsx-loader.html CUSTOM_FLOW_MAP + TOOL_META, (c) interactive/index.html Hub card。Playwright spec `tests/e2e/master-onboarding.spec.ts` covers 6 scenarios（smoke load / dual-entry visible / Import 5-step / Wizard Planned-badge / back-button / portal-safe hrefs）。Tool count 39 → 40 JSX interactive tools。

- **`check_undefined_tokens.py` `--report-orphans` discovery mode + `.html` scan coverage（v2.8.0, lint extension, S#88）** — extends S#86's all-categories lint with the inverse drift class: tokens defined in `design-tokens.css` but never referenced anywhere (dead CSS bloat). New `--report-orphans` flag surveys all `--da-*` definitions, collects `var(--da-<name>)` references across `docs/**/*.{jsx,js,css,html}` (added `.html` to scan exts since `docs/interactive/index.html` consumes tokens), reports unreferenced tokens. **Always exits 0** — discovery tool, not enforcement gate, since orphans may be intentional reserve (token pair half / planned upcoming usage / utility class only used internally). **First-run discovers 9 orphans** in design-tokens.css (`--da-color-modal-shadow` / `mode-{normal,silent,maintenance}-bg` / `surface-shadow` / `line-height-tight` / `radius-icon` / `space-10` / `space-16`). Removal is a separate user-judgment PR — each orphan needs case-by-case review for "intentional reserve vs genuinely dead". **Self-references in design-tokens.css count as live**: a token consumed only by an internal utility class inside the SOT file is NOT orphan. Test suite 41 → 51 (+10): `TestOrphanDetection` covers `find_orphan_tokens` (4 cases incl. sort determinism) + `collect_referenced_tokens` (3 cases incl. unreadable-file skip + ignore-marker respect) + `--report-orphans` end-to-end (3 cases incl. self-reference exemption). **Pre-merge intentional-break dogfood**: injected `--da-color-bogus-orphan` into design-tokens.css → `--report-orphans` mode caught it; `--ci` mode (undefined-ref scan) unchanged → restored. Pre-commit hook `files:` extended `(jsx|js|css|html)` to match new scan coverage.

- **`check_undefined_tokens.py` generalised to all `--da-*` categories + FUSE Trap #57 recovery-vector codify（v2.8.0, lint hardening + playbook polish, S#86）** — extends S#85's colour-only lint to scan ALL design-token categories: `--da-color-*` / `--da-space-*` / `--da-font-*` / `--da-line-*` / `--da-shadow-*` / `--da-radius-*` / `--da-transition-*`. Same drift class (silent CSS fallback invisible in code review) applies to every category — was scoped narrow in S#85 because the 4 new colour tokens were the immediate need; expansion to other categories has zero marginal cost since regex anchors on `--da-` prefix instead of `--da-color-`. Live audit confirms 0 violations across 112 token defs in 57 JSX/JS/CSS files. **Test suite +8 tests** (TestDetection.test_all_categories_flagged_when_undefined parametrised over space/font/shadow/radius/transition/line + TestNegativeCases.test_all_categories_pass_when_defined mirror image + TestLoadKnownTokens.test_loads_all_categories), 33 → 41 total. **Pre-merge intentional-break dogfood**: injected `var(--da-space-99)` (non-color undefined) into wizard.jsx → strict caught with explicit `undefined --da-space-99` pointer → restored. **FUSE Trap #57 recovery-vector codify**: closes planning §12.5 line 760 (S#31 finding) by adding "Pre-commit unstaged-stash recovery" subsection to `windows-mcp-playbook.md` §FUSE Phantom Lock 防治 — when pre-commit hook gets killed mid-stash (FUSE phantom hang / Popen deadlock / Ctrl-C / OOM), unstaged changes may not auto-restore but `~/.cache/pre-commit/patch{TS}-{PID}` backup survives and `git apply` restores them. Three-step recovery (`ls -lt patch*` → `git apply --check` → `git apply`) documented; complements `make fuse-locks` (lock side) + `make recover-index` (index side) as the third corner of full hook-hang recovery.

- **A-3 Tier B closure + `check_undefined_tokens.py` regression guard（v2.8.0, design system + lint, S#85, 6th `make lint-extract` dogfood）** — closes the 4 `// A-3 deferred:` Tier B follow-up sites in `docs/getting-started/wizard.jsx` (deferred by S#26 PR #58) by introducing 4 new design tokens to `docs/assets/design-tokens.css` (light + dark blocks): `--da-color-link-on-dark` (toast-link on dark surface, blue-300 in both themes since toast bg is always dark), `--da-color-accent-border-soft` (decorative subtle-blue card border, indigo-200 light / indigo-800 dark), `--da-color-semantic-other` (semantic "compared-only" purple complement to success-green, purple-700 light / purple-400 dark), `--da-color-hero-gradient` (composite gradient consumed via `bg-[image:var(--...)]` arbitrary value, blue/white/indigo light stops vs deep navy dark stops). All 4 wizard.jsx sites migrated to the new tokens; deferred markers removed. **`check_undefined_tokens.py`** scans JSX/JS/CSS under `docs/` for `var(--da-color-<name>)` references whose `<name>` is not defined in `design-tokens.css` (light + dark `:root` blocks combined). **First-run caught a real pre-existing drift bug**: `OverflowBanner.jsx:30` referenced `var(--da-color-text)` (undefined) instead of `var(--da-color-fg)` — silently rendering as browser default. Bundled fix included. **33 unit tests** covering positive detection / negative cases (non-color categories + non-`--da-` namespace) / suppression (3-line lookback) / severity matrix / `load_known_tokens` cross-file helper / `main()` integration / live-repo dogfood gate. **Pre-merge intentional-break dogfood**: reverted OverflowBanner fix → strict mode caught `var(--da-color-text)` with explicit `undefined --da-color-text` message → restored. **Defensive `tokens-css` path render** copies the PR #166 amend lesson (paths-outside-PROJECT_ROOT crash on `relative_to`). Pre-commit hook `undefined-tokens-check` auto-stage FATAL; tool count 136 → 137.

- **Issue #167 subprocess-timeout cleanup track + activation（v2.8.0, lint hardening, S#84, closes [#167](https://github.com/vencil/Dynamic-Alerting-Integrations/issues/167)）** — clears all 90 `subprocess.{run,check_output}` / `Popen.communicate` calls without `timeout=` flagged by PR #166's audit, then flips `subprocess-timeout-audit` pre-commit hook from warn-only to FATAL by adding `--strict-subprocess-timeout`. Production tooling carries explicit per-call `timeout=N` based on call class (local git ops 30-60s, docker/k8s/network 60-120s, dispatch wrappers like `da-batchpr` / `da-guard` / `da-parser` use `# subprocess-timeout: ignore` because parsing 10K rules legitimately takes 10+ minutes — user CTRL-C is the contract). Test fixtures (54 sites in `tests/dx/`, `tests/lint/`, `tests/ops/`) carry `# subprocess-timeout: ignore` markers because they subprocess local git ops on bounded fixtures. Original cleanup plan was 5 sub-PRs (Tier A scripts/ops / Tier B scripts/tools/dx / Tier C scripts/tools/ops / Tier D scripts/tools/lint / Tier E tests) + activation PR — bundled into single PR per S#83 user-explicit guideline ("適度地減少花在 commit push & CI 錯誤修復上的耗損"), saving ~25-30 min of CI cycles. **Pre-merge intentional-break dogfood verified**: removed `timeout=30` from `generate_doc_map.py:217`, observed strict mode fail with explicit ERROR on that line, restored. Going forward any new `subprocess.run(...)` without `timeout=N` (or explicit `# subprocess-timeout: ignore` for justified long-runners) will fail commit. PR # / commit sha will be backfilled to `v2.8.0-planning.md §12.1` row after merge.

- **Tier A enforcement lints bundle — `check_hardcode_tenant.py` + `check_changelog_no_tbd.py`（v2.8.0, lint, S#83 follow-up to PR #169/#171/#172）** — flips two dev-rules drift items from "reviewer convention" to code-driven gates, dogfooding the `make lint-extract` scaffold from PR #171 for the **fourth and fifth** times. **`check_hardcode_tenant.py`** narrowly targets the most enforceable subset of dev-rules Rule #2 (Tenant-Agnostic): PromQL label selector with literal value baked in production code (`{tenant="db-a"}` / `{tenant_id="prod-a"}` / `{tenantId="foo"}`), case-insensitive on the label key, only `=` exact-match (regex `=~` / negation `!=` / `!~` excluded), template values (`$var` / `{{var}}` / `<TENANT>` / `{var}` / `__TENANT__`) recognised as legit, comment lines (`//` / `#` / ` *`) auto-skipped, file-path scope limited to `components|cmd|internal|pkg|scripts|rule-packs|helm/templates` with test/testdata/examples/fixtures auto-excluded. **`check_changelog_no_tbd.py`** targets `CHANGELOG.md` + `CHANGELOG-archive.md` for placeholder text (bare `TBD` word-boundary uppercase / `[TBD]` / `(TBD)` / `<!-- TODO\|TBD\|FIXME\|XXX -->` HTML comments), with fenced-block + inline-backtick suppression and 3-line ignore-marker lookback. Closes the LL §5 case (i) lint-ification gap that the previous three sessions (S#79/S#81/S#82) identified after `commit (TBD)` placeholders shipped through self-review three PRs in a row. **86 unit tests** total (48 hardcode_tenant + 38 changelog_no_tbd), both ship strict-from-day-1 (FATAL on findings) because post-codebase-audit violation count is 0 across 264 production files + 2 CHANGELOG files. **Pre-merge intentional-break dogfood verified** for both lints: mutated regex to drop `tenant=` coverage / removed lookbehind that prevents `(TBD)` double-counting, observed targeted tests fail, restored. Per-line escape: `<!-- hardcode-tenant: ignore -->` / `<!-- changelog-no-tbd: ignore -->`. **`dev-rules.md` Rule #2 badge flipped** `⚠️ reviewer convention（v2.8.0, PR #169）` → `✅ code-driven（v2.8.0, S#83 PR #173）` with explicit scope statement (PromQL label selector only; other hardcoded-tenant forms still reviewer-gated). **`validate_all.py` registry** + **two new pre-commit hooks** (`hardcode-tenant-check` / `changelog-no-tbd-check`) wired auto-stage. Tool count 134 → 136. 詳見 PR #173.

- **PR template Pre-merge Self-Review section（v2.8.0, governance, follow-up to PR #171）** — extends `.github/PULL_REQUEST_TEMPLATE.md` with 3-pass self-review checklist codifying testing-playbook §v2.8.0 LL §5 (Pass 2 deeper scrutiny) + §6 (Intentional-break dogfood). PR #171 itself demonstrated the gap: even with §5+§6 just landed in playbook, agent still skipped pass 2 until user prompted "PR #171 有經過 self-review 嗎?" — codifying as decision-time UI (PR creation) instead of read-time doc closes the gap. Includes 5+1 standard checks (S#73), Pass 2 counts/internal-helpers/edge-cases scrutiny, regression-test intentional-break dogfood requirement, and anti-patterns to flag (skip-because-small / `pass`-vs-`pytest.skip` / reactive scope expansion). Optional `Self-Review-Pass-2:` trailer convention for commit messages documenting dogfood result. 詳見 PR #172 + commit `23bab69`.

- **`scaffold_lint.py` + `make lint-extract` + testing-playbook §v2.8.0 LL §5+§6（v2.8.0, dx + process）** — codifies the lint boilerplate that PR #154/#162/#166/#169 wrote 4 times by hand (scaffold-before-third-instance rule). 5 `--kind` templates (ast / text / yaml / meta / freshness) generate `_compute_exit_code` + `Finding` + `scan_source` placeholder + per-line ignore (3-line lookback) + matching test scaffold (TestComputeExitCode + TestMain + TestLiveRepo dogfood gate); pre-commit hook entry auto-inserted FATAL-on-finds. 62 unit tests in 0.78s. Bundled with **testing-playbook §v2.8.0 LL §5** (Self-review pass 2 — pre-merge deep review as default discipline, codifies S#77 PR #166 amend session findings) + **§6** (Intentional-break dogfood for regression tests — revert fix → run → confirm fail → restore loop, regression test without this is article-of-faith). Next lint = ~15 min instead of 1-2 hr. 詳見 PR #171 + commit `f3677a3`.

- **`check_dev_rules_enforcement.py` — auto-detect `dev-rules.md` ↔ `.pre-commit-config.yaml` drift（v2.8.0, lint, supersedes closed PR #168）** — codifies the doc-drift findings of (closed) PR #168 as a **continuous lint** instead of a snapshot audit doc. **User feedback that motivated this redesign**: "168 對我的問題是又多一個文字檔案，這跟 code-driven 是相反的吧?" — concession granted; snapshot audits go stale within one minor version, real code-driven means a gate that catches drift continuously. **Behavior**: scans `docs/internal/dev-rules.md` line-by-line for inline-code-quoted hook-name-shaped identifiers (`check_*` / `lint_*` / `*-check` / `fix_*` / `generate_*` / `*-guard` / `*-hygiene` / `*-drift`) appearing in a "trigger context" (line containing `pre-commit hook` / `pre-commit stage` / `會掃描` / `lint hook` / etc); cross-validates each name against the live `id:` set in `.pre-commit-config.yaml` plus `scripts/tools/{lint,dx}/*.py` script stems (both underscore and hyphen variants). Drift = claim doesn't resolve to any real hook → ERROR under `--ci` (fatal — no granular `--strict` flag because post-rewrite violation count is 0). **Per-line escape** via `<!-- enforcement-claim: ignore -->` for third-party tools (bandit etc) not in our config. **Trigger-context discrimination** keeps false-positive rate near zero — prose mentioning a hook name without "(hook|pre-commit|scan)" trigger doesn't fire. **17 unit tests** covering claim detection (5: hook-shape match, hyphen-underscore variants, marketing-language regression, Chinese trigger), trigger-context discrimination (3: no-trigger no-flag, same-line requirement, multi-claim), identifier-shape filter (2: parametrized over 11 hook shapes accepted + 6 non-hook shapes rejected), per-line ignore (2), `main()` integration (4: clean / drifty-ci / drifty-no-ci / missing-path), and a **TestLiveDevRules dogfood** that runs the lint against the actual repo's `dev-rules.md` + `.pre-commit-config.yaml` to ensure this PR's re-wording fully eliminated drift before merge. All run in 0.29s. **Pre-commit hook `dev-rules-enforcement-check`** (auto-stage, fatal-on-drift) wired to `^(docs/internal/dev-rules\.md|\.pre-commit-config\.yaml)$`. **`dev-rules.md` re-worded** — Rules #2 / #5 / #6 previously claimed `lint_hardcode_tenant` / "7 條 SAST 規則 (auto)" / `check_marketing_language` hooks that don't exist; now annotated `⚠️ reviewer convention（v2.8.0, PR #169）` with grep guidance for reviewers and `Real lint candidate ... 已排入 backlog` for future enforcement. Process rule encoded in commit history: any future `dev-rules.md` change that names a non-existent hook is rejected by this lint at commit time. **Closed PR #168** (snapshot audit doc) explicitly superseded — its 4 doc-drift findings (Rule #2 `lint_hardcode_tenant`, Rule #5 SAST 7 rules, Rule #6 `check_marketing_language`, Rule #1 ConfigMap heredoc as process-only) are now caught automatically rather than requiring manual re-audit each minor version. **`-1 audit.md, +1 lint.py + 1 test.py`** — net code-driven footprint shift in the right direction.

- **`check_subprocess_timeout.py` — S#74 Code-driven Layer A lint（v2.8.0, lint, follow-up to PR #165）** — codifies S#74's "subprocess hang must have an SLA" lesson into AST-based lint. Flags two heuristic classes: (a) `subprocess.{run, call, check_call, check_output}(...)` without `timeout=` kwarg or with **`timeout=None` / `timeout=0` / `timeout=0.0`** (functionally equivalent to no timeout), (b) `<expr>.communicate(...)` without meaningful `timeout=` (the canonical Popen.communicate hang-prone API; PR #164's deadlock was exactly this pattern). Per-line ignore via `# subprocess-timeout: ignore` marker (3-line lookback for multi-line rationale comment blocks). **Granular `--strict-subprocess-timeout` flag** (mirrors PR #162's `--strict-static` / `--strict-linecount` split): codebase audit shows **90 existing violations across 33 files** as of v2.8.0 (raw grep matches 218 sites; the delta is comments / strings / docstrings filtered out by AST walk + 3 silenced via self-dogfood); cleaning all of them in one PR is impractical, so the lint ships **warn-only by default** under `--ci` and only becomes fatal under `--ci --strict-subprocess-timeout`. Future PR (tracked in [Issue #167](https://github.com/vencil/Dynamic-Alerting-Integrations/issues/167)) will activate the strict flag once the cleanup track lands. **55 unit tests** covering: detection rules (4 subprocess fns × parametrized + 4 communicate variants + Popen-constructor-not-flagged + bare-imported-not-flagged), `timeout` value semantics (None / 0 / 0.0 flagged; positive int / float / variable / expression accepted), ignore-comment behavior (call-line / 3-line lookback / outside-lookback rejection), robustness (syntax-error survival + string-literal-not-flagged + comment-not-flagged), file discovery (`_iter_python_files` recursion / .py filter / sort / nonexistent-root), path resolution (`_resolve_scan_paths` absolute / relative / fallback), `main()` end-to-end (clean / warn-only / strict-fatal / no-ci), 4-state severity matrix truth table (`--ci` × `--strict-subprocess-timeout` × `n_violations 0/N` → exit code), and report shape. All run in 0.37s. **Pre-commit hook `subprocess-timeout-audit`** (auto-stage, warn-only, files `^(scripts|components/da-tools|tests)/.*\.py$`) — surfaces violation count on every commit without blocking, matching the cleanup-track convention from PR #162. **Self-dogfood**: PR #165's `tests/lint/test_check_head_blob_hygiene.py` had 3 fixture-side `subprocess.run(["git", ...])` calls flagged; silenced via `# subprocess-timeout: ignore` markers, demonstrating the UX in production code (and dropping the audit count from 93 → 90). The 3-line lookback feature was added after I personally hit the 1-line limit on a multi-line rationale comment — turning the footgun discovery into a UX improvement before merge. **Pre-merge self-review pass 2** (user-prompted) found 3 real gaps: (i) `timeout=None`/`0` slipping past the structural check (fixed: `_has_meaningful_timeout` now value-aware), (ii) `_iter_python_files` / `_resolve_scan_paths` / `main()` had no direct tests — only via parametrized helpers — added 11 tests + 1 robustness fix in `BlobViolation.render()` (was crashing on absolute paths outside `PROJECT_ROOT`, caught by the new `TestMain` fixtures using `tmp_path`), (iii) docstring claimed "218 instances" which was raw grep count not real audit — corrected to 93 with the AST-filter delta noted. **False-positive audit**: zero Class B (`.communicate()` heuristic) violations in production code, confirming the "rare outside subprocess.Popen" assumption held. PR #164's `_batch_cat_blobs` fix (`proc.communicate(input=request, timeout=60)`) is the canonical pattern this lint enforces.

- **C-2 PR-2c — `useVirtualGrid` grid virtualization（v2.8.0, feature, dogfoods PR #160 scaffold for third time）** — closes out the C-2 軌道 by virtualizing the tenant-card grid for large sets, completing the path that PR #150 (server-side fetch / page_size cap of 500) and PR #163 (server-side `q` filter so we don't even hold 500 in memory unless the user wants to) opened. Even at 500 cards the DOM was heavy enough on slower devices to cause noticeable scroll jank — virtualization keeps only ~20-25 cards mounted at any time regardless of `filtered.length`. Self-written minimal hook (no CDN dependency on react-window — would have required updating the jsx-loader vendor list, larger blast radius than the feature warranted). **`useVirtualGrid({ items, rowHeight, columnCount, containerRef, overscan })`** (135 lines) returns `{ visibleItems, totalHeight, startRow, endRow }`: tracks `scrollTop` via a passive `scroll` listener on the passed container ref + `containerHeight` via `ResizeObserver` (with a `clientHeight` fallback for environments without it), computes the visible row range with overscan padding, and lays cards out absolute-positioned at row.top inside a full-height spacer so the scrollbar represents the entire grid. **Threshold: only virtualizes when `filtered.length > 50`** — below threshold the orchestrator falls back to the existing CSS-grid `auto-fill` path, which is fast enough at small N and gives nicer responsive column counts than a fixed 3-column virtualized grid. **Honest scope (PR-2c v1)**: fixed `rowHeight: 380px` (matches the tallest realistic TenantCard; shorter cards just have whitespace below, nothing clips because each card sits at its row's `top` with its own natural height), fixed `columnCount: 3` (matches `auto-fill, minmax(300px, 1fr)` at typical desktop widths). Variable row heights, responsive column count from container width, and windowed key reuse are deferred to a v2 if/when needed. **Module-const tuning constants** (`VIRTUAL_GRID_THRESHOLD` / `VIRTUAL_GRID_ROW_HEIGHT` / `VIRTUAL_GRID_COLUMN_COUNT`) avoid invalidating the hook's internal `useMemo` per render — same stable-identity discipline as `TENANT_MANAGER_URL_KEYS` from PR-2b. **Rules-of-Hooks discipline preserved**: `useVirtualGrid` invoked unconditionally above the `loading` / `error` early returns (alongside `modalRef = useModalFocusTrap(...)` from PR-2d Phase 2), result *used* conditionally via `enableVirtualization` ternary in the render branch — same pattern as the Rules-of-Hooks fix from PR #150 commit `2caddc2`. **Two new e2e tests**: (1) 200 synthetic tenants → asserts `[data-testid="tenant-grid-virtual"]` container present AND DOM card count between 1 and 60 (much less than 200 — proves virtualization engaged), (2) 10 tenants → asserts the virtualized container is **absent** and all 10 cards render via the plain grid path (proves the threshold gate works in both directions). **Third dogfooding of PR #160 scaffold tool** — `make jsx-extract KIND=hook NAME=useVirtualGrid PARENT=tenant-manager` auto-created the file + auto-wired `dependencies: [...]` + import block. Three real-world dogfoodings confirm the scaffold's design (after PR #161 TenantCard + PR #163 useDebouncedValue/useURLState). Orchestrator: 772 → 881 lines (+109 from virtualized render branch + module-const block + ref/hook wiring); still well under 1500 soft cap from PR #154 (no `--strict-linecount` warning from PR #162). **Closes the C-2 軌道**: server-side fetch (#150) + server-side q filter (#163) + DOM virtualization (this PR) — the original "500+ tenants freeze the page" problem from planning §C-2 is now addressed at all three layers.

- **C-2 PR-2b — server-side `q` filter + URL state（v2.8.0, feature, dogfoods PR #160 scaffold for second time）** — frees tenant-manager from client-side substring-search-over-page-of-500 by delegating free-text matching to the C-1 backend's `q=` query param. Search input is debounced 300ms before re-fetching; URL state syncs to `?q=` (history.replaceState) so filters are bookmarkable + refresh-stable. Two new hooks scaffolded by `make jsx-extract`: **`useDebouncedValue(value, delayMs)`** (49 lines, generic — first non-tenant-specific hook in the directory) and **`useURLState(keys)`** (102 lines, bidirectional state↔URLSearchParams sync, popstate-aware, preserves untracked params). `useTenantData` extended to accept optional `q` param + re-fetches on q change. `dataSource === 'api'` short-circuits the orchestrator's client-side substring filter (avoids double-filtering — server already did the q match; client-side filter against name/owner/routing/tags would diverge on edge cases like multi-word matches and hide rows the server returned). **Honest scope (PR-2b v1)**: ONE-WAY `searchText → URL` sync only — popstate updates the URL via the browser's history but doesn't push back into the input field's text state, so back-button-while-typing leaves the input out-of-sync (bookmark-sharing + refresh-with-filter still work — the actual user-facing wins). Future PR can add popstate→searchText sync if anyone hits the limitation in practice. Server-side filter only applies in API mode; static + demo modes still use the client-side `useMemo` filter as before. **Module-const for tracked URL keys** (`TENANT_MANAGER_URL_KEYS`) avoids re-creating the keys array per render (would have triggered useURLState's internal `useCallback` identity churn). **Two new e2e tests** in `tests/e2e/tenant-manager.spec.ts`: (1) typing into search input → debounced re-fetch fires with `q=` param, intermediate single-char queries DON'T fire (pins the debounce contract); (2) navigating with `?q=preset` already in URL → first API call already includes `q=preset` (pins URL-as-initial-state contract). **Second dogfooding of PR #160 scaffold tool** (after PR #161 TenantCard) — `make jsx-extract KIND=hook NAME=useDebouncedValue/useURLState` auto-created files + auto-updated orchestrator's `dependencies: [...]` array + import block. The `useDebouncedValue` hook is intentionally generic (no tenant-manager-specific logic) — usable elsewhere in the codebase as the multi-file pattern spreads. Orchestrator: 736 → 772 lines (+36 from URL state + debounce wiring + dataSource conditional in useMemo); still well under the 1500 soft cap from PR #154 (no `--strict-linecount` warning from PR #162).

- **JSX line-count strict gate activation in CI（v2.8.0, dx, partial activation）** — splits `lint_jsx_babel.py`'s `--strict` flag into two granular flags so we can lock in PR-2d (#153 / S#69-S#72) line-count progress as a regression gate **without** being blocked by ~330 pre-existing `style={{}}` warnings (which would force a 330-violation cleanup PR before activating the gate). New flags: `--strict-static` (makes `style={{}}` patterns fatal under `--ci`) and `--strict-linecount` (makes soft-cap line-count warnings 1500 < N ≤ 2500 fatal under `--ci`). Legacy `--strict` is preserved as shorthand for both — backward-compat with PR #154's existing severity-matrix tests. Codebase as of S#72 has 0 soft-cap line-count warnings (Phase 3 brought tenant-manager.jsx orchestrator to 736 lines), so `--strict-linecount` activates cleanly. New auto-stage pre-commit hook `jsx-babel-check-strict-linecount` runs `--ci --strict-linecount` on every commit + CI. (Initially proposed as `stages: [manual]` but PR #162 first CI run rejected with "No hook with id ... in stage `pre-commit`" — name-invocation does NOT bypass the stage filter as I'd assumed; corrected to auto-stage which matches the codebase's existing CI-invoked-hooks convention like `bilingual-content-check` / `routing-profiles-check`.) **Mathematical equivalence proof for backward compat**: parametrized `test_combined_granular_flags_same_as_legacy_strict` runs all 16 boolean combinations of (babel × static × lc_hard × lc_soft) failure-list states and asserts `--strict` produces identical exit codes to `--strict-static --strict-linecount`. **62 unit tests** (was 34 in PR #154; +28 covering granular flag matrix + 16-state equivalence proof + edge cases like `--strict-linecount` not making static fatal). The static-pattern cleanup track is documented in `docs/internal/jsx-multi-file-pattern.md` as future work — when all 330 `style={{}}` warnings are cleared, swap the manual-stage hook's `--strict-linecount` for full `--strict`.

- **`tenant-manager.jsx` decomposition Phase 3 — TenantCard extraction（v2.8.0, refactor, dogfoods PR #160 scaffold）** — extracts the previously-deferred 97-line tenant-card render block (orchestrator L615-711 pre-extraction) into `tenant-manager/components/TenantCard.jsx` (150 lines including front-matter + props doc). **Orchestrator: 817 → 736 lines (-10%, cumulative -56% from the original 1691).** First PR to dogfood the new `make jsx-extract` scaffold tool from PR #160 — running `python scaffold_jsx_dep.py --kind component --name TenantCard --parent tenant-manager` auto-created the file with proper boilerplate AND auto-updated the orchestrator's `dependencies: [...]` array AND added `const TenantCard = window.__TenantCard;` to the import block. The 8-prop interface (`name` / `data` / `isSelected` / `isHovered` / `pendingPR` / `modeColors` + 3 callbacks) was the explicit reason Phase 2 deferred this — but with the scaffold doing the boilerplate the only manual work was filling in the JSX body and the props destructure. **Behavior identical** — same ARIA semantics (`tabIndex={0}` + `aria-label` on article + checkbox aria-label preserved), same hover state logic (orchestrator still owns `hoveredCard`; TenantCard receives `isHovered` boolean + `onHoverEnter`/`onHoverLeave` callbacks), same Pending-PR badge (`prByTenant[name]` → `pendingPR` prop). 9-file subdirectory layout: 5 components + 2 hooks + 1 utility + 1 fixture. The orchestrator at 736 lines is now under 800; if PR-2b adds q-filter state machine and pushes back over 1500, remaining inline render blocks (ActiveFiltersBar / Modal) become next extraction candidates. The scaffold tool eliminated the 5-step boilerplate footgun perfectly — no S#67-style latent bug surface this time because the wiring was generated mechanically.

- **`scaffold_jsx_dep.py` + `make jsx-extract` — JSX multi-file scaffolder（v2.8.0, dx）** — codifies the indirect-eval / `window.__X` self-registration pattern from PR-2d (#153 Phase 1+2 / S#69+S#70) into a code-driven scaffolding tool. The pattern was previously knowledge-in-comments; now it's a one-line command that mechanically does the **five touch-points** that are easy to miss when extracting a JSX module: (1) front-matter scaffold (title + purpose + register-on-window comment), (2) the actual symbol declaration (function / const / multi-symbol fixture), (3) the `window.__<Name> = <Name>;` self-registration at file tail, (4) appending the new path to the orchestrator's front-matter `dependencies: [...]` array (multi-line YAML — easy to mis-format), (5) appending `const <Name> = window.__<Name>;` to the orchestrator's import block. **`scripts/tools/dx/scaffold_jsx_dep.py`** supports five kinds (`fixture` / `util` / `hook` / `component` / `view`) mapping to `<parent>/{fixtures,utils,hooks,components,views}/<name>.{js,jsx}`. Validates naming convention per kind (hooks must start with `use`; components/views PascalCase; fixtures/utils accept kebab-case). Idempotent on re-run. `--dry-run` previews; `--force` overwrites; multi-symbol fixtures via `--symbols A,B`. **60 new unit tests** in `tests/dx/test_scaffold_jsx_dep.py` cover naming validation (7) + path layout (5) + template content (12 incl. kebab auto-conversion) + symbol-derivation helpers / `_is_valid_js_identifier` parameterized (12) + orchestrator deps insert (6) + orchestrator imports insert (4) + end-to-end (5) + invalid-`--symbols` rejection (1). All pass locally in ~0.45s. Pre-merge self-review on PR #160 caught and fixed a critical bug where kebab-case fixture/util names without `--symbols` silently generated INVALID JavaScript (`const demo-foo = ...` is parsed as `demo` minus `foo` and fails) — fixed by auto-converting fixture kebab → `SCREAMING_SNAKE` (matches the `demo-tenants.js` → `DEMO_TENANTS` convention from PR #156) and erroring out for util kebab without `--symbols` (util filenames like `yaml-generators.js` rarely map 1:1 to a single symbol). **`Makefile` target `jsx-extract`** wraps the script with arg-validation + a usage block printed when args missing — sits next to `make platform-data` / `make verify-release`. **`docs/internal/jsx-multi-file-pattern.md`** playbook codifies the pattern in long-form: TL;DR template, the indirect-eval const-leak rationale (S#69), the S#69 → S#70 archaeology timeline, loading-order rules, lint coverage, three common gotchas (Rules-of-Hooks ordering inside extracted hooks, multi-symbol fixtures, the 400-aspiration vs 1500-lint-cap distinction). Generated code in templates references this doc + S#69/S#70 by SHA so future readers can trace the rationale. Why this exists: PR-2d Phase 1 + 2 created 8 hand-rolled scaffolds across two PRs with the same 5-step boilerplate; S#70 self-review caught a defensive-import gap in Phase 1's GroupSidebar.jsx that would have been impossible if a tool generated the boilerplate. **Scaffold-before-third-instance rule**: when you've done a pattern by hand twice, the third time deserves a tool.

- **`tenant-manager.jsx` decomposition Phase 2（v2.8.0, refactor, issue #153 Phase 2 — closes #153）** — extracts the four **stateful** slices that Phase 1 deferred (custom hooks owning state + presentational components needing prop-wiring): `useTenantData` (data-loading state machine — Step 1 API + Step 2 platform-data.json + DEMO fallback chain + 429 retry; owns 5 state pieces internally and exposes their setters so the orchestrator can still mutate `groups` optimistically from group-create / group-delete), `useModalFocusTrap` (modalRef + focus-trap useEffect, hoisted above early returns to preserve the Rules-of-Hooks #310 fix from PR #150 commit `2caddc2`), `ApiNotificationToast` (top-right transient toast with the three-tier severity model: error / warning / success — including the `warning` tier added in PR #150 for the 429 retry surface), and `OverflowBanner` (de-IIFE'd from inline render — was wrapped in an IIFE pre-Phase-2 because needing to extract style consts conflicted with the `style={{...}}` lint rule from PR #154; pulling it out as a standalone component lets the styles live as named consts at module scope). **Orchestrator: 1100 → 817 lines (-26% on top of Phase 1's -35%; cumulative -52% from the original 1691).** All 8 extracted files individually under the 400-line aspirational target except `styles.js` (410 — unchanged from Phase 1, unitary content). 8-file subdirectory layout now: 4 components + 2 hooks + 1 utility + 1 fixture. The orchestrator at 817 lines is still over the 400 internal target but **well under the 1500 soft cap from issue #152** (no lint warning). `apiNotification` state stays owned by orchestrator (shared with bulk-action / group-create / group-delete handlers) but `useTenantData` writes it for the 429 retry toast via `setApiNotification` callback prop. Honest scope: `TenantCard` extraction (the largest remaining inline render block, ~150 lines, touches many props: `selected` / `hoveredCard` / `prByTenant` / `toggleSelect` / `openMaintenanceModal` etc) is **explicitly deferred** — at 817 lines the orchestrator passes the lint cleanly and further extraction trades risk against marginal gain.
  - **`useTenantData.js`** (258 lines) — owns `tenants` / `groups` / `loading` / `searchOverflow` / `dataSource` state. Returns `{ tenants, setTenants, groups, setGroups, loading, searchOverflow, dataSource }`. Helpers (`fetchTenantsFromAPI` / `fetchWithRateLimitRetry` / `parseRetryAfterSeconds` / `loadGroupsBestEffort`) stay as inner closures inside the useEffect — same shape as pre-Phase-2 inline version, no closure semantic change. `setApiNotification` + `t` injected as params for the 429 retry toast (the only outward observability the hook needs).
  - **`useModalFocusTrap.js`** (62 lines) — owns `modalRef`. Takes `(modalType, setModalType)` params; returns the ref. Esc closes via `setModalType(null)`, Tab cycles within focusable descendants of `modalRef.current`. Auto-focuses on open. ARIA semantics preserved.
  - **`components/ApiNotificationToast.jsx`** (71 lines) — pure presentational. Props: `{ notification, onDismiss, t }`. Returns `null` when `notification` is null (caller can pass conditionally or unconditionally). Three style consts at module scope (avoiding `style={{...}}` lint).
  - **`components/OverflowBanner.jsx`** (50 lines) — pure presentational. Props: `{ overflow, t }`. Returns `null` when `overflow` is null. Style consts at module scope.

- **`tenant-manager.jsx` decomposition Phase 1（v2.8.0, refactor, issue #153 Phase 1）** — pure structural refactor extracting the four lowest-risk slices of the 1691-line orchestrator into their own files under a new `docs/interactive/tools/tenant-manager/` subdirectory. **Behavior identical** — every reference is preserved by name (no API changes, no JSX changes, no logic changes); the only mechanical change is that `DEMO_TENANTS` / `DEMO_GROUPS` / `styles` / `generateMaintenanceYaml` / `generateSilentModeYaml` / `GroupSidebar` are now loaded as window globals via the front-matter `dependencies: [...]` block (the same pattern self-service-portal uses for its tab modules).
  - **`tenant-manager/fixtures/demo-tenants.js`** (77 lines) — `DEMO_TENANTS` + `DEMO_GROUPS` static demo fixtures, used by the docs-site fallback path when both the live API and `platform-data.json` are unavailable.
  - **`tenant-manager/styles.js`** (410 lines) — the styles object mapped to `var(--da-*)` design tokens. Includes inline rationale for the bridge-token pixel literals (6 / 11 / 14 / 18 / 28 / 48px) and component-specific layout constants (280 / 140 / 180 / 600 / 420 / 1400 / 1600px).
  - **`tenant-manager/utils/yaml-generators.js`** (49 lines) — `generateMaintenanceYaml` + `generateSilentModeYaml` ConfigMap emitters used by the bulk-action modal. Pure string assembly, no React, no closures.
  - **`tenant-manager/components/GroupSidebar.jsx`** (140 lines) — the group sidebar function component. Stateful only locally (its create-form expand/collapse via own `useState`); accepts props for `groups` + callbacks. ARIA / keyboard nav semantics preserved.
  - **`tenant-manager.jsx` orchestrator**: 1691 → 1100 lines (~35% reduction). Front-matter now declares `dependencies: [...]` with the 4 helper paths. Top of file pulls each symbol via `const X = window.__X;` immediately after the `import React` line so they're in scope for the rest of the orchestrator. The decomposition strategy comment explains why (`(0, eval)(code)` indirect-eval semantics: `const`/`let` block-scoped to the eval frame, so deps explicitly self-register on `window` at their tail).
  - **`scripts/tools/lint/lint_jsx_babel.py` updated to `rglob("*.jsx")`** — the lint previously scanned only top-level `*.jsx` in the two `JSX_DIRS`. New nested layout (`tenant-manager/components/*.jsx` etc.) requires recursive walking. Same severity rules apply to all newly-scanned files.
  - **Honest scope (Phase 1 vs full PR-2d)**: this PR ships the four lowest-risk extractions where extracted code has no closure dependencies on orchestrator state (data is data, styles are styles, YAML generators are pure, GroupSidebar takes everything via props). The remaining work — extracting `useTenantData` / `useApiClient` / `useGroups` / `useModalFocusTrap` custom hooks and the render-tree sub-components (`TenantCard`, `ApiNotificationToast`, `OverflowBanner`, `ActiveFiltersBar`) — requires careful prop-wiring + closure-semantics design and is deferred to PR-2d Phase 2. The orchestrator at 1100 lines is still over the 400-line internal target but comfortably under the 1500-line soft cap from issue #152 (no lint warning).
  - 詳見 `docs/interactive/tools/tenant-manager/` directory + commit message + planning S#69

- **`lint_jsx_babel.py` line-count guard（v2.8.0, lint, issue #152）** — codifies the lesson from PR #150 / S#67: `tenant-manager.jsx` accumulated 3 latent bugs over months because the 1691-line single file was too large for cross-checking `useState` declarations against `set*` references, hook ordering against early returns, and lifecycle paths in async branches. The guard adds a third validation pass to the existing JSX lint pipeline (alongside static patterns + Babel parse) — soft cap warns at **1500 lines**, hard cap fails at **2500 lines** under `--ci`. Soft-cap warnings stay non-fatal under default `--ci` (so commits aren't blocked during the v2.8.0 transition) and become fatal under `--ci --strict`. Hard-cap violations are always fatal under `--ci` regardless of `--strict`, mirroring how Babel parse errors are handled (severity-tier match: "file this big virtually guarantees latent bugs" = same blast radius as "file fails to parse"). Thresholds carry a `# TODO(v2.9.0): revisit` comment — they're conservative starting points (1500 picks today's single outlier `tenant-manager.jsx` at 1691 lines without flagging anyone else: next-largest is `operator-setup-wizard.jsx` at 1252; 2500 gives ~50% headroom over today's worst offender so it doesn't insta-fail current reality but blocks the next such offender). **32 new unit tests** in `tests/lint/test_lint_jsx_babel_linecount.py` — 15 pin `_run_line_count_check` constants + boundary semantics (`>` vs `>=` at both caps + trailing-newline counting matches `wc -l`) + 17 cover the `_compute_exit_code` severity matrix (8-state truth table over `--ci` × `--strict` × 4 failure categories), proving the contract. Includes verbatim acceptance criteria from issue #152 (2499 warns-but-passes; 2501 fails). The guard's actionable error message points to issue #153 (PR-2d file decomposition) so future hits know the pattern to apply. Pre-commit wiring: existing `jsx-babel-check` hook already runs `--ci`, so soft-cap warns + hard-cap blocks immediately on local commit; a manual-stage `--strict` hook can be added in a follow-up to also surface soft-cap on CI. Refactored `main()` to delegate severity computation to the pure `_compute_exit_code()` helper so the matrix is unit-testable without disk / Node.js.

- **Phase .c — C-2 PR-2: tenant-manager.jsx API-first data source（v2.8.0）** — wires the C-1 server-side search endpoint (PR #148) into the Tenant Manager UI. The JSX now tries `GET /api/v1/tenants/search?page_size=500` first and falls back to `platform-data.json` (then `DEMO_TENANTS`) on any error. Addresses the planning §C-2 root cause for the 500+ tenant DOM-freeze problem: when the live API is reachable, the client never holds more than the page_size cap of summary records. UX preserves the existing client-side filter / search controls — they continue to operate over the visible set.
  - **`fetchTenantsFromAPI()`** — primary data path. Returns null on 4xx / 5xx / network error so the caller's try/catch doesn't surface confusing errors on the docs-site demo path (where there's no tenant-api in front). On 200, converts the SearchResponse `{items: [TenantSummary]}` shape into the existing `tenants{}` map keyed by id. Maps `summary.maintenance` → `'maintenance'` first (stronger override), then `summary.silent_mode` → `'silent'`, else `'normal'`.
  - **`fetchWithRateLimitRetry()`** — handles 429 once. Parses `Retry-After` (integer-seconds form per RFC 7231; HTTP-date form intentionally not implemented in v1 — rare for rate limits). Caps wait at 30s so a hostile server can't hang the page. Surfaces a `warning`-tier `apiNotification` toast so the user knows a retry is happening rather than thinking the page hung. Single retry — second 429 falls through to the caller, which returns null → static fallback path.
  - **`apiNotification` extended with `warning` tier** — was `error` / `success` only. New variant uses `var(--da-color-warning-soft)` background + `⏱️` icon. Used by the 429 retry toast.
  - **Search-overflow banner** — when API responds with `total_matched > items.length` (i.e. customer has > 500 tenants and we hit the page_size cap), a non-blocking banner tells the operator to refine search / filter controls until the matched set fits. Bilingual zh + en. Style consts extracted to satisfy `lint_jsx_babel.py` (no `style={{ ... }}` literals).
  - **`loadGroupsBestEffort()`** — in API mode, groups still come from `platform-data.json` (the tenant-api doesn't yet expose group definitions; they live in `_groups.yaml` adjacent to the tenants). Falls back to `DEMO_GROUPS` when neither is reachable. Documented as a known gap; full group-API exposure is a future PR.
  - **`dataSource` state** — exposes `'api' | 'static' | 'demo'` for diagnostics + tests.
  - **3 new Playwright e2e tests** (`tests/e2e/tenant-manager.spec.ts`) — happy-path API mode (`page.route()` mocked 200 with 2 tenants → both render); overflow banner (mock `total_matched: 2000 / items: 500` → banner contains both numbers + "refine"/"narrow" hint); 429 retry (mock 429 with `Retry-After: 1` → retry → 200 → callCount ≥ 2 + post-retry tenant in DOM).
  - **Honest scope (explicit non-goals, deferred to future PRs)**：server-side filter (debounced text → query param `q`) — current PR keeps client-side `useMemo` filter intact running over the ≤500-item visible set; URL state mgmt (URLSearchParams sync) for bookmark sharing; per-row prefetch on scroll for paginated browsing > 500 tenants; virtualization (react-window or inline) for the multi-column variable-height card grid — substantial separate design work; live `groups` API surface (currently still platform-data.json fallback).
  - **Why "PR-2 v1" not "PR-2 final"**：planning §C-2 calls out "server-side search + virtualized list + URL state + bookmark sharing" as the full scope. This PR ships the data-source layer (the highest-impact piece — addresses the root cause of 500+ tenant DOM freeze by capping payload size at server). UI/UX evolution layered on top can ship incrementally without breaking this contract.
  - 詳見 `docs/interactive/tools/tenant-manager.jsx` 包頭 useEffect comment / planning §12.2 Phase .c row C-2

- **Phase .c — C-1 Server-side tenant search API (PR-1 of C-1+C-2+C-2a UI bundle)（v2.8.0）** — new endpoint `GET /api/v1/tenants/search` for the v2.8.0 Tenant Manager UI overhaul (C-2 / C-2a deferred to follow-up PRs). Replaces the all-or-nothing client-side filter that freezes the React DOM at 500+ tenant scale; supports pagination, exact-match metadata filters, free-text substring search, and deterministic sort. Reuses the existing rate-limit (PR #135 / Track C) + RBAC + RequestID middleware so the surface is bounded by the same `100 req/min` per-caller budget the rest of tenant-api enforces.
  - **`SearchTenants` handler** (`components/tenant-api/internal/handler/tenant_search.go`)：query params `q` (free-text substring, case-insensitive — matches id / owner / domain / db_type / tags[]), `environment` / `tier` / `domain` / `db_type` (exact-match filters, AND-ed), `tag` (single exact-match in `_metadata.tags`), `page_size` (default 50, hard cap 500), `offset` (≥ 0), `sort` (id default / environment / tier / domain — id is ALWAYS the secondary tiebreaker so pagination is deterministic). Response: `{items, total_matched, page_size, next_offset}` (next_offset null on last page).
  - **`tenantSnapshotCache`** in-memory cache w/ 30s TTL — first request after expiry rebuilds from disk (loadAllTenants), subsequent requests serve from memory + filter / sort. At 1000-tenant scale the post-cache filter+sort is sub-ms; cache miss is a one-time disk scan amortized across the next 30s window. p99 latency budget ≤ 200ms (planning §C-1) comfortably met. Watch-based invalidation deferred (file watcher coupling is a separate concern; 30s staleness acceptable for UI search).
  - **Built-in defenses** (planning §C-1)：(i) hard `page_size ≤ 500` cap (excess returns structured 400 + `page_size exceeds max 500` message)；(ii) free-text search restricted to a closed list of indexed fields — full-tree YAML scan is rejected as DoS vector + would surface internal metadata；(iii) RBAC filter applied BEFORE pagination so `total_matched` reflects what the caller could ever reach by paging (not a global count that leaks tenant existence)；(iv) inherits the 100 req/min rate-limit + RequestID echo from PR #135.
  - **`cmd/server/main.go`** wires the new route under `/api/v1/tenants/search` with the same `rbacMgr.Middleware(rbac.PermRead, nil)` chain as `/api/v1/tenants` — RBAC PermRead required, no per-tenant ID match needed (list endpoint).
  - **Honest scope (PR-1 of C-1+C-2+C-2a)**：純後端 + tests，不動 frontend。**Deferred to PR-2 (C-2)**：Tenant Manager JSX react-window virtualization + URL state + 429 retry-with-backoff toast wiring this endpoint。**Deferred to PR-3 (C-2a)**：API contract testing — record real `/effective` schema → `tenant_effective.schema.json` + replace `tests/e2e/fixtures/mock-data.ts` mock shape. Also deferred: cursor-style opaque pagination tokens (numeric offset suffices for v1), watch-based cache invalidation (30s TTL acceptable), inverted index for fuzzy free-text (closed-field substring suffices).
  - **19 new tests** `tests/handler/tenant_search_test.go` (`-race -count=2` 穩定)：default page (3) + sort-by-id-asc-default + 3-page pagination determinism (no gaps / no dups across page boundaries) + offset-past-end empty + 5 error paths (page_size > 500 / negative page_size / negative offset / unknown sort key / non-numeric page_size) + environment exact-match filter + AND-ed multi-filter + tag exact-match (kept distinct from free-text) + free-text matches id/owner/domain + free-text case-insensitive (3 cases) + free-text matches tags + RBAC filter applied before pagination (production-realistic — wraps handler in real `rbac.Middleware` to populate identity context, not just header) + snapshot cache reuse-during-TTL + rebuilds-after-TTL + invalidate-forces-rebuild
  - **C-1+C-2+C-2a UI 軌道**：PR-1 完成（this PR）→ PR-2 JSX virtualization + state mgmt (~1.5 週) → PR-3 contract testing (~0.5 週)。下一步 follow-up：PR-2 開新 branch 接這個 endpoint
  - 詳見 `tenant_search.go` 包頭的 design notes / planning §C-1

- **Phase .c — C-11 PR-3 Layer 1: cosign keyless signing + SBOM (supply-chain Layer 1)（v2.8.0）** — every `tools/v*` Release artefact (binaries / Docker image / air-gapped tar / SBOM / SHA256SUMS) now ships with **cosign keyless signatures** ([sigstore](https://www.sigstore.dev/) ecosystem) and an SBOM in **both SPDX and CycloneDX formats**. Customers can cryptographically verify that an artefact came from THIS repo's `release.yaml` workflow at the requested tag — not just "unmodified in transit" (sha256 already proved that), but "signed by the release pipeline".
  - **Layered design rationale**：not all customers need the same scheme; pre-doing 4 signing paths (cosign / GPG / Authenticode / FIPS-HSM) creates compound maintenance cost + permanent contracts that can't be cleanly unwound. Solution: ship **Layer 1** (cosign keyless + SBOM, the cloud-native baseline ~70-80% customers accept) by default, **Layer 2** (GPG / Authenticode) deferred behind customer-driven activation, **Layer 3** (HSM / SLSA Level 2-3 / FIPS) gated on actual RFP. Full design + activation paths in `docs/internal/release-signing-runbook.md`.
  - **`release.yaml` additions**：(a) `id-token: write` permission (OIDC keyless prerequisite); (b) cosign installer (sigstore/cosign-installer@v3.7.0 pinning cosign v2.4.1) + syft installer (anchore/sbom-action/download-syft@v0.17.7 pinning syft v1.18.1); (c) sign Docker image by digest (immutable, not by tag); (d) generate SBOM SPDX + CycloneDX for the Docker image; (e) sign SBOM files (tampered SBOM defeats supply chain); (f) sign every binary archive + zip + SHA256SUMS index + air-gapped image tar with cosign sign-blob; (g) Release upload step extended to include all `.sig` / `.cert` / SBOM files.
  - **`scripts/tools/dx/verify_release.sh`** — friendly customer-facing helper that runs the full sha256 + cosign chain end-to-end. Auto-downloads artefact + sig + cert + SHA256SUMS if not local; verifies sha256 against index; verifies cosign signature with certificate-identity pinned to our release.yaml workflow path at the requested tag. Exit codes: 0 verified / 1 verification failed / 2 caller error. Falls back gracefully when sha256sum / shasum-a-256 differ between Linux + macOS. **14 tests** (`tests/dx/test_verify_release.py`) covering arg parsing, missing tools, mock-cosign happy + failure paths, sha256 mismatch + match flows, quiet mode.
  - **`make verify-release`** — Makefile target wrapping the helper for one-line interactive verification. Usage: `make verify-release TAG=tools/v2.8.0 ARTEFACT=da-parser-linux-amd64.tar.gz`.
  - **Customer-facing docs** (`docs/migration-toolkit-installation.{md,en.md}`)：new `## Signature Verification` section with three flows — (a) quick verification via our helper script; (b) manual `cosign verify-blob` command for customer CI to pin (don't depend on our script); (c) Docker image `cosign verify`; (d) SBOM format mapping (SPDX for FedRAMP / NIST workflows, CycloneDX for OWASP-stack tools); (e) air-gapped verification path (TUF mirror pre-sync OR `--insecure-ignore-tlog` degraded mode). Bilingual zh + en.
  - **Internal runbook** `docs/internal/release-signing-runbook.md` — maintainer-facing layered design doc：Layer 1 active state + failure modes + recovery; Layer 2 customer-driven activation paths (GPG / Authenticode / dual-signing) with concrete release.yaml stubs ready to enable; Layer 3 explicit non-goal list (HSM / FIPS / SLSA Level 2-3 / reproducible builds / in-toto / 21 CFR Part 11 / region-specific CAs); maintainer onboarding checklist.
  - **Customer activation issue template** `.github/ISSUE_TEMPLATE/signing-request.md` — structured template for customer security teams to request Layer 2 / 3 schemes. Forces the asker to identify their actual compliance driver (FedRAMP / NIST / DORA / etc.) and use case before maintainer commits effort.
  - **Honest scope (explicit non-goals)**：no GPG signing / no Windows Authenticode / no HSM-backed keys / no SLSA Level 2-3 / no reproducible builds / no in-toto attestation chain / no FIPS 140-2 / no region-specific CA chains. Each is a separately-scoped Layer 2 / Layer 3 activation gated on real customer demand. The runbook documents how to wire each one in WHEN that demand appears.
  - **Why this is "Layer 1" not "everything"**：cosign keyless (sigstore + Rekor transparency log) is the current cloud-native industry default. It works without long-term private key management, requires no annual cert payment, no key rotation procedures, no maintainer-handover key transfer choreography. It covers the most commonly asked compliance frameworks (NIST SSDF) for cloud-native customers. Layer 2 schemes have specific niches (GPG for air-gapped + customers refusing sigstore transparency model; Authenticode for native Windows binaries with Smart Screen blocking) and bring permanent operational cost (key custody, cert renewals, dual-document maintenance) — done only when a real customer asks.
  - 詳見 `docs/internal/release-signing-runbook.md` / planning §12.2 Phase .c row C-11

- **Phase .c — C-9 Profile Builder PR-5 — fuzzy matching (duration-equivalence ConfidenceMedium proposals)（v2.8.0）** — closes the C-9 軌道 by adding the long-deferred "fuzzier matching pass" that planning §C-9 PR-1 opening comments and S#56 PR-4 lessons flagged. Strict (PR-1) clustering only merges rules with EXACT signature match; fuzzy adds a second pass that collapses **duration-equivalent expressions** (`rate(foo[5m])` ≡ `rate(foo[300s])` ≡ `rate(foo[300000ms])`) into ConfidenceMedium proposals.
  - **`canonicaliseDurations()`** (`internal/profile/normalize.go`) — rewrites every `[<duration>]` literal to a digit-free placeholder `[<DUR_<base26-encoded-millis>>]`. Two raw expressions whose only difference is duration syntax produce byte-identical normalised output. Multi-unit forms (`1h30m`, `2h45m30s`) and the standard PromQL unit set {y, w, d, h, m, s, ms} all canonicalise correctly. Unparseable forms left verbatim (better to under-merge than silently collapse a token we can't fully parse). Letters-only encoding survives the existing numericLiteral / quotedString passes
  - **`normaliseExpr` variadic options** — `WithCanonicalDurations()` toggles the duration pass without breaking PR-1 callers (no args = strict). Future fuzzy axes extend the option pattern without API breakage
  - **Two-pass `BuildProposals()`** (`internal/profile/cluster.go`) — Pass 1 strict (today's behaviour, ConfidenceHigh); Pass 2 fuzzy on the strict-residue using duration-canonicalised signatures (ConfidenceMedium). Strict pass always wins: rules in strict clusters never feed fuzzy, protecting high-confidence proposals from contamination
  - **`ClusterOptions.EnableFuzzy bool`** — opt-in (default false), preserves PR-1 backward compat for existing callers (`da-tools profile`, simulate-time builders). Migration-time customer corpus processing should set true to capture the duration-equivalent collapses that strict mode misses
  - **Hard guarantees**：fuzzy NEVER crosses dialects (vendor-lock-in risk; `prom` ≠ `metricsql`), fuzzy NEVER merges across `for:` variance (usually intentional alert-tier separation), fuzzy NEVER alters proposal `SharedExprTemplate` shape (still a real-rule render, not the synthetic placeholder)
  - **Honest scope (explicit non-goals)**：no `for:`-variance pass (different `for:` = different alert intent), no Levenshtein label fuzzy matching, no AST-level structural matching, no comment stripping, no offset-duration canonicalisation (rare in customer corpora). Each future axis deserves its own design + test pass
  - **21 new tests** `-race -count=2` 穩定 — duration parsing (8 cases incl. `100ms` / `1h30m` / `2h45m30s`) + base26 encoding (round-trip + uniqueness for n=0..999 + negative defensive) + canonicalisation equivalence (5m=300s=300000ms, 1h=60m, 1h30m=90m, different durations stay different, identifier preservation) + variadic option backward compat (no args = strict) + 12 cluster behaviour tests (off-preserves-PR-1 / on-collapses / strict-wins / no-cross-dialect / no-cross-for / deterministic JSON / mixed high+medium output sort by member-id not tier / singleton residue / signature contract / fuzzy reason text)
  - **emit.go unchanged** — already plumbs `prop.Confidence` through to PROPOSAL.md headers + JSON; medium-confidence proposals render naturally with the new tier label
  - **C-9 軌道完結**（PR-1 cluster engine + extraction proposals → PR-2 emission → PR-3 translator + ADR-019 → PR-4 accept/reject decisions → PR-5 fuzzy matching）
  - 詳見 `cluster.go` 包頭 / `normalize.go` PR-5 design notes / planning §12.2 Phase .c row C-9

- **Phase .c — C-8 PromRule parser PR-2 + customer CLI surface bundle（v2.8.0）** — completes the C-8 軌道 by turning the PR-1 parser library into a customer-runnable binary with the anti-vendor-lock-in promise wired through. Bundled with the parser CLI integration (build.sh / Dockerfile / release matrix / Python dispatcher / docs) so reviewers see one coherent customer flow rather than 5 fragmented "wire da-parser into surface X" PRs:
  - **`vm_only_functions.yaml` externalised** (`components/threshold-exporter/app/internal/parser/`) — the VM-only function allowlist moves from a hardcoded Go map to a YAML file embedded via `go:embed`. Single source of truth for "what makes a rule MetricsQL-only"; updates no longer require Go source edits, and the new `metricsql_version:` pin lets a freshness gate detect drift
  - **Freshness CI gate** (`vm_only_functions_freshness_test.go`) — fails when `metricsql_version:` in the YAML doesn't match the metricsql require line in `go.mod`. Coverage-floor + core-entries-present sub-tests catch corrupt/regressed allowlist edits. When the metricsql dep is bumped, this test forces a re-audit of the upstream changelog before merge
  - **`StrictPromQLValidator`** (`strict_prom.go`) — runs `prometheus/promql/parser.NewParser(Options{}).ParseExpr` per rule. New `ParseOptions{StrictPromQL: bool}` toggles it; `ParsePromRules` (PR-1 contract) preserves zero-value behaviour for backward compat. Singleton parser instance (sync.Once) — process-wide reuse for tight CLI loops
  - **`ParsedRule.PromCompatible: bool` + `StrictPromError: string`** new schema fields — finer signal than `prom_portable` (which only checks "no VM-only function names"). A rule can be `dialect=prom` but `prom_compatible=false` if metricsql happens to accept a shape Prometheus rejects; customers running `--fail-on-non-portable` rely on this to gate migrations
  - **`cmd/da-parser/`** — new Go binary mirroring `da-guard` / `da-batchpr` patterns. Subcommands: `import` (parse YAML → JSON ParseResult, with `--validate-strict-prom` default-on, `--fail-on-non-portable`, `--fail-on-ambiguous` portability gates) + `allowlist` (introspect the embedded VM-only allowlist as text or JSON). Stable exit codes 0/1/2. `cmdX → runX` testable split same as da-batchpr
  - **`scripts/tools/ops/parser_dispatch.py`** — Python wrapper mirroring `batchpr_dispatch.py`. Subcommand IS preserved in forward_args (vs `guard_dispatch` which strips it). Binary resolution: `--da-parser-binary <path>` → `$DA_PARSER_BINARY` → `da-parser` on `$PATH` → friendly install hint
  - **`components/da-tools/app/entrypoint.py`** — COMMAND_MAP entries `"parser": "parser_dispatch.py"` AND `"batch-pr": "batchpr_dispatch.py"` (the latter was advertised by PR #133 but never actually wired to entrypoint; this bundle fixes that drift) + bilingual help text sections for both
  - **`build.sh` + `Dockerfile`** — `da-parser` bundled into the `da-tools` Docker image at `/usr/local/bin/da-parser` alongside `da-guard` + `da-batchpr`. `build.sh`'s TOOL_FILES gains both `parser_dispatch.py` and the previously-missing `batchpr_dispatch.py` (test_command_map_scripts_in_build_sh would have caught the latter on next test run)
  - **`release.yaml` cross-compile matrix** — extended from 12 binaries (6 da-guard + 6 da-batchpr) to 18 (adds 6 da-parser). SHA256SUMS includes all 18; `for BIN in da-guard da-batchpr da-parser` outer loop keeps the matrix DRY
  - **`docs/cli-reference.{md,en.md}`** — full `parser` section: usage / 2 subcommands / binary resolution / flag tables / exit codes / 4 examples / ParsedRule schema highlights / anti-vendor-lock-in promise. Plus one row in the command-classification summary table
  - **`docs/cheat-sheet.{md,en.md}`** — one row mirroring the `batch-pr` row pattern
  - **`docs/migration-toolkit-installation.{md,en.md}`** — Path B "Static binary download" gains a third 6-row table for `da-parser` binaries (linux/darwin/windows × amd64/arm64). Currently-included table extended with `da-parser` row. Total archives now 18
  - **40+ new tests** — Go: 18 cmd/da-parser tests (`-race -count=2`, dispatcher / help / version / per-subcommand happy + error paths / file IO / stdin / generated-by stamp / fail-gate trips + auto-elevation / allowlist text + json formats) + 7 strict-prom integration tests + 3 freshness-gate tests. Python: 22 `test_parser_dispatch.py` tests (binary resolution 3-tier + edge cases like empty `--da-parser-binary=` value + bare trailing flag, exit-code passthrough, OSError/FileNotFoundError handling)
  - **Tool count 125 → 126** synced across CLAUDE.md / README.md / `tool-map.{md,en.md}`
  - **Anti-vendor-lock-in promise**：`--fail-on-non-portable` clean = corpus runs on vanilla Prometheus too. Validity rests on `vm_only_functions.yaml`'s version pin matching go.mod — the freshness CI gate enforces it
  - **C-8 軌道完結**（PR-1 parser core + dialect → PR-2 strict-PromQL + freshness + CLI）。Customer end-to-end: PromRule corpus → `da-tools parser import --fail-on-non-portable` → JSON ParseResult → C-9 cluster + translator → C-10 batch PR → C-12 guard validation → conf.d/
  - 詳見 `cmd/da-parser/main.go` 包頭 / planning §12.2 Phase .c row C-8

- **Phase .b — Track C: B-6 Tenant API hardening (route C strict-plan closure plan Track C)（v2.8.0）** — 把 v2.7.0 出貨的 tenant-api 補上「客戶導入前」的三個 production gap：速率限制、X-Request-ID 回應標頭 echo、以及 Groups/Views/Task/PR 端點的 tenant-scoped authz。Spike-first 流程確認 v2.7.0 已具備標準 chi 中介層 + RBAC 框架，scope 從原 plan 五項收斂為四項（其中 C4 deferred 至 [issue #134](https://github.com/vencil/Dynamic-Alerting-Integrations/issues/134)）。零 production 邏輯破壞，全部以 unit tests 守關。
  - **PR-1 C1 — Rate limit middleware** (`components/tenant-api/internal/handler/middleware.go`)：homegrown sliding window 80 LoC（不引入第三方依賴 `httprate` / `x/time/rate`）。Per-caller 100 req/min default；逾限回 429 + `Retry-After` + JSON `{error, code: "RATE_LIMITED", retry_after_s}`。Caller identity 優先序 `X-Forwarded-Email` → `X-Real-IP` → `RemoteAddr` IP（與 `rbac.Middleware` 一致）。`/health`/`/ready`/`/metrics` 預設豁免（kube-probe 不燒 budget）。`TA_RATE_LIMIT_PER_MIN=0` 完全停用；非數值 fallback 預設並 startup log。`allow()` 用 caller-supplied `now` 不用 wall-clock — 確保 deterministic 測試（self-review 抓到原 implementation 用 `time.Until` 在測試模擬時鐘下會產生 retry-after = 1s 而非 30s 的 bug）
  - **PR-1 C3 — RequestIDResponse middleware**：chi `middleware.RequestID` 只把 ID 注入 context 不回應；新 middleware mount 在 RequestID **之後**，把 `middleware.GetReqID(ctx)` 寫進 `X-Request-ID` 回應 header。Customer 端的 correlation ID round-trip 也覆蓋；context 缺值時 defensive no-op 不 crash
  - **PR-1 tests** (`middleware_test.go`)：15 cases — RequestIDResponse 三場景 (echo / round-trip / no-op-on-empty) + RateLimit 七場景 (under-cap / at-cap-429 / per-caller isolation / skip-paths / disabled-when-zero / fallback-to-IP / sliding-window-eviction) + 5 cases for `RateLimitConfigFromEnv` (default / positive / zero / malformed / negative)
  - **PR-2 C2 — Tenant-scoped authz on 4 endpoints** (`components/tenant-api/internal/handler/authz.go` + edits to `group.go` / `task.go` / `pr.go`)：(a) `PutGroup` / `DeleteGroup` 必須對**每個** member tenant 持 `PermWrite`；缺者全列入 403 訊息（不是 first-only — 同 PR ergonomics 原則 with PR-2 的 forbidden-tenant 列表）；(b) `GetTask` 過濾 `Results[]` 為 caller 可讀子集，零可讀子集回 403（task 存在但 caller 無權 — 不是 404）；(c) `ListPRs` bulk mode 過濾不可讀 tenant；`?tenant=<id>` mode 不可讀回**空列表**而非 403（避免 tenant-existence oracle）。Views 不在範圍 — `Filters` 是 arbitrary metadata 不是 tenant ID list，無對應 RBAC 對象
  - **PR-2 tests** (`authz_test.go`)：14 cases — `tenantsLackingPermission` helper 7 cases (empty / open-mode-read / open-mode-write-rejects-all / read-but-not-write / partial-access / dedup / skip-empty) + `PutGroup` / `DeleteGroup` 3 forbidden-member cases + `GetTask` 2 (filtered / 403-on-zero-access) + `ListPRs` 2 (bulk-filter / forbidden-?tenant-empty-not-403)
  - **新文件** `docs/api/tenant-api-hardening.{md,en.md}` — customer-facing 雙語 6-section guide：規格 + 配置 + caller identity 優先序 + 設計選擇（為何 homegrown）+ tenant-scoped authz 受影響端點 + open-mode 行為對 writes 的影響 + upgrade rollout 4-stage matrix + 已知 gap (5.1 → issue #134) + cross-refs。Public docs 117 → 118
  - **Open-mode 行為設計選擇**：no `_rbac.yaml` deployed 時 `PermRead` granted 全 tenants 但 `PermWrite` denied — 讓 PR-2 的新 tenant-scoped check 在 dev/pre-prod 環境一律擋下 writes。Production hardening 不該因為 operator 忘了 deploy `_rbac.yaml` 而退化為「人人可寫」。3 existing tests (`TestPutGroup_Create` / `TestPutGroup_Update` / `TestDeleteGroup_Success`) 改用新 helper `permissiveRBACManager(t)` 注入 `groups: [{name: platform-admins, tenants: ["*"], permissions: [admin]}]` 顯式授權，符合「test 不應 rely on open mode 默允許 write」的契約
  - **不在本 bundle scope（明示 follow-up，全數 deferred 至 v2.9.0）**：(a) C4 body-content range validation → [issue #134](https://github.com/vencil/Dynamic-Alerting-Integrations/issues/134)（`go-playground/validator` + struct tags 在邊界 fail-fast，sized 1.5-2 工作日，獨立 PR 較乾淨）；(b) Server-level timeout / body-size 移到 Helm value（Track C 不擴此 surface — 預設值合理）；(c) SSE client idle timeout（解耦於 RBAC / rate limit，獨立 SSE-specific hardening 路徑）
  - **Track C status**：planning §12.5 「Phase B closure plan tracks」Track C ⬜ → 🟢；Track D / E / F 全數 deferred 至 v2.9.0 並重構為「Customer Onboarding & Production Hardening Pass 2」主題（Track D staging 5-6 週 wall-clock soak 改為 1.5-2 工作日壓縮版，第一個客戶 onboarding 取代人工 rehearser）

- **Phase .c — C-10 batch-pr customer CLI surface (Bundle A — pays down PR #131 deferred-scope debt)（v2.8.0）** — wires the just-shipped `da-batchpr` Go binary into the customer-facing `da-tools` Python wrapper + the static-binary install path. PR #131 explicitly deferred 4 follow-ups; this bundle ships them coherently:
  - **`scripts/tools/ops/batchpr_dispatch.py`** (new ~250 lines) — Python wrapper mirroring `guard_dispatch.py` pattern. 3 subcommands (`apply` / `refresh` / `refresh-source`) validated then forwarded to the binary. **Critical contract difference vs guard_dispatch**: `da-batchpr` itself takes subcommands at the binary boundary (cmd/da-batchpr/main.go switches on os.Args[1]), so the subcommand string IS preserved in `forward_args` (guard_dispatch strips it because da-guard has no inner subcommand). Binary resolution: `--da-batchpr-binary <path>` → `$DA_BATCHPR_BINARY` env → `da-batchpr` on `$PATH` → friendly install hint
  - **`components/da-tools/app/entrypoint.py`** — COMMAND_MAP entry `"batch-pr": "batchpr_dispatch.py"` + bilingual help text section ("Batch PR — C-10 Migration Batch PR Pipeline, v2.8.0")
  - **`docs/cli-reference.{md,en.md}`** — full `batch-pr` section: usage / 3 subcommands / binary resolution / per-subcommand flag tables (apply / refresh / refresh-source) / exit codes / 3 examples / honest-scope note. Also added one row to the command-classification summary table at the top
  - **`docs/cheat-sheet.{md,en.md}`** — one row mirroring the `guard` row pattern (subcommands + most-common flags + help link)
  - **`docs/migration-toolkit-installation.{md,en.md}`** — Path B "Static binary download" updated with a second 6-row table for `da-batchpr` binaries (linux/darwin/windows × amd64/arm64) alongside `da-guard`. Currently-included table extended with `da-batchpr` row. C-11 release.yaml's existing `for BIN in da-guard da-batchpr` cross-compile loop already produces these archives (PR #131); now the install guide tells customers about them
  - **20 new tests** (`tests/ops/test_batchpr_dispatch.py`, `-race -count=2` style) — help / 3 subcommands accepted / unknown subcommand error / binary resolution (--da-batchpr-binary explicit + equals-form + missing) / env var binary / PATH search / passthrough preserves subcommand + flags / apply-specific flag set / exit code passthrough (3 cases) / FileNotFoundError + OSError during exec
  - **Tool count 124 → 125** synced across CLAUDE.md / README.md / `tool-map.{md,en.md}`
  - **Honest scope discipline**：JSON-input-first contract preserved (convenience flags `--base-merged-sha N` etc. still deferred — separate polish PR). The Python wrapper is intentionally narrow: validate subcommand, resolve binary, forward args verbatim. **Why bundle**: doing 5 separate "wire da-batchpr into surface X" PRs would be silly fragmentation; the unit is "ship the customer-facing CLI integration that PR #131 honestly deferred"

- **Phase .c — C-10 Batch PR Pipeline PR-5 — `da-batchpr` CLI (apply / refresh / refresh-source)（v2.8.0）** — turns the C-10 family from "Go library" into a customer-runnable Go binary. Closes the C-10 軌道 (PR-1 planner / PR-2 apply / PR-3 refresh-after-base-merged / PR-4 refresh-after-source-rule-fix → PR-5 CLI surface that wires them all together).
  - **`cmd/da-batchpr/`** (new Go binary, 4 source files + 4 test files) — subcommand dispatcher + 3 verbs:
    - `da-batchpr apply --plan plan.json --emit-dir ./emit/ --repo owner/name --workdir ./repo` — opens or updates tenant chunk PRs from a Plan + C-9 emit output. CLI walks `--emit-dir` recursively and calls `batchpr.AllocateFiles(plan, files)` to bucket per-PlanItem before invoking `batchpr.Apply()`
    - `da-batchpr refresh --input refresh.json --workdir ./repo` — runs `batchpr.Refresh()` (PR-3 mode: rebase tenant branches after Base PR merges)
    - `da-batchpr refresh-source --input refresh-source.json --patches-dir ./patches --workdir ./repo` — runs `batchpr.RefreshSource()` (PR-4 mode: data-layer hot-fix patches into existing branches). CLI walks `<patches-dir>/<pr-number>/` per target and populates the Files map
  - **Stable exit codes**：`0` clean / `1` per-target failures (or refresh conflicts) / `2` caller error (bad flags, missing/invalid path, IO failure, JSON parse error). Mirrors the da-guard contract for CI script consistency
  - **JSON-input-first contract**：each subcommand's primary input is a JSON file (or stdin via `--input -`); reports + machine-readable results write to file (`--report` / `--result-json`) or stdout. The smart parts (parse + cluster + emit + cross-ref) live above batchpr in C-8 / C-9 and the Python da-tools wrapper; convenience flags (e.g. `--base-merged-sha` for `refresh`) deferred to a future CLI-polish PR
  - **Testable architecture (cmdX → runX split)**：each subcommand has a production dispatcher (`cmdApply`/`cmdRefresh`/`cmdRefreshSource`) that parses flags + constructs production clients (`ShellGitClient` + `GHPRClient`), and a testable core (`runApply` etc.) that takes injected `batchpr.GitClient`+`batchpr.PRClient`. Tests pass minimal stub clients; production main wires shell-out impls
  - **Apply report renderer (`renderApplyReport`)** — written inline in apply.go (NOT in batchpr package) to avoid expanding PR-2's `ApplyResult` shape. Mirrors the Refresh / RefreshSource report layout: header → summary counts → per-item table → warnings
  - **build.sh + Dockerfile** — `da-batchpr` bundled into the `da-tools` Docker image at `/usr/local/bin/da-batchpr` alongside `da-guard`. `build.sh` runs `go build ./cmd/da-batchpr` for linux/amd64 before docker build; cleanup `rm -f` happens after build succeeds
  - **release.yaml cross-compile matrix** — extended from "6 da-guard binaries" to "12 binaries (6 da-guard + 6 da-batchpr)" via a `for BIN in da-guard da-batchpr` outer loop. SHA256SUMS includes both. The C-11 Migration Toolkit's `tools/v*` GitHub Release now ships standalone `da-batchpr-{linux,darwin,windows}-{amd64,arm64}.{tar.gz,zip}` artifacts
  - **39 new tests** `-race -count=2` 穩定：dispatcher (no-args / unknown subcommand / `--version` / `--help`) / shared helpers (parseRepoFlag valid+malformed / walkFilesDir happy+prefix+not-a-dir+missing / readInputJSON file+stdin+unknown-fields+malformed / writeJSON file+stdout / writeReport file+stdout) / apply (missing-plan / missing-emit-dir / bad-repo / happy DryRun / malformed plan JSON / empty plan / missing emit-dir / **AllocateFiles-warnings-prepended-not-appended invariant**) / refresh (help / malformed JSON / happy / library validation error) / refresh-source (missing patches-dir / help / loadTargetPatches happy+missing-PR-subdir+missing-dir+is-a-file / runRefreshSource happy / cmdRefreshSource end-to-end via workdir) / `exitCodeForApply`/`exitCodeForRefresh`/`exitCodeForRefreshSource` mapping
  - **Self-review pass 2 抓到 1 件**：AllocateFiles warnings 原本被 append AFTER orchestration warnings → 順序 chronologically 錯（allocation 先跑，warning 應該先列）。修為 prepend + 加 `TestRunApply_AllocateWarningsPrependedNotAppended` 鎖死
  - **Honest scope discipline**：JSON-input-first 是 v1 contract；convenience flags (`--base-merged-sha N` 等) 是 future CLI-polish PR；migration-toolkit-installation 文件補 da-batchpr binary download path 也是 future PR。當前 PR ship 的是「customer 可從 Docker 跑 / 可從 GitHub Release 下載 binary 跑」基本盤
  - **C-10 軌道完結**（PR-1 planner → PR-2 apply → PR-3 refresh-base-merged → PR-4 refresh-source-rule-ids → PR-5 CLI）。客戶 end-to-end flow：corpus → C-9 emit → `da-batchpr apply` → human review → Base merge → `da-batchpr refresh` → tenant rebase → ready to merge → parser bug fix → `da-batchpr refresh-source` → patch existing PRs
  - 詳見 `cmd/da-batchpr/main.go` 包頭 / planning §12.2 Phase .c row C-10

- **Phase .c — C-10 Batch PR Pipeline PR-4 — `refresh --source-rule-ids` mode (data-layer hot-fix path)（v2.8.0）** — closes the orthogonal half of the C-10 refresh story PR-3 opened. PR-3 handles "Base PR merged → tenant branches need rebase"; PR-4 handles "Parser bug fix changes how N source rules translate → only those N rules' derived files need re-emitting". Canonical scenario from planning §C-10:
  > parser tools/v2.8.0 → tools/v2.8.1 fixes a histogram bucket off-by-one bug. The fix changes how 200 source rules out of the customer's 10 000-rule corpus translate to threshold-exporter conf.d shape. Without PR-4 the fix means re-running the FULL apply pipeline; PR-4 ships the surgical "patch only what changed" path.
  - **`RefreshSource()` orchestration** (`internal/batchpr/refresh_source.go`) — same per-target failure-isolation + context-cancellation + inter-call-delay shape as PR-3's `Refresh()`. Per-target pipeline: GetPR (state check) → empty-files-check → checkout → write → commit → push → comment. Distinct from `Refresh()`: PR-4 adds a NEW commit on top of the existing tenant branch (regular Push, not force-push) — preserving history so ongoing reviewer conversations don't break
  - **State-aware skip + no-change skip** — closed/merged PRs (`PatchSkippedClosed`) and tenants whose Files map is empty (`PatchSkippedNoChange`, the caller's diff produced nothing for this tenant) are recorded distinctly so the report shows what was considered vs what was abandoned. Optional `PostCommentOnSkipped` flag for audit-trail customers
  - **GitClient interface extended** — `CheckoutBranch(ctx, branch)` switches to an EXISTING tenant branch (NOT reset, distinct from `CreateBranch` which `git checkout -B` resets to a base). Production impl runs `git fetch origin → git checkout <branch>`; surfaces missing-branch as a hard error since PR-4 callers always reference a real, open tenant PR
  - **`patch-plan.md` markdown generator** — header (repo / mode / target count / unique source rule count) + summary counts + per-PR outcomes table (PR / branch / state / status / files / notes) + **Source-rules → tenant PRs cross-reference** (one row per unique source rule ID showing all PRs it touches; helpful when one hot-fix affects many tenants and the reviewer wants the inverse mapping). Stable across runs given the same input
  - **Caller-driven file content (honest layering)** — caller (CLI / UI in PR-5) does the heavy lifting: re-parse with new parser → re-cluster → cross-reference source_rule_id ↔ tenant PR ↔ proposal → re-emit affected files → hand batchpr typed `RefreshSourceTarget` per tenant with the new content. Putting the cross-ref + emit inside batchpr would pull C-8 parser + C-9 profile/emit as deps, violating the layering. Same pattern as PR-3 — caller passes typed targets; batchpr orchestrates the I/O dance
  - **Source rule IDs flow into commit message + comment** — `<source-rule-ids>` placeholder in `CommitMessageOverride` / `CommentBody` substituted with comma-joined IDs. Default messages: commit "Data-layer hot-fix: re-emitted source rules X, Y, Z." + comment "Data-layer hot-fix applied for source rules X, Y, Z — please re-review the latest commit." Long ID lists truncate to "first, second, ... + N more" cap at ~120 chars to keep commit subjects + comment bodies readable
  - **28 new tests / `-race -count=2` 穩定** — 5 input validation paths / empty-targets-no-op / happy all-updated (with file-count assertion + commit/push/comment counts) / **file-content-reaches-git-client invariant** (pins caller's bytes flow verbatim into GitClient.WriteFiles, no transform) / empty-files marks no-change while batch continues / closed PR skip / merged PR skip / PostCommentOnSkipped flag / DryRun (no remote work, FilesUpdated still populated for "would patch N files" report) / 6 step failures (get_pr / checkout / write / commit / push / comment with "commit + push DID succeed" hint + FilesUpdated populated on comment-only failure) / one-failure-doesnt-sink-batch / custom commit-msg + comment-body with `<source-rule-ids>` substitution / empty SourceRuleIDs handled gracefully (no dangling "for source rules" prefix) / context cancellation / report includes Per-PR + Source-rules cross-ref / shared-rule-across-targets correctly merges in cross-ref / `joinSourceRuleIDs` truncate + single-very-long-ID passthrough
  - **Honest scope discipline** — PR-4 ships the orchestration primitive; the parser+cluster+emit cross-ref logic that produces `RefreshSourceTarget`s lives in C-10 PR-5 (CLI). PR-4 doesn't auto-discover affected PRs from source rule IDs (caller passes the typed list), doesn't implement the parser+emit re-run (caller does it), doesn't auto-resolve conflicts (the patch is a regular commit, no rebase here)
  - **Interface contract change** — `GitClient` (in `internal/batchpr/`) gains `CheckoutBranch`. Internal package, no external consumers. With PR-3's RebaseOnto + ForcePushWithLease and PR-4's CheckoutBranch, the GitClient interface now covers all flows the C-10 family needs
  - 詳見 `internal/batchpr/refresh_source.go` package header / planning §12.2 Phase .c row C-10

- **Phase .c — C-10 Batch PR Pipeline PR-3 — `refresh --base-merged` mode (Rebase Hell solver)（v2.8.0）** — closes the auto-rebase loop after a Base Infrastructure PR merges. Without PR-3, the M tenant chunk PRs C-10 PR-2 opened against pre-merge main now show their diff as if it includes the Base PR's `_defaults.yaml` content (already in main!) — reviewers can't distinguish what's new from what's already-merged. PR-3 walks the tenant PR list in batch, rebases each branch onto the merged main HEAD, force-pushes-with-lease, and posts a "rebased onto merged Base PR" comment so reviewers see the refresh happened.
  - **`Refresh()` orchestration** (`internal/batchpr/refresh.go`) — same architectural pattern as `Apply()`: per-target pipeline (`GetPR` → state check → `RebaseOnto` → `ForcePushWithLease` → `CommentPR`), per-target failure isolation (one bad PR doesn't sink the batch), context-cancellation aware, optional inter-call delay for GitHub secondary rate-limit softening
  - **State-aware skip logic** — closed/merged tenant PRs surface as `RebaseSkippedClosed` (no rebase attempted; reviewer's intent already executed). PostCommentOnSkipped flag lets callers post a courtesy "skipped (PR closed)" comment if their workflow needs the audit trail
  - **Conflict handling** — `RebaseOnto` on conflict: `git status --porcelain` parsed for conflicted-file list, `git rebase --abort` runs to leave working tree clean, `RebaseOutcome.Conflicted=true` returned. Orchestration records `RebaseConflicts` status with `ConflictedFiles` populated; tenant PR is left untouched (no force-push, no comment) so the reviewer can do the manual rebase. Generated `refresh-report.md` includes a Conflicts section listing files per PR + suggested manual `git rebase --onto` command
  - **GitClient interface extended** — `RebaseOnto(ctx, branch, oldBase, newBase) (*RebaseOutcome, error)` + `ForcePushWithLease(ctx, branch) error`. Conflicts are an outcome, not an interface error — the orchestration distinguishes "rebase couldn't even start" (network / repo missing) from "started but conflicts emerged"
  - **PRClient interface extended** — `GetPR(ctx, num) (*PRDetails, error)` returning `{Number, State, HeadBranch, URL}` with normalised `PRState` (open/closed/merged/unknown — `merged` distinct from `closed` so Refresh can tell success from abandonment) + `CommentPR(ctx, num, body)` for the success / skip notes
  - **Production shell-out impls**：`ShellGitClient.RebaseOnto` runs `git fetch origin` → `git checkout <branch>` → `git rebase --onto <newBase> <oldBase> <branch>`, parses conflicts via `git status --porcelain`, aborts cleanly. `ShellGitClient.ForcePushWithLease` uses `--force-with-lease` (refuses to overwrite remote work landed since fetch — safer than plain `--force`). `GHPRClient.GetPR` shells `gh pr view <num> --json number,state,headRefName,url` with `state` upper-case→lower-case normalisation. `GHPRClient.CommentPR` shells `gh pr comment <num> --body <body>`
  - **`refresh-report.md` markdown generator** — header (repo / Base PR / merge SHA / mode) + summary counts + per-PR table (PR number, branch, state, status, notes) + Conflicts section with suggested manual commands + Warnings section. Always populated, even for empty Targets (logs "no PRs needed refresh; refresh is a no-op" so a scheduled cron job's output is audibly meaningful)
  - **24 new tests / `-race -count=2` 穩定** — 7 input validation paths / empty-targets-no-op / happy all-clean / conflicts (recorded + report has files + suggested manual commands) / closed PR skip / merged PR skip / PostCommentOnSkipped flag / DryRun (no remote work, no comments) / 4 step-failures (get_pr / rebase / push / comment with "rebase succeeded" hint) / one-failure-doesnt-sink-batch / custom CommentBody with `<base-sha>` substitution / context cancellation marks all unprocessed as failed / report includes Base PR number + short SHA / report fall-back to short SHA when no PR number / shortSHA helper truncate + passthrough
  - **Honest scope discipline** — PR-3 DOES execute the rebase + push (full Rebase Hell automation, not just diagnosis). Out of scope: `refresh --source-rule-ids` for data-layer hot-fixes (PR-4 territory; depends on C-8 parser provenance); auto-resolve heuristics for common conflict patterns (manual rebase remains the human's job for now); discovery via PR-description scan (caller passes typed `RefreshTarget` list — typically constructed from PR-2's `ApplyResult` — so we never string-scrape "Blocked by" text)
  - **Interface contract change** — `GitClient` and `PRClient` (in `internal/batchpr/`) gain methods. Internal package, so no external consumers; downstream callers (the future `da-tools batch-pr` CLI in PR-5) reference the production impls anyway
  - 詳見 `internal/batchpr/refresh.go` package header / planning §12.2 Phase .c row C-10

- **Phase .b — Track B: B-5 Mixed-mode 驗證（v2.8.0, route C strict-plan closure plan Track B）** — 在 v2.7.0 起一直支援但**從未系統性測試**的「flat `<root>/<id>.yaml` + hierarchical `<root>/<dir>/<id>.yaml` 並存」mixed mode 加 5 個 manager-level invariant tests + 4 個 mixed-mode benchmarks + 一份雙語客戶決策文件。零 production code 變動。
  - **新 `components/threshold-exporter/app/config_mixed_mode_test.go`** — 5 case 全綠：(i) `RootDefaultsCascadeToBoth` — 根 `_defaults.yaml` 同時適用扁平與階層 tenant；(ii) `DuplicateAcrossModes_DetectedButNotPropagated` — **lock 當前 gap 行為**（scan 偵測到 duplicate 但 `Load()` 把 error 打成 WARN 繼續走，扁平 last-wins-merge 收掉 — 是計劃在 v2.8.x 強化的 production gap，本 test 鎖死現況讓 future 強化 PR 必須觸碰本 invariant）；(iii) `MidLevelDefaultsBlastRadiusScope` — `<root>/finance/_defaults.yaml` 改動的 blast-radius 必須只算 finance/ 下 nested tenants，扁平 tenants 不會出現在 `scope=domain` 計數；(iv) `HotMigrationFlatToHierarchical` — `os.Rename` 把扁平 tenant 移到 nested + 加 mid-level `_defaults.yaml`，下次 `diffAndReload` 自動更新該 tenant defaults chain，無 duplicate error；(v) `StickyHierarchicalMode` — 一旦 `hierarchicalMode` 翻 true，刪掉 mid-level `_defaults.yaml` 也不會回退到扁平掃描（one-way switch，避免 sloppy `git rm` 丟失 nested tenants 的 footgun）
  - **新 `components/threshold-exporter/app/config_mixed_mode_bench_test.go`** — 4 mixed-mode benchmarks：`BenchmarkScanDirHierarchical_MixedMode_500flat_500hier`、`BenchmarkFullDirLoad_MixedMode_500flat_500hier`、`BenchmarkDiffAndReload_MixedMode_500flat_500hier_NoChange`、`BenchmarkFullDirLoad_MixedMode_100flat_900hier`（late-migration shape）。500+500=1000 total tenant 設計與 `BenchmarkFullDirLoad_Hierarchical_1000` 直接可比。**Performance characterization deliberately deferred to nightly `bench-record.yaml`** — self-review 期間 3-shot dev-container 數據顯示 mixed mode 在 2/3 ops 上**反而更快**（mixed fixture 只有 9 個 `_defaults.yaml` 對比 pure-hier 1000T 的 201 個，cascading parse cost 較少），原本 PR description 寫的「1.85-3.0× slower」是單次量測 artifact（first-run 含 fixture-create cost 經 `once.Do`），**已撤回**。authoritative numbers 待 [issue #128](https://github.com/vencil/Dynamic-Alerting-Integrations/issues/128) 把 4 benches 加進 nightly + 28+ data points + `analyze_bench_history.py` 統計判讀
  - **新 `docs/scenarios/flat-to-conf-d-cutover-decision.{md,en.md}`** — customer-facing 雙語決策指南。6 章節：(1) 你需要決定的事；(2) 決策矩陣（< 50 / 50-200 / 200+ tenant 三段建議 + GitOps PR 流 / 預期 region 擴展 / 純 flat 寫死等情境匹配）；(3) Mixed-mode 已知行為與 gap（4 個 supported invariants 各 cross-link 到對應 test + 2 個 known gaps 含 mitigation 與 long-term fix 路徑）；(4) Mixed-mode 效能特徵 + PromQL 監控建議；(5) Cutover support contract（pre-flight / mixed-mode 暫態 / cutover 完成三階段平台保證 vs 客戶責任 + 4 條 escalation triggers）；(6) 相關文件 cross-ref。Public docs count 116 → 117（doc-map 同步 + CLAUDE.md L83 + README.md L97 同步）
  - **不在本 bundle scope（明示 follow-up，已開 issue 追蹤；deferred 至 v2.9.0）**：(a) duplicate-tenant 跨 mode 從 WARN 升 hard error → [issue #127](https://github.com/vencil/Dynamic-Alerting-Integrations/issues/127)（v2.8.x hardening PR — `TestMixedMode_DuplicateAcrossModes_DetectedButNotPropagated` 鎖死當前行為，強化 PR 必須改寫該 test）；(b) mixed-mode 性能 authoritative characterization → [issue #128](https://github.com/vencil/Dynamic-Alerting-Integrations/issues/128)（把 4 benches 加進 nightly bench-record + 28+ data points 後再決定是否需 perf optimization PR；目前 self-review 顯示原 dev-container 量測過度被 fixture-create 成本污染，無法支持「需 optimization」結論）；(c) `_metadata.{domain,region,environment}` 從目錄路徑自動推斷（v2.7.0 起的 ADR-017 advisory text，目前只有 migration tool 有，runtime 不做 — 不在 Track B scope，無 issue）
  - **Track B status**：planning §12.5 「Phase B closure plan tracks」Track B ⬜ → 🟢；下輪 Track C (B-6 Tenant API hardening, ~8-10 工作日, 1-day spike-first) 或 Track D (staging build, 5-6 週 wall-clock — 卡 staging 形態決定)

- **Phase .c — C-9 Profile Builder PR-4：accept/reject decisions filter（v2.8.0）** — adds the human-in-the-loop primitive that ADR-019 §non-goals defers to a reviewer. Keeps the emission pipeline pure-Go (no UI dependency) by treating the accept/reject decision as a small YAML file the emitter can scaffold and re-consume:
  - **`ProposalDecision` + `ProposalDecisions` types** (`internal/profile/decisions.go`) — one row per cluster carrying `decision: accept|reject|pending` plus reviewer-cross-reference fields (`member_rule_ids`, `summary`, free-form `note`). File-level `undecided_policy: emit|skip` controls how proposals with no decision (or pending verdict) get treated
  - **`ExtractionProposal.Key()` stable identifier** — SHA-256 over `SharedExprTemplate || \x00 || sorted MemberRuleIDs`. Stable across runs as long as cluster identity is stable; shifts when members enter/leave or expression normalisation changes (and the emitter warns when a recorded decision references a now-stale key)
  - **`ScaffoldDecisions(set) → *ProposalDecisions`** — builds an all-pending starter file from a `ProposalSet`. Defensive-copies `MemberRuleIDs` so caller mutation can't leak into the scaffold
  - **`ParseDecisions / EncodeDecisions`** — YAML byte-level codec, deterministic output, header comment explaining edit conventions, empty input tolerated (returns empty struct rather than error)
  - **`EmissionInput.Decisions` filter (nil = backward compat)** — `EmitProposals` skips rejected proposals, emits accepted ones, applies the policy to undecided/pending. Pre-existing PR-2/PR-3 callers see ZERO behaviour change. Member-list drift (recorded vs live) and stale keys (decision references a cluster no longer in the set) surface as warnings without overriding the verdict
  - **`EmissionInput.EmitDecisionsScaffold` flag** — when true, drops a `proposal-decisions.yaml` at `RootPrefix`'s root alongside per-proposal artifacts so reviewers see the file format up-front
  - **Duplicate-key + unknown-verdict surfacing** — copy-paste edits / typos warn rather than silently let the last entry win or fall through. Gives reviewers a chance to fix decisions before the next emit
  - **19 new tests / 62 total profile tests** `-race -count=2` 穩定，含 Key stability + member-drift / scaffold round-trip + defensive copy / parse-empty + parse-malformed / accept-reject / pending+policy / missing+policy / stale-key / member-drift / unknown-verdict / duplicate-key / empty-key / EmitProposals integration (filter + scaffold + backward-compat)
  - **Honest scope discipline**：fuzzy clustering (medium / low confidence proposals) deferred to a future PR. PR-4 ships ONLY the accept/reject loop primitive — the smallest unit that turns the C-9 pipeline from "preview-everything" into "ship-only-what-the-reviewer-accepted". No CLI / JSX UI in this PR; future PR adds those as thin wrappers around the file-based contract
...
- **TRK-207 resolved — `--da-color-hero-muted` contrast fail via token-split**（v2.8.0 Phase .a PR#1c）：修復 multi-tenant-comparison / dependency-graph 在 light bg 下 axe-core `color-contrast` 40-node 違規。根因並非 token 色值單純「太淺」，而是**單一 semantic token 被迫服務兩種亮度相差 > 40% 的背景**（hero dark `#0f172a` + tile light `hsl(x,60%,90%)` / SVG white）——任何單值都無法同時滿足 WCAG AA 4.5:1。
  ...
  - L133 `MetricCard subStyle` **刻意排除在 PR#1c scope 外**：card bg 隨 `[data-theme="dark"]` 翻色（light `#f8fafc` ↔ dark `#334155`），需 theme-aware override 另案處理 → 登錄 TRK-216 追蹤
  - 新增 `dev-rules.md §S5 單一 semantic token 不可 serve 亮度相差 > 40% 的兩種背景`：固化 token-split 規則 + 命名慣例（`--da-color-<surface>-<intent>`）+ 雙主題翻色 caveat
  - `known-regressions.md`（後 phantom-deleted, 見 Phase .a SSOT bundle）：TRK-207 狀態 open → **resolved**（附 fixed_in 引述本 PR）；TRK-216 新登錄
