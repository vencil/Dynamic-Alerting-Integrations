...
- **`bench-gate-pr.yaml` 補上 applier 角色驗證 — 關閉「Ghost Green」race condition（v2.8.0, [issue #433](https://github.com/vencil/Dynamic-Alerting-Integrations/issues/433) W2 governance defense-in-depth, Gemini round-7 抓漏）** — `bench-override-audit.yaml` 是 label-applier 權限的主要 enforcement，但兩個 workflow 都在同一 `labeled` event 觸發、且 bench-gate preflight (~5s) 比 audit (~20s) 跑得快。如果 unauthorized 使用者 apply label，preflight 只憑 label 存在性就會 mark skip=true 並讓 workflow exit 0 綠燈；audit 雖然紅燈但 branch protection 若沒設「audit 必過」則 PR 可挾帶 bypass 入 main。修正：preflight 自己也用 `gh api /repos/{repo}/issues/{pr}/timeline` 查最近一次該 label 的 applier，再 `gh api /repos/{repo}/collaborators/{user}/permission` 驗角色，只有 admin/maintain 才認可 skip；其他角色 → 視為 label 不存在繼續跑 bench（audit 並行 revert label）。雙保險：任一防線單獨壞掉另一邊仍然撐住。
