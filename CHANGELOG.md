...
- **`jsx-i18n-check` hook 死分支 + 已自停用的 TOOL_META 檢查退役（同批 TRK-242 殘留）**：files filter 三分支中兩個指搬遷前舊路徑（`docs/interactive/tools/` / `docs/getting-started/`，已無 `.jsx`），且 lint 本體掃描常數同樣指舊路徑 → `.jsx` 的 `window.__t` 重複參數掃描自搬遷起 silent no-op；TOOL_META ↔ CUSTOM_FLOW_MAP 一致性檢查則因 TOOL_META 隨 TRK-230z 移除而恆 self-skip。修法：filter 與掃描路徑改指 `tools/portal/src/`，TOOL_META 檢查正式退役（該守備已由 `test_flows_e2e.py` + `sync_tool_registry.py` 承接，不重造重複 gate）；保留 `window.__t` 重複參數與 language-toggle 檢查。
