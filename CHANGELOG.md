...
- **RBAC metadata 範圍過濾補上 fail-closed 能力（#962 身分→權限 LD-6 前置，opt-in、預設 SHADOW＝零行為變化）**：以環境 / 域 metadata 過濾租戶清單時,「沒標記該 metadata 的租戶」過去在受限規則下一律**放行**(fail-open);現改為 mode-aware——預設 **SHADOW** 仍放行、但每次計入新指標 `tenant_api_scope_would_deny_total{axis="metadata"}` 供落單租戶稽核,operator 把 counter soak 到 0 後以 `--rbac-metadata-scope-enforce`(或 `TA_RBAC_METADATA_SCOPE_ENFORCE=1`)切成 **fail-closed**(沒標記就拒絕)。純 tenant-api Go;預設部署(seed 無 env/域 規則)逐位元組不變;沿用專案 audit→enforce 上線紀律,並鋪一條可復用的 shadow→enforce rail 供後續組織範圍授權使用。細節見 [#962](https://github.com/vencil/Dynamic-Alerting-Integrations/issues/962)。
