...
- **Federation read-path proxy Helm chart（ADR-020 IV-2a，issue [#506](https://github.com/vencil/Dynamic-Alerting-Integrations/issues/506)）**：新增 `helm/federation-proxy` chart — ADR-020 Layer 3 的租戶隔離 proxy。`proxy.engine` 二選一：`prom-label-proxy`（改寫 PromQL、把 `{tenant_id="<X>"}` 強制注入每個 selector 與 metadata API，用於 Prometheus / Thanos / VictoriaMetrics 單機）或 `vmauth`（VM cluster 的 accountID 路徑路由）。實作盤點修正了 ADR 的一個架構誤解 — vmauth 是 auth router、**不**解析 PromQL 也不注入 label，故單機 VM 改由 prom-label-proxy 承接。metadata API enforcement（`-enable-label-apis`）hardcode 不可 override；NetworkPolicy 預設限定只有 federation gateway 能連入（proxy 信任 gateway 設的 tenant header，跳層即破防）；HPA 依 CPU 70% 擴容。proxy image pin：prom-label-proxy `v0.13.0` / vmauth `v1.143.0`。audit log 因 prom-label-proxy 無原生支援，移交 gateway（IV-2b）。詳 [ADR-020](docs/adr/020-tenant-federation.md) §Blast radius Layer 3。
