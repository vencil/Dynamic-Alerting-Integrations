...
- **Bundle PR — `check_marketing_language.py` lint + testing-playbook §v2.8.0 LL §5/§6 process patterns（v2.8.0, lint+process, follow-up to PR #169 / S#74-S#77 lessons）** — bundled two low-dep / single-CI-cycle items per user direction "dependency 少的優先，bundle 起來，減少在 CI 的消耗". **(1) `check_marketing_language.py` (~250 LOC + 39 tests, 0.51s)** — codifies dev-rules.md §6 「推銷語言不進 repo」 from `⚠️ reviewer convention` to actual `pre-commit hook check_marketing_language`. **Curated zh+en keyword list** (high-precision: 業界領先 / 革命性 / industry-leading / revolutionary / game-changing 等); deliberately **excludes high-FP technical terms** (`the only` → "the only safe way" 是合法 technical claim; `unmatched` → "unmatched braces" 是技術 idiom; `premier` → 過於 noisy)，false-positive control 第一線靠 keyword 選擇而不是事後 ignore。**Suppression**: fenced code blocks (` ```...``` `) + inline backticks 自動排除 (illustrative quotes 不會 fire); per-line `<!-- marketing-language: ignore -->` marker (3-line lookback) for genuine anti-pattern examples (e.g. dev-rules.md §6 rule definition itself listing banned words). **Codebase audit**: post-rewrite **0 hits across 180 files** — ship strict (`--ci` fatal) from day 1。**Pre-commit hook `marketing-language-check`** wired to `^(README(\.en)?\.md|CHANGELOG\.md|docs/.*\.md)$`. **Self-dogfood**: `TestLiveDocs::test_live_repo_has_no_marketing_language` runs lint against actual repo before merge — gates PR from landing in broken state. **Cross-validated by PR #169's `check_dev_rules_enforcement.py`**: dev-rules.md Rule #6 從 `⚠️ reviewer convention` 升級為 `✅ enforced — check_marketing_language.py pre-commit hook` (real hook reference); lint 自動驗 hook name doesn't drift. **(2) testing-playbook §v2.8.0 LL §5+§6** — codifies 2 process patterns from S#77 (PR #166 amend session): **§5 "Self-review pass 2 (user-prompted)"** documents how second-pass deeper review抓到 pass 1 沒抓到的 3 real bugs + 1 robustness crash (timeout=None/0 slipping past structural check / `_iter_python_files` 沒直接 tests / `BlobViolation.render()` crash on absolute paths outside `PROJECT_ROOT`); pattern: pre-merge ship 前最後一輪深層 self-review 應該變 default discipline，"if production regresses, would this test catch it?" 自驗必跑。**§6 "Intentional-break dogfood"** documents the empirical regression-test verification loop (revert fix → run test → confirm fail → restore → re-run pass); critical finding: 沒 dogfood loop 的 regression test 是 article-of-faith — PR #166 原 headline test 用 300×1KB total 不會在 Windows runner 觸發 deadlock，1000×2KB 才可靠 trigger，**path count 比 total bytes 更關鍵**。Three-layer defense ladder codified: pytest-timeout marker / soft elapsed assert / inner `communicate(timeout=N)` — 任一被 reverted 還剩兩層守關。**Bundle rationale**: zero cross-dependencies between (1) and (2); both < 100 LOC core + tests; one CI cycle covers everything; lands 1 of 3 dev-rules drift annotations going from ⚠️ → ✅ (Rules #2 hardcode-tenant + #5 SAST bandit profile remain backlogged due to higher allowlist-design / cleanup-track cost).
