...
- **admin config（`_rbac.yaml` / `_domain_policy.yaml` / `_tenant_orgs.yaml`）pre-merge schema 驗證（Gemini #1056 外審 disposition 3b；hybrid lint = jsonschema 引擎 + Vibe wrapper）**：tenant-api 授權邊界 admin 檔以 strict（yaml KnownFields）解析——key typo（`permissons:` / `tenant_org:`）或壞 permission enum（`readonly`）在 runtime 是**載入錯誤**（startup fatal 或 hot-reload last-good 靜默掩蓋，即前一支 PR 補的 runtime 可見性面），過去**無 pre-merge 結構驗證**、壞檔可 merge 進 main、部署後才炸。新增 `docs/schemas/rbac.schema.json` + `tenant-orgs.schema.json`（鏡射 `internal/rbac` / `internal/tenantorg` 的 Go struct，SoT=struct、加欄時同步）+ 復用既有 `domain-policy.schema.json`，以新 wrapper `check_admin_config_schema.py`（jsonschema 引擎 + basename→schema 映射；`files:` 正則配對三檔於 repo **任何位置**、pass_filenames）驗證，pre-commit + CI Lint job 雙掛。**能表達的（shape / key-typo / enum）由 schema 擋；不能表達的 cross-field fail-closed 規則（空 `match: {}`、org-scope claim key 須宣告）仍由 parser 強制**——schema/wrapper docstring 誠實標明界線、不假裝涵蓋。缺席檔容忍（P4a 刻意未 ship seed `_tenant_orgs.yaml`）、空/註解檔為 benign（比照 rbac/tenantorg io.EOF）。純工具/schema（無 runtime 影響）。細節見 [#962](https://github.com/vencil/Dynamic-Alerting-Integrations/issues/962) LD-6 P4a fast-follow。
