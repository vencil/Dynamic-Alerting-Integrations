...
- **`bench-gate-pr.yaml` 並行 sharding 加速 — wall time 11 min → ~4-5 min（v2.8.0, issue [#433](https://github.com/vencil/Dynamic-Alerting-Integrations/issues/433) W2 後續優化）** — 在 PR [#435](https://github.com/vencil/Dynamic-Alerting-Integrations/pull/435) 初版（2 jobs：base + pr parallel）的基礎上，再把 bench 工作分 3 shards × 2 sides = **6 個並行 runner**。分片以 source-grep（`grep -rh "^func Benchmark"`）+ `(NR-1) % SHARDS` modulo partition 完成，~0.1s 列舉成本，**deterministic 且新增 bench 自動分配**不需手動維護 list。Compare job 收 6 個 artifact 後 `cat` 串接每側（Go bench 輸出格式 stack-safe）再 benchstat。統計效力**無變動** — 每個 benchmark 仍在單一 runner 跑 COUNT=6 樣本；sharding 切的是 benchmarks 不是 counts。CI 分鐘 +20%（每 shard 各付 ~30s setup-go + cache-restore 獨立 overhead），換 ~55% wall-time 節省。Tail-variance 風險：6 runners 比 2 runners 更易遇 noisy-neighbor 拖尾 — 若 FP rate >10%/2 weeks 則觸發 Larger Runners 升級（per rollout 設計）。`SHARDS_TOTAL` 為 workflow-level env var 方便調整，但 matrix 仍須手動同步（GitHub Actions matrix 不能直接讀 env）。
