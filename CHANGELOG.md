...
- **Phase .c — C-9 Profile Builder PR-3 — PromRule→threshold translator + ADR-019（v2.8.0）** — closes the C-9 emission gap that PR-2 honestly punted: PR-2 shipped intermediate-format artifacts (`shared_expr_template`, `dialect`, etc.) but the threshold-exporter runtime needs `defaults: {<metric_key>: <numeric>}` shape. PR-3 adds the bridge:
  ...
  - **ADR-019 written + linked** — [`docs/adr/019-profile-as-directory-default.md`](docs/adr/019-profile-as-directory-default.md) (zh + `.en.md`) pins: §1 Profile-as-Directory-Default (cluster median in `_defaults.yaml`, divergent overrides only) / §2 metric_key resolution order / §3 cluster aggregation rules / §4 operator handling / §5 status + fallback / §6 emission dispatch + 6 explicit non-goals. ADR README + index + cross-ref to ADR-017/018 updated zh + en
