...
- **L3 supply-chain digest verification at release time**（issue [#445](https://github.com/vencil/Dynamic-Alerting-Integrations/issues/445) AC iii / Chunk 2 of 5）：在 `.github/workflows/release.yaml` 4 個 release job（exporter / da-tools / portal / tenant-api）的 image push 步驟後加 `Verify image digest` step，透過共用 shell helper `scripts/ops/verify_release_digest.sh` 跑 `skopeo inspect --creds $GITHUB_ACTOR:$GITHUB_TOKEN docker://ghcr.io/vencil/<img>:v<TAG>` 取 digest，並寫進 GitHub Actions job summary 作審計軌跡。**Semantic**：對 Helm-chart-based 元件（threshold-exporter / da-portal / tenant-api）以 `Chart.yaml appVersion` 為 tag source（不是 chart `version:`），刻意容納 tenant-api 的合法 decoupling（chart 2.8.0 wraps appVersion 2.7.0）；對 da-tools（無 chart，VERSION file 為 SoT）直接以 `$VERSION` 為 tag source。**Catches**：(a) 上游 `docker/build-push-action` silent push 失敗（轉移性 GHCR 503 / manifest list inconsistency）；(b) Chart.yaml appVersion claim 對應的 image 不存在於 registry（chart 宣稱 v9.0.0 binary 但從未 build）；(c) GHCR 短暫 outage。**順道補 release-portal 缺漏的 Chart.yaml-vs-tag version-check**（其他 3 job 都有，唯 portal 漏掉）。Auth 走 same-registry `GITHUB_TOKEN`（per AC 不用 OIDC，已 `packages: write` permission inherited）。skopeo 用 `sudo apt-get install` 在 ubuntu-latest runner 上裝（idempotent）。Helper script 3 個 exit code（1=arg error / 2=image not found or skopeo failed / 3=env misconfig）讓 CI failure log 直接讀懂。詳：[`scripts/ops/verify_release_digest.sh`](scripts/ops/verify_release_digest.sh) 內 docstring。
...
- **Backlog frontmatter migration — dx-tooling-backlog**（issue [#379](https://github.com/vencil/Dynamic-Alerting-Integrations/issues/379) chunk 3）：18 個 `### TRK-001` ~ `TRK-018` entries 加 ADR-020 frontmatter spec yaml 區塊（`id` / `tracking_kind: dx` / `status` / `domain` / `created_at` / `updated_at`）。Default `status: proposed`，TRK-006 / -010 / -011 個別覆蓋為 `in-progress` / `done` 反映實際進度。`docs/internal/planning-index.md` 從 1 entry (ADR-021) 擴張為 19 entries (`in-progress 2 / done 1 / proposed 16`)。順帶修 chunk 2a `SECTION_YAML_RE` 的 multi-line heading bug：`(?P<heading>.+?)` with `re.DOTALL` 能跨多 H2/H3 backtrack 撈出整段 prose 當 title；改 `(?P<heading>[^\n]+?)` 限制單行。新增 regression test `test_heading_does_not_span_multiple_sections` pin 此 contract。Follow-up：其餘 backlog files（`frontend-quality-backlog.md` 等）migration 待後續 PR 處理。
...
- **`da-tools profile build`**：cluster 相似 rules → median 演算法決定 cluster 共通閾值 → 寫 `_defaults.yaml`、偏離 tenant 寫 `<id>.yaml` 只含 override；fuzzy matching opt-in 套 duration-equivalence canonicalisation（`[5m]` ≡ `[300s]` ≡ `[300000ms]`）；遵循 [ADR-019](docs/adr/019-profile-as-directory-default.md) Profile-as-Directory-Default
...
### ADR 新增（ADR-019 / 020 / 021，3 條）

- **ADR-019** Profile-as-Directory-Default：`_defaults.yaml` 為 cluster 共通閾值的權威位置
- **ADR-020** Planning SSOT：`TRK-NNN` namespace 統一政策（Option C refined hybrid）
- **ADR-021** Tenant Federation outline（v2.9.0+ design seed）
