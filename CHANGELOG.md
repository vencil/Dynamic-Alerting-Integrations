...
- **AI-agent 長時驗證可觀測性協議（`vibe-subagent-review` skill 擴充 + `make agent-progress`，內部 DX）**：codify 長時（>15 分鐘）reviewer／verifier subagent 的觀測慣例——多階段 verify 一律 `Workflow` 編排（`phase()`／`log()` 原生串流到 live view）；raw 背景 `Agent` 例外時 spawn prompt 強制內嵌 append-only `dev/<scope>/PROGRESS.jsonl` 里程碑 ledger 契約（`gate-mapped`→`repro-built`→`repro-ran`→`verdict`，卡住 >5 分鐘寫 `blocked`）；單 agent 範圍上限 ~15 分鐘、超過拆 staged agents。新增 `make agent-progress SCOPE=dev/<scope>` 一鍵 tail ledger。動機：security-audit fix-重驗實測，兩個背景 verifier 各跑 46–71 分鐘完成前零訊號、其一卡在過度優化 ~71 分鐘無從止損。（協議全文見 `.claude/skills/vibe-subagent-review/SKILL.md`；`vibe-security-audit` fix-重驗流程同步引用。）
