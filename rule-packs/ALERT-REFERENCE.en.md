...
| ContainerOOMKilledCritical | critical | [{{ $labels.tier }}] {{ $labels.tenant }}: OOM/restart loop  | Check alert metrics and review related logs; contact platform team for assistance if needed | rule_pack_kubernetes:container_oom_killed_critical:core |
