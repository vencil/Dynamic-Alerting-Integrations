...
| MariaDBHighCPU | warning | [{{ $labels.tier }}] {{ $labels.tenant }}: CPU threshold breached  | Check alert metrics and review related logs; contact platform team for assistance if needed | tenant:mysql_threads_running:avg1m |
| MariaDBHighCPUCritical | critical | [{{ $labels.tier }}] {{ $labels.tenant }}: critical CPU saturation  | Check alert metrics and review related logs; contact platform team for assistance if needed | tenant:mysql_threads_running:avg1m |
