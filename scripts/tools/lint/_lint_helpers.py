...
# The portal's CLI Playground command catalog. ⛔ The SOURCE, never
# docs/assets/dist/*.js — that bundle is regenerated by `make portal-build`,
# and keeping it in step with this file is the job of TRK-239's
# `portal dist commits without matching source change` hook, not of any
# capability check reading it.
PORTAL_PLAYGROUND_PATH = (REPO_ROOT / "tools" / "portal" / "src" / "interactive"
                          / "tools" / "cli-playground" / "commands.js")
...
# ⚠️ The upper bound is not cosmetic. `NETWORK_MODES` sits directly below
# `COMMANDS` in the same file and its keys (`k8s`, `docker-desktop`, `linux`)
# match the very same shape — scanning the whole file reports those three as
# subcommands the image does not dispatch. Measured on the tree as it stands:
# bounded 18 keys, unbounded 21.
_PORTAL_COMMANDS_OPEN = "const COMMANDS = {"
_PORTAL_COMMANDS_CLOSE = "const NETWORK_MODES"
# ⚠️ `[a-z0-9-]+` (two chars minimum), NOT `*`, deliberately: it is the same
# shape `_COMMAND_MAP_ENTRY_RE` accepts. A one-character key would be
# invisible to the entrypoint parser too, so matching it here would produce
# a finding this oracle cannot actually answer — "the image does not
# dispatch it" would mean "my other parser cannot see it either".
_PORTAL_COMMAND_KEY_RE = re.compile(r"^  '([a-z][a-z0-9-]+)':\s*\{", re.M)


def parse_portal_playground_commands_text(text: str) -> List[Tuple[str, int]]:
    """Parse the portal CLI Playground's ``COMMANDS`` keys → [(key, line), …].

    The keys ARE the subcommands: `engine.js` builds its copy-paste line as
    ``docker run … ghcr.io/vencil/da-tools:vX.Y.Z `` + ``selectedCommand``,
    and `selectedCommand` is a key of this object. So a key that the image
    does not dispatch is a customer pressing Copy and getting
    `Unknown command` (#1836).

    ⛔ Deliberately a BOUNDED regex over one top-level object literal, not a
    JS parser. The day this catalog stops being a literal — generated, merged
    from another module, reindented — the right answer is to die loudly so a
    human re-points the extractor, NOT to return fewer keys: a release would
    then print a green tick over a catalog it had silently stopped reading.
    Hence every failure mode here RAISES. Same rule #1534 set for the doc
    extractor, applied to the second source.
    """
    text = _strip_bom(text)
    start = text.find(_PORTAL_COMMANDS_OPEN)
    if start < 0:
        raise ValueError(
            f"could not find {_PORTAL_COMMANDS_OPEN!r} in the portal CLI "
            f"Playground catalog — the command object is no longer a top-level "
            f"literal with that exact name. Re-point this extractor at wherever "
            f"the playground now gets its subcommands; do not delete the "
            f"anchor check, which is the only thing standing between a moved "
            f"catalog and a release that grades nothing."
        )
    end = text.find(_PORTAL_COMMANDS_CLOSE, start)
    if end < 0:
        raise ValueError(
            f"found {_PORTAL_COMMANDS_OPEN!r} but not the {_PORTAL_COMMANDS_CLOSE!r} "
            f"that closes the scan below it. Without that bound the scan runs to "
            f"end of file and reports the network-mode keys as subcommands. "
            f"Re-point the bound at whatever now follows the catalog."
        )
    block = text[start:end]
    base = text.count("\n", 0, start)
    found = [(m.group(1), base + block.count("\n", 0, m.start()) + 1)
             for m in _PORTAL_COMMAND_KEY_RE.finditer(block)]
    if not found:
        raise ValueError(
            "extracted 0 command keys from the portal CLI Playground catalog. "
            "Both anchors were found, so the catalog is there and the KEY shape "
            "changed under it (reindented, double-quoted, or computed). Zero is "
            "refused rather than returned: it is indistinguishable from a clean "
            "catalog at every call site."
        )
    return found


def parse_portal_playground_commands(path: Path | None = None
                                     ) -> List[Tuple[str, int]]:
    """Read the portal CLI Playground catalog and parse its ``COMMANDS`` keys."""
    return parse_portal_playground_commands_text(
        (path or PORTAL_PLAYGROUND_PATH).read_text(encoding="utf-8-sig"))
