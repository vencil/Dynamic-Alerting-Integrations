...
1. 原首要檢查 TOOL_META ↔ CUSTOM_FLOW_MAP key 一致性已退役 —
   ...
   self-skip。registry key ⊆ loader 與 flow step → dist bundle 的
   守備都由 auto hook `tool-consistency-check`
   （`lint_tool_consistency.py`，error 級）把關 — 原 manual hook
   `flow-e2e-check` 已退役併入該 lint；
...
def run_checks() -> Tuple[List[Dict], Dict]:
    ...
    # (lint_tool_consistency.py: registry key ⊆ loader + flow step →
    # dist bundle, error-level; absorbed the retired manual
    # `flow-e2e-check`) + sync_tool_registry.py (generator).
