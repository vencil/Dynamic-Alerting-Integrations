...
def _gen_tenant_yaml(tenant: str, rule_packs: list[str]) -> str:
    ...
    neither block, yet it takes effect today whenever `<base>` has a value under
    `defaults:` (resolveCriticalRows keys off exactly that). Same test as the
    one that split the rule-pack header three ways in the previous PR — does
    what we say buy the tenant protection or not.
    """
    header = textwrap.dedent("""\
    ...
    # section) inherits that value. Omitting a key _defaults.yaml only DECLARES
    # (its `optional_overrides:` list) inherits NOTHING — the platform asserts
    # no value there, so an unset declared key stays silent. Those keys are
    # listed at the end of this file.
    # A third group is in neither list: `<base>_critical`. Whenever `<base>` has
    # a value under `defaults:`, setting `<base>_critical` here produces a real
    # critical-severity threshold; leaving it out just means no critical row.
    ...
    # stub can only ever advertise keys that file actually declares.
    return append_tenant_declared_stub(
        header + body, shipped_optional_keys_for_packs(rule_packs), lang='en')
