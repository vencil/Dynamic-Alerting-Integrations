...
def write_outputs(output_dir: str, tenant_name: str, defaults_data: dict, tenant_data: dict, report: str, relabel_snippet: str | None = None, mapping_hint: dict | None = None) -> None:
    ...
    tenant_content = (
        ...
        " 省略＝沒有值＝靜默，\n"
        "# 不是「用預設」；那一格只有「填值」與「不填」兩態。清單見檔尾。\n"
        "# 第三類 <base>_critical：兩個區塊都不列，但只要 <base> 在 defaults: 有值，"
        "你填了\n"
        "# 就會產生一條真的 critical 閾值（不填則無此列，base 的 warning 列不受影響）。\n"
    ...
    tenant_content = append_tenant_declared_stub(
        tenant_content, defaults_data.get("optional_overrides") or [], lang="zh")
