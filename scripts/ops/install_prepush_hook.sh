...
if [ ! -r "$_dispatch" ]; then
    {
        ...
        echo ""
        echo "Most likely this tree is checked out before #1689 landed. Either:"
        echo "  * rebase / switch to a commit that has it, or"
        echo "  * from a tree that has it, run:"
        echo "        bash scripts/ops/install_prepush_hook.sh"
        echo ""
