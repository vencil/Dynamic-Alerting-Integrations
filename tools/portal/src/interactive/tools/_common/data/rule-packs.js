...
const RULE_PACK_DATA = window.__PLATFORM_DATA?.rulePacks || {
  mariadb: { label: 'MariaDB/MySQL', category: 'database', exporter: 'mysqld_exporter', configMap: 'prometheus-rules-mariadb', recordingRules: 14, alertRules: 18, defaults: { mysql_connections: { value: 80, unit: 'count', desc: 'Max threads_connected warning', metricClass: 'saturation' }, mysql_cpu: { value: 30, unit: 'threads', desc: 'Running-threads 1m-avg saturation warning; 80→30 = PMM/Nichter \'high\' (#944). NOT host CPU% — key kept as mysql_cpu (config-contract stability; metric/alert renamed, #944 closed)', metricClass: 'saturation' }, mysql_replication_lag: { value: 30, unit: 'seconds', desc: 'Async replication lag warning (seconds behind primary, sql_delay-adjusted); 30 = APA/mysqld-mixin consensus trigger (they page critical at 30s; demoted to warning — PMM\'s lag-template family default is 600s warning). Enabled by default (#1200 WS5-P0-a Q3=C)' } }, metrics: ['connections', 'cpu', 'memory', 'slow_queries', 'replication_lag', 'query_errors'], dependencies: { suggests: ['kubernetes'], reason: { en: 'Container resource alerts complement DB monitoring', zh: '容器資源告警補充 DB 監控' } } },
