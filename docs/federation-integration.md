...
> **v2.0.0-preview.3** — 場景 A 架構藍圖：中央 threshold-exporter + 多邊緣 Prometheus scrape
...
不適用的場景：單叢集部署（直接參考主 README），場景 B Rule Pack 分層（見 §6）。

### 1.2 架構選擇：場景 A vs 場景 B

| 面向 | 場景 A（中央評估） | 場景 B（邊緣評估） |
|------|-------------------|-------------------|
...
本文件專注場景 A。場景 B 的 Rule Pack 分層設計待場景 A 穩定且有明確客戶需求後推進（見 `architecture-and-design.md` §11.2）。
...
場景 B（邊緣評估）需要將 Rule Pack 拆成兩層：

| 層級 | 位置 | 內容 |
|------|------|------|
| Part 1 | 邊緣 Prometheus | Data Normalization（recording rules） |
| Part 2 + 3 | 中央 Prometheus | Threshold Normalization + Alert Rules |
...
此架構需要 Rule Pack YAML 支援拆分標記（如 `placement: edge | central`）和打包工具。待場景 A 穩定且客戶需求明確後推進，詳見 `architecture-and-design.md` §11.2。
