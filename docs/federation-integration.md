...
> **v2.4.0** — 場景 A 架構藍圖：中央 threshold-exporter + 多邊緣 Prometheus scrape
...
## 8. 場景 B：Rule Pack 分層（v2.3.0）

場景 B（邊緣評估）將 Rule Pack 拆成兩層，讓邊緣叢集在本地完成資料正規化，中央叢集只負責閾值比對和告警。適用於 20+ 邊緣叢集或跨區高延遲環境。
...
### 8.2 使用 da-tools rule-pack-split
...
4. 逐叢集切換，過程中場景 A 和場景 B 可並存（不同邊緣叢集各自選擇）
