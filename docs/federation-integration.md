...
本文件專注場景 A。場景 B 的 Rule Pack 分層設計待場景 A 穩定且有明確客戶需求後推進（見 `architecture-and-design.md` §11.7）。
...
此架構需要 Rule Pack YAML 支援拆分標記（如 `placement: edge | central`）和打包工具。待場景 A 穩定且客戶需求明確後推進，詳見 `architecture-and-design.md` §11.7。
