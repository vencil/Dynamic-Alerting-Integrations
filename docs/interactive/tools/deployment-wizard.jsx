      ...
      <div className="max-w-4xl mx-auto">
        ...
        {!showOutput ? (
          <>
            ...
            <div className="mb-8">
              <div className="flex items-center justify-between mb-3">
                <h3 className="text-sm font-semibold text-slate-700 dark:text-slate-300">{t('進度', 'Progress')}</h3>
                <span className="text-xs text-slate-500 dark:text-slate-400">{progress}/{steps.length}</span>
              </div>
                      ...
                      >
                        <div className="flex items-start gap-4">
                          ...
                          <div className="flex-1">
                            <h3 className="font-semibold text-slate-900 dark:text-slate-100">{tier.name}</h3>
                            <p className="text-sm text-slate-600 dark:text-slate-400 mt-1">{tier.desc}</p>
                            <ul className="text-xs text-slate-500 dark:text-slate-500 mt-3 space-y-1">
                              {tier.features.map((f, i) => (
                                <li key={i} className="flex items-center gap-2">
                                  <span className="text-slate-300 dark:text-slate-600">•</span> {f}
                                </li>
                              ))}
                            </ul>
                          ...
                          {config.tier === tier.id && <span className="text-blue-600 dark:text-blue-400 font-bold">✓</span>}
                        </div>
                      </button>
                    ))}
                  </div>
                    ...
                    {ENVIRONMENTS.map(env => (
                      ...
                      >
                        <div className="flex items-center gap-3">
                          ...
                          <div className="flex-1">
                            <div className="font-medium text-slate-900 dark:text-slate-100">{env.label}</div>
                            <div className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">{env.desc}</div>
                          </div>
                          {config.environment === env.id && <span className="text-blue-600 dark:text-blue-400">✓</span>}
                        </div>
                      </button>
                    ))}
                  </div>
                    ...
                    {TENANT_SIZES.map(size => (
                      ...
                      >
                        <div className="flex items-center gap-3">
                          ...
                          {config.tenantSize === size.id && <span className="text-blue-600 dark:text-blue-400">✓</span>}
                        </div>
                      </button>
                    ))}
                  </div>
                    ...
                    {OAUTH2_PROVIDERS.map(provider => (
                      ...
                      >
                        <div className="flex items-center gap-3">
                          ...
                          <div className="flex-1">
                            <div className="font-medium text-slate-900 dark:text-slate-100">{provider.label}</div>
                            <div className="text-xs text-slate-500 dark:text-slate-400 mt-1">{provider.desc}</div>
                            <div className="text-xs text-slate-400 dark:text-slate-500 mt-2">
                              {t('範圍', 'Scopes')}: {provider.scopes.join(', ')}
                            </div>
                          </div>
                          {config.auth === provider.id && <span className="text-blue-600 dark:text-blue-400">✓</span>}
                        </div>
                      </button>
                    ))}
                  </div>
                ...
                <div>
                  ...
                  <div className="mb-4 flex gap-2">
                    ...
                    <button
                      ...
                      className="px-3 py-1.5 text-xs font-medium bg-slate-200 dark:bg-slate-700 text-slate-900 dark:text-slate-100 rounded-lg hover:bg-slate-300 dark:hover:bg-slate-600"
                    >
                      {t('清除', 'Clear')}
                    </button>
                  </div>
                  <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                    {RULE_PACKS.map(pack => (
                      ...
                      >
                        ...
                        <div className="text-xs font-medium text-slate-900 dark:text-slate-100">{pack.label}</div>
                        {config.packs.includes(pack.id) && (
                          <div className="text-blue-600 dark:text-blue-400 text-sm mt-1">✓</div>
                        )}
                      </button>
                    ))}
                  </div>
                  {packWarning && (
                    <div role="alert" className="mt-4 p-3 bg-amber-50 dark:bg-amber-950 border border-amber-300 dark:border-amber-700 rounded-lg text-sm text-amber-800 dark:text-amber-200">
                      {t(
                ...
                <div>
                  ...
                  <div className="bg-slate-50 dark:bg-slate-900 rounded-lg p-6 space-y-4">
                    <div className="flex justify-between items-center pb-4 border-b border-slate-200 dark:border-slate-700">
                      <span className="text-sm font-medium text-slate-600 dark:text-slate-400">{t('部署層級', 'Deployment Tier')}</span>
                      <span className="font-semibold text-slate-900 dark:text-slate-100">{summary.tier}</span>
                    </div>
                    <div className="flex justify-between items-center pb-4 border-b border-slate-200 dark:border-slate-700">
                      <span className="text-sm font-medium text-slate-600 dark:text-slate-400">{t('運行環境', 'Environment')}</span>
                      <span className="font-semibold text-slate-900 dark:text-slate-100">{summary.environment}</span>
                    </div>
                    <div className="flex justify-between items-center pb-4 border-b border-slate-200 dark:border-slate-700">
                      <span className="text-sm font-medium text-slate-600 dark:text-slate-400">{t('Tenant 數量', 'Tenant Count')}</span>
                      <span className="font-semibold text-slate-900 dark:text-slate-100">{summary.tenantSize}</span>
                    </div>
                    {config.tier === 'tier2' && (
                      <div className="flex justify-between items-center pb-4 border-b border-slate-200 dark:border-slate-700">
                        <span className="text-sm font-medium text-slate-600 dark:text-slate-400">{t('認證', 'Authentication')}</span>
                        <span className="font-semibold text-slate-900 dark:text-slate-100">{summary.auth}</span>
                      </div>
                    )}
                    <div className="flex justify-between items-center">
                      <span className="text-sm font-medium text-slate-600 dark:text-slate-400">{t('Rule Packs', 'Rule Packs')}</span>
                      <span className="font-semibold text-slate-900 dark:text-slate-100">{summary.packs}</span>
                    </div>
                  </div>
            ...
            <div className="flex items-center justify-between">
              <button
                ...
                className="px-4 py-2.5 text-sm font-medium text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-slate-100 disabled:opacity-30"
              >
                ← {t('上一步', 'Back')}
              </button>
              <button
          ...
          <>
            ...
            {/* ── Next steps ── */}
            <div className="bg-blue-50 dark:bg-blue-950 rounded-xl border border-blue-200 dark:border-blue-800 p-6 space-y-6">
              <div>
                <h3 className="font-semibold text-slate-900 dark:text-slate-100 mb-4">{t('後續步驟', 'Next Steps')}</h3>
              ...
              {config.tier === 'tier2' && (
                <div className="border-t border-blue-300 dark:border-blue-700 pt-4">
                  <h4 className="font-semibold text-slate-900 dark:text-slate-100 mb-3">{t('Tier 2：建立首個租戶', 'Tier 2: Create Your First Tenant')}</h4>
                  <ol className="text-sm text-slate-700 dark:text-slate-300 space-y-2">
                    <li className="flex gap-3">
                      ...
                      <span>
                        ...
                        <a href="/template-gallery" className="text-blue-600 dark:text-blue-400 underline hover:text-blue-700 dark:hover:text-blue-300">
                    ...
                    <li className="flex gap-3">
                      ...
                      <span>
                        {t('透過 Portal 開啟 ', 'Open ')}
                        <a href="/tenant-manager" className="text-blue-600 dark:text-blue-400 underline hover:text-blue-700 dark:hover:text-blue-300">
                    ...
                    <li className="flex gap-3">
                      ...
                      <span>
                        {t('查看 ', 'Review ')}
                        <a href="/docs/getting-started" className="text-blue-600 dark:text-blue-400 underline hover:text-blue-700 dark:hover:text-blue-300">
