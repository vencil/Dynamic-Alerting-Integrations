        ...
        {CI_OPTIONS.map(opt => (
          <button
            ...
            className={`p-4 rounded-lg border-2 text-left transition-all ${
              config.ci === opt.id ? 'border-blue-500 bg-blue-50' : 'border-gray-200 hover:border-gray-300'
            }`}
          >
      ...
      <div className="grid grid-cols-1 gap-3">
        {DEPLOY_OPTIONS.map(opt => (
          <button
            ...
            className={`p-4 rounded-lg border-2 text-left transition-all ${
              config.deploy === opt.id ? 'border-blue-500 bg-blue-50' : 'border-gray-200 hover:border-gray-300'
            }`}
          >
            <div className="flex items-center gap-3">
              <span className="text-2xl">{opt.icon}</span>
              ...
              {packs.map(p => {
                ...
                return (
                  ...
                  >
                    ...
                    <span className={isSelected ? 'font-medium' : ''}>{p.label}</span>
  ...
  return (
    <div>
      ...
      <div className="flex flex-wrap gap-1.5 mb-4">
        <span className="text-xs text-gray-500 self-center">{t('快速新增：', 'Quick add:')}</span>
    ...
    <div>
      ...
      <div className="grid grid-cols-2 gap-3 mb-4">
        <div className="p-3 bg-gray-50 rounded-lg">
          <div className="text-xs text-gray-500">{t('CI/CD 平台', 'CI/CD Platform')}</div>
          <div className="font-medium text-sm">{CI_OPTIONS.find(o => o.id === config.ci)?.label || '-'}</div>
        </div>
        <div className="p-3 bg-gray-50 rounded-lg">
          <div className="text-xs text-gray-500">{t('部署方式', 'Deployment')}</div>
          <div className="font-medium text-sm">{DEPLOY_OPTIONS.find(o => o.id === config.deploy)?.label || '-'}</div>
        </div>
        <div className="p-3 bg-gray-50 rounded-lg">
          <div className="text-xs text-gray-500">Rule Packs</div>
          <div className="font-medium text-sm">{config.packs.length} + 2 {t('自動', 'auto')}</div>
        </div>
        <div className="p-3 bg-gray-50 rounded-lg">
          <div className="text-xs text-gray-500">Tenants</div>
          <div className="font-medium text-sm">{config.tenants.length}</div>
        </div>
      </div>
        ...
        <>
          ...
          <div className="mb-4">
            <div className="flex items-center justify-between mb-1">
              <span className="text-sm font-medium text-gray-700">da-tools init {t('命令', 'command')}</span>
              <button
                onClick={() => copy(initCmd, setCopiedCmd)}
                className={`text-xs px-2 py-1 rounded ${copiedCmd ? 'bg-green-600 text-white' : 'bg-gray-200 hover:bg-gray-300'}`}
              >
                {copiedCmd ? '✓' : t('複製', 'Copy')}
              </button>
            </div>
          ...
          <div className="mb-4">
            <div className="flex items-center justify-between mb-1">
              <span className="text-sm font-medium text-gray-700">{t('Docker 一鍵執行', 'Docker one-liner')}</span>
              <button
                onClick={() => copy(dockerCmd, setCopiedDocker)}
                className={`text-xs px-2 py-1 rounded ${copiedDocker ? 'bg-green-600 text-white' : 'bg-gray-200 hover:bg-gray-300'}`}
              >
          ...
          {/* File tree */}
          <div className="mb-4">
            <span className="text-sm font-medium text-gray-700">{t('產生的檔案結構', 'Generated file structure')}</span>
            <pre className="mt-1 bg-gray-50 p-4 rounded-lg text-xs font-mono border">
              {fileTree}
            </pre>
          </div>
          ...
          {showPipeline && (config.ci === 'github' || config.ci === 'both') && (
            <div className="mb-4">
              <span className="text-xs font-medium text-gray-500">.github/workflows/dynamic-alerting.yaml</span>
              <pre className="mt-1 bg-slate-900 text-slate-200 p-4 rounded-lg text-xs font-mono overflow-x-auto max-h-64 overflow-y-auto">
          ...
          <div className="mt-4 p-4 bg-blue-50 rounded-lg border border-blue-200">
            <h4 className="text-sm font-medium text-blue-800 mb-2">{t('下一步', 'Next Steps')}</h4>
...
export default function CICDSetupWizard() {
  ...
  return (
    <div className="max-w-3xl mx-auto">
      ...
      {/* Step indicator */}
      <div className="flex items-center mb-6 bg-gray-50 rounded-lg p-2">
        {STEPS.map((s, i) => (
          <React.Fragment key={s.id}>
            {i > 0 && <div className={`flex-1 h-0.5 ${i <= step ? 'bg-blue-500' : 'bg-gray-300'}`} />}
            <button
              onClick={() => setStep(i)}
              className={`flex items-center gap-1.5 px-2.5 py-1.5 rounded-md text-xs font-medium transition-colors ${
                i === step ? 'bg-blue-600 text-white' :
                i < step ? 'text-blue-600 hover:bg-blue-50' : 'text-gray-400'
              }`}
            >
              <span className={`w-5 h-5 rounded-full flex items-center justify-center text-xs ${
                i < step ? 'bg-blue-100 text-blue-700' :
                i === step ? 'bg-white text-blue-600' : 'bg-gray-200 text-gray-500'
              }`}>
                {i < step ? '✓' : i + 1}
              </span>
      ...
      <div className="flex justify-between">
        <button
          ...
          disabled={step === 0}
          className="px-4 py-2 text-sm font-medium text-gray-600 bg-gray-100 rounded-lg hover:bg-gray-200 disabled:opacity-30"
        >
          {t('上一步', 'Back')}
        </button>
        {step < STEPS.length - 1 ? (
