...
> **⚠️ Release-day CVE drift（v2.9.0 實戰）**：`release.yaml` 每個 component job 末段有 `Scan image with Trivy`（`severity HIGH,CRITICAL` + `ignore-unfixed` + `exit-code 1`）**hard gate**——⛔ **它不是「image 出廠前的最後一道線」——這句話 #1337 已實測更正**：那個步驟跑在 **push 之後**，而且五個 job 有四個它就是**最後一步**（`release-exporter` 甚至在 image push 與 `helm push` 都完成之後才掃）。所以它擋的是 **Actions run 變紅**，不是產物被攔下；唯一真的被擋掉東西的是 `release-da-tools`，它後面還有 cosign 簽章／SBOM／跨平台 archive／建立 GitHub Release 共 9 步。且**在 tag push 當下才掃**。CVE 會在「上次乾淨掃描」與「真正打 tag」之間落地，於是 release 當下被擋。v2.9.0 五線 GA 就連撞 3 組剛揭露的 HIGH（Go stdlib `CVE-2026-42504` / nginx `CVE-2026-9256` RCE + `CVE-2026-49975` / golang-jwt `CVE-2025-30204`）。因此：(1) pre-tag audit 要**貼近打 tag 時點**跑、別用幾天前的結果；(2) fixable 一律 patch 不 exempt；(3) 重打 tag 前先用本地預驗 recipe 證明 0/0，避免每修一個 CVE 燒一輪 CI；(4) **自動化偵測（2026-06；#1337 於 2026-08 由 5 擴到 7）**：`nightly-image-scan.yaml`（schedule）每晚對 `main` build 的 **7** 個 self-built image 跑同一份 Trivy 契約——後加的兩顆（`federation-audit-sidecar` / `vector-projection-gate`）隨 chart 出貨、從不發布，`release.yaml` 的 tag-time hard gate 對它們**結構上不適用**——它們的 CVE 覆蓋由夜掃（每晚、對 `main`）＋ `component-docker-build.yaml`（PR 期、informational）＋ `make trivy-scan-all`（pre-tag、informational）三處提供，**唯獨少了 tag-time 那道 hard gate**（`CRITICAL,HIGH` + `ignore-unfixed`），fixable CVE 一出現就開／更新一個 deduped tracking issue（自建桶 label `nightly-cve`、第三方桶 `nightly-cve-thirdparty`，全清自動關）。**⛔ 桶是三個不是兩個（#1337 續）**：第三桶 `scan-delivered`／label `nightly-cve-delivered` 掃的是 **`da-tools init` 寫進客戶 repo 的 4 顆映像**（GitLab apply 階段的 runner image ×3 ＋ git-sync sidecar／init container）。分桶判準是**誰能動手**、不是 Dockerfile 在哪：前兩桶都以「我們可以 merge 的一個動作」收尾，這桶**我們碰不到那個部署**——ref 在 init 執行當下就凍結進客戶 repo，我們 bump 只改變**下一個**產生出來的專案，既有客戶得自己改或重跑 init；混桶會讓兩種互斥的處置指引擠在同一個標題下。⚠️ 這桶量的是**我們今天會遞出去的預設值**，不是任何客戶的實際狀態（那三顆 apply 映像在產生出來的 pipeline 裡是可覆寫的 CI 變數 `DA_KUBECTL_IMAGE` 等，我們對客戶從哪一版產生、覆寫成什麼無遙測）。**通知是 delta 制不是 level 制**：body/title 每晚靜默刷新、只有出現「上一輪沒看過的 CVE」或「更多影像掃不到」才留言（第三方桶長期停在約 120 筆上游未重建的 fixable CVE，每晚 ping 這個數字只會訓練讀者忽略它）；讀不到基準線一律當變差而通知＝fail-open。⚠️ **第三方桶的修復不是「bump tag」**——Renovate（#902 L3）每週已把每個 ref 釘在最新核可 tag，真正能動的是 Renovate Dependency Dashboard [#929](https://github.com/vencil/Dynamic-Alerting-Integrations/issues/929) 上待勾選的 gated major，或換掉該影像，把「發版夜才發現」提早成「平日就看到」——non-blocking、不取代 release.yaml 的 hard gate，只是讓它別在最糟時機才第一次發現。流程細節見 [github-release-playbook.md §Release-gate 陷阱](github-release-playbook.md#release-gate-陷阱component-releaseyaml-實戰v290-首次五線-ga)。
