...
> ⛔ **這三支不會自動安裝（[#1664](https://github.com/vencil/Dynamic-Alerting-Integrations/issues/1664)）。** 安裝只有一條：`bash scripts/ops/install_prepush_hook.sh`（冪等）。「有沒有接上」由 `make pr-preflight` 的 `Local hooks` 回答，它對「沒接上」直接 FAIL 並印那條指令。⛔ **`--skip-hooks` / `make pr-preflight-quick` 略過不了這一問**（[#1811](https://github.com/vencil/Dynamic-Alerting-Integrations/issues/1811)）：那個旗標只跳同一列裡的 `pre-commit run --all-files`。
>
> ⛔ **安裝器會串接既有的 pre-push hook，而不是拒絕**——這不是方便，是必要：本 repo 有 `filter=lfs` 路徑而 `git lfs install` 是全域設定，所以**全新 clone 的 `.git/hooks/pre-push` 一落地就是 git-lfs 的**。實測（Windows host、全新 clone）：拒絕覆寫 ⇒ 安裝器 rc=1，而 `make pr-preflight` 印的補救指令正是它 ⇒ 死路；改用舊指令 `pre-commit install --hook-type pre-push` ⇒ rc=0，但它把 lfs 那支 `#!/bin/sh` 的 hook 遷到 `pre-push.legacy`，之後**每次 push 都死在 `ExecutableNotFoundError: /bin/sh`**（pre-commit 在 Windows 自己解 shebang）。外來 hook 會被移到 `pre-push.chained`，由 dispatcher 用同樣的 argv 與 stdin 繼續呼叫。
>
> ⚠️ **`pre-commit install --hook-type pre-push` 不再是替代方案**：它不會把守衛裝上，而且只要設了 `core.hooksPath`（任何值）它直接 rc=1 拒絕安裝。
...
|---|---|---|---|
| **AST SAST 契約**（`tests/shared/test_sast.py`） | `pytest tests/shared/test_sast.py` | `scripts/tools/` 全檔 AST 掃描（open-encoding / eval / 硬編碼機密 等 **6 規則**；⛔ `subprocess-timeout` **不在這 6 條裡**，它是 `.pre-commit-config.yaml` 的 FATAL **本機** hook `subprocess-timeout-audit`；dev-rules §5 第 4 條 `yaml.load` 自 #1643 起改由 bandit B506 強制，見下一列） | 改 `scripts/tools/**` 後 |
...
| **nested `CLAUDE.md` 的路徑觸發**（TRK-377；目前唯一實例 `tools/portal/CLAUDE.md`） | 🔧 Claude Code 原生機制，**但只在用 Read 工具讀該目錄底下的檔時才帶入** | Bash 的 `cat` / `head` 讀同一個檔**不觸發**（三次探針一致）⇒ 一個習慣用 shell 讀檔的 session 對這一層**零覆蓋**，而畫面與「這個檔沒用」無法區分。⚠️ 機制本身是好的：乾淨 subagent 的 2×2 探針（巢狀檔獨有內容 ×2 ／ root `CLAUDE.md` 陽性對照 ／ 防杜撰陰性對照）全數通過 | [#1757](https://github.com/vencil/Dynamic-Alerting-Integrations/issues/1757)：⛔ **推廣到第二棵子樹之前**，先量一次真實 session 的觸發率（已量的只有「機制會運作」與「Bash 不觸發」，**沒有量**日常回合裡有幾成真的載入），或改走 Tier 3（skill）／Tier 1（root `CLAUDE.md`）。⛔ 自我指涉限制：**無法在產生該檔的那個 session 裡量它對自己的投遞率**（已在 context 就不重送）——要量得換一個「不是我建的」同類物件 |

> ⛔ **最後這三列的漏法是同一族，值得單獨記**：前幾列都是「沒有機械防線」，這三列是 **機制存在、寫得對、在主路徑上綠，但只在某一種形態下能執行**——A-13 那列缺的是 hook 的**依賴**（`node_modules`），#1719 那列缺的是 hook 的**註冊**（settings 檔整份沒載入），nested `CLAUDE.md` 那列缺的是**叫用形式**（同一個檔，Read 觸發、Bash 不觸發）。§3 的「commit 時自動跑，失敗會擋」與 §8 第 2 點的「不要重做 hook-enforced 的事」對它**不成立**——判別時要問的不只是「有沒有 hook」，還有「這個 hook 的**依賴**在我這棵樹裡在嗎、它的**註冊**在這個 session 形態下載入了嗎、以及**我現在的叫用方式**會不會讓它根本不被觸發」。
