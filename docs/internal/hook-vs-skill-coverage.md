...
| `vibe-converge` | 多輪修正的收斂協議（開工前寫一條可跑的 oracle / decidability gate / 跨輪交接契約 / 面積預算 / 輪數上限 5；⛔ 停止條件不是「零 finding」） | advisory（同一缺陷第 2 輪起觸發） | **無對應 hook，刻意的**：`make converge-status` 只觀測不擋，不進 CI / pre-commit（#1457 剛刪掉六支「守衛的守衛」）（TRK-360） |
