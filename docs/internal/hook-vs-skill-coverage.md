...
| **A-13**（`test.fixme()` / bare `test.skip()`）在 **worktree** 內 | 🔧 `playwright-lint` hook，但**只在有 `tests/e2e/node_modules` 的 checkout 跑得起來** | `node_modules` 是 gitignored ⇒ 每一棵新開的 worktree 對它都是壞的、且不會自己好 | #1428：三個呼叫端收斂到 `scripts/tools/lint/e2e_spec_lint.sh`（缺依賴時印 `cd tests/e2e && npm ci` 並 fail）＋`playwright.yml` 加 CI 腿。⚠️ **CI 腿目前是 advisory**（`Smoke Tests (Chromium)` 不是 required check），要 blocking 見該票 |
