...
JSON 輸出格式快照由 `tests/lint/test_snapshot_v2.py` 驗證（更新：`UPDATE_SNAPSHOTS=1 pytest tests/lint/test_snapshot_v2.py`）。

CLI `--help` **不再使用全文快照**（任何 wording / 空白 / 排版微調都會 break，且 py-version 後綴快照在 CI py3.13 只會 auto-create+skip、無回歸價值）。改由 `tests/shared/test_help_contract.py` 斷言結構契約：flag 存在性、required flag、enum choices；描述文字可自由調整不動測試。新增 / 移除 flag 時同步更新該檔 `HELP_CONTRACTS`。
...
7. 🛡️ **main() 的 sys.exit() 要 catch**：validate_all.py main() 走 `sys.exit(0/1/2)`，pytest 需要 `pytest.raises(SystemExit)` 包裹 `[已自動化於 test: test_validate_all]`。⚠️ **不是只有結尾會 exit**：`--ci` 在執行迴圈中間 `sys.exit(1)`，而 `--only`/`--skip` 的未知檢查名走 `EXIT_CALLER_ERROR`（2）**在 `os.chdir` 之前**就早退（[#1620](https://github.com/vencil/Dynamic-Alerting-Integrations/issues/1620)）⇒ 測早退路徑的斷言不能假設工具已經切到 repo root，也不能只覆蓋 0/1
...
**Mechanical safety net**：✅ **codified** — `scripts/tools/lint/check_playwright_rtl_drift.py` (S#96 PR #186) FATAL pre-commit hook 抓 `getByDisplayValue` / `getByLabelText` / `getByPlaceholderText` 在 `tests/e2e/**/*.spec.ts`（`getByAltText` 不入名單 — Playwright 1.27+ 同名同意義）；3-layer 抑制（per-line ignore / TS comment skip / backtick code-span skip）。Drift 自動攔，新 spec 寫到 RTL API 立即 commit-time fail。

**Cross-refs**：PR #184 first-run CI failure (3 scenarios fail with `TypeError`); fix commit `912cf2b` (`readAllInputValues` helper); `tests/e2e/tenant-manager-deeplink.spec.ts` + `simulate-preview.spec.ts`（canonical evaluate pattern）；Playwright docs `https://playwright.dev/docs/locators`（authoritative API surface）。
...
**Mechanical safety net**：✅ **`toBeVisibleWithDiagnostics` matcher** (S#98 PR #188, `tests/e2e/fixtures/diagnostic-matchers.ts`)。`.toBeVisible()` drop-in replacement：success path 零 overhead；failure path 列出當下所有 visible `data-testid`，自動產生 self-explanatory 錯誤（catches cold-start mismatch / async race / validation reject / stale mock / re-mount 等所有 state-mismatch 失敗模式）。**Zero enumeration** — 不需維護 pattern list，每個 Playwright 版本 + custom helper 都 work。Opt-in via spec top `import './fixtures/diagnostic-matchers'` + `.toBeVisibleWithDiagnostics()` call site。

> 原 S#96 codify 提的 Tier 2 (marker-enforced annotation) / Tier 3 (component×spec AST cross-reachability) 已 **deferred indefinitely**：matcher 已涵蓋實際 bug class，加 annotation 或 AST 分析無 proportional value。Tier 1 v1 (text heuristic enumeration via S#97 PR #187) **closed unmerged** — 為何不該走那條路見 §LL §12 closed-vs-open enumeration discipline。
