...
`test.fixme(true, 'TODO: calibrate in real browser')` 會在 CI 報 `skipped`，不報 failed。短期方便但會養成「先 fixme 綁 32 條，之後再說」的債務雪球。**規則**：

- 新增 `test.fixme()` 時**必須** 同時在 [`docs/internal/frontend-quality-backlog.md`](frontend-quality-backlog.md) 登記（測試名、spec 檔、fixme 原因、預計移除版本）
- 單一 spec 檔 `test.fixme()` 數量 > 5 條時觸發強制 calibration sprint（比如 Phase .e 發現的 wizard/playground/config-lint/cost-estimator 4 檔）
- `test.fixme()` 跨版本存留超過 1 個 minor（e.g. v2.7.0 引入、v2.8.0 還在）要走 knowledge annealing 三選一：calibrate / 刪測 / 標 `test.skip()` 並寫明放棄原因

> **A-13 Enforcement (v2.8.0 PR #57)**：以上三條規則已由 `tests/e2e/eslint.config.mjs` + pre-commit `playwright-lint` hook **codified**。`eslint-plugin-playwright/no-skipped-test` 設 `{ allowConditional: false, disallowFixme: true }`，新加 `test.fixme()` / `test.skip()`（bare form）在 commit-time 直接被擋（不需等 CI）。條件式 `test.skip(isChrome, 'reason')` 仍允許——「debt 標記」vs「環境閘門」的區分在自動化層仍有效。若真需要在 E2E 以外的地方 skip，改走 Python 的 `pytest.skip`（不受此規則規範）。
