...
- **Tier 1 — text heuristic** 🟢 **S#97 PR：`scripts/tools/lint/check_playwright_coldstart_drift.py`**。對每個 `test()` block 找 `await expect(...).toBeVisible()` 對 downstream-state testid 的 assertion（pattern：**narrowed during dogfood to `*-state-(ready|success|loaded|error|fail)` only** — `*-result` / `*-output` / `*-preview` 拿掉，因為 `preview` 在本 repo 也是 tool 名稱（`simulate-preview-tenant-id` 是 input testid 不是 state），會誤報；保留 future extension 給 component 真用其他 state-marker convention）。若**前面同一 test block 內**找不到任何 input establishment（`*.fill(` / `*.click(` / `*.dispatchEvent(` / `*.selectOption(` / `*.setInputFiles(` / `*.check(` / `*.uncheck(` / `*.press(` / `*.type(` / `page.goto(<url ?key=value>)` / `page.route(` / `page.evaluate(`），WARN。Block-level escape `// playwright-coldstart: auto-fire`（spec author 已讀 component source 並驗 default state 滿足 render gate；例：`alert-builder` `groupName: 'my-alerts'`、`routing-trace` `labels: {team: 'platform', env: 'prod'}`、`simulate-preview` `tenantId: 'example-tenant'` post-PR #185 fix）。Per-line escape `// playwright-coldstart: ignore` (3-line lookback for JSDoc) 給 residual edge case。**Pre-merge intentional-break dogfood**：移除 `simulate-preview.spec.ts` `auto-simulates on mount` block 的 `await page.route(...)` → lint 立刻抓 `tests\e2e\simulate-preview.spec.ts:82:23 [coldstart-drift] testid='simulate-preview-state-ready' block@L68` exit=1 → restored。Live audit 0 violations across 20 spec files (PR #185 fix already merged)。**49 unit tests** 覆蓋 5 state-suffix variants + 9 input-establishment patterns + JSDoc/comment safety + multi-block independence + describe-not-block + test.skip/only recognition + marker variants × 3-line lookback + edge cases + severity matrix + subprocess integration + live-repo + regression dogfood。**9th `make lint-extract` text scaffold dogfood**。Pre-commit hook `playwright-coldstart-drift-check` auto-stage FATAL；validate_all registry +1；tool count 138→139。
