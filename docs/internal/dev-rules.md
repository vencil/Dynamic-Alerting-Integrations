...
✅ **Codified**：
- `scripts/ops/protect_main_push.sh` pre-push hook 攔截 main push。⛔ **`pre-commit install` 不會裝它**（只裝 pre-commit 那一層），而「誰來跑那支安裝器」有**四條路**：①人類手動 `bash scripts/ops/install_prepush_hook.sh`／②AI session 由 `scripts/session-guards/session-init.py` 的 `_heal_git_hooks` 代跑（沒有「已裝就跳過」的判斷；裝不起來時會同時走 `additionalContext` 與 stderr 出聲，不是靜默失敗）／③Claude Code on the web 由 `.claude/hooks/session-start.sh` 代跑（`SessionStart` hook，只在 `CLAUDE_CODE_REMOTE=true` 動作）／④多 repo 的 web session（[#1719](https://github.com/vencil/Dynamic-Alerting-Integrations/issues/1719)：`.claude/settings.json` 整份不載入 ⇒ ②③ 都不跑，只剩①）。安裝配方始終只有那一條（冪等）。⚠️ **`.git/hooks/pre-push` 在不在，兩種答案都不代表守衛接上了**——裝了 git-lfs 的機器上全新 clone 就有一個（那是 lfs 的，對直推 main 一律放行；安裝器會把它搬成 `pre-push.chained` 再串接），而沒有 git-lfs 的環境（dev container 實測沒有）則是**檔案根本不存在**。要答「接上了沒」只有 `make pr-preflight` 的 `Local hooks` 那一列。`make pr-preflight` 會對「沒接上」直接 FAIL 並印出這條指令。⛔ **`pre-commit install --hook-type pre-push` 不是替代方案（[#1689](https://github.com/vencil/Dynamic-Alerting-Integrations/issues/1689)）**：經 pre-commit 執行的 pre-push hook 只看得到**一列** refspec，於是「同時推 `feat/x` 與 `main`」時 main 對守衛隱形——實測它印 `Passed` 而 main 真的推上去了；而 dev-rule #12 要求的 `feat/` `fix/` `chore/` 全部排序在 `main` 之前
...
- ✅ pre-push guard 取得 refspec 的通道見 `scripts/ops/_prepush_refs.sh`（⛔ 哪幾支 source 它、以及兩欄 `prepush_refs` 與三欄 `prepush_refs_full` 誰用哪一個，以該檔的呼叫端為準，本文刻意不複述——欄數差別正是它檔頭「不要加第三欄」那條禁令的理由）。**「只看得到一列、而且是字典序最前那一列」的殘差已由 [#1689](https://github.com/vencil/Dynamic-Alerting-Integrations/issues/1689) 修掉**——它們不再註冊為 pre-commit 的 pre-push stage（那條路只餵 hook 一列），改由 `scripts/ops/prepush_dispatch.sh` 用 git 的完整 stdin 呼叫。成因仍是活的（那是 pre-commit 的行為，不是我們能改的），所以量測與邊界仍保留在該檔檔頭與 `docs/internal/hook-vs-skill-coverage.md` §1

**執行入口**：`make pr-preflight` ｜ `win_git_escape.bat pr-preflight [PR#]` ｜ `win_git_escape.ps1 pr-preflight [PR#]`（後兩者寫死 `--skip-hooks`）｜沒有 `make` 的 shell 直接跑 `python scripts/tools/dx/pr_preflight.py [--skip-hooks]`（容器／Linux 用 `python3`；⚠️ Windows host 上 `python3` 命中 Microsoft Store stub、exit 49）。⚠️ 報告印 `❌ BLOCKED` 時 **rc 仍是 0**（要非零得加 `--ci`；dev-rule #13 的落差在 [#1472](https://github.com/vencil/Dynamic-Alerting-Integrations/issues/1472)）——真正擋住 push 的是「沒寫 marker」，不是 rc。
