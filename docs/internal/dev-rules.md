...
**規則**：本條無文字敘述——由 `tests/dx/test_pair_bench_ratio.py` 強制（比照 P2「規則本體即為 hook 程式碼」）。承重的是「**跑 producer 讀真實輸出**，與『引入當前 schema 字串那顆 commit』的鍵集合比對」⇒ 改鍵只能與 bump 同一顆 commit 發生。⛔ **本條不宣稱不可繞過**：任何寫在 repo 裡的 gate，改 gate 就能消音。它保證的是**不可能不小心犯**，且**刻意消音會留下一段自我指證的 diff**。⚠️ 四輪盲審都打回東西：前三輪各繞過一次（改字面常數／module-scope decoy 騙過 AST／**只釘了 Python 那半邊**），修法分別是改讀輸出而非原始碼、歧義一律 fail closed、替 workflow 補上同一個歷史錨點；第四輪打回一個**繞過**（`git log -S` 不跟 rename 走 ⇒ rename 那顆 commit 變成它自己的錨、拿自己跟自己比）與一個**偽陽性**（`payload: dict = {...}` 是 `AnnAssign` 不是 `Assign` ⇒ 無害的型別註記讓 gate 空手而歸並誤炸），修法是 `--follow` + 解析歷史檔名、以及兩種賦值形式都認。⛔ 偽陽性和繞過一樣要修：**會亂叫的 gate 會被刪掉**，那是繞過的長路版。⛔ 由來：`60f4523` 替 `bench-paired/v1` 加了含**必需** `status` 在內的三個頂層欄位卻沒 bump，於是每一份合法舊 artifact 都通過 schema 檢查後在下一關被判死；而 `c7d0586` 為一個**選填**欄位 bump 了 v1→v2 ⇒ 破壞相容的沒 bump、不破壞的 bump 了，**規範不存在**而非被疏忽。⚠️ 該 payload 有**兩個** producer（`pair_bench_ratio.py` 與 `bench-record.yaml` 的 `printf` fallback），**各自釘住歷史**，但兩邊強度不對等：GitHub Actions step 無法在測試裡跑，workflow 那半只能比對**文字**（亦即會被騙的那種模型），代價由「抽不到或抽到兩塊一律 fail closed」承擔——宣稱兩邊等強，正是第三輪抓到的病。
