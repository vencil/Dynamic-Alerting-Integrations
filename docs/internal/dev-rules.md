...
**檢查方式**：✅ **code-driven (hard-fail since #1643; baseline v2.9.0 #455)**
...
- **Soak**: ended. `continue-on-error: false` since #1643 — the 2-week soak opened 2026-05-20 and was still open 15 weeks later. Flipped on a surface reporting 0 MEDIUM+ at this floor; ⚠️ **that 0 is held up by the `# nosec` suppressions below** — re-run with `--ignore-nosec` to see what they hide. ⛔ **Not required, and not listable as-is**: the workflow is `paths:`-filtered, so a PR touching nothing in scope never reports that context and would hang on "Expected". ⛔ **Two steps, in this order**: (A) adopt `validate.yaml`'s banner pattern so the workflow always runs (no top-level `paths:`; gate jobs with `if:`, since a skipped job still reports), then (B) add the fail-closed aggregation #1398 asks for — a job skipped because its detect job *broke* also satisfies the check, so **(A) alone gives you a required check that passes when bandit never ran**. Opposite failure direction from this one, which is why #1398 does not already cover it.
- **Suppression**: inline `# nosec B<ID>  # rationale` (dual-hash; em-dash / single-hash gets parsed as test IDs and emits warnings). ⛔ **The `B<ID>` is load-bearing, not decoration**: with no parseable id (`# nosec`, or `# nosec <prose>`) bandit blanket-suppresses **every** rule on that line, B506 included — measured, with `# nosec B310` on a B506 line as the must-fire control. The warning text is identical either way, so rc / finding count / log all look the same. Nothing mechanically checks this.
- **Coverage vs the 7 items**: bandit natively gates items 2/**3 (partly)**/4/5/6 (shell B602/B603/B605, **chmod 0o777 B103**, yaml_load B506, hardcoded password B105/B106 partial, eval/exec/pickle B102/B301/B307); items 1/7 (encoding, stderr routing) have no native bandit rule: item 1's `open()`-encoding half is the pre-commit `open-encoding-audit` hook (builtin `open()` only — `Path.open` / `read_text` / `write_text` have no static guard, [#1992](https://github.com/vencil/Dynamic-Alerting-Integrations/issues/1992)), its BOM half is `tests/shared/test_sast.py::TestOpenEncoding`; item 7 is `test_sast.py::TestStderrRouting`. ⚠️ Item 3's other half (a write-open must be paired with `chmod(0600)`) has no bandit rule and stays in `tests/shared/test_sast.py::TestFileWritePermissions` — B103 only rejects `0o777`. ⛔ **Item 4 is bandit-only since #1643** — the second AST implementation was removed (one exclusive false-red; its **only** increment over B506 was being un-silenceable — a `# nosec` line mutes B506 but never muted the AST copy); items 2/5/6 still carry both, unmeasured. What neither covers is written at rule 4 in `tests/shared/test_sast.py`; the compensating behavioural test itself lives in `tests/ops/test_validate_config.py`.
...
✅ **四層防護**：(1) Prevent (harness) — `preflight_bash.py` PreToolUse hook 攔 `sed -i`+掛載路徑（audit-2026-04 §H1）；(2) Detect — `detect_sed_damage.py` (commit-time NUL+截斷)；(3) Repair — `fix_file_hygiene.py` (auto-補 EOF + 移 NUL)；(4) Shell — `vibe-sed-guard.sh`（shell function，須自行 `source`；repo 內無自動安裝點）。**Symlink 例外**：symlink proxy md 在 `.pre-commit-config.yaml` `exclude` 排除（詳 [windows-mcp-playbook §v2.7.1 LL](windows-mcp-playbook.md#v271-llend-of-file-fixer-會把-symlink-blob-弄壞)）。**行尾同屬檔案衛生**：`scripts/`+`components/`+`helm/` 任何寫出文字的呼叫（含 `Path.open`/`os.fdopen`/`NamedTemporaryFile`/`TextIOWrapper`，不只裸 `open`/`write_text`）須明確傳**字串字面值** `newline=`（`os.linesep` 實測仍是 CRLF、不算表態），否則同一支工具在 Linux 出 LF、Windows 出 CRLF——CI-only gate，詳 [hook-vs-skill-coverage §4.5](hook-vs-skill-coverage.md)。**檔案模式同屬檔案衛生（Windows host）**：本 repo 有 138 個檔 committed 為 `100755`，而 **host 與 dev container 兩邊都觀察不到 exec bit**——host 是 NTFS，container 是 Windows bind mount（實測 `ls -l` 全為 `-rwxrwxrwx`）。⇒ `core.filemode = true` 在這台機器上兩邊都製造幽靈漂移：host 端 138 個純模式 `M`，**container 端 2163 個且方向相反**（`644 => 755`，實測）。後果不只是吵：`git add` 會把不相干的翻轉 staged 進 PR、`git merge` **在 commit 之前**就被擋、pre-push 的 pre-commit 報 `Your pre-commit configuration is unstaged` **並建議 `git add .pre-commit-config.yaml`**（⛔ 照做即是把無關翻轉塞進 PR）。**正解是共用值**：本機 `.git/config` 已設 `core.filemode = false`，**所有 worktree（含新建的）自動繼承**——實測只靠共用值、不設任何 per-tree 覆寫，全新 worktree 漂移為 0。不影響 CI（config 不進版控，Linux runner 自行偵測）。⛔ 兩條經實測否決、勿重試：user-global `includeIf` 贏不了 repo-local；`chmod -x` 那 138 個檔不對。⚠️ **勘誤**：本節曾寫「`git -c` 傳不進 hook 自己 fork 的 git，故須用 `GIT_CONFIG_PARAMETERS`」——**是錯的**，`git -c` 的實作就是把參數寫進 `GIT_CONFIG_PARAMETERS` 再 fork，實測穿透兩層 fork，兩者等價。**驗收**用 `git show --format= --raw <sha>` 確認每行兩側模式相同——⚠️ **只比對狀態為 `M` 的行**：`A`/`D` 的一側是 `000000`（新增/刪除，非翻轉），而歷史上也存在合法的 `100755 → 100644`，不排除就會誤報（我自己的 awk 犯過）（`--numstat` 看不到模式；`git status --porcelain` 在漂移狀態下恆非空，不可當「還原乾淨了嗎」的儀器）。
...
✅ **Codified**：
- `scripts/ops/protect_main_push.sh` pre-push hook 攔截 main push。⛔ **`pre-commit install` 不會裝它**（只裝 pre-commit 那一層），安裝配方只有 `bash scripts/ops/install_prepush_hook.sh`（冪等）。⛔ **也不要假設「一定有人替我裝了」**：自動呼叫端有兩個（`scripts/session-guards/session-init.py`、`.claude/hooks/session-start.sh`，都 shell out 到同一支；裝不起來會出聲），而多 repo 的 web session 兩個都不跑（[#1719](https://github.com/vencil/Dynamic-Alerting-Integrations/issues/1719)）。⚠️ **`.git/hooks/pre-push` 在不在都不是答案**（git-lfs 會佔著那個檔名，安裝器會把它串成 `pre-push.chained`）——「接上了沒」只有 `make pr-preflight` 的 `Local hooks` 那一列回答，沒接上時它 FAIL 並印出上面那條指令。⛔ **`pre-commit install --hook-type pre-push` 不是替代方案（[#1689](https://github.com/vencil/Dynamic-Alerting-Integrations/issues/1689)）**：經 pre-commit 執行的 pre-push hook 最多只看得到**一列** refspec，於是「同時推 `feat/x` 與 `main`」時 main 可能對守衛隱形——實測它印 `Passed` 而 main 真的推上去了；而 dev-rule #12 要求的 `feat/` `fix/` `chore/` 全部排序在 `main` 之前（⛔ 反過來不成立：排在 `main` 之後的名字、以及分支的第一次 push，main 反而不會被藏住，[#1852](https://github.com/vencil/Dynamic-Alerting-Integrations/issues/1852)）
...
- ✅ pre-push guard 取得 refspec 的通道見 `scripts/ops/_prepush_refs.sh`（⛔ 哪幾支 source 它、誰用兩欄誰用三欄，以該檔的呼叫端為準，本文刻意不複述）。**「最多只看得到一列」的殘差已由 [#1689](https://github.com/vencil/Dynamic-Alerting-Integrations/issues/1689) 修掉**（⛔ 全部是刪除列時一支 hook 都不跑，[#1852](https://github.com/vencil/Dynamic-Alerting-Integrations/issues/1852)）——它們不再註冊為 pre-commit 的 pre-push stage（那條路只餵 hook 一列），改由 `scripts/ops/prepush_dispatch.sh` 用 git 的完整 stdin 呼叫。成因仍是活的（那是 pre-commit 的行為，不是我們能改的），所以量測與邊界仍保留在該檔檔頭與 `docs/internal/hook-vs-skill-coverage.md` §1

**執行入口**：`make pr-preflight` ｜ `win_git_escape.bat pr-preflight [PR#]` ｜ `win_git_escape.ps1 pr-preflight [PR#]`（後兩者寫死 `--skip-hooks`）｜沒有 `make` 的 shell 直接跑 `python scripts/tools/dx/pr_preflight.py [--skip-hooks]`（容器／Linux 用 `python3`，Windows host 的 `python3` 是 Store stub）。報告印 `❌ BLOCKED` ⇒ 工具本身 **rc 1**（[#1472](https://github.com/vencil/Dynamic-Alerting-Integrations/issues/1472)；只有 WARN 回 0）。`.ps1` 原值傳出；`.bat` 只保證非零（`:done_err` 把任何非零壓成 1，所以經它走分不出 rc 1 與 rc 2）；`make` 自身固定 rc 2（畫面上印的 `Error 1` 是工具的 rc）。⚠️ rc 不是 push 閘門——擋住 push 的是 marker，而 marker 綁 SHA。⛔ 何時寫、何時撤銷、撤銷幾顆，以 `pr_preflight.py` 的 `write_marker` / `clear_marker` 為準，本文不複述（#1917 有量測）；只記一條會咬人的：marker 目錄**跨全部工作樹共用**。
...
- **`--json`（stdout 契約，#1112 收嚴）**：machine-readable 子命令須提供 `--json`（或既有的 `--json-output` 拼法），且 `da-tools <cmd> --json | jq` idiom 須在 cli-reference 文件化。**契約**：`--json` 模式下 **stdout 恰好一份 JSON 文件**，於**每一條終端路徑**（被信號結束的行程除外）——含 skip / 空輸入 / dry-run / 早退——**所有人類可讀訊息（進度、摘要、警告、狀態）一律走 stderr**。早退路徑亦須吐 JSON：慣例是**沿用該工具既有 schema 的鍵但歸零/清空，追加 `status`（discriminator）與 `reason`**；正常路徑 schema 不動（consumer 用 `.status // "ok"` 分辨）。矛盾的 flag 組合 → `EXIT_CALLER_ERROR`，不得靜默忽略旗標。
- **`--ci`**：Python 工具用 `--ci` 控 fail-on-finding；**Go binary 不引入 `--ci`**（無跨 Python/Go 統一 wrapper 消費者，CI 對 Go 工具用其原生 flag）。
- **認可例外**：`diag_pr_ci.py`（0/1/2/3，exit 3 = network-blocked，runbook 載明）、`patch-config` apply（寫入前後的各種結果另有 ≥3 的結束碼，對照表在 [cli-reference](../cli-reference.md) §patch-config；#1950）、`tenant-verify`（倒置契約 2=驗證失敗，[cli-reference](../cli-reference.md) + rollback runbook 載明）、`init_project.py`（`--ci` 同名異義：`choices=['github','gitlab','both']` 選 CI config 產出對象，非 fail-on-finding）——改動須連帶遷移文件 + CHANGELOG breaking note。
- ✅ **Codified**：exit code → `tests/shared/test_tool_exit_codes.py`（驗 `--help`=0 + bad-flag=2 + SSOT 常數）；**`--json` stdout 契約 → [`tests/shared/test_json_stdout_contract.py`](https://github.com/vencil/Dynamic-Alerting-Integrations/blob/main/tests/shared/test_json_stdout_contract.py)**（`(tool, mode)` recipe 涵蓋 `--json`/`--json-output` 工具，斷言 `json.loads(全 stdout)`；meta-test 硬斷言 scope，新工具無法靜默逃脫）；**`--ci` fail-on-finding 契約 → `tests/shared/test_ci_flag_contract.py`**（每支一份保證產 finding 的 fixture、同 argv ± `--ci` 跑兩次斷言 exit-code 翻轉——no-op 即 fail-open 直接紅；同名異義 `init_project` 走 meta-asserted allowlist）。exit-code 章節見 [`testing-playbook.md`](testing-playbook.md)。**新增 `--json` 子命令或新增次要模式（`--dry-run` / `--skip-*` 等早退路徑）時，須在該 gate 補 recipe**——最陰險的違規都藏在次要模式。
...
**規則**：本條無文字敘述——由 `tests/dx/test_pair_bench_ratio.py`（`bench-paired.json`）與 `tests/dx/test_write_baseline_marker.py`（`bench-baseline.rows`，TRK-371 / #1635 新增）強制（比照 P2「規則本體即為 hook 程式碼」）。⛔ **每個 artifact 各自需要一份**：本條落地時只有前者，而 `grep -c baseline` 對它實測為 **0** ⇒ #1635 替 baseline 側新增必需欄位時，機械層面**沒有任何東西會紅**。所以「本 PR 受 §P5 覆蓋」這句話，只有在該 artifact 自己有一份 gate 時才為真；沒有就補一份，不要引用本條當作已被涵蓋。兩個 producer 的**鍵集合**各自釘在它們宣告的 schema 字串旁，OK 路徑的 payload 由**跑 producer 讀真實輸出**取得（原始碼的**模型**騙得過、輸出騙不過），並交叉斷言兩者 schema 字串相同。⛔ **本條擋的是「不小心」，不是「刻意」**：`60f4523` 不是有人在消音，是加欄位時沒問這個問題；看到紅燈改 `_OK_KEYS` 而不思考的人，這條擋不住——**任何住在自己所守護的樹裡的 gate 都擋不住**。它保證的是**不可能不小心犯**，且**刻意消音會留下一段自我指證的 diff**。⚠️ 兩個 producer 強度不對等：Actions step 無法在測試裡跑，workflow 那半只能比對**文字**，代價由「抽不到或抽到兩塊一律 fail closed」承擔。⛔ **並且記一次撤回**：本條原本把鍵集合錨定在 git 歷史（`git log --follow -S` 找出引入當前 schema 字串那顆 commit），正是為了讓「改常數」救不了。五輪盲審把它拆了：`--follow` 的改名偵測是**相似度啟發式**，一次普通的 rename + 重排版就讓 pickaxe 回答**攻擊 commit 自己**（自我錨定、空洞地綠，兩個 producer 皆然）；而 rename 來回會讓同一字串在歷史裡進出三次、對一個**一個鍵都沒動**的變更觸發歧義守衛並**黏住**。⚠️ **會亂叫的 gate 會被刪掉，那是繞過的長路版**——錨點被移除，是因為它多出來的強度部分是幻覺，而它的偽陽性不是。⛔ 由來：`60f4523` 替 `bench-paired/v1` 加了含**必需** `status` 在內的三個頂層欄位卻沒 bump，於是每一份合法舊 artifact 都通過 schema 檢查後在下一關被判死；而 `c7d0586` 為一個**選填**欄位 bump 了 v1→v2 ⇒ 破壞相容的沒 bump、不破壞的 bump 了，**規範不存在**而非被疏忽。⚠️ 該 payload 有**兩個** producer（`pair_bench_ratio.py` 與 `bench-record.yaml` 的 `printf` fallback），**兩者皆釘、且交叉斷言 schema 字串相同**——「兩處在寫、一處沒跟上」正是原缺陷的形狀。⭐ **baseline 側走了更強的一步**：`bench-baseline.rows` 只有**一個** producer（`write_baseline_marker.py`，`bench-record.yaml` 的主路徑與 fallback 路徑呼叫同一支），所以那個形狀在該 artifact 上**不可能發生**；gate 改為釘住這個前提本身（恰好兩個 baseline step、兩個都呼叫它）。⇒ 面對「同一份 payload 有兩個 writer」時，**先問能不能收斂成一個**，再考慮交叉斷言。
