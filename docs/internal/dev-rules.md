...
- **Coverage vs the 7 items**: bandit natively gates items 2/**3 (partly)**/4/5/6 (shell B602/B603/B605, **chmod 0o777 B103**, yaml_load B506, hardcoded password B105/B106 partial, eval/exec/pickle B102/B301/B307); items 1/7 (encoding, stderr routing) have no native bandit rule and remain reviewer convention. ⚠️ Item 3's other half (a write-open must be paired with `chmod(0600)`) has no bandit rule and stays in `tests/shared/test_sast.py::TestFileWritePermissions` — B103 only rejects `0o777`. ⛔ **Item 4 is bandit-only since #1643** — the second AST implementation was removed (one exclusive false-red; its **only** increment over B506 was being un-silenceable — a `# nosec` line mutes B506 but never muted the AST copy); items 2/5/6 still carry both, unmeasured. What neither covers is written at rule 4 in `tests/shared/test_sast.py`; the compensating behavioural test itself lives in `tests/ops/test_validate_config.py`.
