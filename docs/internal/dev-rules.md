...
✅ **Codified**：
- `scripts/ops/protect_main_push.sh` pre-push hook 攔截 main push。⛔ **`pre-commit install` 不會裝它**（只裝 pre-commit 那一層），而「誰來裝」是**三態**：人類手動跑 `bash scripts/ops/install_prepush_hook.sh`／AI session 由 `scripts/session-guards/session-init.py` 的 `_heal_git_hooks` **無條件代跑同一支**／Claude Code on the web 的 session 兩邊都沒有（[#1719](https://github.com/vencil/Dynamic-Alerting-Integrations/issues/1719)：本 repo 的 `.claude/settings.json` 整份不載入 ⇒ 那支 hook 不執行）。安裝配方始終只有那一條（冪等；實測全新 clone 只跑前者 ⇒ `.git/hooks/pre-push` 不存在、直推 main 成功且滿螢幕綠）。`make pr-preflight` 會對「沒接上」直接 FAIL 並印出這條指令。⛔ **`pre-commit install --hook-type pre-push` 不是替代方案（[#1689](https://github.com/vencil/Dynamic-Alerting-Integrations/issues/1689)）**：經 pre-commit 執行的 pre-push hook 只看得到**一列** refspec，於是「同時推 `feat/x` 與 `main`」時 main 對守衛隱形——實測它印 `Passed` 而 main 真的推上去了；而 dev-rule #12 要求的 `feat/` `fix/` `chore/` 全部排序在 `main` 之前
