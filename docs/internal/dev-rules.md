...
**執行入口**：`make pr-preflight` ｜ `win_git_escape.bat pr-preflight [PR#]` ｜ `win_git_escape.ps1 pr-preflight [PR#]`（後兩者寫死 `--skip-hooks`）｜沒有 `make` 的 shell 直接跑 `python scripts/tools/dx/pr_preflight.py [--skip-hooks]`（容器／Linux 用 `python3`，Windows host 的 `python3` 是 Store stub）。報告印 `❌ BLOCKED` ⇒ **rc 1**（[#1472](https://github.com/vencil/Dynamic-Alerting-Integrations/issues/1472)；只有 WARN 回 0）。⚠️ 但 rc 不是 push 閘門——擋住 push 的是「FAIL 時不寫 marker」，而 marker 綁 SHA。
