...
- ⛔ **另外三條已知限制（#1396 第三輪盲審開出,刻意不在該 PR 修——都要動整體錯誤策略或引入票的身分識別）**:
...
| `perf-trend` 票沒關,body 出現 `STILL TRACKED BY THIS ISSUE, NOT VERIFIED TONIGHT` | 今夜是 CLEAR,但**這張票在追的 bench 今夜沒被評估**（停止回報 / 同類夜不足） | **正常防線**（fail-closed）。看 body 列出的成因:`no-recent` → 查該 bench 為什麼停跑（perf timeout / crash / harness 變更）;`thin-anchor` → 等同類夜累積。⚠️ bench 若已永久移除,票需人工關閉 |
