...
| `perf-trend` 已修好卻沒自動關 | #1396 後關票要**同機器類別**且低於**凍結基線**；stderr 會印 `NOT closing … not comparable` / `still ≥ frozen baseline` | 若是 `not comparable`,等一晚落在凍結那個機器類別即可（body 有窗口機器組成）；確認真的修好就人工關 |
...
| stdout 出現 `⚠️ INCONCLUSIVE — nothing evaluable tonight` | 今夜機器類別在窗口內的夜數不足（< K recent + 3 settled） | **正常防線**:不 fire 也不關票,等同類夜累積。連續多晚代表 runner pool 太雜,考慮拉長 `--trend-limit` |
