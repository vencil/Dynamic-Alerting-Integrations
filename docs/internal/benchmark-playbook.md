...
- **同類主機分層 + 三態（#1396）**:GitHub hosted runner pool 是異質的——實測一個 30 夜窗口落在 4 種 CPU（Intel Xeon 8370C / XEON 8573C / AMD EPYC 7763 / EPYC 9V74）,IO/CPU 比 **11.8 / 19.1 / 25.3 完全分離**。混機器的 anchor 比的是 CPU 不是 commit,所以 recent 與 anchor **兩個窗口都只取今夜同類的夜**（分層鍵 = artifact `cpu:` 表頭的**原始字串**,精確相等;⛔ 刻意不把兩顆 Xeon SKU 歸成一家——那等於把這個洞重造一次）。同類 recent 夜不足 K、或同類 settled 夜 < 3 → 該 bench **INCONCLUSIVE**（不 fire、**也不關票**）。⛔ **三態的重點是 CLEAR ≠ INCONCLUSIVE**:舊碼 `findings == []` 就關掉所有 open issue,於是「今晚沒有東西可評估」被當成「已復原」。**fire 判定的算式未動**,只換了它跑在哪個族群上。⚠️ **「未分層退路」只在「全窗每一夜都沒有 `cpu:`」時才走**（#1396 follow-up E）:只有今夜讀不到、其他夜讀得到 → **INCONCLUSIVE**,不退回未分層。理由是退回會同時做兩件事——把今夜拿去跟窗內任何機器比,**而且**把 settled 下限從 3 降回 2——實測正是會讓 #1396 窗口再 fire 的組態,一行 `cpu:` 沒解析到就能把 CLEAR 夜翻成一張 30.8% sustained 新票。
- **INCONCLUSIVE 一定會出聲（#1396 follow-up C）**:過去 INCONCLUSIVE 是**綠燈 + exit 0 + 零 annotation + 零 step summary + issue 完全不動**——一支注入的真 +30% 永久 regression 落在同類夜不足的夜,對外完全看不見。現在同時寫三處:stderr `::warning::`、`$GITHUB_STEP_SUMMARY` 一行、以及（有 open issue 時）**刷 body 標明「今晚未評估（host class X,同類夜 N/需 M）」+ 這批數字取自哪一夜**。exit code 仍為 0（watchdog 不擋任何東西,annotation 才是訊號）。
- **marker 解析取最後一個匹配 + `cpu:` 字串無害化（#1396）**:這兩條看起來像 state 機制的細節,其實是上面「揭露 `cpu:` 表頭」開出來的洞。body 把**原始 `cpu:` 字串印在 marker 之前**,而解析取的是第一個匹配——一個含假 marker 的 `cpu:` 表頭實測可劫持解析（真 marker 在 offset 1773、regex 卻匹配 596）,watchdog 讀到的「上一晚狀態」於是由 artifact 自己撰寫,足以憑空發出一則「Recovered (no longer flagged)」。兩邊都修:解析取**最後**一個匹配（watchdog 永遠把自己的 marker 附在最後）,且 body 輸出的 CPU 字串一律 escape `<!--` / `-->`。
- **關票是「每支 bench」的主張,不是「每夜」的（#1396 第三輪）**:三態擋掉了「今晚什麼都沒評估」,沒擋掉「今晚評估的不是這張票在講的那一支」。⛔ 實測:票為 `BenchmarkA` 而開（marker 記著）、A 今夜停止回報（**這正是 perf timeout/crash 的徵狀**）、B 乾淨 ⇒ `status=CLEAR, evaluated=[B], inconclusive=[A]` ⇒ **關票**,而同一份 body 的 marker 還寫著 A 是 sustained。現在關票前先讀該票 marker 的 flagged 集合,**任一支不在今夜 `evaluated_benches` 就不關票**（判準是「有沒有被評估」而非「有沒有落在 inconclusive」——一支從窗口整個消失的 bench 兩張名單都不在,它並不比被略過的那支更可信）,並在 body / stderr / `::warning::` / step summary 四處講明「今夜無法驗證 X」。marker 讀不到（legacy／人工開的票）**維持原行為關票**——那種票沒有可查的 per-bench 主張。同一套判準也擋住通知層:marker **把今夜量不到的列往前帶**（帳本不遺忘,下一個 CLEAR 夜的守衛才還武裝著;且前後狀態一致 ⇒ 假的 `Recovered` comment 從源頭產生不出來）,`recovering` label 的 `add` 也被擋（`remove` 不擋,它由今夜真的量到的 sustained 驅動）。⚠️ **代價是 fail-closed**:一支 benchmark 不再回報後,為它開的票會一直開著（沒有任何夜能再驗證它）,需要人工關閉——這優於反過來的「沒量到就當復原」。**觸發條件與受害範圍見下一條**（第四輪更正,原本這裡寫的低估了兩個量級）。
- **帳本只會變長不會變短——held-open 夜必須剪掉今夜量到且乾淨的列（#1396 第四輪）**:上一條的守衛「加什麼進帳本」是對的,「該拿掉什麼」從沒被問過。held-open 路徑原本把整份舊帳本原樣寫回（`state=prior_state`,而 FINDINGS 路徑用的是 `state=state_for_body` = 今夜 findings + 量不到的列）,於是「今夜量到且乾淨」的列永遠不會被剪掉。⛔ 實測三種**方向相反**的錯誤:①兩支 bench 輪流不可評估 ⇒ 票永遠關不掉（**分層本身就會製造這種 parity 交替**）;②帳本殘留 `B sustained`、隔夜 B 真的 sustained regression ⇒ 前後狀態相同 ⇒ **transition comment = `None`,訂閱者收到 0 則通知**;③帳本殘留 `B sustained`、隔夜 B 只有 creep ⇒ 被說成 `**Eased to creep:**`（好轉）,正確帳本下應是 `Newly flagged`。現在寫入 `state=tracked`(等價於 `_carry_forward_state([], tracked)`,與 FINDINGS 路徑同一條規則)。同批:**關票時保留 prior marker**（原本寫空 marker,人工 reopen 後守衛永久失效——而 reopen 正是自動關票留言自己建議的補救動作）;`recovering` label 的 `add` 只被**prior 為 sustained 且今夜未驗證**的 bench 擋（原本被整個 prior 集合封死,一支無關的 creep 列就能吃掉真的好消息）。
- ⛔ **per-bench 守衛的真實代價（#1396 第四輪更正,原文低估兩個量級）**:**觸發條件是「改名」不是「永久移除」**——marker 與分層鍵都用**字串完全相等**比對,`BenchmarkFoo` → `BenchmarkFooV2` 與「Foo 消失」無法區分,而改名是例行重構;**受害範圍是整個 repo 的自動關票,不是那一張票**——FINDINGS 只在 `open_issues` 為空時才開新票、且只更新 `[0]`,所以卡住的那張票會把之後**所有** bench 的回歸都 carry 進自己,新票也開不出來。守衛擋住關票時的 `::warning::` / step summary 現在會指名**卡住的是哪一支、幾夜沒回報**（`_nights_since_seen`,從既有窗口推導,不動 marker 格式）,並在該 bench 整個窗口都沒出現時印第三種成因 `absent-from-window`（原本落到「一支 benchmark 都沒出現」的兜底句,而它是印在 CLEAR 夜、緊鄰其他 benchmark 的 CLEAR 判定旁邊的自相矛盾句）。**結構性修法（bench 身分要能撐過改名）不在該 PR 範圍,見下一條已知限制。**
- **INCONCLUSIVE 的成因分兩類,不再一律歸咎窗口（#1396 第三輪）**:`no-recent`（該 bench 沒在最近 K 個同類夜全部回報 = **停跑徵狀**）vs `thin-anchor`（同類 settled 夜不足）。舊碼一律印後者,在全窗同類的窗口會印出「only 14 of 14 window nights ran on tonight's host class」這種自相矛盾句,把讀者推去等更多同類夜、而真正該查的是 bench 為什麼停跑。body、stdout、annotation、step summary 四處同步。
- **canary 零樣本 ≠ 零抖動（#1396 第三輪）**:`_cv` 對「沒量到」與「量到而完全穩定」回傳同一個 `0.0`,body 因此印 `0.00%`——那是「已測得零抖動」的正面斷言。現在 `canary_n < 2` 時印「**not measured**」並明講 floor 不含 canary 成分。
- **watchdog 沒跑成也一定會出聲**:除了 INCONCLUSIVE,另外兩條靜默路徑同批補上——**夜數不足的短窗 bail-out**（`< K + 2` 夜,原本只 print 到 stderr）現在同樣 `::warning::` + step summary;**`gh issue create` 兩次都失敗**時明講「一張票都沒開、沒有人被通知」,而不是像舊碼那樣在第二次嘗試**之前**就先宣告「已改開不指派的 issue」（第二次也失敗就變成宣稱一張不存在的票）。第三輪再補三條:**靜默退回未分層**（已知會誤報的模式）現在也 `::warning::` + step summary;**update / close 路徑的 `gh` 失敗**從一行 stderr 升為 annotation + step summary;**close 失敗不得再貼「Auto-closing」留言**（舊碼無條件貼 ⇒ 一張還開著的票上掛著宣告它已關閉的留言）。
- **狀態化 issue lifecycle（#754 follow-up,治洗版）**:已開 issue 時**每晚原地改寫 body**(`gh issue edit`,永遠反映當前表格),**只在「被旗標的 bench 集合改變」時才留 comment**(新增 / 復原 / creep↔sustained 升降級);狀態以 body 內隱藏 HTML-comment marker（`<!-- perf-trend-state v1 [[bench, kind]] -->`;`kind` ∈ `sustained` / `creep`）持久化、下次 run 解析回來比對。舊版每晚 append 一整張表 → #702 累積數十筆雜訊。⚠️ INCONCLUSIVE 夜刷 body 時**沿用上一晚的 marker**,不寫入今夜的空 finding set——今夜什麼都沒量到,把 marker 清空等於下一個真的判得動的夜會把每支 bench 都當成「新旗標」重新通知。**recovering label**:當 sustained 已清、只剩 creep,自動上 `perf-trend:recovering`(任一 sustained 回來或 issue 關閉即移除),讓訂閱者一眼分辨「仍惡化 vs 復原中」。無 marker 的 legacy body 視為「無變化」→ 遷移那一晚靜默(只補 marker、不誤發 comment)。
- ⛔ **已知未修:關票仍用滑動 anchor（#1396 未結,只修了偵測面）**:滑動 anchor 對**偵測**是對的、對**復原**是致命的——永久 regression 會滑進 settled window,anchor 漲上去迎合它,finding 自行消失,CLEAR 夜就把票關掉。⚠️ **但發生頻率未量測**:此處原本引用過一組關票率（68% / 160-of-236）,該數字來自一支**從未 import 受測模組、自行重寫「no findings → close」**的 scratchpad 腳本,量到的是模型不是這份 code,已移除且**不以另一個數字取代**——沒有誠實的數字可用時,錯的數字比沒有數字更糟。要重做必須直接驅動 `run_trend_watch` 跑連續多夜,並把每夜 render 出的 body 當成下一夜的 issue body 餵回去（marker 是這支工具唯一的跨夜記憶,跳過它的 harness 測的不是這份 code）。因此**關票留言只講它真的比過的東西**（「今夜同類窗口內沒有 bench 高於 floor」）並明講該判定基於會漂的 anchor,不宣稱 perf 回到當初開票的水位。⚠️ 實務上:`perf-trend` 票自動關掉**不等於**該 regression 已修——關票後請對照 bench 的絕對數值,或等重新開票。凍結 anchor（首次 fire 就把當時 anchor + 機器類別凍進 marker、關票須低於凍結基線且同機器類別）的修法已實作過並在合併前整組退回:算式對,但周邊狀態生命週期（marker 帳本語意、marker 健康度、永遠無法判定的列）六處各自可重現的破口,且其 fail-closed 安全網從未被任何測試執行過。留在 #1396 重新設計。
- **關票時帳本一律清空（#1396 第五輪盲審 A1;上一輪「保留帳本供 reopen」整組回退）**:走到關票那一行時 `unverified` 必為空（held-open 守衛在它之前就 `continue` 掉了）,代表 marker 裡**每一列今夜都被評估過且都在 floor 之下**——把它寫回去等於重申一份當夜剛被推翻的帳本,而且該路徑不傳 `tracked=`,連一行 STILL TRACKED 揭露都沒有。實測症狀:body 散文寫 `### ✅ Nothing above its floor tonight (CLEAR)`,隱藏 marker 卻是 `[["BenchmarkA","sustained"],["BenchmarkB","creep"]]`;reopen 後下一夜的 transition 把真正的 `Newly flagged` 講成 `**Eased to creep**`（好消息）、`recovering` label 可被錯加、帳本在關票路徑上是只增不減的固定點。⚠️ 立論前提也是錯的:自動關票留言實際寫的是 `A new issue is filed if it regresses again.`,**從未建議過 reopen**。**代價（已知限制,見下）**:reopen 的票不再有 per-bench 守衛。
- **held-open 警告新增第四種成因「在窗口裡,但不在今夜的 host class stratum」（#1396 第五輪盲審 A2）**:舊碼 `_nights_since_seen` 掃**整個窗口**、成因卻只從**今夜同類別 series** 判,沒有 reason code 就一律 fallback 成「改名或刪除」——於是同一句 warning 同時出現「1 夜前才回報過」與「一夜都沒出現…改名或刪除才會到這個狀態且永遠出不來」。異質 runner pool 下這是常態不是改名。現在:兩個 helper 都只看今夜同類別的夜（措辭也改成 `same-class night(s)`）,成因分四類,off-class 明講「會自己好」、rename 才講「永遠出不來」。
- **marker「讀不到」≠「沒有」;部分評估的 CLEAR 夜不是 all-clear;`thin-anchor` 拆成兩種成因（#1396 第六輪盲審 F1–F3）**:狀態機本身這輪被確認乾淨（跨夜 marker 串接、四條不變式、76 例手改 marker × 四路徑零崩潰、注入面雙防線有效）,剩下兩處都在外圍可觀測性與散文層。⭐ **F1 marker 解析失敗只有一行裸 stderr**——實測同一份 body 只差 marker 末尾一個 `]`:完好 ⇒ 不關票／1 個 `::warning::`／有 step summary;少一個 `]` ⇒ **關票／0 個 annotation／step summary 檔案根本沒建立**（regex 貪婪吃到最後一個 `]` 仍匹配,`json.loads` 才死,CLEAR 路徑於是拿到一張「什麼都沒在追蹤」的票）,而 body footer 無條件寫著 `never while ANY benchmark this issue is tracking went unmeasured`。現在解析失敗補 `_warn` + `_step_summary`,**absent（legacy 票,本來就沒有 per-bench 主張,維持靜默）與 unreadable（有主張、讀不到）分成兩種訊息**,footer 那句加上「marker 可讀時」限定。⛔ **關票行為刻意不變**——不引入 marker 健康度狀態機,那是被退回的設計。⭐ **F2 只評估到部分 bench 的 CLEAR 夜**——實測 14 夜同 host class、一支 bench 只在最新 3 夜出現且帶真實 +30%（settled 夜 0 ⇒ 判不動）:stdout 一個綠勾、stderr 全空、summary 檔案沒建立,而 `meta` 裡寫著 `evaluated=[另一支]`／`inconclusive=[它]`。現在 `inconclusive_benches` 非空時補 `::warning::` + step summary（訊息取 `meta` 裡已算好的成因）,✅ 那句不再無條件。⭐ **F3 `thin-anchor` 的自相矛盾第三次復發**——全窗同一個 host class 時印「reported tonight but could not be judged — only 14 of 14 window nights ran on tonight's host class」,前半說它今晚就在這台機器上跑過,後半怪 host class 組成,真正的原因（這支 bench 自己只有 3 夜歷史）沒提。改由**哪一個計數短**決定成因:`thin-anchor` = 窗口的同類 settled 夜不足（**沒有任何** bench 判得動）、新的 `thin-bench-history` = 窗口夠而這支 bench 自己不夠（新加或剛改名）。兩者互斥（窗口短時每支 bench 的 anchor 都短）,新成因明講「不是窗口的 host-class 組成」。**fire 判定的算式一個字都沒動**:真實 30 夜資料重放 17 個窗口 + 240 個 δ=20% 注入情境,detector 輸出與修前逐位元相同。
- **footer 承諾了它兌現不了的事;而它唯一真的會出聲的那個情況,annotation 在同一夜就被自己洗掉（#1396 第七輪盲審 F1–F2）**:偵測核心、狀態機、注入面、崩潰面經七輪判定乾淨,本輪只收這兩條、其餘記票。⭐ **F1 footer 的「edited-away 或損毀的 marker…the run says so in its log, its annotations and the job summary」有一半是空頭支票**——實測把 marker **整行刪掉**:關票、**零個 marker 相關 annotation**、step summary 對 marker 一個字都沒有（`grep -c marker` = 0）。根因不是漏了誰:`_parse_state_marker` 對 ABSENT **刻意**沉默,理由它的 docstring 早就寫著——ABSENT 與 legacy 票**在型別上不可分辨**,legacy 票本來就沒有 per-bench 主張,對它出聲等於每夜對每張舊票發噪音。⛔ **因此修措辭不修機制**（不加辨識欄位、不對 legacy 票發警告）:footer 現在只承諾「marker 可讀時」與「marker **損毀**時會出聲、且**每夜都出聲直到修好**」,並改為**揭露**刪掉 marker 會靜默解除守衛（見已知限制 11）。⭐ **F2 上一輪補的 annotation,在回報它的同一夜就被自己覆寫掉**——`_parse_state_marker` 回 None（PRESENT but UNREADABLE）發 `::warning::` + step summary,但**同一夜的 body refresh 把壞 marker 覆寫成乾淨的空 marker**,隔夜起每夜全綠,證據永久消失而 per-bench 守衛仍然是瞎的。實測 `night N warnings=2／寫回 <!-- perf-trend-state v1 [] -->` ⇒ `night N+1 (CLEAR) closed=True／warnings=[]／無 summary`;對照組（N 夜不覆寫）⇒ N+1 夜仍發 1 個 warning。兩條路徑都會洗:INCONCLUSIVE（`_parse_state_marker(...) or []` → 寫回 `state=[]`）與 FINDINGS（壞 marker 記的 bench 被今夜 finding set 取代 ⇒ 永久消失）。⚠️ INCONCLUSIVE 路徑**自己的註解**明寫「marker 從 PRIOR state 重寫,**不是**從今夜的空 finding set」——`or []` 在 unreadable 分支做的正是它禁止的事。修法是**對既有寫入加守衛**:UNREADABLE 時兩條路徑都不覆寫 body,讓 annotation 每夜重發。⛔ **不新增生命週期狀態／marker 欄位／判別機制**——ABSENT/UNREADABLE 的區分上一輪就存在（annotation 靠它分的）,直接沿用;**ABSENT 維持照常寫入**（legacy 遷移路徑,擋掉會讓舊票永久出不了生命週期）;**關票路徑刻意不動**（上一輪明示保留）。**fire 判定的算式一個字都沒動**:真實 30 夜資料重放 17 個窗口（11 個可評估、false-positive bench-nights = 0）+ 240 個 δ=20% 注入情境,detector 輸出與 HEAD 逐位元相同。⚠️ **測試斷言只讀 marker 那一條 `::warning::`,不用「warning 數 > 0」**——同夜的 CLEAR-but-INCOMPLETE 等 annotation 會遮蔽它,上一輪就是這樣差點假綠（`test_other_warnings_on_the_same_night_would_have_masked_this` 把遮蔽釘住）。
- ⛔ **另外十三條已知限制（#1396 第三／四／五／六／七輪盲審開出,刻意不在那些 PR 修——都要動整體錯誤策略或引入票／bench 的身分識別）**:
  1. **`gh issue list` 失敗是致命的**——其餘 `gh` 呼叫皆 best-effort（`_gh_write` 永不 raise）,只有這一支例外,列 open issue 失敗會讓整晚不做事。要修就要一併決定「讀失敗」與「寫失敗」的策略是否該一致。
  2. **`--ci` 的 run-reliability gate 從不執行**——`--trend-watch` 在它之前就 `return`,那道 `>= 26/28 runs` 的 gate 只對 GO/NO-GO 變異表模式有意義,對 watchdog 模式是死碼。
  3. **任何掛 `perf-trend` label 的票都被當成自己的**——人工開的、其他工具開的都會被 update 甚至 close。要修需要在票上放身分識別（例如 marker 內加 owner 欄位）並處理 legacy 票的遷移。
  4. **bench 的身分撐不過改名**——marker 記的是名字,改名後舊名永遠不可驗證,那張票就卡住（連帶卡住全 repo 的自動關票,見上）。要修需要 bench 身分識別（或 marker 記 host class + 一套 rename 對映）。
  5. **四個真 subprocess 邊界函式零執行**——`_gh` / `list_recent_runs` / `download_artifact` / `night_records_from_gh`。測試的 seam 就開在 `ab._gh`,這四支本身沒有任何測試跑過（要修需要一層 fake CLI 或 record/replay fixture）。
  6. **held-open 夜的帳本收縮不發 transition comment**——該路徑本來就不留言,所以一支 bench 在 held-open 夜恢復時沒有人被通知。比反過來的假復原安全,但仍是缺口。⚠️ **不對稱（第六輪 F6 補記）**:FINDINGS 路徑對**同一個 delta** 會發留言（它走 `_state_transition_comment`）,held-open 路徑不會——於是「同一支 bench 在哪一種夜恢復」決定了訂閱者會不會收到通知。剪枝本身是對的（第四輪）,缺的是通知。
  7. **reopen 的票沒有 per-bench 守衛**(第五輪 A1 的代價)——關票時帳本清空,人工 reopen 後 marker 是 `[]`,下一個 CLEAR 夜會直接再關一次、不檢查它當初是為哪一支 bench 開的。要修需要 issue 生命週期狀態（`closed_on` 之類的新欄位已明確否決:在這個檔案裡加生命週期狀態,五輪下來每次都長出新缺陷）。
  8. **`tonight-unknown` 夜的 body 首句與第十句互相矛盾**(第六輪 F4)——`span` 只讀 `meta["stratified"]` 這個布林,於是 TONIGHT_UNKNOWN 共用 LEGACY 的文案「Detected across the last N nightly `bench-record` runs (no host-class stratification — see below)」,而同一份 body 的 host-class 區塊寫的是「正因為不能未分層地判,所以今晚什麼都沒判」;「Detected」本身也在一個什麼都沒偵測到的夜上。同源兩處（一個布林餵兩種 stratification）,要修得讓 `span` 讀三態 `stratification` 並為 TONIGHT_UNKNOWN 另寫一句——同時該分支的 LEGACY 文案實務上幾乎不可達（要全窗每一夜都沒有 `cpu:` 表頭）,故列為已知限制而非本輪修法。
  9. **`Recovered (no longer flagged)` 沒有 SLIDING-anchor 的 scope 限定**(第六輪 F5)——自動關票留言在第四／五輪補上了「該判定基於會漂的 anchor,不代表 perf 回到開票水位」,但 transition 留言的 `Recovered` 是**同一個 anchor 產生的同一種主張**,字串仍是 main 時代的裸句。⚠️ 這個不對稱是本條線造成的（補了關票留言那一邊、沒補通知那一邊）,兩處措辭應一起改。
  10. **FINDINGS 路徑的 body 寫入回傳值被丟棄**(第六輪 F7,**main 既有**)——`_gh_write(["issue","edit",…,"--body",body])` 回傳 bool 卻沒被讀,transition 留言不受它成敗牽制。該處註解自陳的設計是「body（帶已推進的 marker）先寫,寫失敗就該讓下一夜不再重發同一則留言」,但 body 寫失敗時留言照發 ⇒ 隔夜 marker 仍是舊的 ⇒ 同一則 transition 再發一次,正是那段註解要殺的洗版。要修需要決定「部分失敗」的整體策略（與已知限制 1 同一類:讀失敗與寫失敗的策略是否該一致）。
  11. **人工把 marker 整行刪掉會靜默解除 per-bench 關票守衛**(第七輪 F1 的代價)——`_parse_state_marker` 對 ABSENT 回 `None` 且不出聲,而那是**刻意**的:ABSENT 與 legacy 票在型別上不可分辨,對它出聲等於每夜對每張沒有 marker 的舊票發噪音。於是刪掉 marker 那一行的票,下一個 CLEAR 夜會直接關掉、**三個管道（log／annotation／step summary）一個字都不提 marker**（實測 `grep -c marker` = 0）。⛔ 損毀（PRESENT but UNREADABLE）**不是**這一類,它每夜都會出聲且 body 不再被覆寫（第七輪 F2）。要修需要能把「這張票曾經有過 marker」與 legacy 票分開,亦即在票上放身分識別——與已知限制 3／7 同一類,而**在這個檔案裡加生命週期狀態,七輪下來每次都長出新缺陷**。目前的補償是 footer 直接寫「不要刪掉 body 最後一行」。
  12. **同類夜少於 `recent_k` 時,每一支 bench 都被歸因成「benchmark 停止回報」**(第七輪 F3)——`recent = [n.medians.get(b) for n in series[:recent_k]]` 之後的 `len(recent) < recent_k` 與 `any(v is None ...)` 共用同一個 `continue`,而前者為真時**與這支 bench 本身無關**:短的是今夜同類 series 的長度。實測（`recent_k=3`、某 bench **14 夜全部有回報**）:`n_class_nights` 1–2 ⇒ 成因 `no-recent`,印出「did not report on all 3 newest same-class nights (a benchmark that STOPS reporting is the classic perf-timeout/crash symptom, **not a thin window**)」——最後五個字正好是真相的反面;`n_class_nights` ≥ 3 ⇒ 正確落到 `thin-anchor`。誤診的方向最貴:它把讀者推去查 perf timeout / crash / harness 變更,而該做的是等同類夜累積。與第六輪 F3 是同一個病的第四次復發（成因由「哪一個計數短」決定,而這個分支還沒被拆),修法應與那次一致——多一個成因碼,但那要動 `_inconclusive_causes` / `_unverified_causes` / body / stdout / annotation / step summary 六處措辭,不在只收兩條的本輪範圍。
  13. **CLEAR 的「部分評估」守衛看不到今夜 stratum 以外的 bench**(第七輪 F4)——`skipped = meta["inconclusive_benches"]`,而 `inconclusive` 只從 `benches = {b for n in series ...}`（**今夜同類別的** series）產生。一支出現在窗口裡、但**今夜這個 host class 一夜都沒跑過**的 bench,`evaluated` 與 `inconclusive` **兩邊都不在**,於是那道守衛不觸發、印出無條件綠勾。實測（14 夜、tonight=EPYC、某 bench 只在 3 個 XEON 夜出現）:`status=CLEAR`／`evaluated=['BenchA']`／`inconclusive=[]`／`window_benches=['BenchA','BenchOffClass']`,守衛不觸發。⚠️ 這正是第六輪 F2 修的那個病在**另一個母體**上的重演——F2 修的是「stratum 內判不動」,這條是「根本不在 stratum 內」。`meta["window_benches"]` 已經備妥（`_unverified_causes` 就在讀它,用來分辨 off-class 與 rename）,缺的是 CLEAR 分支去讀它;但那會把「今夜沒排到這台機器」變成每夜的常態警告（異質 runner pool 下 off-class 是常態不是異常),需要先決定噪音策略——與已知限制 6 同一類。**有 open issue 時這條不致命**:per-bench 關票守衛讀的是 marker 而非 `inconclusive_benches`,票仍會被 held open;漏的是**沒有 open issue 那一夜**的告知。
...
| `perf-trend` **自己關掉了但 regression 還在** | ⛔ 已知未修（見上節最後一條）：關票用的是會漂的滑動 anchor，永久 regression 滑進 settled window 後 anchor 漲上去迎合它 | 別把自動關票當成「已修復」的證據。對照 bench 絕對數值；仍高就手動 reopen 或等它重新開票。⚠️ **reopen 的票沒有 per-bench 守衛**（關票時帳本清空,見已知限制 7）——它會在下一個 CLEAR 夜再關一次,所以 reopen 只適合當「人工待辦」用,別指望它自己撐開 |
| body 顯示 `stratification: OFF (NOT stratified)` | 該夜 artifact 沒有 `cpu:` 表頭（舊 artifact / 解析失敗） | 該夜的 finding **可能是 runner 換機造成**,不是 regression;查 run log 的 `cpu:` 行再決定 |
| stdout 出現 `⚠️ INCONCLUSIVE — nothing evaluable tonight` | 今夜機器類別在窗口內的夜數不足（< K recent + 3 settled）,或今夜 `cpu:` 讀不到而其他夜讀得到 | **正常防線**:不 fire 也不關票,等同類夜累積。⚠️ 但**這一夜等於沒體檢**（annotation + step summary 會這樣寫）,不要當成「perf 沒事」。連續多晚代表 runner pool 太雜,考慮拉長 `--trend-limit` |
| stderr 出現 `::warning::perf-trend watchdog did NOT run` | 窗口內可用夜數 < `--recent-nights + 2`（nightly 剛啟用、連續多晚 workflow 失敗、或 artifact 抓不到） | **這一夜沒體檢**,不是「perf 沒事」。查 `bench-record.yaml` 近幾晚的 run 是否綠;夜數補齊後自動恢復 |
| stderr 出現 `::warning::… NO issue could be filed` | 偵測到 regression 但 `gh issue create` 兩次（指派 + 不指派）都失敗 | **沒有任何票被開、沒有人被通知**。看同一個 job 的 step summary 取 finding 清單並人工開票;查 `GITHUB_TOKEN` 權限 / API 狀態 |
| `perf-trend` 票沒關,body 出現 `STILL TRACKED BY THIS ISSUE, NOT VERIFIED TONIGHT` | 今夜是 CLEAR,但**這張票在追的 bench 今夜沒被評估**（停止回報 / 同類夜不足） | **正常防線**（fail-closed）。看 body 列出的成因（四類）:`no-recent` → 查該 bench 為什麼停跑（perf timeout / crash / harness 變更）;`thin-anchor` → 等同類夜累積;`DID report inside the window, but never on tonight's host class` → **只是今夜排到別台機器**,會自己好,不要當改名處理;`did not appear in ANY of the N nights … RENAMED or deleted` → **該 bench 已改名或刪除**,票必須人工關閉（否則它會卡住整個 repo 的自動關票,見上節）。warning 會直接寫出卡住的是哪一支、幾個**同類別**夜沒回報 |
| stderr 出現 `::warning::perf-trend watchdog is running UNSTRATIFIED` | 窗口內**沒有任何一夜**有 `cpu:` 表頭（全是舊 artifact / 解析全失敗） | 該夜的 finding **可能是換機造成**。查 `bench_filter.go` 的 `retainPrefixes` 與近幾晚 artifact 是否真的少了 `cpu:` 行 |
| stderr 出現 `::warning::perf-trend watchdog: <操作> FAILED` | update / close 路徑某個 `gh` 寫入失敗（body / label / comment / close） | **issue 沒反映今夜的結果**。若是 close 失敗,票仍開著且**刻意不會**貼 Auto-closing 留言;查 API 狀態後人工補做該操作 |
| stderr 出現 `::warning::… state marker is PRESENT but UNREADABLE`,**且每夜重複** | 該票 body 末尾的隱藏 marker 有東西但 parse 不了（多半是人工編輯 body 時弄壞,例如少一個 `]`） | **人工修 marker 是唯一出路**。該票的 per-bench 關票守衛今夜是瞎的（marker 讀不到 ⇒ 不知道它為哪支 bench 開的）。⚠️ **每夜重複是設計**(第七輪 F2):UNREADABLE 時 body **刻意不再被刷新**,好讓損毀留在票上、警告持續發——所以那張票的表格會停在最後一次可讀的夜。修法:把最後一行改回合法的 `<!-- perf-trend-state v1 [["<bench>","sustained"]] -->`（或整行刪掉,但那會**靜默**解除守衛,見已知限制 11）|
| body 出現 `Control-canary night-to-night CV: **not measured**` | 今夜同類夜裡 `BenchmarkControlCanaryCPU` 樣本 < 2（canary 沒 emit / 該類夜太少） | floor 退回固定下限（不含 canary 成分）。**不是「零抖動」**。查 canary 為什麼沒進 artifact |
...
> ```
>
> 產出：`bench.out.txt`（僅 `goos:` / `goarch:` / `pkg:` / `cpu:` / `BenchmarkX ns/op` / `PASS` / `FAIL` 行）、`bench.err.log`（`go test` 的 fd 2，成功跑時為空——見下）、`bench.raw.jsonl`（`go test -json` 原 event stream 供 debug）。
>
...
>
> ⚠️ **編譯失敗的診斷落在哪裡取決於 Go 版本**：go1.24+ 的 `bench.err.log` 連編譯錯誤都是空的，診斷由 `bench_filter.go` 印到 stderr（[#1872](https://github.com/vencil/Dynamic-Alerting-Integrations/issues/1872)）。各版本的當前語意以 `bench_wrapper.sh` 檔頭為 SSOT，不在本檔複述。
>
