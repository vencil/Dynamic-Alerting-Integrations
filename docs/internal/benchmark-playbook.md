...
- **凍結 anchor — 假復原防線（#1396 Critical）**:滑動 anchor 對偵測是對的、對**復原**是致命的——永久 regression 會滑進 settled window,anchor 漲上去迎合它,finding 自己消失,舊碼接著關票並宣稱「perf has returned below the floor」（30 夜重放:71% 的真 regression issue 自行關票,中位 6 夜）。因此 state marker 升 v2,首次 fire 就把當時的 anchor 與機器凍進 marker;**關票必須該 bench < frozen_anchor × (1 + floor) 且同一機器類別**,跨類別＝無從比較＝維持 open。留言只講它真的比過的東西。
- **marker 是「尚未證明復原」的帳本,不是今夜 finding 的快照（#1396 follow-up A,High）**:舊碼每晚只用**今夜的 findings** 重建 marker,於是一支「今夜剛好沒 fire」的 bench 其凍結基線被**驅逐**——而 anchor 漂到 regression 上正是它今夜沒 fire 最可能的原因。兩支 bench 錯開復原時間就足以拿回假關票。現在 row 的 kind 除 `sustained`/`creep` 外多一個 **`held`**（曾凍結、今夜未 fire、也尚未通過復原檢定）,**只有該 bench 通過復原檢定才會把該列移除**。`held` 列不進 transition comment、也不影響 recovering label,「Recovered (no longer flagged)」維持原語意。
- **關票用的是 recent 窗中位數,不是單一夜（#1396 follow-up H）**:fire 要 K 夜一致、close 卻只看今夜,等於同一支工具裡「關」比「開」寬鬆 √K 倍——一個運氣好的夜就能退掉真 regression。close 改讀 `analyze_trend` 從**同一組分層 series** 算出的 `recent_medians`（同樣要求該 bench 在 K 夜全數在場）,成本為零。
- **讀不到的 marker 一律 fail-CLOSED（#1396 follow-up B,High）**:舊碼 marker 只要有一列壞掉就整段丟棄 → `frozen={}` → 走 legacy 關票路徑 → **帶病自動關票**（實測 `[[...],["oops"]]` 一列壞掉,今夜 +50% 仍關票;人工刪掉 marker 同理）。現在壞列**跳過不致命**,並區分四種 marker 健康度:`v2 ok` / `v1 legacy`(照舊關票路徑) / **`absent`** / **`damaged`**——後兩者**擋住關票並發 `::warning::`**,且**不覆寫 body**(拿一個空 marker 蓋掉讀不懂的狀態,等於下一晚照樣關)。
- **關不掉又無從驗證 → 揭露,不自動關（#1396 follow-up F）**:bench 消失、或凍結的機器類別再也沒出現,復原檢定永遠不會通過 = 永久楔子。刻意**不**自動關票（守望器寧可卡著也不能無證據關掉),改成看得見:marker row 第 5 格記「連續無法判定夜數」,body 多一張 **Held — awaiting proof of recovery** 表（哪些 bench、凍結基線、為什麼、擋了幾晚）,連續 ≥3 晚再自動上 **`perf-trend:unverifiable`** label 交人決定。能重新量到就自動歸零、label 自動拿掉。
- **marker 解析取最後一個匹配 + `cpu:` 字串無害化（#1396 follow-up G）**:body 把原始 `cpu:` 字串印在 marker **之前**,而解析取的是第一個匹配——一個含假 marker 的 `cpu:` 表頭實測可劫持解析（真 marker 在 offset 1773、regex 匹配 596、凍結基線全歸零）。兩邊都修:解析取**最後**一個匹配（watchdog 永遠把自己的 marker 附在最後）,且 body 輸出的 CPU 字串一律 escape `<!--` / `-->`。
- **狀態化 issue lifecycle（#754 follow-up,治洗版）**:已開 issue 時**每晚原地改寫 body**(`gh issue edit`,永遠反映當前表格),**只在「被旗標的 bench 集合改變」時才留 comment**(新增 / 復原 / creep↔sustained 升降級);狀態以 body 內隱藏 HTML-comment marker（**v2**:`<!-- perf-trend-state v2 [[bench, kind, frozen_anchor_ns, frozen_cpu_model, unverifiable_nights?]] -->`;`kind` ∈ `sustained` / `creep` / **`held`**;第 5 格為 0 時省略;**v1 `[[bench, kind]]` 仍可解析**——既有 open issue 遷移不掉狀態,只是沒有凍結基線、該 bench 退回舊關票行為）持久化、下次 run 解析回來比對。舊版每晚 append 一整張表 → #702 累積數十筆雜訊。**recovering label**:當 sustained 已清、只剩 creep,自動上 `perf-trend:recovering`(任一 sustained 回來或 issue 關閉即移除),讓訂閱者一眼分辨「仍惡化 vs 復原中」。無 marker 的 legacy body 視為「無變化」→ 遷移那一晚靜默(只補 marker、不誤發 comment)。
...
| `perf-trend` 已修好卻沒自動關 | #1396 後關票要**同機器類別**（⚠️ 機器類別 `unknown` **不算相符**）且 **recent 窗中位數**低於**凍結基線**；stderr 會印 `NOT closing … not comparable` / `still ≥ frozen baseline` / `absent from the recent same-class window` | 若是 `not comparable`,等一晚落在凍結那個機器類別即可（body 有窗口機器組成）；確認真的修好就人工關 |
...
| issue 上了 `perf-trend:unverifiable` label | 某支凍結中的 bench 連續 ≥3 晚無法判定（從 recent 窗消失,或凍結的機器類別沒再出現） | 打開 issue 看 **Held — awaiting proof of recovery** 表:bench 被改名/移除 → 人工關票;只是機器類別沒輪到 → 等或拉長 `--trend-limit`。⛔ watchdog **刻意不自動關**——無證據關票正是 #1396 要根治的事 |
| stderr 出現 `::warning::… state marker is absent / damaged` | issue body 的 marker 被人工編掉、或壞掉 | watchdog 不知道這張票在看什麼,**不會自動關**（fail-closed）。確認 perf 沒問題後人工關票即可;不要只補一個空 marker |
