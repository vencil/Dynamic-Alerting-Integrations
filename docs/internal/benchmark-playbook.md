...
| `perf-trend` **自己關掉了但 regression 還在** | ⛔ 已知未修（見上節最後一條）：關票用的是會漂的滑動 anchor，永久 regression 滑進 settled window 後 anchor 漲上去迎合它 | 別把自動關票當成「已修復」的證據。對照 bench 絕對數值；仍高就手動 reopen 或等它重新開票 |
...
| `perf-trend` 票沒關,body 出現 `STILL TRACKED BY THIS ISSUE, NOT VERIFIED TONIGHT` | 今夜是 CLEAR,但**這張票在追的 bench 今夜沒被評估**（停止回報 / 同類夜不足） | **正常防線**（fail-closed）。看 body 列出的成因:`no-recent` → 查該 bench 為什麼停跑（perf timeout / crash / harness 變更）;`thin-anchor` → 等同類夜累積;`did not appear in ANY of the N nights` → **該 bench 已改名或刪除**,票必須人工關閉（否則它會卡住整個 repo 的自動關票,見上節）。warning 會直接寫出卡住的是哪一支、幾夜沒回報 |
