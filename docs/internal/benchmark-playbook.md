...
- **watchdog 沒跑成也一定會出聲**:除了 INCONCLUSIVE,另外兩條靜默路徑同批補上——**夜數不足的短窗 bail-out**（`< K + 2` 夜,原本只 print 到 stderr）現在同樣 `::warning::` + step summary;**`gh issue create` 兩次都失敗**時明講「一張票都沒開、沒有人被通知」,而不是像舊碼那樣在第二次嘗試**之前**就先宣告「已改開不指派的 issue」（第二次也失敗就變成宣稱一張不存在的票）。
