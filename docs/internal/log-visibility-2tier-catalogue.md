...
| `suspicious_audit` | ⛔ platform-only | demux 的 origin spoof guard 判定 `pod_owner` 不符 gateway workload 名前綴時的落點（#566 T2-1），平台資安自用。⚠️ **它是被動保留、不是主動偵測訊號**：這個 class **沒有任何告警或 dashboard panel**，只是留在 `0:0` 可查詢；不要據此以為偽造會被通知。（`TenantProjectionFanoutDiscardSpike` 是 component 層的 discard 總量 tripwire，`configmap-rules-platform.yaml` 自己就註明它把本 class 與其他合法 drop 原因合計，分不出來。）⛔ 另見 [#1293](https://github.com/vencil/Dynamic-Alerting-Integrations/issues/1293)：guard 曾因比對錯 `pod_owner` 形狀而把**全部**合法 audit 列丟進這裡，因為本 class 無人消費而完全靜默 |
