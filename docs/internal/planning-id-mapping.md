...
| TRK-363 | [#1545](https://github.com/vencil/Dynamic-Alerting-Integrations/issues/1545) | `flat_scanner.go` mtime guard 的 2 秒安全窗：批次寫入後每次 `IncrementalLoad` 重算**全部**檔案雜湊，實測 2.6×。⚠️ 開票當日已就地更正一句宣稱過頭的話——本票**不**解釋 #1497 的雙峰方向（補量的資料反駁它：污染更重的 `NoChange` 反而是全集最穩的一群）。缺陷仍在 | ADR-032 / #1497 |
| TRK-364 | [#1557](https://github.com/vencil/Dynamic-Alerting-Integrations/issues/1557) | `bench-canary/canary_test.go` 說 sleep canary 是 INFORMATIONAL ONLY 的那一句被多處散文引用，而該引用在 #1439 / TRK-359 的五輪盲審裡漂掉兩次（未標省略又改標點；修一處漏第三處）。修法是把每一處引用逐字釘回 Go 原始碼——去註解前綴扁平化後逐字 `in` 比對，⛔ **只釘這一句**，通用的「所有引用都要對得上來源」是 TRK-365。⚠️ **票內寫的「三處」在開工當下已經過期**：機械枚舉發現 #1536 併入後 `paired_trend_watch.py` 自己就有數處，另有中文改寫式引用從未被計入 ⇒ 站點清單由 `CITATIONS` 明列，每一處各自釘住。⛔ **最後一輪三份獨立盲審（機制／覆蓋稽核／爆炸半徑）導致把「巡邏」那一半整個刪掉**：原本另有一道掃全 repo 的枚舉守衛負責攔「沒登記的新引用站點」，但這個模組**所有**的環境缺陷都來自它（它要 shell out 到 git，分辨「沒有 checkout」與「git 因別的理由失敗」改了三次、每次都新開一個靜默 skip 的洞），它會對一般英文誤觸（往常改的 playbook 加一條無關條目就紅），而覆蓋稽核逐行突變顯示它幾乎沒有被驗到（拿掉命中掃描的任一迴圈、或任一觸發字串，整套仍綠）。失去的是「發現全新且從未登記的站點」，改為人工步驟（`git grep -inE "informational[ -]only" \| grep -i canary`）；票本身要修的「引用從原文漂走」仍然被守住。⛔ **本項最後一輪的結論是「散文才是主要缺陷來源」**：帳本 25 條 finding 有 15 條是散文，其中絕大多數是沒有機械 SSOT、靠記憶重述的數字，已一律從測試檔／CHANGELOG／PR body 刪除，改為可查的程式碼或可自己跑的重現指令。⚠️ **本列同時是補登**：TRK-364 這個號碼此前只活在 #1557 的標題裡，登記表在 TRK-362 與 TRK-369 之間是空的（TRK-363 / 365–368 仍是洞，屬 #1627 範圍） | TRK-359 |
| TRK-365 | [#1558](https://github.com/vencil/Dynamic-Alerting-Integrations/issues/1558) | 通用「引用必須逐位元對得上來源」守衛：#1439 / TRK-359 的五輪盲審裡**九條修正有五條是同一類**——散文裡被反引號括起來、看起來像原文的字串與來源不符（引用不存在的輸出、漏尾綴、把 stderr 併進 stdout 的表、省略未標、同一句修一處漏另一處）。⛔ **需先 brainstorm 標註慣例，不可直接開工** | ADR-032 |
| TRK-366 | [#1564](https://github.com/vencil/Dynamic-Alerting-Integrations/issues/1564) | `converge_status.py` 的 `CONVERGED` 規則會在「什麼都沒審的記帳列」上誤判收斂，4 行帳本可重現。**已隨 [#1573](https://github.com/vencil/Dynamic-Alerting-Integrations/pull/1573) 刪除 `CONVERGED` 而消滅**（該票三個缺陷分別消滅／失去承重／被否決）| TRK-360 |
| TRK-367 | [#1571](https://github.com/vencil/Dynamic-Alerting-Integrations/issues/1571) | `bench-paired/v1` 是分裂版本：`pair_bench_ratio.py` ↔ `paired_trend_watch.py` 已知六個月沒跟到底，而唯一覆蓋它的測試是假綠的。⛔ 這不是「發現了沒人知道的 bug」——分裂本身早就寫在 `paired_trend_watch.py` 的註解裡，票追的是**沒有機制**這件事 | ADR-032 / TRK-359 |
| TRK-368 | [#1574](https://github.com/vencil/Dynamic-Alerting-Integrations/issues/1574) | 沒審任何東西的記帳列會燒掉 `ROUND-CAP` 的預算——**誠實記帳的人受罰**。#1564 的衍生問題，在該票三缺陷消滅後成為唯一沒消失且變 blocking 的一項。⇒ 已落地：只帶 `question` 的記帳列不再花預算 | TRK-360 |
...
| TRK-373 | [#1731](https://github.com/vencil/Dynamic-Alerting-Integrations/issues/1731) | `bench-probe-write-latency.yaml` 的 Summarize inline python 與 `analyze_probe.py` 是同一份計算的**兩份拷貝**，無任何一致性機制。已知 4 處分歧（校準輪**歧義時的報告方式**——⚠️ 兩邊留下的列等價，早期把它寫成「辨識」差異不準；記錄比對錨定；相關係數最低輪數；NaN 字面呈現），由 [#1716](https://github.com/vencil/Dynamic-Alerting-Integrations/pull/1716) 的盲審找到。⭐ 四條的重現指令與兩邊各自的輸出寫在票內；其中前三條是跑出來的，第四條是讀格式化字串比對（票內已分開標注強度）。⚠️ 修法方向是**去重**不是加差分測試——測試只會把重複鎖死 | — |
| TRK-374 | [#1732](https://github.com/vencil/Dynamic-Alerting-Integrations/issues/1732) | `bench-probe-2026-09/README.md` §四 的反事實 harness 從未被 commit、現已不存在 ⇒ 該節數字無法用該目錄的〈重算〉指令重現。房規先例是 `bench-trend-2026-08/counterfactual.py`。**依賴 TRK-373**：兩份拷貝分歧時「驗的是哪一份」才有答案。#1716 只收窄了宣稱，未補回驗證。⇒ **已落地**：`bench-probe-2026-09/counterfactual.py`，照先例形制、驅動真模組（`load` → `reject` → `render_ci` / `render_archive`），數字從工具**印出來的表**刮回來、不自己算相關係數。最關鍵的一項檢查是 §三 第二欄的直接證據：純 episode 下兩個分量的相關係數**都是約 +1.000**，sd 佔比卻一個只有幾個百分點、一個接近全部。⛔ **舊數字未重現、也沒有試**；新參數另選並印在輸出最前面，舊值與差異說明留在 README §四。⛔ **每一輪修法本身也盲審，每一輪都被打穿（含修法自己帶進來的新洞），最後再補一次整份檔案的盲審**——逐條 finding 與重現指令在 PR [#1795](https://github.com/vencil/Dynamic-Alerting-Integrations/pull/1795) 與 CHANGELOG，**本列刻意不重述**（本表的職責是 ID 翻譯，而同一段歷史寫三份已經對不上過一次）。⚠️ **兩個已知盲點不修只寫**：三種形狀都對 round total 單調 ⇒ `corr()` 換成別的保序統計量會整份全過；`level()` / `above()` 的加法偏移**補不了**——原始值從不被印出，而相關係數與 sd 都平移不變 ⇒ 對任何消費者都不可觀測。⇒ owner 裁決後**已接上 `tests/dx/test_counterfactual_harness.py`**：不只斷言跑得過（那會是沒東西能弄紅的綠），同時釘住判別式改壞 ⇒ rc 1、報表讀不出來 ⇒ rc 3 兩種紅分得開，各有對照格；`verify_diff` 在工具或 harness 任一被改時都會選到它。⚠️ 先例 `bench-trend-2026-08/` 仍無任何東西在跑 | — |
| TRK-375 | [#1733](https://github.com/vencil/Dynamic-Alerting-Integrations/issues/1733) | probe summary 對格式壞掉的輸入是裸 traceback 而非明確拒收；⛔ 其中一種發生在部分內容已 `tee` 進 `$GITHUB_STEP_SUMMARY` 之後 ⇒ Summary 頁顯示一份印到一半、外觀正常的表，**失敗看起來像成功**。併收跨列 `iters` 不一致不警告、`PROBETAIL` 死碼。⇒ **前兩項已落地**：`load()` 改容錯把壞記錄帶到 `reject()` 裁決、渲染改原子輸出（崩潰只印一行 `::error::`，不留半份）、`iters` 不一致改拒收並涵蓋 `--archive` 的跨檔池化路徑。⇒ **第三小項（`PROBETAIL` 死碼）亦已落地**：解析整段刪除，判準是四種輸入 × 兩種輸出模式逐位元相同；記錄本身仍由 Go 探針產生、仍在存檔裡，只是沒人讀，原處留註解說明下一個人該先加的是**消費者不是解析器**。⚠️ 票給的刪除理由（「會誤導後人」）在 #1731 留下 `DEAD as of writing` 註解後已不成立，改用「死碼要花維護成本」。⚠️ **票寫的行號（`yaml:278`、`py:65-67/127/232`）在 #1731 去重後全部過期**，且 workflow 的 inline python 已不存在——重新定位過。⚠️ **票寫「失敗看起來像成功」需要限定**：job 本來就是紅的（`set -o pipefail`），被騙的是只看 Summary 頁的讀者。⛔ **修正本身引入過兩個缺陷、由盲審逐輪抓到**：校準列缺陷誤拒（回歸）→ 豁免；豁免又造成靜默丟棄 → 具名說出；而那則警告一度印在 Markdown 圍籬內、自己不會渲染成 callout。⭐ 過程中砍掉一個純詞形變化的死分支（單複數）而不是為它養 fixture，並把一個被寫兩遍、分歧結構上觀測不到的述詞改成單一分流 | — |
| TRK-376 | [#1734](https://github.com/vencil/Dynamic-Alerting-Integrations/issues/1734) | `bench-probe-write-latency.yaml` 的 `PIPESTATUS` 註解技術理由錯誤（pipefail 下 `$?` 已是管線的失敗狀態），且它守的那三行在 `set -e` 下**是死碼**——探針真失敗時自訂的 `::error::` 永遠不會印。既有債，來自已合併的 [#1678](https://github.com/vencil/Dynamic-Alerting-Integrations/pull/1678)，不在 #1716 範圍。⇒ **已落地**：刪掉那段守衛與那句假註解，換成釘住「不要在這裡加 PIPESTATUS/rc 檢查」與「不要拿掉 `set -o pipefail`」的註解，重現指令寫在註解旁邊。⛔ **票裡給的替代寫法（把管線放進 `if !` 條件後讀 `PIPESTATUS[0]`）未採用**：探針成功而 `tee` 失敗時 `PIPESTATUS[0]` 是 **0** ⇒ 訊息說謊且 `exit 0` 讓 step **變綠**，正是 TRK-375 的形狀（重現指令見 CHANGELOG 該條目）。⚠️ **票裡與本列原文寫的「那三行是死碼」不完整，在此更正**：拿掉 `pipefail` 後那段守衛會醒 ⇒ 它只在 `pipefail` 存在時是死的，同時是一層冗餘後備；刪除移走了它，所以新註解把 `pipefail` 一併釘住。⛔ 未新增回歸測試，也不宣稱新增偵測力。同一形狀的第二個實例另開 [#1771](https://github.com/vencil/Dynamic-Alerting-Integrations/issues/1771) / TRK-381 | — |
...
| TRK-378 | [#1741](https://github.com/vencil/Dynamic-Alerting-Integrations/issues/1741) | squash merge 附加 `Co-authored-by:` 時插入的空行劈開 trailer block，於是 `Refs:` / `Self-Review-Pass-2:` / `Resolves:` 在 `main` 上被 git 丟棄（掃最近 80 顆：70 顆受損，未受損的 10 顆恰好是 GitHub 沒附加 co-author 的那些——資料自帶對照組）。⛔ **作者側沒有寫錯**，劈開發生在 merge。而 `Validate Self-Review-Pass-2 trailer` 跑在 `pull_request`、量的是 merge 前的形狀 ⇒ **它驗的是一個接著會被 merge 摧毀的狀態**，結構上看不到。本票不預設要修：先量「有沒有任何消費端從 merged 之後的 main 讀 trailer」 | — |
| TRK-379 | [#1746](https://github.com/vencil/Dynamic-Alerting-Integrations/issues/1746) | 以 subprocess 呼叫工具的測試對 `coverage.py` **完全不可見**（它不追子行程），於是一支測試齊全的工具在 coverage 報表上讀作「從未被 import」——「量不到」被呈現成「量了沒事」。實測（[#1745](https://github.com/vencil/Dynamic-Alerting-Integrations/pull/1745) / TRK-373）：`analyze_probe.py` 248 statements，15 個 subprocess 測試 → `No data was collected`，2 個 in-process 測試 → 91.5%；而那 15 個的 intentional-break dogfood 是 8/8，**偵測力在、可見度是 0**。⚠️ 量法本身有坑：`--cov` 給**路徑**時連 in-process 測試都報 `never imported`（壞掉的儀器不是結果），要給**模組名**。⛔ 範圍未定：機械枚舉出 78 個測試檔用 `sys.executable`，但**不宣稱那是 78 個盲點**——另有 98 個檔直接 import，重疊多少沒量，真實受影響集合尚無人產生。⭐ in-process 進入點買到的不只是可見度：實測把 `main()` 改成回傳 `None`，**只有 in-process 那條抓到**（`sys.exit(None)` 即 exit 0，subprocess 介面結構上看不見回傳契約）。⛔ **本列是一次撞號的裁決紀錄**：本項最初也自稱 `TRK-378`，與 [#1741](https://github.com/vencil/Dynamic-Alerting-Integrations/issues/1741) 相撞。配號當下機械枚舉本表得到「下一個可用 378」，那個讀數在當下為真，但 [#1742](https://github.com/vencil/Dynamic-Alerting-Integrations/pull/1742) 隨後把 378 登記給了 #1741。⚠️ **這正是 [#1627](https://github.com/vencil/Dynamic-Alerting-Integrations/issues/1627) 在講的那個病**（表上有洞 / 讀數會過期 ⇒ 配號會撞），這次是踩在「讀數會過期」那一半。依本表 TRK-372 那列已立下的先例「不讓已 merge 的歷史變假」裁決改號的是本項：#1741 的 TRK-378 已隨 `009e638c` 進 main 並登錄本表，本項當時只存在於未推的 commit、未發布的 CHANGELOG 與該票標題 ⇒ **只改 #1746 的標題，刻意不動它的 body**（同 TRK-372 的處置） | — |
| TRK-380 | [#1757](https://github.com/vencil/Dynamic-Alerting-Integrations/issues/1757) | nested `CLAUDE.md` **只有 `Read` 工具會觸發，`Bash` 讀檔不會**（乾淨 subagent 2×2 探針量到），而這個限制原本**只記在 CHANGELOG**——一個歷史載體，決定「要不要推廣到第二棵子樹」時沒有人會去讀它 | TRK-377 |
| TRK-381 | [#1771](https://github.com/vencil/Dynamic-Alerting-Integrations/issues/1771) | `scripts/tools/ops/bench_wrapper.sh` 的 `GO_RC="${PIPESTATUS[0]}"` 區塊在該檔的 `set -euo pipefail` 之下**不可達**，那則 `[bench_wrapper] go test exited …` 訊息從來沒印過。TRK-376 / #1734 同一形狀的第二個實例。⇒ **已落地**：owner 裁決走**刪除 + 註解釘住**（不救活——`exit 1` 與逃出去的管線 rc 只在 `go test` 被 SIGKILL 的 **137** 上有差，救活反而有損）。⚠️ 原註解的機制寫錯了：pipefail 取的是**最右邊**的非零不是最左邊（實測 `(exit 5)\|(exit 0)\|(exit 7)\|(exit 0)` → rc 7），已更正。⛔ **「還有沒有第三個實例」仍然沒有機制，這一格與 #1734 相同**：本則落地後全樹真正的 `PIPESTATUS` 讀取點只剩 `weekly-fuzz.yaml` 與 `scripts/ops/run_hooks_sandbox.sh`，兩者都用 `set +e` 包住、是合法用法（重算：`git grep -n 'PIPESTATUS\[' -- '*.sh' '.github/workflows/*'`；⚠️ `bench_wrapper.sh` 那兩行命中是釘住 pipefail 的註解、不是讀取），但那是一次性讀數，下一個新檔不會被任何東西擋。[#1820](https://github.com/vencil/Dynamic-Alerting-Integrations/pull/1820) 曾附一支類別級守衛，**owner 裁決移出該 PR**（述詞經多輪對抗式盲審未收斂），改走 inline 豁免制另案處理 | — |
| TRK-382 | [#1845](https://github.com/vencil/Dynamic-Alerting-Integrations/issues/1845) | errexit＋pipefail 下讀 `PIPESTATUS`（TRK-376 / TRK-381 同形狀）改由機制守：`scripts/tools/lint/check_pipestatus_errexit.py`（pre-commit `pipestatus-errexit`）。⛔ 它是**檔案級文字判定**，不判可達性（那條路在 #1820 已被打死）；誤紅以行內 `# pipestatus-ok: <why>` 豁免接住，豁免理由無法驗證（#1443 的殘留，不宣稱解掉）。已知界線與兩個方向的測試在工具 docstring 與 `tests/lint/test_check_pipestatus_errexit.py` | — |
| TRK-383 | [#1984](https://github.com/vencil/Dynamic-Alerting-Integrations/issues/1984) | 只被 subprocess 執行、沒有專屬測試的 pre-commit 閘門是不是活的——owner 裁決先做**存活探針**（只量、不補測試）：逐字跑各支的 pre-commit `entry:`，對種下的違規與合法輸入各量一次 rc。⚠️ 量到兩個不是「死」但與「閘門」字面不符的形狀：`check_open_encoding` 的 hook 當時只帶 `--ci`（設計上 warn-only），偵測到也回 0；`check_ad_hoc_git_scripts` 手動給不可解析的 `--diff-base` 時 `git diff` 失敗被吞成「OK」、rc 0（pre-commit 路徑走 `resolve_diff_base()` 會先驗 ref，不受影響）。後續：`check_open_encoding` 對 `scripts/`、`components/da-tools` 轉阻擋（[#1985](https://github.com/vencil/Dynamic-Alerting-Integrations/pull/1985)）；`check_ad_hoc_git_scripts` 另開 [#1987](https://github.com/vencil/Dynamic-Alerting-Integrations/issues/1987)；兩份 `open()` 判定器的分歧另開 [#1992](https://github.com/vencil/Dynamic-Alerting-Integrations/issues/1992)。探針的違規／合法輸入記在票上，供之後補行為測試時當 fixture | TRK-379 |
| TRK-384 | [#1987](https://github.com/vencil/Dynamic-Alerting-Integrations/issues/1987) | diff-only lint 用 `git diff --name-only` 列檔時，把 git 失敗或逾時吞成空清單 ⇒ 印 OK、回 0，「量不到」被呈現成「量了沒事」。顯式 `--diff-base` 會跳過 `resolve_diff_base` 的 ref 驗證，所以不存在的 ref 會直接走到這裡。同一個形狀共有三支：`check_ad_hoc_git_scripts`、`check_bat_ascii_purity`、`check_repo_name`，已改走共用的 `_lint_helpers.diff_changed_paths`，失敗時丟出 `DiffScanError`，由各自的 `main()` 轉成 rc 2。盲審另外抓到同一形狀的既有缺陷：git 會把非 ASCII 路徑加引號跳脫，於是這類檔名的違規被靜默放過；helper 改用 `-z` 一併修掉。逐行型的同族工具（走 `get_diff_added_lines`）在 git 失敗時會退回全檔掃描，判錯方向安全，不在本項範圍 | TRK-383 |
...
| TRK-390 | [#2025](https://github.com/vencil/Dynamic-Alerting-Integrations/issues/2025) | `diff_changed_paths` 用 `--diff-filter=AM` 列檔，git 判成改名的檔狀態是 `R` 而被濾掉 ⇒ `git mv` 到違規位置（含搬移時一併改內容）三支 diff 掃描型 lint 都看不到。改加 `--no-renames`：改名拆成 D＋A，新路徑以 A 出現。不選「把 `R` 加進 filter」的理由是使用者的 `diff.renames=copies` 會把複製判成 `C`，`AMR` 一樣漏；`--no-renames` 覆寫該設定 | TRK-384 |
