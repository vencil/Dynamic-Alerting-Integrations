...
| TRK-381 | [#1771](https://github.com/vencil/Dynamic-Alerting-Integrations/issues/1771) | `scripts/tools/ops/bench_wrapper.sh` 的 `GO_RC="${PIPESTATUS[0]}"` 區塊在該檔的 `set -euo pipefail` 之下**不可達**，它那則 `[bench_wrapper] go test exited …` 訊息永遠不會印。TRK-376 / #1734 同一缺陷形狀的第二個實例，由 owner 裁決另開票，該 PR 刻意不跨到 shell script 這一面以保盲審視野單一。⚠️ **不可照抄 #1734 的修法**：⑴ 這裡的**註解技術上是對的**（pipefail 確實讓 go test 的失敗浮現成非零 wrapper exit），錯的只是它下面那段執行不到，而 #1734 的註解本身是假話；⑵ 死碼裡寫的是 `exit 1` 而非 `exit "$GO_RC"` ⇒ 刪除或改活它會改變 wrapper 對呼叫端的 **exit code 語意**，動它之前先跑 `git grep -n bench_wrapper` 讀呼叫端。⛔ **未量、不要當成已知**：呼叫端是否依賴具體 exit code、該自訂訊息的實際價值、是否還有第三個同形狀實例。⚠️ **TRK-380 沒有自己的一列**（`grep -c '^| TRK-380 |' docs/internal/planning-id-mapping.md` → 0；換成 379 或 381 → 1），它只活在 issue 標題裡，與 TRK-366／367／368 同屬 [#1627](https://github.com/vencil/Dynamic-Alerting-Integrations/issues/1627) 的洞 | — |
| TRK-381 | [#1771](https://github.com/vencil/Dynamic-Alerting-Integrations/issues/1771) | `scripts/tools/ops/bench_wrapper.sh` 的 `GO_RC="${PIPESTATUS[0]}"` 區塊在該檔的 `set -euo pipefail` 之下**不可達**，它那則 `[bench_wrapper] go test exited …` 訊息永遠不會印。TRK-376 / #1734 同一缺陷形狀的第二個實例。⇒ **已落地**：owner 裁決走**刪除 + 註解釘住**（不救活），註解同時釘住「不要讀 PIPESTATUS/rc」與「不要拿掉 `pipefail`」，重現指令寫在旁邊；並補上**類別級守衛** `check_unreachable_pipestatus.py` + `tests/ops/test_unreachable_pipestatus.py` + pre-commit `unreachable-pipestatus`（auto-run 108 → 109）——這是與 #1734 的差別，#1734 明確未加守衛。⚠️ **本列原文寫的「這裡的註解技術上是對的」只對一半，在此更正**：結論（pipefail 讓 go test 失敗浮現成非零 wrapper exit）成立，但機制「propagates the **leftmost** non-zero」是假的——bash 取的是**最右邊**的非零（實測 `(exit 5)|(exit 0)|(exit 7)|(exit 0)` → rc 7）；沒被發現是因為這條管線每種失敗恰好都收斂成 1。⚠️ **本列原文的 exit-code 顧慮量出來幾乎為零**：go test 所有失敗模式都正規化成 1，`exit 1` 與逃出去的管線 rc 相等；唯一可觀察差別是 `go test` 被 SIGKILL 時的 **137** ⇒ 照原樣 `exit 1` 救活反而**有損**，刪除才保留它。呼叫端四處全是 pass/fail（不讀具體 rc）；全樹**沒有第三個實例**（`weekly-fuzz.yaml:94`、`run_hooks_sandbox.sh:101` 都用 `set +e` 包住，合法）。⚠️ **TRK-380 仍沒有自己的一列**，與 TRK-366／367／368 同屬 [#1627](https://github.com/vencil/Dynamic-Alerting-Integrations/issues/1627) 的洞 | — |
| TRK-381 | [#1771](https://github.com/vencil/Dynamic-Alerting-Integrations/issues/1771) | `scripts/tools/ops/bench_wrapper.sh` 的 `GO_RC="${PIPESTATUS[0]}"` 區塊在該檔的 `set -euo pipefail` 之下**不可達**，那則 `[bench_wrapper] go test exited …` 訊息從來沒印過。TRK-376 / #1734 同一形狀的第二個實例。⇒ **已落地**：owner 裁決走**刪除 + 註解釘住**（不救活——`exit 1` 與逃出去的管線 rc 只在 `go test` 被 SIGKILL 的 **137** 上有差，救活反而有損）。⚠️ 原註解的機制寫錯了：pipefail 取的是**最右邊**的非零不是最左邊（實測 `(exit 5)|(exit 0)|(exit 7)|(exit 0)` → rc 7），已更正。「還有沒有第三個實例」不再靠人掃——補了類別級守衛 `check_unreachable_pipestatus.py` + pre-commit `unreachable-pipestatus`，這是與 #1734 的差別（#1734 明確未加守衛） | — |
