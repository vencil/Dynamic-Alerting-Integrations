...
| TRK-361 | — | 跨 AI agent 中性 SSOT — `agents/{skills,roles}/` + `AGENTS.md`（AAIF 標準）成為單一真相源，`.claude/**` 降為 `gen_agent_adapters.py` 的生成物 + drift gate；用複製而非 symlink（Windows host 實測 symlink 壞，#1457）（**無 issue，直接以 PR 落地**） | — |
