...
| TRK-383 | [#1984](https://github.com/vencil/Dynamic-Alerting-Integrations/issues/1984) | 只被 subprocess 執行、沒有專屬測試的 pre-commit 閘門是不是活的——owner 裁決先做**存活探針**（只量、不補測試）：逐字跑各支的 pre-commit `entry:`，對種下的違規與合法輸入各量一次 rc。⚠️ 量到兩個不是「死」但與「閘門」字面不符的形狀：`check_open_encoding` 在 `--ci` 下**設計上 warn-only**，偵測到也回 0，hook 從不擋 commit（殘量重算：`check_open_encoding.py --ci --strict-open-encoding` 的 `Total:` 行）；`check_ad_hoc_git_scripts` 手動給不可解析的 `--diff-base` 時 `git diff` 失敗被吞成「OK」、rc 0（pre-commit 路徑走 `resolve_diff_base()` 會先驗 ref，不受影響）。兩者去留待 owner 裁決。探針的違規／合法輸入記在票上，供之後補行為測試時當 fixture | TRK-379 |
