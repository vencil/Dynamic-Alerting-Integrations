...
| TRK-374 | [#1732](https://github.com/vencil/Dynamic-Alerting-Integrations/issues/1732) | `bench-probe-2026-09/README.md` §四 的反事實 harness 從未被 commit、現已不存在 ⇒ 該節數字無法用該目錄的〈重算〉指令重現。房規先例是 `bench-trend-2026-08/counterfactual.py`。**依賴 TRK-373**：兩份拷貝分歧時「驗的是哪一份」才有答案。#1716 只收窄了宣稱，未補回驗證。⇒ **已落地**：`bench-probe-2026-09/counterfactual.py`，照先例形制、驅動真模組（`load` → `reject` → `render_ci` / `render_archive`），數字從工具**印出來的表**刮回來、不自己算相關係數。檢查中最關鍵的一項是第二欄的直接證據（純 episode 下兩個分量相關係數**都是約 +1.000**，sd 佔比一個幾個百分點、一個接近全部）；另一項把故意改壞內建成檢查（一組壞掉的判別式，每一項可評估的檢查至少被其中一種打紅；基準線已紅者報 `NOT ASSESSABLE` 不記功）。⛔ 通過數與 mutant 數刻意不寫進散文——它們已經漂過一次。⛔ **舊數字未重現、也沒有試**：參數不可從數字回推，新參數另選並印在輸出最前面，舊值與差異說明保留在 README §四。⛔ **兩處先前的錯誤就地更正**：工具自印的「沒有實測背書」與 README §三第 2 點的「不是量出來的」；以及本 harness 第一版的檢查 5 沒取基準線，會把「本來就紅」記成 mutant 的功勞（現報 `NOT ASSESSABLE`）。⚠️ **驗的是形狀已知的合成資料**，不證明真實資料只有這兩種形狀；⛔ **兩輪盲審打穿 harness 自己七處**。第一輪：`total()` 從未被改壞測過（改成只回 `load_sum` 全過）⇒ 加第三種形狀與一項對應檢查；sd 欄被刮出卻沒被斷言（除數 `1e6`→`1e3` 仍全過）⇒ renderer 間 sd 比對與「佔比 == sd ÷ 總 sd」恆等式；工具**執行期**崩潰以裸 traceback + rc 1 逃出 ⇒ 每次呼叫包成 `CannotMeasure`。第二輪（全是第一輪修法自己帶進來的）：寫入側形狀每輪寫入成本是常數 ⇒ `write_p50 × iters` 與 `write_sum` 恆等，新檢查仍看不見用錯哪一個 ⇒ 改成寫入側 episode；該檢查全是比值 ⇒ 兩邊一致的 1000 倍單位錯誤照樣全過 ⇒ 補絕對值錨點；`SystemExit` 不是 `Exception` ⇒ 工具裡的 `sys.exit()` 一字不印就決定出口碼；sibling import 留在 `sys.modules` ⇒ 換 `--tool` 拷貝沉默沿用前一份。⚠️ 盲審要求把「工具 `corr()` 壞掉」由 rc 3 降為 rc 1，**未照做**——出界的數字分不出是刮錯欄位還是工具壞了；改的是訊息。第三輪（又有兩處是第二輪修法帶進來的）：`SystemExit` 的修法只加在 `drive()`，同一個洞留在 `load_module`；harness 自己的刮取碼沒保護 ⇒ 欄數不對的表炸成 `IndexError` + rc 1；相關係數與 sd 都平移不變 ⇒ `total()` 加法偏移整份全過（補中位數錨點）；「報表讀不出來」被當成偵測力 ⇒ 分開計，要在報表讀得出來時被打紅才算數。第四輪：`load_module` 前兩輪各修一行，而 `try:` 上面的敘述句仍露在外面（`--tool` 指到 `.txt` ⇒ 裸 `AttributeError` + rc 1）⇒ 邊界改成整個函式本體。第五輪：`main()` 的 `path.resolve()` 與 `table_roles()` 自己的 `read_text(utf-8)` 仍在邊界外（symlink loop / latin-1 cookie 的合法 `.py`）⇒ 裸 traceback + rc 1。⛔ 改為套用判準而非再補一行：**凡碰到工具的操作都走 `drive()`**，harness 自己的邏輯不走。⛔ 同批做減法：harness 散文 291→253 行而**程式碼未動**（445→445），README §四 的逐條 finding 清單刪除改為指向 CHANGELOG。⚠️ **兩個已知盲點不修只寫**：三種形狀都對 round total 單調 ⇒ `corr()` 換成別的保序統計量會整份全過；`level()` / `above()` 的加法偏移**補不了**——原始值從不被印出，而相關係數與 sd 都平移不變 ⇒ 對任何消費者都不可觀測。⛔ 另刪沒有機械 SSOT 的數字（docstring 的「gap under 1 pp」；README 的 `50.8%` / `f≈0.663` 只存在於 issue 內文）。⚠️ **無任何 CI / 測試在跑它**（先例亦然），是否接上 `tests/dx/` 未裁決 | — |
