...
| TRK-374 | [#1732](https://github.com/vencil/Dynamic-Alerting-Integrations/issues/1732) | `bench-probe-2026-09/README.md` §四 的反事實 harness 從未被 commit、現已不存在 ⇒ 該節數字無法用該目錄的〈重算〉指令重現。房規先例是 `bench-trend-2026-08/counterfactual.py`。**依賴 TRK-373**：兩份拷貝分歧時「驗的是哪一份」才有答案。#1716 只收窄了宣稱，未補回驗證。⇒ **已落地**：`bench-probe-2026-09/counterfactual.py`，照先例形制、驅動真模組（`load` → `reject` → `render_ci` / `render_archive`），數字從工具**印出來的表**刮回來、不自己算相關係數。六項檢查，其中第 3 項是第二欄的直接證據（純 episode 下兩個分量相關係數**都是約 +1.000**，sd 佔比一個幾個百分點、一個接近全部）、第 5 項把故意改壞內建成檢查（三種壞掉的判別式，檢查 2–4 各自至少被一種打紅）。⛔ **舊數字未重現、也沒有試**：參數不可從數字回推，新參數另選並印在輸出最前面，舊值與差異說明保留在 README §四。⛔ **兩處先前的錯誤就地更正**：工具自印的「沒有實測背書」與 README §三第 2 點的「不是量出來的」；以及本 harness 第一版的檢查 5 沒取基準線，會把「本來就紅」記成 mutant 的功勞（現報 `NOT ASSESSABLE`）。⚠️ **驗的是形狀已知的合成資料**，不證明真實資料只有這兩種形狀；⛔ **三輪盲審打穿 harness 自己三處**：`total()` 從未被改壞測過（改成只回 `load_sum` 六項全過）⇒ 加第三種形狀與檢查 7；sd 欄被刮出卻沒被斷言（除數 `1e6`→`1e3` 仍全過）⇒ 檢查 6 加 renderer 間 sd 比對與「佔比 == sd ÷ 總 sd」恆等式；工具**執行期**崩潰以裸 traceback + rc 1 逃出 ⇒ 每次呼叫包成 `CannotMeasure`。⛔ 另刪兩個沒有機械 SSOT 的數字（docstring 的「gap under 1 pp」實測 1.13 pp；README 的 `50.8%` / `f≈0.663` 只存在於 issue 內文）。⚠️ **無任何 CI / 測試在跑它**（先例亦然），是否接上 `tests/dx/` 未裁決 | — |
