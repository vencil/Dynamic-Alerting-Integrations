...
安裝：解壓、`sha256sum -c` 驗 binary、`pip install --no-index` 離線裝 `jsonschema`/`pyyaml`
（或 vendored wheel）。起隔離 vmsingle（`-retentionPeriod=100y`、`-httpListenAddr=:8428`）。
...
硬編外部 URL。對方可自審：`grep -nE "urlopen|urllib|http" scripts/tools/dx/*.py
tests/rulepacks/vm_harness.py` → 目標全為 `{base_url}/...`。可於無出向網路的隔離網段執行。
...
```
python3 scripts/tools/dx/inject_waveform.py pack.yaml --rules <候選VM規則路徑> --json
python3 scripts/tools/dx/waveform_score.py <inject報告.json> --tolerances tol.yaml --json
```
...
範本的 `shape_class`（step / ramp / spike / plateau / dip / oscillation）**不是全集**。真實
故障若不落在這些形狀（如多階段複合、狀態機式），**不要硬套一個近似形狀**——那會製造
一個「能踩閾值但不像真故障」的波形，catch 了也無意義（同義反覆）。正確做法：
...
2. **人類 attestation**（連一行都不放）：對方 cleared 人員簽署「跑了 vX、得 PASS/FAIL、
   N=K 個簽章」——出關的是簽名聲明、不是資料。
