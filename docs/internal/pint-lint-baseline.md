...
| Match（rule name） | Rationale |
|---|---|
| `.+ExporterAbsent` / `VersionAwareThresholdInert` | platform-scoped sentinels：expr 刻意把 `tenant` 聚合掉（告警是平台級、render 空 → drop），但 repo 的 required-labels policy（`lint_custom_rules.py`）**強制**每條規則帶 `tenant` label → 此處「template 用了 query 不會有的 label」**是 by design 非 bug**。新規則若真的砍掉它需要的 label，name 不 match → **照樣被抓**。 |
