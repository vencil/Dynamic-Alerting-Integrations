...
- **三步合一 commit workflow**：`make win-commit MSG=_msg.txt FILES="a b" SKIP=<hook>`（見 [§修復層 C.1](#修復層-c1escape-helpersmcp-60s-timeout--fuse-cache-bypass)）
...
| 三步合一 commit（stage + commit-file + push） | `make win-commit MSG=... FILES=... SKIP=...` | 包 `win_git_escape.bat` 三個子命令，避開陷阱 #46（cmd 引號）和 `--no-verify` |
...
為此打包了 3 個 helper，取代自己寫一次性 PowerShell snippet：

| Helper | 解決問題 | 典型用法 |
|--------|---------|---------|
| [`scripts/ops/win_async_exec.ps1`](../../scripts/ops/win_async_exec.ps1) | MCP 60s timeout | 派工 → 拿 PID → sandbox 側輪詢 log 檔案 |
| [`scripts/ops/win_read_fresh.ps1`](../../scripts/ops/win_read_fresh.ps1) | FUSE dentry cache | 用 Win32 `ReadAllBytes` 讀 source → 寫到新 inode（預設 `<path>.fresh`） |
| `make win-commit` | 包 `win_git_escape.bat` 三步（add + commit-file + push） | `make win-commit MSG=_msg.txt FILES="a b" SKIP=head-blob-hygiene` |
...
#### 3. `make win-commit` — 包三步的 commit workflow

`win_git_escape.bat` 已經提供 `add` / `commit-file` / `push` 三個子命令，但實際 session 常需三個連著跑（stage → commit with message file → push）。這個 Make target 把它們封成一行：

```bash
make win-commit MSG=_msg.txt FILES="scripts/ops/win_async_exec.ps1 docs/internal/windows-mcp-playbook.md" SKIP=head-blob-hygiene
```

必填：`MSG=<message-file>`（UTF-8 without BOM）
選填：`FILES=<space-separated paths>`（省略時假設已 staged）、`SKIP=<hook-name>`（targeted skip 代替 `--no-verify`）

設計選擇：
- 沒有 `--no-verify` escape hatch — 若需跳過 hook，用 `SKIP=<name>`（pre-commit 原生支援的 env var），保留其他 hooks 把關
- Message 一律走檔案（`commit-file` 子命令）— 避開陷阱 #46（cmd 對 em-dash/CJK 引號解析崩潰）
- Sandbox 側呼叫時自動 detect `cmd.exe`，不存在則印出可複製的 Windows 指令給 user 手動執行
