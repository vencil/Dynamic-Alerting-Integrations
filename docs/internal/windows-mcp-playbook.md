...
| 61 | PowerShell `Out-File -Encoding utf8` 寫 commit message 加 U+FEFF BOM → commitlint 三層 fail | **✅ Codified（PR #56, v2.8.0）**：`scripts/tools/dx/pr_preflight.py::check_commit_msg_file()` 讀檔前先 `detect_commit_msg_bom()` 掃首 3 bytes 偵測 UTF-8 / UTF-16 LE / UTF-16 BE BOM；命中時 stderr 直接印對應 PS `[IO.File]::WriteAllText(…, UTF8Encoding(false))` / bash `printf` 修法 + `git filter-branch --msg-filter sed` recovery SOP。commit-msg hook 以 exit 1 擋 commit，不會流到 commitlint cascade。原 RCA + 修法矩陣 → [`archive/automation-origins/trap-61-bom.md`](archive/automation-origins/trap-61-bom.md) |
...
| 64 | Cowork VM proxy 擋 api.github.com → `diag_pr_ci.py` / 任何 `gh api` 走 Cowork 時 timeout 或 403 | **✅ Codified（PR TBD, [#446](https://github.com/vencil/Dynamic-Alerting-Integrations/issues/446)）**：`diag_pr_ci.py` 第一步 prerequisite probe（`gh api /rate_limit` 帶 3s timeout）會 fast-fail 並 print 「Switch to Windows MCP or Dev Container」hint，exit 3（與 exit 2「gh 缺失/未認證」區分）。**為何 Cowork VM 會卡**：sandbox 走 proxy，`git push` 走得通（git 協議通道）但 `gh api` / `curl https://api.github.com` 回 403 或 timeout — 同 trap #31。**操作建議**：日常 `gh` 用法（pr view / issue view / api calls）一律走 Windows MCP 或 Dev Container；`diag_pr_ci.py` 因為已 codify probe，撞到 exit 3 換 host 就好。**任何新工具用 `gh api`**：複用同款 3-step probe（gh exists / auth / network），不要 reimplement |
| 63 | Microsoft Store Python "App Execution Alias" stub 通過 `command -v` 但執行 exit 49 → commit-msg + pr_preflight 全卡 | **✅ Codified（PR TBD, [#436](https://github.com/vencil/Dynamic-Alerting-Integrations/issues/436)）**：Windows 11 fresh install 在 `%LOCALAPPDATA%\Microsoft\WindowsApps\` 放了 `python3.exe` / `python.exe` placeholder（App Execution Alias），任何 `command -v python3` 都會回 success（檔案實存），但實際執行 stub 印 `Python was not found; run without arguments to install from the Microsoft Store...` 並回 exit 49。**影響**：(a) `scripts/hooks/commit-msg` 的 candidate loop 選到 stub 後 preflight 失敗，每次 commit 被擋；(b) `pr_preflight.py::check_scope_drift()` 的 `subprocess.run(["python3", ...])` 走 `CreateProcess` PATH lookup 撈到 stub，preflight 永遠回報 `❌ Scope drift`。**修法**：(i) shell-side（commit-msg）candidate list 改成 `"py -3" "py" python3 python ...` 並加 `$PY --version >/dev/null 2>&1` 探活，stub 失敗即跳下一個；(ii) Python-side（`pr_preflight.py` / `check_pr_scope_drift.py`）用 `sys.executable` 取代 bare `"python3"`，由當前 interpreter 自我 fork，繞過 PATH lookup。**Workaround（已修前）**：bash shim 把 `python3` 指到 `py -3`（對 bash hook 有效；Python subprocess 不認 shebang，仍需 patch）|
| 65 | Bash 工具的 `/tmp` ≠ Windows `C:\tmp` → 跨工具讀同名檔靜默讀錯檔（false-green） | Git Bash / MSYS 把 `/tmp` 解析成 **MSYS 自己的 temp mount**（通常 `C:\Users\<user>\AppData\Local\Temp` 或 MSYS 安裝樹下），**不是** `C:\tmp`。當你用 Write / Read 工具在 **`C:\tmp\foo`**（Windows 路徑）建檔，再用 Bash 跑 `grep ... /tmp/foo`，grep 讀的是另一個（多半不存在的）檔 → 回空 → **假綠**。燒過 v2.9.0 release：release-body 建在 `C:\tmp\`，用 `/tmp/` grep 驗 placeholder 殘留 → 回「無 placeholder」實為讀錯檔，差點漏掉沒填的 `<FILL>`。**正解**：跨工具一律用**同一種路徑寫法**——Bash 端也用 `C:/tmp/foo`（Git Bash 認 `C:/` drive 寫法），或全程 `/tmp`（但那 Read/Write 工具的 Windows 絕對路徑就得對應 MSYS 實體位置）。**鐵則**：Write/Read 用 `C:\tmp\x` 建的檔，Bash 要碰就用 `C:/tmp/x`，**別混用 `/tmp/x`** |
...
pre-push hook 擋路時，第一件事：**證明失敗是否跟這次 commits 有關**。用 `git worktree` 跳到 base commit 重跑同一個 hook，若結果一樣 → drift 跟這次無關，**用那一格自己的旗標繞過**（mkdocs strict：`MKDOCS_STRICT_BYPASS=1`；preflight marker：`GIT_PREFLIGHT_BYPASS=1`，需 owner 核准）；若結果不同 → 這次 commits 引入新問題，必須修。⛔ **不要用 `--no-verify`**：它一次關掉 pre-push 上的**每一道**守衛（含擋直推 main 那道——實測它不認任何 bypass 變數，也含串接的 git-lfs hook），而不是你剛證明無關的那一道。逐字禁止它的那幾句住在 `protect_main_push.sh`、`require_preflight_pass.sh` 與 `prepush_dispatch.sh` 的「helper 不見」錯誤訊息與註解裡，**不在檔頭**（去檔頭找會撲空；mkdocs 那支則完全沒有這句）。⚠️ 相反的活字仍在三個載體：`protect_main_push.sh` 被擋時的橫幅印著「緊急 hotfix？加 `--no-verify`」、`win_git_escape.bat push` 無條件帶它、`github-release-playbook.md` 的 force-push 範例也帶——行為面在 [#1487](https://github.com/vencil/Dynamic-Alerting-Integrations/issues/1487) 追蹤。
⛔ 上面那個 A/B 一行式（`pre-commit run <hook-id>`）**對這三道 pre-push 守衛不適用**：它們不是 pre-commit hook、沒有 hook id（以 `yaml.safe_load` 數過，全部 id 裡 0 個）。危險的不是它會紅，是**它會綠**——實跑得到 `No hook with id …`，而這段自己的度量 `grep -c "error:"` 在 base 與 head **兩邊都是 0**，於是決策樹判成「pre-existing drift」叫你繞過。要 A/B 就直接跑 `bash scripts/ops/prepush_dispatch.sh`（檔案沒有執行位元，Linux 上少了 `bash` 會 Permission denied）：remote 名與 URL 走 argv、`<local_ref> <local_sha> <remote_ref> <remote_sha>` 走 stdin。⛔ **stdin 忘了餵就是 rc=0 全綠**（零列＝沒有東西要判），而且它會連帶執行 `.git/hooks/pre-push.chained`（真機上那是 git-lfs，會連遠端）。
