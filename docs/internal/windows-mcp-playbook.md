...
| 58 | `make git-preflight` 把自身 bash 程序誤判為「活躍 git 程序」跳過清理 | preflight helper `scripts/ops/git_check_lock.sh` 用 `pgrep git` 偵測 active git 程序決定要不要清 `.git/*.lock`，卻把 Makefile 本身啟動的 bash subshell（其 argv 含 `git` 字串的 path）當成活 git，於是**永遠跳過清理**。**表現**：`make git-preflight` 回報「lock exists but git active → skip」但實際沒有 git 在跑，lock 永久存在。**修法**：過濾自身 PID + parent PID：`pgrep git \| grep -v -E "^($$\|$PPID)$"`；或改偵測 `.git/index.lock` 的 mtime（> 60s 無進度視為 stale）。歸檔於 v2.8.0-planning §12.4 #2，排入 A-12 子項 (v) 施工週。**手動繞道**：直接 `rm -f .git/*.lock` 或 Windows 側 `Remove-Item .git\*.lock -Force` |
