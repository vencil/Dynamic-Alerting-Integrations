...
⭐ **暖機期就是曲線本身**（2026-09-01 新增）。[#1497](https://github.com/vencil/Dynamic-Alerting-Integrations/issues/1497) 的逐迭代分支計數顯示：前 W 次迭代裡 1001 個檔案全部錯過 mtime 守衛被重新雜湊，之後**恰好 1 個**。把它寫成單參數模型 `t(N) = t_steady + C×min(N,W)/N` 並**只用本檔資料**擬合（`N≥100`）：
