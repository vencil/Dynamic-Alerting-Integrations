...
**v2.7.0 進度**：未推進，且 `scaffold_tenant.py` 於 v2.9.1 新增 `--topology=1:N`、`--mapping-instance`、`--mapping-filter`、`--routing-profile` 參數後反而略為增長；`validate_docs_versions.py` 同理（1,032 → 1,109 行，因加入歷史版本 skip 規則）。**重新確認：大型工具的「每次小改」會讓瘦身壓力持續累積，必須排入 minor release 的明確 phase 才會發生。**
