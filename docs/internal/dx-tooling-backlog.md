...
**導入前置：SHA-pin 連動（#902 L3）。** repo 對下載的第三方工具走「download + `scripts/ops/_verify_download.sh` 驗 pinned SHA-256」（golangci-lint @ `validate.yaml`；hadolint / promtool / pint / kube-linter @ `ci.yml` 等，政策見 #849/#851）。Renovate 預設只 bump 版號字串、**不會重算 install.sh / binary 的 SHA-256** → 升級 PR 會在 `_verify_download.sh` 直接紅燈、自動升級在此斷鏈。導入時必須客製一條 `regexManagers` + `customDatasources`：提取版號 → 抓對應 release 的 SHA-256 → 同步更新 `*_SHA256` 變數。（來源：#912 對抗 review）
