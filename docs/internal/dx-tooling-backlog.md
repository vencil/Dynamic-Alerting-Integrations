...
**v2.5.0 完成項目**（已從候選清單移除）：
- Release Notes Generator (JSX) — 從 CHANGELOG 自動產生角色分流更新摘要
- Threshold Heatmap (JSX) — 跨 tenant 閾值分佈視覺化 + 離群偵測 + CSV 匯出
- Interactive Tool Playwright E2E — 5 個 critical path smoke tests（38 test cases）+ CI 整合
- Full CRUD API Server — tenant-api（Go）+ RBAC + groups + views + domain policy（ADR-009, ADR-010）
- check_doc_reading_time.py — 偵測 >15 分鐘閱讀量的文件
- check_doc_freshness.py — 偵測 >90 天未更新的文件
- Deployment Profile Wizard (JSX) — 問答式產出 Helm values
- RBAC Setup Wizard (JSX) — 問答式產生 _rbac.yaml
...
v2.5.0 已建立 5 個 critical path smoke tests（38 test cases）。擴展至覆蓋全部 39 支 JSX 工具：每支工具可載入、基本互動正常、無 console error、i18n 切換不破版。加入 axe-core 自動化無障礙測試。
...
### Python 大型工具結構瘦身（v2.4.0 Phase B.5 標記）

**現狀**：6 支 1,000+ 行 Python 工具未進行結構性重構，合計 7,419 行。

| 工具 | 行數 | 建議方向 | 時機 |
|------|------|---------|------|
| `generate_alertmanager_routes.py` | 1,474 | Phase C API server 會共用 routing 邏輯，屆時自然拆分 | Phase C |
| `init_project.py` | 1,404 | 將 template rendering 抽出，main 變純 orchestration | v2.5.0 |
| `migrate_rule.py` | 1,225 | AST 引擎，邏輯內聚，僅需補充 inline 註解 | 維持現狀 |
| `scaffold_tenant.py` | 1,153 | template generation 與 file I/O 分離 | v2.5.0 |
| `onboard_platform.py` | 1,131 | 與 `init_project.py` 有功能重疊，評估合併 | v2.5.0 |
| `validate_docs_versions.py` | 1,032 | 拆出 version pattern registry 為獨立 data file | v2.5.0 |

**決策理由**：Phase C 的 API server 會重塑 `generate_alertmanager_routes.py` 和 `validate_docs_versions.py` 的呼叫模式。現在動刀是改兩次。其餘工具可在 v2.5.0 作為獨立瘦身衝刺處理。

---

## 候選 — v2.5.0 開發過程新發現

### Playwright axe-core 無障礙自動化

v2.5.0 Phase D/E 的手動 WCAG 審查發現 2 Critical + 4 Major 問題。整合 `@axe-core/playwright` 到現有 5 個 smoke tests，自動偵測色彩對比、缺失 ARIA 屬性、表單標籤等常見無障礙問題。

### tool-registry.yaml ↔ JSX 完整對齊

目前 tool-registry 有 32 個 entry，但實際 JSX 檔案有 39 個（含 v2.5.0 新增的 deployment-wizard, rbac-setup-wizard, release-notes-generator, threshold-heatmap 等）。需將未註冊的 JSX 工具補入 registry。

### Batch response Summary 欄位

Phase B R2 建議：group batch operation response 加入 `Summary` 欄位（"5 succeeded, 1 failed"），區分全成功/部分/全失敗狀態，改善 UI 錯誤回饋。

### Version matrix 自動追蹤

Phase D R2 建議：建立 `docs/internal/version-matrix.md` 統一追蹤各檔案版本號，搭配 pre-commit hook 自動檢查。v2.5.0 曾因 README.en.md / e2e/package.json / deployment-wizard.jsx 版號落後而需手動修正。
...
**問題**：42 個檔案中的 Docker image tag（`da-tools:v2.4.0`）、Helm `--version` flag、`Chart.yaml`、`VERSION` 檔案、`mkdocs.yml` extra vars 全部手動更新，遺漏率極高。
