...
>
> **v2.7.0 進度**：TRK-006 的 `make test-skip-audit` target 已在 v2.7.0 落地（Makefile L387，BUDGET=5），但 CI gate 尚未接上。其他項目 TRK-001~TRK-005、TRK-007~TRK-009 仍為候選，延續至 v2.8.0。

### TRK-001: `check_noqa_hygiene.py` — noqa/nosec 必要性驗證
```yaml
id: TRK-001
tracking_kind: dx
status: proposed
...
### TRK-002: `make test-impact` — 變更影響測試自動縮減
```yaml
id: TRK-002
tracking_kind: dx
status: proposed
...
### TRK-003: Pre-commit hook CI gate
```yaml
id: TRK-003
tracking_kind: dx
status: proposed
...
### TRK-004: Lint tool self-test framework（negative fixtures）
```yaml
id: TRK-004
tracking_kind: dx
status: proposed
...
### TRK-005: `check_test_isolation.py` — 測試隔離驗證
```yaml
id: TRK-005
tracking_kind: dx
status: proposed
...
### TRK-007: Lint test coverage 補齊（18 支缺測試）
```yaml
id: TRK-007
tracking_kind: dx
status: proposed
...
**前置依賴**：TRK-004（negative fixtures 目錄建立後可共用）。
**驗收**：auto hook 的 lint tools 全部有 `test_*.py` + 每支至少 2 tests。

### TRK-008: CI ignore 文件化與 test-map 更新
```yaml
id: TRK-008
tracking_kind: dx
status: proposed
...
### TRK-009: Coverage source 一致性 lint
```yaml
id: TRK-009
tracking_kind: dx
status: proposed
...
>
> **分類邏輯**：
> - **Flow** — 整個 session / release 流程層級（TRK-010、TRK-014、TRK-015、TRK-017）
> - **Lint** — 靜態檢查 / pre-commit 層級（TRK-012、TRK-013、TRK-016）
> - **Testing Strategy** — 測試設計本身的根因修復（TRK-011、TRK-018）

### TRK-010: Flake 自動重試 CI Policy（不是盲目全域 retry）
```yaml
id: TRK-010
tracking_kind: dx
status: in-progress
...
**驗收**：
1. `flaky-tests.yaml` 收錄 `TestWatchLoop_DebouncedReload_DetectsFileChange` + TRK-011 的根因 PR 鏈接
2. v2.7.0 期間 rerun 手動次數的 TP99 from 3 → 0
3. expire_at 過期時 CI 失敗（驅動根因修復）

### TRK-011: Fake-Clock 注入（根因修復 Go 時間相依測試）
```yaml
id: TRK-011
tracking_kind: dx
status: done
...
> **狀態**：原始 motivating test (`TestWatchLoop_DebouncedReload_DetectsFileChange`) 已透過更輕量的 `tickOnce()` 抽取 + `debounce=0` synchronous fallback 達成 deterministic（無 clockwork dep）。`flaky-tests.yaml` 已 shrink 到空 — 「root-cause fix → registry shrinks」的設計閉環已驗證。
>
> **後續仍需做**：其他 `_debounce` / `_retry` / `_backoff` 命名的 test 仍可能再撞坑；下次發生時可考慮 clockwork 注入（不一定要在這次 sweep 全做）。本條目暫保留為 *候選*，不是 *進行中*。

**問題來源**：TRK-010 是症狀治療，TRK-011 才是根因。`TestWatchLoop_DebouncedReload_DetectsFileChange` 使用真實 `time.Sleep()` 等 300ms debounce 觸發，在 CI runner 負載浮動時容易 flake。類似問題會出現在未來每一個 `_debounce` / `_retry` / `_backoff` test。
...
3. TRK-010 的 `expire_at: v2.8.0` 能安全過期

### TRK-012: ADR / 內部連結檔名一致性 Lint
```yaml
id: TRK-012
tracking_kind: dx
status: proposed
...
### TRK-013: Spoke 文件 Freshness Gate（防「空頭支票」）
```yaml
id: TRK-013
tracking_kind: dx
status: proposed
...
### TRK-014: FUSE-side Git Write 防護 Wrapper
```yaml
id: TRK-014
tracking_kind: dx
status: proposed
...
### TRK-015: Session 起手式 PATH+PATHEXT Smoke Test
```yaml
id: TRK-015
tracking_kind: dx
status: proposed
...
### TRK-016: CHANGELOG 計數一致性 Lint（tool count / JSX count / hook count）
```yaml
id: TRK-016
tracking_kind: dx
status: proposed
...
### TRK-018: `engineering:testing-strategy` Skill 驅動的測試設計還債
```yaml
id: TRK-018
tracking_kind: dx
status: proposed
...
**問題來源**：前述 TRK-010 / TRK-011 是「兩個 test 的局部修復」，但專案有 ~4232 tests，類似 flake 陷阱可能散佈全域。需要一次性用 `engineering:testing-strategy` skill 對測試策略做系統化還債。
...
**v2.7.0 進度**：未推進，且 `scaffold_tenant.py` 於 v2.9.0 新增 `--topology=1:N`、`--mapping-instance`、`--mapping-filter`、`--routing-profile` 參數後反而略為增長；`validate_docs_versions.py` 同理（1,032 → 1,109 行，因加入歷史版本 skip 規則）。**重新確認：大型工具的「每次小改」會讓瘦身壓力持續累積，必須排入 minor release 的明確 phase 才會發生。**
...
六線版號（platform/exporter/tools/portal/recipe-preview/tenant-api）的 tag + GitHub Release 仍是手動流程。可用 GitHub Actions 監聽 tag push 自動觸發 Release Notes 產生（基於 CHANGELOG section）和 OCI image build/push。v2.3.0 CI matrix 已到位，為自動化提供基礎。

### dependabot / renovate

**Renovate 已導入（#902 L3，self-hosted），目前範圍鎖定第三方容器映像。** `renovate.json`（`enabledManagers: custom.regex`，3 個 customManager 涵蓋 14 個 #902 L2 pinned 映像的 tag+digest，annotation 錨定 split-knob chart）+ `.github/workflows/renovate.yaml`（週排程 + `workflow_dispatch` dry-run）。Offline 守門 `tests/ops/test_renovate_config.py`（regex 真匹配 + 14 完整覆蓋 + matrix==deploy depName 一致，讓 Renovate bump 兩邊一起動、drift-guard 保綠）。**這關閉了 L1+L2 的 toil loop**：digest 凍結後 nightly 掃描無法靠上游 tag-patch 自動 close，由 Renovate 定期重解 digest（並 bump tag）。

**啟用步驟（owner，一次性）**：(1) 建 fine-grained PAT（Contents + Pull requests + **Workflows**: read/write——Workflows 必須，因為 Renovate 會改 `nightly-image-scan.yaml` 這個 `.github/workflows/` 檔，GitHub 拒絕無 workflow scope 的 token 動 workflow 檔）；(2) repo Settings → Secrets and variables → Actions 加 `RENOVATE_TOKEN`；(3) Actions → Renovate → Run workflow，**Dry run=true** 先讀 log 驗證；(4) 確認無誤後讓週排程接手。⚠️ **必須用 PAT 而非 GITHUB_TOKEN**——否則 Renovate 的 PR 不會觸發其他 workflow 的 CI（drift-guard 安全網失效）。選 self-hosted 而非 Mend-hosted app：本 repo 全鏈自管/pin，不授權外部 SaaS 寫入，且 self-hosted 是下方 Category B 的必要路徑。若 Docker Hub 匿名 lookup 撞 rate limit（GH runner 共享 IP，nightly scan 已為 pull 加過 `DOCKERHUB_USERNAME`），在 `renovate.json` 加 `hostRules`（`matchHost: docker.io` + token）即可——dry-run log 會先浮現。**⚠️ PAT 365 天會過期**：fine-grained PAT 上限 365 天，過期後 secret 實體還在（workflow 的 token-presence guard 仍綠）但 Renovate 撞 401、排程 run 紅燈卻無人看 → digest 靜默重新凍結（workflow 的 `Explain a likely failure cause` step 讓紅 run 自我解釋；#902 L1 nightly scan 是慢速兜底——凍結的 digest 累積 CVE）。**長期解（defer，Category B/C 前做）**：PAT 換成 GitHub App token（`actions/create-github-app-token`，1 小時壽命、永不過期、PR 掛 `renovate[bot]` 身分而非 owner 個人帳號，權責切分更清晰）。

**待擴充（同一份 `renovate.json` 增量加 manager，各有 trigger）：**

- **C — lang-deps（Python / Go / npm）**：原 backlog 項，CI matrix 穩定後導入（把 `pip_requirements`/`gomod`/`npm` 加進 `enabledManagers`）。確保安全修補及時套用。**勿與 image 綁同一 PR**：image group 是 `matchDatasources: ["docker"]`、lang-deps 為不同 datasource 自然分流；導入時走 **domain-based grouping**（如 `monitoring-images` / `backend-deps` / `frontend-deps` 各自 PR），在 review noise 與 bisection 可除錯性間取平衡——目前 14 image 併一 PR 是合理 ROI，但混入 lang-deps 後單一紅燈會難定位是哪個升級（外部 review）。
- **B — 下載工具的 version+SHA-256 連動。** repo 對下載的第三方工具走「download + `scripts/ops/_verify_download.sh` 驗 pinned SHA-256」（hadolint / promtool / pint / kube-linter / vector @ `ci.yml`；golangci-lint @ `validate.yaml`；政策見 #849/#851）。Renovate 預設只 bump 版號字串、不重算 SHA → 升級 PR 會在 `_verify_download.sh` 紅燈、自動升級斷鏈。**解法需 self-hosted `postUpgradeTasks`**（升級後跑腳本重算 SHA 寫回——hosted app 做不到，故 L3 選 self-hosted）+ 客製 `customManagers` 讀對應 release `checksums.txt` 的 datasource 同步 `*_SHA256`。

  > ⚠️ **golangci-lint 額外陷阱（#914 對抗 review 已驗證）**：它現在驗的是 **`install.sh`**，而官方 release 的 `checksums.txt` **只列 binary tarball 的 SHA、不含 `install.sh`**（v2.12.2 實測：`install.sh` 0 筆、`golangci-lint-2.12.2-linux-amd64.tar.gz` 有）。所以 Renovate **無法**靠 release checksums 更新 install.sh 的 hash。**Category B 落地前須先把 golangci-lint 改成直接下載 binary tarball**（`golangci-lint-<ver>-linux-amd64.tar.gz`，其 SHA 必在官方 checksums），即可用 Renovate 內建 datasource 無縫同步——hadolint/promtool/pint/kube-linter 本來就下載 binary、無此問題。
