...
**導入前置：SHA-pin 連動（#902 L3）。** repo 對下載的第三方工具走「download + `scripts/ops/_verify_download.sh` 驗 pinned SHA-256」（golangci-lint @ `validate.yaml`；hadolint / promtool / pint / kube-linter @ `ci.yml` 等，政策見 #849/#851）。Renovate 預設只 bump 版號字串、不重算 SHA → 升級 PR 會在 `_verify_download.sh` 紅燈、自動升級斷鏈。導入時須客製 `regexManagers` + 讀對應 release `checksums.txt` 的 datasource 同步更新 `*_SHA256`。

> ⚠️ **golangci-lint 這條有額外陷阱（#914 對抗 review 已驗證）**：它現在驗的是 **`install.sh`**，而官方 release 的 `checksums.txt` **只列 binary tarball 的 SHA、不含 `install.sh`**（v2.12.2 實測：`install.sh` 0 筆、`golangci-lint-2.12.2-linux-amd64.tar.gz` 有）。所以 Renovate **無法**靠 release checksums 更新 install.sh 的 hash。**L3 解法 = 把 golangci-lint 改成直接下載 binary tarball**（`golangci-lint-<ver>-linux-amd64.tar.gz`，其 SHA 必在官方 checksums），即可用 Renovate 內建 datasource 無縫同步——hadolint/promtool/pint/kube-linter 本來就下載 binary、無此問題。
