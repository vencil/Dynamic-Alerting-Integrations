...
v2.5.0 Phase D 完成影響評估文件（`docs/internal/archive/ssot-language-evaluation.md`）。**v2.7.0 進度**：Phase 1 工具準備完成（`migrate_ssot_language.py` 支援 `--dry-run` / `--execute --git`，lint hooks 支援 `.en.md` + `.zh.md` dual-mode auto-detect，pilot report 見 `docs/internal/archive/ssot-migration-pilot-report.md`）。**v2.8.0 執行 Phase 2**：66 對檔案 + mkdocs.yml 原子性 commit，配合 minor release 的 migration window。**Status (v2.8.0 S#101)**：execution cancelled — both evaluation docs archived；trigger conditions for re-evaluation 見 dev-rules §9b。
