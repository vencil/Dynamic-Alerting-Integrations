...
| `nightly-race.yaml` | `schedule` + `workflow_dispatch`（含 `file_issue` 輸入） | ✅ #1275 新增 `report` fan-in job（`if: always()`），開 issue 另由 `file_issue`（預設 false）閘住。`file_issue=false` 走完 fragment 載入→判定→body 組裝→step summary 而**不寫任何 issue**；label 建立、issue 查詢與 create/edit 分支則被跳過，要驗那半段得用 `file_issue=true`（在無失敗可報時它不會建立任何東西）。這條路徑無法在自己的 PR CI 裡跑（schedule/dispatch-only），dispatch 是唯一的上線前驗證手段 |
| `nightly-mutation-pilot.yaml` | `schedule` + `workflow_dispatch` | ✅ 已有手動 dispatch（#350） |
...
make release-tag-recipe-preview                 # 從 recipe-preview Chart.yaml version 推導（同步升 = 平台同版）
...
#    刪 remote tag + 逐個重推才生效。六線時更要逐個推。
for t in exporter/v<EXP_CHART_VER> tools/v<TOOLS> portal/v<PORTAL> recipe-preview/v<RP_CHART_VER> tenant-api/v<TENANT_API_CHART_VER>; do
...
| 22 | ⚰️ **此陷阱已不存在**（原文：`scope-enum` allowlist 不含 `lint`） | 本列上一版把 enum 抄成散文（「只允許 `exporter, tools, docs, rule-packs, ci, k8s, helm`」七個值），而 SSOT 實際遠不止那七個（`lint` / `dx` 都在內），照它寫 `fix(tools)` 反而是錯的建議。⛔ **這裡不再複製那份清單，也不寫「共幾個」**——複製出來的第二份必然先腐爛，而「共幾個」是同一個錯換個形式：本列改寫時就順手寫了一個數字，當場就是錯的，被 review 抓出來。本列是活例 | scope 的 SSOT 是 [`.commitlintrc.yaml`](https://github.com/vencil/Dynamic-Alerting-Integrations/blob/main/.commitlintrc.yaml) 的 `scope-enum`，改 scope 前直接讀它。⚠️ 編號保留（不刪列、不重編）是因為第 33 列以「陷阱 #26 / #30」形式交叉引用本表 |
| 23 | 改 PR title 後 commitlint 不重跑 | `commitlint.yaml` 的 `pull_request.types` 只列 `[opened, synchronize, reopened]`，**沒有 `edited`** | `gh pr edit --title` 後需要額外觸發 `synchronize` 事件：① `git commit --amend` 改 commit message 符合 scope 規則 → ② `GIT_PREFLIGHT_BYPASS=1 git push --force-with-lease`（amend 換了 sha，preflight marker 必然不存在；⛔ 不要用 `--no-verify` 代替——它連擋直推 main 那道一起關掉，而那道沒有自己的旗標，#1487） → synchronize 事件會同時觸發 commitlint 重跑和逐 commit 檢查 |
...
| 25 | commit body 每行超過 100 字元會被擋，`-m "<單行長 body>"` 一定爆 | ⚠️ **卡住你的不是 commitlint**：本 repo 的 [`.commitlintrc.yaml`](https://github.com/vencil/Dynamic-Alerting-Integrations/blob/main/.commitlintrc.yaml) 把 `body-max-line-length` 覆寫成 **200**。真正把每行卡在 **100 字元**的是本地 `commit-msg` hook（`POST_HEADER_MAX_LINE_LENGTH`，含 trailer 行），見 [`commit-convention.md`](commit-convention.md)。用 `git commit -m "title" -m "<body>"` 時第二個 `-m` 整段變一行，中文/URL/錯誤訊息很容易超過 | 用 `git commit -F <file>`，在檔案裡把 body 以 ≤ 100 字元手動斷行；或 `git commit -m "title" -m "line1" -m "line2"`（每個 `-m` 各自換行）。用 Windows 寫訊息檔時**必須 UTF-8 無 BOM**（`UTF8Encoding(false)`）否則 git 會把 BOM 當成訊息第一個字元 |
...
| 25 | PowerShell 寫 commit message file 帶 BOM → commitlint 三層 fail | **✅ Codified（v2.8.0 PR #56）**：`scripts/tools/dx/pr_preflight.py::detect_commit_msg_bom()` BOM-first precedence 偵測 + 寫檔正解提示，commit-msg hook 直接 exit 1 擋下不流到 commitlint。寫檔正解：`[IO.File]::WriteAllText($p, $m, [Text.UTF8Encoding]::new($false))` 或 Git Bash `printf '%s\n' "$msg" > file`。原 RCA / recovery SOP → [`archive/automation-origins/trap-61-bom.md`](archive/automation-origins/trap-61-bom.md)（同 [Windows-MCP Pitfall #61](windows-mcp-playbook.md#已知陷阱速查)）|
