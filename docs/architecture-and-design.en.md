...
| **Rule Packs** | Zero PR conflicts for cross-team parallel development | 16 Projected Volumes + three-part structure + bilingual annotations | [rule-packs.en.md](design/rule-packs.en.md) |
...
The platform manages **16 independent rule packs**. Each Rule Pack is a self-contained three-part structure:
...
The table below is generated by `generate_rule_pack_stats.py` from the Rule Pack source YAML (guarded by the `rule-pack-stats-check` hook) — **do not edit by hand**. This section previously carried a hand-written "139 Recording + 99 Alert", while the real figures had long since moved to 166 / 161; pulling in the generated fragment stops the two from drifting apart again.
