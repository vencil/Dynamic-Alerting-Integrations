...
- **範圍**：只用 GET。但 pod 名稱是生成的，`resourceNames` 縮不到 exporter ⇒ 這條等於能對該 namespace **每個 pod 的每個 port** 發 GET，包含平常由 sidecar（例如認證 proxy）擋在前面的 container port。要更窄，就把 exporter 放在自己的 namespace 並用 `--exporter-namespace` 指過去。
