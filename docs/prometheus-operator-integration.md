...
> **相關 ADR**：[ADR-008 — Operator CRD 路徑](adr/008-operator-crd-path.md)
...
## 步驟 1：ServiceMonitor — 注入 tenant 標籤 + 抓取 threshold-exporter

### Step 1a. 為資料庫 Exporters 建立 ServiceMonitor
...
### Step 1b. 為 threshold-exporter 建立 ServiceMonitor
...
## 步驟 2：PrometheusRule — 自動轉換規則包

### 使用 da-tools operator-generate 自動產出
...
## 步驟 3：AlertmanagerConfig — 動態路由配置

### 前提：Alertmanager Operator 設定
...
### 3a. 使用 da-tools operator-generate 產出 AlertmanagerConfig
...
## 遷移路徑：從 ConfigMap 到 Operator CRD
...
#### Q3：Alertmanager 無法識別 AlertmanagerConfig
...
#### Q4：Namespace selector 跨越多個 ns 時無法抓取
...
A：不建議。若必須，使用 `additionalPrometheusRules` Helm value 注入額外的 rule 文件，避免 ServiceMonitor/PrometheusRule 重複。詳見 [ADR-008](adr/008-operator-crd-path.md)。
...
| [ADR-008 — Operator CRD 路徑](adr/008-operator-crd-path.md) | ⭐⭐⭐ |
