...
- AM 舊 config / Prom config 同時消失（前面 Phase 4 catalog 第 4 條已警告）
...
新規則抓到一個**舊系統漏抓 2 年的緩慢 memory leak**——某 Java service GC behavior 慢慢退化，每天記憶體少 ~50MB，舊靜態閾值要等到 service 撐 ~3 週快要 OOM 才警告（已是事故發生中）；新 predict_linear 在斜率改變後 ~4 小時內就警告，給 ops 充裕時間 restart / 滾動更新。客戶 SRE lead 的反應：「⋯⋯ 那我們 2 年來都怎麼活的？」（→ catalog Phase 3 第 1 條 anchor 對應；同時是 §6「為什麼 subset overlap < 100% 不見得是壞事」的活生生案例）。
