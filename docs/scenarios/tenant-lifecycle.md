...
⚠️ `expires` 必須是完整的 RFC3339 時戳（含 `T` 與時區，如 `2099-12-31T23:59:59Z`，建議加引號）。exporter 解析不了時只印 WARN（每次 scrape 都會出現）、把它當成**沒有期限**——靜默不會自動結束。`expires` 已在過去 ⇒ 寫入後立即視為過期、不靜音，並發出 `da_config_event{event="silence_expired"}` 事件。`target:` 冒號後要留空白；JSON 寫法（`{"target": …}`）不會被認得。
