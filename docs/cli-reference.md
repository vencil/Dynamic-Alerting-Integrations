...
「宣告」以 **exporter 的讀法**為準：只讀第一個 YAML 文件、key 取原始文字（`on:` 就是租戶 `on`、`007:` 就是 `007`），而且整份檔必須通過 exporter 的解碼——租戶 body 是純量、同一個 key 出現兩次、其他欄位型別錯誤、YAML 語法錯誤的檔，exporter 會**整份拒收**、不宣告任何租戶。這樣的檔若提到了這次要求的租戶（或 init 無法判斷 exporter 是否接受它），init 會**拒絕執行、rc 1、不寫入任何檔案**，並點名檔案與原因，請先修好那個檔；沒提到要求租戶的，只在 stderr 以 WARN 點名。以下情形也會拒絕：init 要覆寫的 `conf.d/<t>.yaml` 同時宣告了這次要求的另一個租戶（覆寫後那個租戶將無處宣告）；init 要寫的任何路徑旁已有**只差大小寫**的既有項目（例如 `DB-C.YAML` 之於 `db-c.yaml`，在不分大小寫的檔案系統上兩者是同一個檔）。
