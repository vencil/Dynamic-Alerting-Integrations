...
3. Whether you accidentally used mode B (receiver containing `{{tenant}}`) — per-tenant enforced routes carry a `tenant="<name>"` matcher, and platform alerts have no tenant, so they are **structurally unreachable**.
