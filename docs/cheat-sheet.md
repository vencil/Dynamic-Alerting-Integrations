...
| `guard` | Dangling Defaults Guard（C-12 PR-4），shell-out 至 `da-guard` Go binary 驗 conf.d/ schema + routing + cardinality | defaults-impact 子命令 + --config-dir <PATH>, --scope, --required-fields, --cardinality-limit, --format md\|json | `da-tools guard --help` |
| `batch-pr` | Migration Batch PR Pipeline（C-10 PR-5），shell-out 至 `da-batchpr` Go binary 跑 apply / refresh / refresh-source orchestration | apply\|refresh\|refresh-source 子命令 + --plan, --emit-dir, --input, --patches-dir, --workdir, --repo, --dry-run | `da-tools batch-pr --help` |
| `parser` | PromRule parser（C-8 PR-2），shell-out 至 `da-parser` Go binary 解析 PrometheusRule YAML + dialect / VM-only / strict-PromQL 相容性分類 | import\|allowlist 子命令 + --input, --output, --validate-strict-prom, --fail-on-non-portable, --fail-on-ambiguous | `da-tools parser --help` |
| `tenant-verify` | 印 tenant effective config + merged_hash（Phase B Track A，B-4 rollback checklist item 6） | <tenant-id>, --conf-d <PATH>, --expect-merged-hash <H>, --all, --json | `da-tools tenant-verify --help` |
