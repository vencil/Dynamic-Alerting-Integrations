...
| `deprecate` | 下架指標：從 `defaults:`／`optional_overrides:`／租戶檔移除 `<m>`、`<m>_critical`、`custom_<m>`、`custom_<m>_critical`；租戶平面另含這四個名字的維度鍵 `<名字>{…}` | --config-dir <PATH>, --execute, --plane {root,subtree} | `da-tools deprecate --help` |
