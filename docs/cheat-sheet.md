...
| `shadow-verify` | Shadow Monitoring 就緒度與收斂性三階段驗證 | --mapping <FILE>, --report-csv <FILE>, --json | `da-tools shadow-verify --help` |
| `byo-check` | BYO Prometheus & Alertmanager 整合驗證 | --prometheus <URL>, --alertmanager <URL>, --json | `da-tools byo-check --help` |
| `federation-check` | 多叢集 Federation 整合驗證（edge / central / e2e） | --prometheus <URL>, --edge-urls <URLS>, --json | `da-tools federation-check --help` |
| `grafana-import` | Grafana Dashboard ConfigMap 匯入（sidecar 自動掛載） | --dashboard <FILE>, --verify, --dry-run | `da-tools grafana-import --help` |
...
- **Prometheus API 工具**：需要能連到 Prometheus HTTP API
  ...
  - `cutover` — 一鍵切換（遷移最後一步）
  - `shadow-verify` — Shadow Monitoring 三階段驗證
  - `byo-check` — BYO 整合驗證
  - `federation-check` — 多叢集 Federation 驗證
  - `grafana-import` — Dashboard ConfigMap 匯入
