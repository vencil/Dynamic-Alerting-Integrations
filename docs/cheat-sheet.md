...
| `fed-key` | 產生 / 輪替 federation JWT 簽章金鑰（ADR-020 IV-2l）：私鑰 Secret manifest + 公鑰 JWKS | --rotate, --existing-jwks <FILE>, --jwks-out <FILE>, --namespace <NS> | `da-tools fed-key --help` |
...
| `validate-config` | 一站式配置驗證：YAML 格式、schema、routing、policy、版本一致性 | --config-dir <PATH>, --policy <FILE>, --json | `da-tools validate-config --help` |
| `offboard` | 下架 tenant 配置與相關資源 | --config-dir <PATH>, --backup <DIR>, --cleanup-rules | `da-tools offboard --help` |
| `deprecate` | 下架指標：從 `defaults:`／`optional_overrides:`／租戶檔刪除其 key | --config-dir <PATH>, --execute, --plane {root,subtree} | `da-tools deprecate --help` |
...
| `gitops-check` | GitOps Native Mode 就緒度驗證（repo / local / sidecar） | --url <URL>, --dir <PATH>, --namespace <NS> | `da-tools gitops-check --help` |
...
| `rule-pack-split` | Rule Pack 分層拆分（edge Part 1 + central Parts 2+3） | --rule-packs-dir <DIR>, --output-dir <DIR>, --scenario | `da-tools rule-pack-split --help` |
| `opa-evaluate` | OPA Rego 策略評估橋接（OPA 整合） | --config-dir <PATH>, --opa-url <URL>, --opa-binary, --policy-path, --dry-run | `da-tools opa-evaluate --help` |
| `guard` | Dangling Defaults Guard（v2.8.0），shell-out 至 `da-guard` Go binary 驗 conf.d/ schema + routing + cardinality | defaults-impact 子命令 + --config-dir <PATH>, --scope, --required-fields, --cardinality-limit, --format md\|json | `da-tools guard --help` |
| `batch-pr` | Migration Batch PR Pipeline（v2.8.0），shell-out 至 `da-batchpr` Go binary 跑 apply / refresh / refresh-source orchestration | apply\|refresh\|refresh-source 子命令 + --plan, --emit-dir, --input, --patches-dir, --workdir, --repo, --dry-run | `da-tools batch-pr --help` |
| `parser` | PromRule parser（v2.8.0），shell-out 至 `da-parser` Go binary 解析 PrometheusRule YAML + dialect / VM-only / strict-PromQL 相容性分類 | import\|allowlist 子命令 + --input, --output, --validate-strict-prom, --fail-on-non-portable, --fail-on-ambiguous | `da-tools parser --help` |
| `tenant-verify` | 印 tenant effective config + merged_hash（v2.8.0；incremental migration playbook rollback checklist item 6） | <tenant-id>, --conf-d <PATH>, --expect-merged-hash <H>, --all, --json | `da-tools tenant-verify --help` |
...
- **採用與初始化**
  - `init` — 專案骨架產生（CI/CD + conf.d + Kustomize）
  - `config-history` — 配置快照與歷史追蹤
  - `gitops-check` — GitOps Native Mode 就緒度驗證
  - `state-reconcile` — 遷移狀態目錄一致化（schema 驗證 + manifest 重建）
  - `rule-pack-diff` — Rule Pack 兩版本機械比對（升級審計用）
  - `silencer-drift-check` — AM silence 漂移偵測（offline，cutover 必跑）

- **Operator + Federation**
  - `operator-generate` — Operator CRD 產生（PrometheusRule / AlertmanagerConfig / ServiceMonitor）
  - `operator-check` — Operator CRD 部署狀態驗證
  - `rule-pack-split` — Rule Pack 分層拆分（Federation Scenario B）
  - `opa-evaluate` — OPA Rego 策略評估橋接
