...
- [ ] `blind-spot` 掃描無預期外的盲區（所有叢集 DB instance 都有對應 tenant 配置）
...
| 工具 | 用法 | 效果 |
|------|------|------|
| **Onboard→Scaffold pipeline** | `onboard` → `scaffold --from-onboard <hints>` | 一鍵完成遷移準備（v1.9.0） |
| **Auto-convergence detection** | `validate --auto-detect-convergence --stability-window 5` | 連續 N 輪 match 後自動產出 `cutover-readiness.json`（v1.9.0） |
| **One-command cutover** | `cutover --readiness-json <path> --tenant <t>` | 單一指令完成 Shadow Monitoring 切換全流程（v1.10.0） |
| **Post-cutover health report** | `batch-diagnose` | 自動發現 tenants → 並行 `diagnose` → health score（v1.9.0） |
| **Blind spot discovery** | `blind-spot --config-dir <dir>` | 掃描叢集 targets 與 tenant 配置交叉比對盲區（v1.10.0） |
| **Rule Pack gap analysis** | `analyze-gaps --config <path>` | 三層匹配分析 `custom_` 規則對應的官方 Rule Pack（v1.9.0） |
| **Threshold backtest** | `backtest --git-diff --prometheus <url>` | PR threshold 變更 7 天歷史回測 + 風險等級（v1.9.0） |
| **Config diff** | `config-diff --old-dir <old> --new-dir <new>` | 目錄級配置差異 blast radius 報告（v1.10.0） |
| **Config patch preview** | `patch-config --diff <tenant> <metric> <value>` | 單一 metric 的 live before/after 對比（v1.9.0） |
