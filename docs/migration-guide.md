...
```bash
# CLI 模式 — 一行搞定
docker run --rm -v $(pwd)/output:/data ghcr.io/vencil/da-tools:1.4.0 \
...
```bash
# 預覽模式 — 不產出檔案，只顯示分析結果
docker run --rm -v $(pwd):/data ghcr.io/vencil/da-tools:1.4.0 \
...
spec:
  template:
    spec:
      containers:
        - name: da-tools
          image: ghcr.io/vencil/da-tools:1.4.0
          args:
            - scaffold
            - --tenant
            - db-c
...
# 確認 Alert 狀態
docker run --rm --network=host ghcr.io/vencil/da-tools:1.4.0 \
...
```bash
# 預檢模式 — 確認無外部依賴
docker run --rm -v $(pwd)/conf.d:/data/conf.d ghcr.io/vencil/da-tools:1.4.0 \
  offboard db-a
