...
>
> **提示：** 所有 `da-tools` 指令可透過 Docker 直接執行（`docker run --rm --network=host ghcr.io/vencil/da-tools:v2.8.0 <cmd>`），以下範例使用簡寫 `da-tools <cmd>` 形式。

## 安裝 Migration Toolkit（v2.8.0+）

開始遷移前先安裝工具集。三條路徑（Docker / static binary / air-gapped tar）任選一條，詳見 [`migration-toolkit-installation.md`](migration-toolkit-installation.md)。最常見的兩個指令：
...
完整指令、hash 驗證、air-gapped 流程：見 [`migration-toolkit-installation.md`](migration-toolkit-installation.md)。
...
# 含 N:1 namespace-tenant mapping
...
da-tools 也可以直接作為 K8s Job 運行（`image: ghcr.io/vencil/da-tools:v2.8.0`），省去 port-forward 設定。叢集內 da-tools 可透過 K8s Service 直接存取 Prometheus（`http://prometheus.monitoring.svc.cluster.local:9090`），適合 `check-alert`、`validate`、`baseline` 等需要 Prometheus API 的命令。
...
上面 4.1 示範的三件套結構對所有場景適用。以下表格列出常見指標的關鍵差異：
...
當 `da-tools migrate` 遇到無法解析的規則，它會產出可直接交 LLM 的 Prompt。`migration-report.txt` 中包含可直接交 LLM 的 System Prompt 模板，指導 LLM 抽取閾值、產出 Recording Rule（含 sum/max 選擇理由）、Alert Rule（含 `group_left` + `unless maintenance`），並標記需要平台額外處理的項目。
...
> **提示**：切換前可用 `da-tools shadow-verify all` 一鍵執行完整就緒度檢查（preflight + runtime + convergence），替代手動逐步驗證。

`cutover-readiness.json` 由 `validate --auto-detect-convergence` 在達到收斂條件時自動產出。如果是手動確認收斂（無 readiness JSON），可用 `--force` 跳過 readiness 檢查。詳見 [Shadow Monitoring SOP §7](shadow-monitoring-sop.md#7-退出-shadow-monitoring)。
...
`blind-spot` 會列出叢集中有 exporter 在跑但未被 tenant 配置涵蓋的 instance（blind spots）。遷移完成後建議定期執行（如每次新增 exporter 時），確保所有 DB instance 都受閾值監控保護。
...
| 工具 | 用法 | 效果 |
|------|------|------|
| **Onboard→Scaffold pipeline** | `onboard` → `scaffold --from-onboard <hints>` | 一鍵完成遷移準備 |
| **Auto-convergence detection** | `validate --auto-detect-convergence --stability-window 5` | 連續 N 輪 match 後自動產出 `cutover-readiness.json` |
| **One-command cutover** | `cutover --readiness-json <path> --tenant <t>` | 單一指令完成 Shadow Monitoring 切換全流程 |
| **Post-cutover health report** | `batch-diagnose` | 自動發現 tenants → 並行 `diagnose` → health score |
| **Blind spot discovery** | `blind-spot --config-dir <dir>` | 掃描叢集 targets 與 tenant 配置交叉比對盲區 |
| **Rule Pack gap analysis** | `analyze-gaps --config <path>` | 三層匹配分析 `custom_` 規則對應的官方 Rule Pack |
| **Threshold backtest** | `backtest --git-diff --prometheus <url>` | PR threshold 變更 7 天歷史回測 + 風險等級 |
| **Config diff** | `config-diff --old-dir <old> --new-dir <new>` | 目錄級配置差異 blast radius 報告 |
| **Config patch preview** | `patch-config --diff <tenant> <metric> <value>` | 單一 metric 的 live before/after 對比 |
