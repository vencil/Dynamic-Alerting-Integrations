...
```bash
# CLI 模式 — 一行搞定
docker run --rm -v $(pwd)/output:/data ghcr.io/vencil/da-tools:1.9.0 \
...
以下工具已在 v1.9.0 實作，降低大型遷移的人工介入：

| 工具 | 用法 | 效果 |
|------|------|------|
| **Onboard→Scaffold pipeline** | `onboard` 自動產出 `onboard-hints.json` → `scaffold --from-onboard <hints>` | 一鍵完成遷移準備，省去手動複製 extracted-tenants.yaml |
| **Auto-convergence detection** | `validate --auto-detect-convergence --stability-window 5` | 自動偵測所有 metric pair 收斂，連續 N 輪 match 後產出 `cutover-readiness.json` |
| **Post-cutover health report** | `batch-diagnose`（da-tools CLI） | 自動發現 tenants → 並行 `diagnose` → 彙整 health score + remediation |
| **Rule Pack gap analysis** | `analyze-gaps --config <path>`（da-tools CLI） | 三層匹配（Exact → Prefix → Token Overlap）分析 `custom_` 規則對應的官方 Rule Pack |
| **Threshold backtest** | `backtest --git-diff --prometheus <url>` | PR 修改 threshold 時用 7 天歷史數據模擬 firing count 變化 + 風險等級 |
| **Config diff preview** | `patch-config --diff <tenant> <metric> <value>` | 顯示 patch 前後的 before/after 對比 + 受影響 alert rule（類似 `terraform plan`） |
