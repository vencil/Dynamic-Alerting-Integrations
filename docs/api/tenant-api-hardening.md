...
寫平面是**單寫者**（ADR-023）。為消除滾動更新交疊期的「幽靈副本」多寫者 correctness bug（#677 / TRK-324），tenant-api Deployment 採 **`strategy: Recreate`**（殺舊 pod 再起新 pod，無交疊）。**對價**：每次部署都會結束所有開啟中的 `GET /api/v1/events` SSE 連線。

- **優雅關機（#675 / TRK-322）**：SIGTERM 時 `Hub.Shutdown` 先廣播一則 `server_shutdown` 控制事件、再關閉所有 SSE 連線，**然後**才 `http.Server.Shutdown`。兩個效果：(1) SSE 連線永不 idle，若不先關，`srv.Shutdown` 會卡滿 15s grace period 才硬切——先關讓 Shutdown 毫秒級完成；(2) client 收到的是**可行動的訊號 + 重連提示**而非生硬的 connection reset。事件契約：`{"type":"server_shutdown","reconnect_delay_ms":2000}`——well-behaved client 應等 `reconnect_delay_ms` **加自身隨機 jitter** 再重連，把重連流量打散離開尚未 ready 的新 pod。〔前端 jitter 重連屬 Portal 對接 future work——目前無 SSE consumer，server 先發出契約事件作為基礎。〕
