...
- **偵測護欄（本節）**：alert `TenantApiSingleWriterBreach`（severity `critical`、會 page）把 trigger 從 **lagging**（等資料毀損事故才發現）改成 **leading**：`max by (namespace, deployment) (kube_deployment_spec_replicas{namespace="tenant-api", deployment="tenant-api"}) > 1`，`for: 2m`。一旦 live Deployment 宣告 >1 副本即在 2 分鐘內 page。**為何 spec 非 status**：`kube_deployment_spec_replicas` 是宣告值——最早的訊號，且正好是 L1 `replicaCount==1` guard 的執行期鏡像，`kubectl scale` / HPA / GitOps-patch 一改 intent 就抓到（互補面 `kube_deployment_status_replicas > 1` 看實際 Running pod，刻意不編進來以保契約單一訊號可 promtool 測）。**為何聚合**：`max by (namespace, deployment)` 把 scrape 帶的 `job`/`instance` label 收斂成單一邏輯 series（也撐過 KSM rescrape 換 instance），免得一個雜散 label 讓 `> 1` 比對**靜默永不命中**——與 §5.6 SSE 告警同一套 anti-silent-fire 紀律。**為何 `for: 2m`**：debounce 一個 GitOps self-heal 在 sync loop 內把手動 scale revert 回 1 的瞬態（修正過的瞬態不 page），但對**卡住**的破口仍快速 page——本來就沒有合法的 2-副本狀態，任何持續觸發都是真的。KSM 以無 `--namespaces` 過濾運行，故 `tenant-api` namespace 確實被涵蓋（KSM 本身掛掉是 KSM-absence 告警的事、非本規則）。
- **同時是 #787 的 codify re-trigger**：此告警 fire 即「執行期多寫者已成真實風險」→ reopen ADR-023 Deferred A3 實作 leader-election（**勿**抬高 replicaCount）。與 §5.6 `TenantApiReadHANeeded`（A4 的 codify re-trigger）對稱：把延後決策的觸發條件變成會自己叫的訊號，而非關掉的 issue 裡沒人看的 TODO。行為契約：`tests/rulepacks/platform-single-writer-breach_test.yaml`（4 場景：2-副本持續觸發+聚合剝離 label／單副本不觸發／sub-`for` self-heal 瞬態不觸發／3-副本 HPA 向量觸發）。
