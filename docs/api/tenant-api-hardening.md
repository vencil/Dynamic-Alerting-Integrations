...
`http.Server{ReadTimeout: 15s, WriteTimeout: 30s, IdleTimeout: 60s}` 與 1MB body limit 都寫死在 code 裡。未來如客戶 ops 需要不同環境的調參，再透過 Helm value 暴露。**本次硬化不擴此 surface** — 預設值已在合理範圍。
