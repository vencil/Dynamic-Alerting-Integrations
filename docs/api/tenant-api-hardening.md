...
### 5.1 Body 內容範圍校驗（C4，[issue #134](https://github.com/vencil/Dynamic-Alerting-Integrations/issues/134) 追蹤）

`POST /api/v1/tenants/batch` 的 `BatchOperation.Patch` 等 body 欄位目前只檢查 size（≤1MB）+ JSON 合法性，**不**檢查值範圍：
...
**Mitigation**：下游（threshold-exporter / GitOps writer）的 schema 驗證最終會擋，但這不是 fail-fast。v2.8.x hardening PR 會引入 `go-playground/validator` + struct tag 在邊界先擋。
