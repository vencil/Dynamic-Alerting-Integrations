...
- **read-HA 是 deferred（#678 / #788，已關閉並 codify re-trigger）**：給讀取 zero-downtime 的正解是讀寫拆分部署（read deployment 多副本 RollingUpdate + binary read-only enforcement、write deployment 維持單副本 Recreate）。**刻意 defer**——成本中等，且「讀取 HA 是需求」這個前提目前**無 field data 驗證**（讀路徑為低頻 admin UI、發版數秒 blip 未經量測證實有害）；在需求成形前先蓋＝服務一個幻影需求。為免 defer 流為「關掉的 issue 裡一句沒人看的 TODO」，trigger 已**codify 成會自己叫的 alert** `TenantApiReadHANeeded`（severity `info`、不 page）：對同一個 `tenant_api_sse_clients` gauge 取 **7d 平均並發 > 2**（`avg_over_time(...[7d:1h])`，取平均非峰值＝只認「持續多人」、不被「單日尖峰」誤觸）即「read-HA 成為真實需求」、屆時 reopen ADR-023 Deferred A4 實作讀寫拆分。**⛔ 觸發時注意**：A4 單做只買到**讀** zero-downtime，**寫**路徑（Save）發版仍中斷，需與 A3（K8s Lease，#787）**同排**、勿單獨出 A4。互補可一併看 `rate(tenant_api_requests_total)`（讀為主、寫為極小的 `writes_total`）。行為契約：`tests/rulepacks/platform-read-ha-trigger_test.yaml`。在那之前 Recreate 的數秒讀取 blip 是接受的對價。
