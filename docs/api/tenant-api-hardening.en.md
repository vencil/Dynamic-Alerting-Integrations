...
- **Detective guard (this section)**: the `TenantApiSingleWriterBreach` alert (severity `critical`, pages) turns the trigger from **lagging** (discovered only via a corruption incident) into **leading**: `max by (namespace, deployment) (kube_deployment_spec_replicas{namespace="tenant-api", deployment="tenant-api"}) > 1`, `for: 2m`. It pages within 2 minutes the moment the live Deployment declares > 1 replica. **Why spec not status**: `kube_deployment_spec_replicas` is the declared value — the earliest signal, and the exact runtime mirror of the L1 `replicaCount==1` guard; it catches `kubectl scale` / HPA / GitOps-patch the instant they mutate intent (complementary face `kube_deployment_status_replicas > 1` watches pods actually Running — deliberately not encoded, to keep the contract single-signal + promtool-testable). **Why aggregate**: `max by (namespace, deployment)` collapses the scrape-added `job`/`instance` labels into one logical series (and survives a KSM rescrape under a new instance), so a stray label can't make the `> 1` comparison **silently never match** — the same anti-silent-fire discipline as the §5.6 SSE alerts. **Why `for: 2m`**: it debounces a GitOps self-heal that reverts a manual scale to 1 within its sync loop (no page for a corrected transient) yet still pages fast on a **stuck** breach — there is no legitimate 2-replica state, so any sustained firing is real. KSM runs with NO `--namespaces` filter, so the `tenant-api` namespace IS covered (a dead KSM is the KSM-absence alert's job, not this rule's).
- **Also the codified re-trigger for #787**: when this alert fires, runtime multi-writer is now a real risk → reopen ADR-023 Deferred A3 and build leader-election (do **not** raise replicaCount). Symmetric to §5.6's `TenantApiReadHANeeded` (A4's codified re-trigger): it turns a deferred decision's trigger into a self-firing signal rather than an unread TODO in a closed issue. Behavioural contract: `tests/rulepacks/platform-single-writer-breach_test.yaml` (4 scenarios: sustained 2-replica fire + label-stripping aggregation / single-replica no-fire / sub-`for` self-heal transient no-fire / 3-replica HPA-vector fire).
