...
### 5.6 Deployment strategy `Recreate` and SSE reconnect (read-HA trade-off, #677 / #740)

The write plane is a **single writer** (ADR-023). To eliminate the rolling-update overlap "phantom replica" multi-writer correctness bug (#677 / TRK-324), the tenant-api Deployment uses **`strategy: Recreate`** (kill the old pod before starting the new one — no overlap). **The price**: every deploy hard-drops all open `GET /api/v1/events` SSE streams.
...
- **Observability guard (#740 / TRK-326)**: the `TenantApiSSEReconnectFailure` alert catches a **failed** reconnect — three clauses sustained `for: 10m`: `tenant_api_sse_clients == 0` (none connected now) AND **`tenant_api_uptime_seconds < 1800`** (the pod **restarted** in the last 30m — load-bearing: it anchors the alert to the post-deploy window; without it the alert can't tell a reconnect failure from a user simply closing their Tenant-Manager tab during normal use, a constant false alarm on a low-frequency admin UI) AND `max_over_time([30m]) > 0` (there were clients in the last 30m, so there's something to reconnect). A normal single reconnect (recovers in seconds) does not false-positive; a genuinely-idle API (max=0) does not either; closing a tab outside a deploy window (high uptime) does not either; it auto-resolves once clients reconnect or uptime passes 30m. The behavioural contract is locked by the promtool test `tests/rulepacks/platform-sse-reconnect_test.yaml` (4 scenarios).
