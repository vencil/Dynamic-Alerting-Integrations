...
> ℹ️ This table is generated by `scripts/tools/dx/generate_byo_rulepack_table.py` from the projected volume in `k8s/03-monitoring/deployment-prometheus.yaml` (ConfigMap names and file lists) plus the Rule Pack source YAML (rule counts). **Do not edit it by hand** — the `byo-rulepack-table-check` hook will reject that. Mount a new Rule Pack into the projected volume and the row appears here on its own.
