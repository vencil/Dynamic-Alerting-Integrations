...
>
> **本指南寫給租戶端的操作者（SRE / NOC）** —— 你要拿 token、設定自己的 Prometheus / Grafana 拉取。你若是**平台工程師**要部署 federation（gateway / proxy / storage cap、策展 whitelist），看 `helm/federation-gateway` 與 `helm/federation-proxy` 的 chart README + [ADR-020](../adr/020-tenant-federation.md)，不是這篇。

## 0. 快速開始

趕時間？三步把自己的 metrics 拉出來（細節見後續各節）：

1. **取得 token** —— 由對你的租戶有 admin 權限者執行：
   ```sh
   curl -X POST "$TENANT_API/api/v1/federation/tokens" \
     -H "Content-Type: application/json" \
     -d '{"tenant_id": "<你的 tenant>", "description": "quickstart"}'
   ```
2. **設定拉取** —— 把回傳的 JWT 放進租戶側 Prometheus 的 `/federate` scrape 或 Grafana data source（一律走 `Authorization: Bearer` header；範例見 §4）。
3. **驗證**：
   ```sh
   curl -H "Authorization: Bearer <jwt>" "$FED_GW/api/v1/query?query=process_open_fds"
   ```
   回 `200`、且 result 內每個 series 的 `tenant` label 都是你自己 = 成功。

撞到錯誤碼看 §6、其他症狀看 §8；**上線後**的兩個行為（撤銷有延遲、斷線後別狂補資料）務必先讀 §7。
...
token TTL 4h、**無 sliding refresh** —— 持續性的拉取必須自動換發，否則每 4h 斷一次。最簡單的做法：一個 cron job 每隔 < 4h（例如每 3h）重新 `POST` 簽一把新 token、寫進你的 Prometheus / Grafana 設定後觸發 reload（Prometheus `kill -HUP` 或 `/-/reload`）。

注意每租戶同時最多 **16 把**有效 token —— 換發腳本應把上一把 `DELETE` 掉、或讓它自然過期（4h），別讓舊 token 累積撞 16 上限（會回 `409`）。
...
scrape_configs:
  - job_name: platform-federation
    ...
    scheme: https
    # honor_labels: true 不可移除 —— federate 來的 metric 自帶 tenant / instance /
    # job 等 label；honor_labels: false（Prometheus 預設）會在 label 衝突時把它們
    # 改寫成 exported_*，你的 dashboard 會壞圖。
    honor_labels: true
...
| 碼 | 意義 | 怎麼辦 |
|---|---|---|
| `429` | 撞到 per-token 速率上限（chart 預設 15 req/min，平台可調 15–60）| **走 `/federate` scrape**：Prometheus **不讀** rate-limit header，會照 `scrape_interval` 繼續撞牆 —— 調大 `scrape_interval`（15s → 30s/60s）、或別讓多個 scrape job 共用一把 token。**走 script / Grafana**：照回應的 `x-ratelimit-reset` header 退避。兩種情況都可多簽 token 分流 |
...
| Grafana "Save & Test" 顯示錯誤 / 紅字 | Grafana 會探測 `/api/v1/status/buildinfo` 做後端版本偵測，但 prom-label-proxy 不提供該 endpoint（回 `404`）—— "Save & Test" 可能因此報錯。**這不代表連線壞了**：只要實際 dashboard query 有資料就是通的，可忽略該按鈕的錯誤。若 query 也沒資料，才檢查 data source URL 是 `$FED_GW`、`Authorization: Bearer` header 是否設對 |
