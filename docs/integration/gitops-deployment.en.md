...
| A produced ConfigMap that is not your tree | Once built, this step **reads back the manifest it is about to write** (nothing has landed yet): the key set and every value's length must equal the carriers it selected. ⚠️ **Two** different causes land here: (1) the **argument layer** — a `,`, a `"` or an `=` in the path makes kubectl mangle the `--from-file` argument before it ever opens a file (it splits on CSV first, then on `key=path`) — one tenant short, or one key nobody declared; the remedy is a path holding none of those characters. (2) The **content layer** — the key set is right and only a **length** differs, which has nothing to do with the path: the value did not survive kubectl's YAML emitter and this loader byte for byte. Moving the tree reproduces that failure exactly — compare the manifest's value with the file instead. |
