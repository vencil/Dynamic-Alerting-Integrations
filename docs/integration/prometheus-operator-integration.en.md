...
In `operator` mode the chart renders only the ServiceMonitor. PrometheusRule and AlertmanagerConfig are generated by [`da-tools operator-generate`](../cli-reference.en.md#operator-generate) (ADR-008: the chart stays path-agnostic); the receiver and Secret are given as flags:
