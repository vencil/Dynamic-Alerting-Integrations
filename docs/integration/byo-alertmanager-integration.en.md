...
Both Silent Mode and Maintenance Mode support structured object configuration, including `expires` (RFC3339, e.g. `"2099-04-01T00:00:00Z"`; if it cannot be parsed the exporter only logs a WARN and treats it as having no expiry) auto-expiry and `reason` field to prevent "set and forget".
