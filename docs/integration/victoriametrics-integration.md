...
| **MetricsQL extensions** | non-portable function 偵測 | ✅ 完整 | `da-parser` `vm_only_functions.yaml` allowlist + freshness CI gate（細節見 [`cli-reference.md`](../cli-reference.md) §C-8 MetricsQL-as-Superset） |
...
詳細 CLI + JSON ParseResult schema：[`cli-reference.md` §C-8 MetricsQL-as-Superset PromRule parser](../cli-reference.md)（L2320-2397）。
...
| **da-parser MetricsQL handling**：CLI + JSON spec | [`cli-reference.md` §C-8](../cli-reference.md) |
