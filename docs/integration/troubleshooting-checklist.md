...
PR #401 提供了一個 humbling data point：作者按 9-item checklist 逐項 self-review、宣稱所有 entries pass，但 reviewer（Gemini）仍抓到一條漏網之魚——§1.1.2 分支 C 的「重啟 vmagent 會 lose buffer」陳述違反 vmagent persistent buffer 的物理特性（落盤 PVC、重啟後仍在），照原命令跑只會陷入 OOM 輪迴。
...
| Part 1 §1.4.4 Cardinality 暴漲 | ✅ ship（PR #400，含 `labeldrop` 自爆陷阱） |
| Part 1 §1.5.1 HA Prom reload race | ✅ ship（PR #400，含 AM dedup 失效 + `kubectl delete pod` brute-force fallback） |
| Part 2 §2.2 da-guard 4-layer 失敗 | ✅ ship（PR #400，4 sub-layer + local-validation 命令） |
| Part 1 §1.1.2 vmagent ingest gap | ✅ ship（**本 PR**，3 分支 root cause + clock skew edge） |
| Part 1 §1.6.2 SLO dashboard 誤判 | ✅ ship（**本 PR**，alert-count-as-SLI antipattern + universal recommendation） |
| Part 2 §2.1.3 Orphan rule | ✅ ship（**本 PR**，3 形狀分類 + 不要 prune 警告） |
| Part 2 §2.3 Migration state inconsistency | ✅ ship（**本 PR**，3 形狀分類 + per-cluster split 升級路徑） |
| §4.1 Entry template | ✅ ready |
| §4.2 Runbook PR review SOP（will-this-actually-deploy-at-3am） | ✅ ship（PR #400） |

**進度**：**19/19 entries 已 ship（100%）**。I-4 Migration Troubleshooting Checklist 從 outline + 7 entries（PR #398）→ 12 entries（PR #399）→ 15 entries（PR #400）→ **19 entries（本 PR）** 完成 4 波 batch ship。後續 maintenance：(1) 真實客戶 Issue # 由 maintainer 在 review 中陸續補入對應 anchor；(2) 新 incident 模式（catalog 沒列、第一次踩到的）依 §4.2 SOP 走 PR 補進；(3) EN mirror（待 SSOT policy 觸發條件達成）。
