...
# 分支 C：vmagent buffer 已滿、disk-overflow 也卡住
# 看 vmagent pvc usage
kubectl exec <vmagent-pod> -- df -h /tmp
# 若 vmagent persistent buffer 寫滿 → 重啟 vmagent 會 lose 該 buffer 的 in-flight data
# 接受資料丟失、bump vmagent memory + persistent disk size 後重啟
```
