...
| 成員模式 | 靜態 `members[]` list | 可預測、可 review、可 diff；filter-based auto-membership 延後至 v2.6.0+ |
...
Filter-based auto-membership 保留在 `filters` 欄位中，但 v2.5.0 不啟用自動匹配邏輯。預計 v2.6.0+ 作為進階功能加入。
...
| Group member 引用不存在的 tenant ID | v2.5.0 寫入時不驗證（soft reference），v2.6.0+ 加入 lint hook |
...
**下一步（v2.6.0+）**：
1. **Filter-based auto-membership**：啟用 `filters` 欄位，依 metadata 自動匹配 tenant 進群組，減少手動維護成本
2. **Group member lint hook**：寫入時驗證 member 引用的 tenant ID 存在，從 soft reference 升級為 validated reference
3. **Group nesting**（v2.7.0+）：群組可包含子群組，支援階層式組織結構
