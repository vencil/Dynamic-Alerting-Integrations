...
- **v2.1.0**（已完成）：`federation_check.py` 遷移至共用 `query_prometheus_instant`，支援邊緣/中央雙模驗證。場景 B（Rule Pack 分層）已實作——`da-tools rule-pack-split` 支援邊緣正規化 + 中央聚合 + Operator CRD 輸出
- **v2.6.0**（已完成）：`operator-generate --kustomize` 支援多叢集 Federation 場景的 CRD 部署；`drift_detect.py --mode operator` 可偵測跨叢集 CRD 漂移
