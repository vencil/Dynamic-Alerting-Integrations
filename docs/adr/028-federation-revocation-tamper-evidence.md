...
2. 查 VictoriaLogs 取「窗口內所有 `federation_token_revoked` 事件」（#1237 起以 `log_type:"federation_evidence"` 限定串流，與 heartbeat canary 同一串流類別；見 D3 §偵測查詢的來源限定）。
...
- **⛔ 偵測查詢的來源限定（#1237 / TRK-351，2026-07 補）**：D1 的三支 LogsQL 查詢（撤銷事件、fail-open、reload-rejected）原本只比對**內容**（欄位值或訊息片語）、不限定來源——任何能把一行日誌寫進同一個 VictoriaLogs 的來源都能灌入假訊號（製造假警 → 告警疲勞 → 真警被消音，即偵測失能）。修法在查詢端加上**平台寫入、payload 無法偽造的分類欄位**：事件查詢加 `log_type:"federation_evidence"`（與 heartbeat canary **同一串流**，兩者由測試釘死一致——分歧會製造「事件查詢瞎掉回零、canary 仍綠」的靜默全綠）；fail-open / reload-rejected 兩支加 `log_type:"gateway_operational"` + `app:"envoy"`（`app` 把類別收窄到 Envoy 容器，排除同 pod 的 mtail / logrotate sidecar 的非 JSON stderr；容器名以跨檔測試釘回 chart）。
  - **實測佐證（fail-open 那半確實有效）**：租戶請求內容經 `audit_extract.lua` 進 access log 的 `query` 欄位，但那行是 JSON、會 parse 成 `federation_audit`（有 top-level `query`），永遠到不了 `gateway_operational`；對真 Envoy（`distroless-v1.38.0`）探測確認**任何注入位元組**（含控制字元、非法 UTF-8）都被編碼成單行合法 JSON，不會拆行或落錯類。
  - **誠實邊界（這一步擋什麼、不擋什麼）**：`log_type` 綁的是「Vector 把它歸為哪一類」，**不是「哪個 producer 產的」**——evidence 分支的入場券**在 chart 0.10.0（[#1288](https://github.com/vencil/Dynamic-Alerting-Integrations/issues/1288)）之前只有 pod label**，故能在受採集範圍建立帶對應 label 之 pod 的主體即可偽造事件（含偽造 heartbeat 讓 canary 說謊）。**0.10.0 起入場券為「pod label ＋ 設定的 namespace」**（`evidenceChannel.podNamespace` / `.gatewayNamespace`，設 `""` 回到僅 label），門檻因而升到「能在**該 namespace** 建 pod 者」——**但這仍不是 producer 身分驗證**：該 namespace 內 label 依舊是自宣告的。查詢端欄位把偽造門檻從「任何能寫入日誌庫者」**升到**「能在受採集範圍建 pod 者」，但**未跨過來源身分驗證這條線**。**`gateway_operational` 那半（fail-open／rejected）共享同一邊界，不因加了 `app` 而不同**：`app` 值等於 k8s container_name，攻擊者在自己的 pod spec 裡可任意設為 `envoy`，故 `app:"envoy"` 只排除同 pod **合法** sidecar（mtail／logrotate）的噪音、不提供來源身分驗證；且 `origin_split.gateway` 的路由條件同樣只看 pod label，而 demux 的 pod_owner spoof 檢查**只套用在 `federation_audit` 分支、不涵蓋 `gateway_operational`**（見 `helm/vector/templates/configmap.yaml` demux）。真正封死需在 evidence 與 gateway **兩條** origin 分支都補 producer 綁定（demux 現有的 pod_owner 檢查僅及 `federation_audit`）＋收緊 `evidenceChannel.events`/`keepFields` 的成員檢查——**兩項已於 [#1288](https://github.com/vencil/Dynamic-Alerting-Integrations/issues/1288) 落地（chart 0.10.0）**，但綁的是 **namespace 不是 pod_owner**：pod_owner 只是工作負載的**名字**，攻擊者把自己的 Deployment 取名 `federation-gateway` 即可通過前綴比對（實測），所以它與 pod label 屬同一個信任等級；namespace 由控制面指派、pod 無法自選，是這組 metadata 裡唯一不可自宣告的。⚠️ **這是門檻提升，不是來源身分驗證**：綁定後的門檻是「能在該 namespace 建 pod」，而 gateway 那一半的 namespace 是**共用的**（prometheus／grafana／alertmanager／KSM 都在），且能在其中建 pod 者可直接寫入 VictoriaLogs :9428（無認證、以自貼 label 准入，見 [#1295](https://github.com/vencil/Dynamic-Alerting-Integrations/issues/1295)）——對該主體此綁定不增加成本，它關掉的是「叢集他處帶著該 label 的工作負載」這個較廣的情境。真正的來源身分驗證仍需 producer 端簽章（本 ADR 保留的 `audit_signature` seam）或 admission control。業界共識（MITRE CTID《Summiting the Pyramid》、Sigma `logsource` 官方定位）一致：查詢端來源限定是「選對資料」，信任邊界須在攝取端建立。⚠️ **本段描述的邊界在 [#1294](https://github.com/vencil/Dynamic-Alerting-Integrations/issues/1294) 之前其實比寫的更弱**：`app` / `k8s_namespace` / `pod_name` 與 demux 的 `pod_owner` spoof 檢查全都在「把 producer 自己的 JSON deep-merge 進事件」**之後**才讀，因此攻擊者連「把自己的 container 取名 `envoy`」都不必——payload 裡塞一個 `kubernetes` 物件即可覆蓋平台記錄的來源，spoof 檢查也一併失效。#1294 把這些欄位改為只從 **merge 前的快照**讀取，本段敘述才與實作相符；同理 evidence 列的 `timestamp`（sink 的 `_time_field`、reconciler `_time:` 視窗依據）先前亦可被 payload 決定，屬 ADR 自身威脅模型（tenant-api RCE）可觸及的範圍。
- **⛔ 取樣式偵測的固有盲區（#1238 / TRK-352，2026-08 補）——本 ADR 最重要的一條誠實邊界**：D1 的偵測端是一支**週期輪詢**的 reconciler（300s 一輪），而它發出的 gauge **每輪從頭重算**。這產生兩個**性質不同**的缺口，先前被混為一談：
  - **(a) 已取樣到的偵測會被丟掉（已修）**：`tamper_suspected` 的非零狀態只維持一輪（300s），而 `FederationRevocationTamperSuspected` 的 `for:` **恰好也是 5m**，於是單輪偵測**結構上永遠撐不到門檻**（promtool 實測：任何 eval 時點都不觸發）。同型病在另外兩條上更隱蔽：`federation_gateway_revoked_set_reload_rejected` 是 ~600s 窗的列數、每 300s 重寫，單次拒載最多非零 600s ＝ `for: 10m`，**同樣永不觸發**；`federation_gateway_revoked_set_missing` 則視輪次對齊而定（600s 觸發／300s 不觸發），**同一個事件會不會 page 取決於排程巧合**。三條均改為 `max_over_time(...[1h]) > 0` **鎖存**，把「是否曾經偵測到」與「此刻是否仍成立」拆開——對 tamper-evidence 而言只有前者有意義。
  - **(b) 取樣本身可能從未發生（未修，且無法用告警式修法修）**：un-revoke 的最小可行窗遠短於一輪。gateway 的 reload 掛在**請求路徑**上（Envoy Lua filter 無 timer API），攻擊者自己的請求即可觸發 reload 並在同一個請求被放行，所以攻擊時長 ≈ kubelet 投影延遲 ＋ ≤`reloadIntervalSeconds`。**攻擊窗短於取樣週期時，偵測器根本沒看到任何東西，任何 expr 都救不了。** 鎖存把「必然逃脫」改成「每次使用擲一次骰子」（重複使用會累積），這是它買到的全部——**不是關洞**。
  - **為何 reconciler 這個形狀天生如此**：Kubernetes 官方設計文件對 level-based reconciler 的定位是「**regardless of how many intermediate state updates may have been missed**」，並明文把 edge-triggered 降為最佳化——也就是說「漏掉中間狀態」是這個模式**被接受的性質**，因為它的目標是**收斂**而非**偵測**。真正的結構解是讓偵測事件驅動（watch／API-server audit stream），追蹤於 [#1322](https://github.com/vencil/Dynamic-Alerting-Integrations/issues/1322)（TRK-357）；⛔ 該方案在本 chart 上**不是**單純加一個 watch：reconciler 今天刻意**沒有 SA token、沒有任何 RBAC、egress 只開 DNS + VictoriaLogs**（`serviceaccount.yaml` 自述「deliberately less privileged than a compromised tenant-api」），而 RBAC 的 `resourceNames` **限制不了 `list`/`watch`**，故加 watch 等於同時放寬三道並給出全 namespace 的 ConfigMap 讀取權。此權衡須與偵測收益一起評估，不可當成純增益。
  - **⛔ 鎖存移除了一個原本免費的偽陽性吸收器，故同時在來源端補上 `projectionGraceSeconds`**：舊的 `for: 5m` 順帶擋掉了「合法撤銷剛寫入、但 reconciler 自己那份 kubelet 投影還沒更新 → 該 token 讀起來不在 live set → 誤判 tamper」這種單輪假陽性。鎖存之後它會直接 page，所以改在**事件年齡**這個維度處理：事件必須老於 `max(settleSeconds, projectionGraceSeconds)` 才進對帳。⛔ **不可改用「連續兩輪缺席才算」**——攻擊窗短於一輪，結構上不可能被取樣兩次，那種規則會把 (a) 原封不動地裝回去。⛔ 也**不可**改成調大共用的 `settleSeconds`：它同時是 gateway 三支查詢的窗近端，調大會**縮小**那個 600s 窗，讓 reload-rejected 更難觸發。
  - **reset time 是刻意付出的代價**：鎖存窗 W = 1h 同時是告警的最短復歸時間（Google SRE 把 reset time 列為告警四屬性之一）。條件修好之後告警仍會續燒至多 1h，**這是設計而非 bug**——對一條 critical 安全訊號，寧可讓人確認過再消失。
