...
✅ **Accepted**（v2.7.0, 2026-04-16）— 實作見 commit `ec07914`
...
v2.7.0 必須把它吃進 token 系統，但三種遷移路徑互有取捨：
...
- **Runtime axe-core 後續發現的 `--da-color-tag-bg` + `--da-color-muted` 對比不足
  不是 Option A 造成的 → 是 token 定義層問題，但 Option A
...
- v2.7.0 起，所有 JSX token 遷移一律採 Option A 寫法（不允許 B）
...
- 後續 runtime contrast audit 發現的 token-pair 對比問題（非本 ADR 直接造成，已另案追蹤）
