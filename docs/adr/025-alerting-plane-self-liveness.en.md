...
- **runtime canary tenant / end-to-end synthetic probe (interop) / backend-compatibility staleness-temporal semantics** — still deferred-with-trigger (triggers in the "Deferred" table below).
...
| **End-to-end synthetic probe (interop, not self-built)** | A mature customer usually already runs blackbox / synthetic probing; we **establish a contract** rather than rebuild — reserve a **dedicated sinkhole route**: an alert carrying `component="synthetic-probe"` is guaranteed routed to `synthetic-receiver` with `continue:false` (the customer's own probe verifies travel THROUGH our layer, and a test alert is zero-risk — it can't wake on-call) | After heartbeat + canary ship, when a "rule evaluation silently failed" incident occurs; a self-built probe defers (interop usually suffices) |
