...
- **runtime canary tenant** — **design-ready**: the full design, the config-locus decision (`conf.d/` GitOps + a reserved tenant), the honest two-layer bad-tenant-isolation account, plus a promtool demo produced through the real compiler and run in CI (`tests/rulepacks/runtime-canary{.rules,_test}.yaml`), all in [Runtime Canary Design](../design/runtime-canary.en.md). The pipeline is wired in production; **what still defers is the resident deployment** (a heartbeat-source emitter + wiring the meta-alert to real on-call).
...
|---|---|---|
| **Canary tenant (runtime)** — **design-ready**, see [Runtime Canary Design](../design/runtime-canary.en.md) | A permanent fake tenant + an always-firing alert (`CustomAlertPipelineCanaryDown` dead-man's-switch), end-to-end proving the "rule compilation + routing" chain isn't broken and catching the exporter / compile-pipeline death the Watchdog can't see. **Bad-tenant isolation**, stated honestly (correcting the original wording): this platform achieves it via **two layers**, not "the broken config still compiles" — (1) compile/CI is **fail-closed + names the offending file** (a bad config is blocked as a batch and never deployed); (2) at runtime, **per-tenant row independence** (`max by(tenant)` + `on(tenant) group_left` — another tenant's missing series cannot blank my row). The canary is the end-to-end **proof that layer (2) is alive** | **Design + a CI promtool demo are done** (`tests/rulepacks/runtime-canary*`); the **resident deployment** defers: deploy as a safety net before a major rule-compiler refactor / multi-tenant routing change, or after the first production custom-alert "evaluation silently failed" incident to prevent recurrence |
