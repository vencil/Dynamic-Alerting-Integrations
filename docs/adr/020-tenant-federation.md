...
| 4. Storage backend tuning | platform Prometheus deployment manifest 加 `--query.max-samples` / `--query.timeout`；BYO-VM 的 `-search.maxUniqueTimeseries` 等於本表文件化（VM 非平台部署元件）| 2h |
