...
**Admission validator（soft gate + force override，**adversarial review surfaced**）**：whitelist 加入新 metric 時，IV-2 admission validator 對「過去 24h 該 metric 在後端 storage 至少有一筆帶 `tenant_id` label 的 sample」做檢查。**輸出分三種**：

| 觀察結果 | Validator 行為 | 為何 |
|---|---|---|
| 有 sample 且帶 `tenant_id` label | ✅ Pass | 預期情境 |
