...
**GitLab 模式**（`--write-mode pr-gitlab`，v2.6.0 新增）：
...
| **Platform Interface** | `internal/platform/platform.go`（v2.6.0 新增） | 平台無關的 `Client` + `Tracker` interface |
...
| **GitLab Client** | `internal/gitlab/client.go`（v2.6.0 新增） | 封裝 GitLab REST API v4，實作 `platform.Client` |
| **GitLab Tracker** | `internal/gitlab/tracker.go`（v2.6.0 新增） | In-memory pending MR 快取 + 定期 sync，實作 `platform.Tracker` |
...
| 方案 | 評估 | 棄用原因 |
|------|------|---------|
| **GitLab MR**（而非 GitHub PR） | ✅ **已實作** | v2.6.0 實作 `platform.Client` 抽象層 + `internal/gitlab/` 套件。`--write-mode pr-gitlab` 啟用 |
