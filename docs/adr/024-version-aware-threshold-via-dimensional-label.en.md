...
**Write-path asymmetry (stated honestly)**: eol-expansion is a stateful check (it needs the old+new delta). Only the **`/custom-alerts` write path** loads the current config + the PUT, so only it can compute the delta and has teeth. The **shared stateless shape preflight** (`PutTenant` / batch full-config writes via `gitops.Writer.validate`) cannot see the delta, so those paths validate shape only; the **CI / GitOps-direct compiler** sees the whole conf.d tree and cannot tell new from existing → it can only warn. Adding stateful eol-expansion at the writer layer (covering PutTenant/batch) is a follow-up; the portal recipe-authoring flow uses `/custom-alerts`, so the primary tenant-facing path is covered.
