...
emission 的 `<RootPrefix>/<ProposalDir>/` 對應 ADR-017 的 directory level。**Caller**（batch PR pipeline 是主要使用者）決定 `ProposalDirs[i]` 落在 L1 / L2 / L3 哪一層。**Profile Builder 不做目錄推斷**；那是 batch PR pipeline 的工作。

### 與 Dangling Defaults Guard

Emission 直接吃 ADR-018 deepMerge 形狀後，Dangling Defaults Guard 自然套用：
