...
接受的依據是 §證據 4 的跨機 A/A 實測。判準在看到任何資料前寫死（放大倍率 ≥ 3× 且跨機比較超過 5% floor 的比例 ≥ 5%），實測 **4.0×** 與 **12.5%**，兩條都過。

⚠️ **餘裕比第一版報告的薄得多，原因是第一版報告的對照組挑錯了。** 初判用「全體跨機配對」得到 37×，但那描述的是 **#1396 之前**的設計；現行夜跑已經同類分層，而且參考點是同類多夜的**中位數**而非單一夜。兩項都修正後是 4.0×（外部 review 又指出 PAIRED 與 CROSS 的樣本母體不同，取同母體後由 3.8× 修正為 4.0×）。門檻是 3×，**只多 1.0**，且該估計只有 n=48。詳見 §證據 4。
...
|---|---|---|---|---|
| **PAIRED 同機、同 12 台（本提案）** | **0.34%** | 1.57% | **2.1%** | 48 |
| **CROSS 同類，對同類多夜中位數（現行設計）** | **1.38%** | **5.26%** | **12.5%** | 48 |
| PAIRED 同機、全部 16 台（母體不同，勿與上一列相比） | 0.37% | 1.40% | 1.6% | 64 |
| CROSS 同類，單夜對單夜 | 2.32% | 6.81% | 21.1% | 280 |
| CROSS 跨類，單夜對單夜（#1396 之前） | 23.23% | 80.87% | 89.7% | 680 |

**對現行設計的放大倍率 = 4.0×。** 前兩列用同一種函數形式（ratio-of-medians）、每側同樣多的量測數、**同一組 12 台機器**，唯一差別是分母來自哪台機器。

⚠️ **兩個必須同時避開的陷阱：**

1. **對照組要選第二列，不是第四或第五列。** 現行夜跑經 #1396 之後只比較同類主機，且參考點是同類多夜的中位數（中位數會壓低參考端噪音）。用單夜對單夜會得到 6.8×，用全體跨機會得到 37×——那兩個數字描述的是**已經不存在的設計**。
2. **PAIRED 要取同母體那一列。** `CROSS` 只能對「有 ≥2 台同型號同伴」的 shard 成立，16 台裡只有 12 台符合。拿 16 台的 PAIRED（0.37%）去比 12 台的 CROSS，變動的就不只是分母的機器身分，還有樣本母體，得到的 3.8× 不是乾淨的比較。**這一點由外部 review 指出**；修正後倍率反而從 3.8× 上升到 4.0×——更正沒有救援結論，而是讓它更銳利。

#### 覆蓋率：被排除的 4 台本身就是證據

`CROSS` 排除的那 4 台，正是現行設計**連評估都做不到**的機器——無同類同伴，真實夜跑會回 `INCONCLUSIVE`：
...
這與 #1396 修法付出的「約 1/3 的夜不可評估」代價直接對應，是放大倍率之外的第二項可量測優勢。
...
2. ~~殘餘噪音實測值，以及交錯到底有沒有效~~ —— **已結（§證據 4）**：真 runner 上配對殘餘 0.34%（同母體、6 輪）/ 0.66%（1 輪），對現行設計放大 4.0×。
...
- **§證據 4 的決定性數字 4.0× 只有 n=48**，因為「同類多夜中位數」需要至少 3 台同型號機器，只有 EPYC 7763（8 台）與 EPYC 9V74（4 台）兩群符合。門檻是 3×，餘裕僅 1.0。
...
- [`audit-reports/bench-xmachine-2026-08/`](../internal/audit-reports/bench-xmachine-2026-08/README.md) —— **§證據 4 的完整資料集、重現腳本與 negative validation**（`reproduce.py` 一行重現本 ADR 引用的每個數字）
