...
| [020](#020-planning-ssot-frontmatter-contract-discovery-based-index) | Planning SSOT — Frontmatter Contract + Discovery-based Index | ✅ Accepted | 跨檔 8 處散落 planning（CHANGELOG / dx-tooling-backlog / frontend-quality-backlog / S# planning / roadmap / ADR / code TECH-DEBT / flaky-tests）統一治理：frontmatter contract + discovery-based index generator + active CI sync check。TD/HA/REG → TRK 統一 namespace；ADR-NNN / S#NNN 保留語意純度 |
| [021](#021-tenant-federation-label-injection-proxy-over-self-built-endpoint) | Tenant Federation — Label-Injection Proxy over Self-Built Endpoint | 🟡 Proposed | Tenant 拉自己 metrics 回 tenant 側自管 federation。採 vmauth（VM 客戶）/ prom-label-proxy（Prom 客戶）做 label-enforced read proxy，不自寫 endpoint。2-tier policy（platform whitelist + tenant subset）+ 4h TTL token（無 server-side revocation）+ blast radius 三件組（concurrency / timeout / series cap）|
...
## 020: Planning SSOT — Frontmatter Contract + Discovery-based Index

**文件**: [`020-planning-ssot.md`](./020-planning-ssot.md)

v2.8.0 ship via PR #376。跨檔分散的計畫追蹤（tech-debt / dx-backlog / known-regression / future-roadmap / sprint planning / ADR / code TECH-DEBT / flaky-tests 共 8 處 source）統一治理。三層設計：(1) frontmatter contract（每個 planning item 必填 `id` / `tracking_kind` / `status` / `domain` 等欄位）、(2) discovery-based index generator（glob + `tracking_kind:` enum 過濾，避免硬編 source 清單）、(3) active CI sync check（PR body `Resolves TRK-NN` 自動驗 status:done + pr_ref 同步）。Namespace 策略：TD/HA/REG → TRK 統一；ADR-NNN / S#NNN 保留語意純度（design history vs sprint planning vs ongoing tracking）。實作 follow-through 在 issue [#379](https://github.com/vencil/Dynamic-Alerting-Integrations/issues/379)。
