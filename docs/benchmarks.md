...
- **dual-hash 效果**：defaults 變動時若 merged_hash 未改（例如 comment-only），跳過整個 reload（`da_config_defaults_change_noop_total` counter 累計）
- **B-4 telemetry（v2.7.0 已實裝）**：以下 metrics 可在 exporter 端即時觀測 reload 行為：
