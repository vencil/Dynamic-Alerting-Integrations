...
`)}function H(e){let o=["conf.d"];return e.deploy==="kustomize"&&o.push("kustomize"),e.deploy==="helm"&&o.push("environments"),o.push("rule-packs"),o.sort()}function G(e){if(e.deploy!=="kustomize"&&e.deploy!=="helm")throw new Error(`cicdGenerateGitHubActionsPreview: unknown deploy method ${JSON.stringify(e.deploy)} \u2014 expected 'kustomize' or 'helm'. A new deploy method needs an apply block here AND in scripts/tools/ops/init_project.py.`);let o=k(e),r=M(o)?`# ${o} can be repointed at different code without this file changing - :latest moves by design, any other tag at its registry's discretion. Pin a digest if this pipeline has to be reproducible.
