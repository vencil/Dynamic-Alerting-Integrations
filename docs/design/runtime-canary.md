...
- 一個**常駐的 heartbeat 來源**（發 `canary_pipeline_heartbeat=1` 的小 emitter——一段 pushgateway cron 或 exporter self-metric）。這是 canary 唯一的小型常駐元件。
- **把 meta-alert 接上真實 on-call**（`CustomAlertPipelineCanaryDown` → 真的 pager，而非 demo sink）。
