...
ADR-025「之後再說」表原本把 canary 的信任資產寫成：「故意注入一個損壞的租戶設定，canary 仍**成功編譯**、繞過單點錯誤、正確送出」。這句話寫下當時對照程式碼是**錯**的（編譯彼時整批 fail-closed，一個壞租戶讓所有人一起紅）；[#1008](https://github.com/vencil/Dynamic-Alerting-Integrations/issues/1008) 把編譯器改為 per-recipe fail-soft 後，它才真的在**編譯層**成立。但單句敘述仍掩蓋了結構——本平台的「壞租戶不拖垮好租戶」其實是**兩個不同的層**達成的，不是「壞的還能編譯」（壞 recipe 從不編譯，是被隔離）：

**第一層 — 編譯期 per-recipe fail-soft 隔離（quarantine）+ 租戶可定位。** 編譯**不再是**整批 fail-closed（舊行為：一個 schema 損壞的租戶設定讓**整個** `compile_custom_alerts.py` 回 exit 2——等於任一租戶的壞設定都能弄紅共用 CI gate、擋下**所有**租戶的 PR merge，一種跨租戶 availability DoS）。自 [#1008](https://github.com/vencil/Dynamic-Alerting-Integrations/issues/1008)（[PR #1016](https://github.com/vencil/Dynamic-Alerting-Integrations/pull/1016)）起改為逐 recipe 驗證：無效的 recipe（缺必要欄位、非法 shape 參數如非法 window 或非法的 threshold severity 標記、name / severity 撞名、超出 own-recipe cap，乃至整個載不進來的 conf.d 檔案）被**大聲隔離**——stderr 逐筆警告 + 總結行，**指名 tenant、宣告所在檔（`origin`）與原因**（[`loader.py`](https://github.com/vencil/Dynamic-Alerting-Integrations/blob/main/scripts/tools/dx/custom_alerts/loader.py) `build_shapes` 的 per-recipe 驗證迴圈 + [`compile_custom_alerts.py`](https://github.com/vencil/Dynamic-Alerting-Integrations/blob/main/scripts/tools/dx/compile_custom_alerts.py) 的 quarantine 報告）——**其餘 recipe 照編**；exit 2 只保留給 config 層級錯誤（config dir 不存在、負值 cap、emit-time template-safety invariant）。關鍵不變式沒變：**quarantined recipe 不進 pack、永不部署**。所以隔離靠的是**大聲隔離 + 可定位診斷**：壞的被指名丟出、永不部署，好的照編照部署。**canary 在此沒有特殊魔法**——鄰居租戶的壞設定不會再拖垮它的編譯，而 quarantine 也絕不靜默（CI log 逐筆留痕）。（租戶本來就**不寫 PromQL**，編譯器代寫，所以租戶無法注入會在執行期炸掉的表達式。）由此可知：一個**畸形的 recipe**（非法 window、非法的 threshold severity 標記）在這一層即被隔離、**永不進入執行期**——所以第二層的執行期隔離測試（demo case 2）餵的是壞**資料**（鄰居帶極端值），而非壞 recipe；「執行期還在跑的壞 recipe 鄰居」在這個架構裡**不存在**。
