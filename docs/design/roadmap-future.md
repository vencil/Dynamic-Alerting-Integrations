...
- **維運自動化** — Rule Pack × threshold-calculator 資料流：[#457](https://github.com/vencil/Dynamic-Alerting-Integrations/issues/457) R0 已拍板 → **採納 STAGE-1** `threshold_recommend --export-patch`（吐 conf.d patch）+ **延後**全自動 PR adapter（trigger：客戶 toil / RFP / maintainer poll）+ **不做**寫回 rule pack schema；論證見 [#457](https://github.com/vencil/Dynamic-Alerting-Integrations/issues/457) / Local try-it-yourself onboarding（exporter / tenant-api / portal / da-tools standalone）
