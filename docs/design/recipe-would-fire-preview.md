...
> 本機已實測：值 1500 > 閾值 1000 → 失敗、輸出帶完整告警；值 500 → 成功。整套用既有工具即可，不需要另起一個 Prometheus 實例。
...
5. **`promtool` 版本鎖**——上面的 returncode／輸出格式是跟版本綁的契約（marker 由 `ci.yml` 的 `Preview would-fire e2e` 一步對當下 pin 的版本每次重驗）；服務 image 要鎖版本、啟動時記錄版本。
