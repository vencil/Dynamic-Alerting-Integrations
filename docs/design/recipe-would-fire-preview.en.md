...
> Verified locally: value 1500 > threshold 1000 → failure, output carries the full alert; value 500 → success. The whole thing uses existing tools — no separate Prometheus instance needed.
...
5. **A pinned `promtool` version** — the returncode / output format above is a version-bound contract (ci.yml's `Preview would-fire e2e` step re-verifies the markers against whatever version is pinned); the service image pins the version and logs it at startup.
