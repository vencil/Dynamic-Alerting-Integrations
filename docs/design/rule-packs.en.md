...
> Counts and per-pack rule numbers are sourced from [`docs/assets/platform-data.json`](../assets/platform-data.json) (generated by `generate_platform_data.py`); the table below is a snapshot. `custom-alerts` (the tenant-compiled pack) is deliberately excluded from the standard count.
