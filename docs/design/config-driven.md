...
tenants:
  db-a:
    ...
    # mysql_threads_running 未指定 → 使用預設值 30
    # 維度標籤
...
#### 階層式 conf.d/ + 繼承引擎 (v2.7.0，ADR-016 / ADR-017)
...
- [ADR-016：`conf.d/` 階層式目錄 + 混合模式遷移](../adr/016-conf-d-directory-hierarchy-mixed-mode.md)
- [ADR-017：`_defaults.yaml` 繼承 + Dual-Hash 熱重載](../adr/017-defaults-yaml-inheritance-dual-hash.md)
...
**自動失效 **：`_silent_mode` 和 `_state_maintenance` 支援結構化物件（向後相容純量字串），帶 `expires` RFC3339 時戳（如 `"2026-04-01T00:00:00Z"`；exporter 解析不了時只印 WARN、當成沒有期限，不會自動失效）。Go 引擎 `time.Now().After(expires)` 過期即停止 emit sentinel metric，alert 自動恢復正常。失效時產出瞬時 gauge `da_config_event{event="silence_expired"}` 搭配 `TenantConfigEvent` alert rule 通知。
...
`--validate` 檢查 YAML 合法性 + webhook domain allowlist（exit 0/1，供 CI 消費）；搭配 `--strict` 時 domain policy（ADR-007）違規由 WARN 升級為 ERROR 並 fail（CI 跑 `--validate --strict`）。`--apply` 直接合併 fragment 至 Alertmanager ConfigMap 並觸發 reload。產出支援 webhook、email、slack、teams、rocketchat、pagerduty 六種 receiver type。
