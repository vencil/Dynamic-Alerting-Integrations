...
- **租戶自訂告警**：7 種參數化 recipe（threshold / rate / ratio / absence / p99_latency / forecast **已隨 v2.9.0 落地**，epic #741，[ADR-024 §Custom Alerts](adr/024-version-aware-threshold-via-dimensional-label.md)；slo_burn_rate 依 [ADR-031](adr/031-slo-burn-rate-recipe.md) 新增）+ 生命週期（`status: active / deprecated / eol`）。**仍為 future**：Level 2 bounded-DSL / Level 3 raw-PromQL 逃生門、全域 rule-count budget（規劃中）。
...
`absence` 的底層是 `count_over_time(<metric><selectors>[<window>])`——每次評估會掃 `<metric>` 在窗內**所有符合 selectors 的 series**。**高基數指標不給 `selectors`** = 掃全部 series → 記憶體／CPU 峰值（在 VictoriaMetrics 等後端尤其明顯）。務必用**拓撲維度 selector**（`instance`／`pod` 等）限到具體實例。⚠️ **只給事件維度 selector 不夠**：對一個 N-pod 資源池，`{errno="1236"}` 仍會配出 N 條 series（每 pod 一條）→ 一樣昂貴；真正限縮掃描量的是**拓撲** selector。label-form 的價值正在於**鼓勵用 selector 精確定位目標實例**、從源頭壓低 TSDB 掃描量。
