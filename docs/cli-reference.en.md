...
"Declares" means **what the exporter reads**: the first YAML document only, keys by their raw text (`on:` is tenant `on`, `007:` is `007`), and only a file that passes the exporter's decode as a whole — a scalar tenant body, a key defined twice, another field of the wrong type or a YAML syntax error makes the exporter **reject the whole file**, which then declares nothing. If such a file names a tenant this run was asked for (or init cannot tell whether the exporter accepts it), init **refuses with rc 1 and writes nothing**, naming the file and the reason — fix that file first; one that names no requested tenant is only named in a WARN line on stderr. init also refuses when the `conf.d/<t>.yaml` it would overwrite also declares another tenant this run was asked for (after the overwrite that tenant would be declared nowhere), and when any path it would write has an existing entry beside it that differs **only in case** (e.g. `DB-C.YAML` next to `db-c.yaml` — one file on a case-insensitive filesystem).
