...
In `conf.d/`, init owns only two kinds of path: `_defaults.yaml` and `<tenant>.yaml` at the **root**. A tenant already declared by **another file** of yours (judged by the keys of that file's `tenants:` mapping, not by its name — `db-c.yml`, `DB-C.YAML`, a multi-tenant `team.yaml` and files in subdirectories all count) is **skipped, and that file is left untouched**; stderr and the summary name it (e.g. "db-c is already declared by conf.d/db-c.yml — conf.d/db-c.yaml was not generated") and the exit code stays 0. Likewise `_defaults.yaml` is not written when the root already has a defaults carrier in another spelling (e.g. `_defaults.yml`). If init's own path **already** sits beside such a carrier (e.g. `db-c.yaml` and `db-c.yml` both declare db-c, or `_defaults.yaml` next to `_defaults.yml`), init **refuses with rc 1 and writes nothing** (`.da-init.yaml` included), under `--dry-run` too — the exporter rejects a tree that declares a tenant twice, and which file to keep is your call.

How "declares" is decided: init judges for itself whether the exporter accepts a file, and whom it declares, only inside a **strict whitelist** — the whole file is UTF-8 without a BOM, contains no tab, no `%YAML` directive, no explicit tag (`!!str`, `!foo`, …), no anchor/alias/`<<` merge, every key reads as a string in every YAML version (so `on:` and `007:` do not qualify), no timestamp, and every integer fits in int64. Only inside it does init mirror the exporter's decode field by field (first YAML document only, keys by their raw text; a scalar tenant body, a key defined twice or another field of the wrong type makes the exporter **reject the whole file**). Everything outside it is "init cannot tell", deliberately: the differences between two YAML implementations have no bound, so init refuses rather than guesses. A file the exporter rejects or init cannot judge that mentions a tenant this run was asked for in any form (block, flow, JSON, inside quotes, UTF-16 and other encodings) makes init **refuse with rc 1 and write nothing**, naming the file and the reason — rewrite that file first; one that mentions no requested tenant is only named in a WARN line on stderr. init also refuses when the `conf.d/<t>.yaml` it would overwrite also declares another tenant this run was asked for (after the overwrite that tenant would be declared nowhere), and when any path it would write has, at **any level** below the output directory, an existing entry that differs only in case (e.g. `Conf.D/` for `conf.d/`, `DB-C.YAML` for `db-c.yaml` — one path on a case-insensitive filesystem).
