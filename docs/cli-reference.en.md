...
| The subcommand raises an uncaught exception (traceback) | `1` | The Python interpreter's default; the dispatcher deliberately has no `try/except` (that would break pass-through). This is the residual collision with the "findings" `1` — when you read `1`, check stderr for `Traceback` |
