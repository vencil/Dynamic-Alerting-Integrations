...
# 印出: Loaded image: ghcr.io/vencil/da-tools:v2.8.0
...
之後客戶內部 CI / pre-commit 直接用 `internal-registry.corp/da-tools:v2.8.0` 即可。`da-guard` binary 已 bundle 在 image 內 `/usr/local/bin/da-guard`，無需另外傳輸。
