...
> **適用版本**：`tools/v2.9.0` 起（Migration Toolkit packaging 落地後的所有 release）。  
...
**Trivy CVE scan** 在 release 時自動跑（`CRITICAL` / `HIGH` 級別 fail-fast）。Image SBOM + **cosign keyless 簽章**自 `tools/v2.9.0` 起隨每個 artefact 發佈（驗證方式見下方 [§Signature Verification](#signature-verification)）。
...
之後客戶內部 CI / pre-commit 直接用 `internal-registry.corp/da-tools:v2.9.0` 即可。`da-guard` binary 已 bundle 在 image 內 `/usr/local/bin/da-guard`，無需另外傳輸。
...
`tools/v2.9.0` 起，每個 Release artefact 都附帶 **cosign keyless 簽章**
...
跨主版本（例 `tools/v2.x → tools/v3.x`）升級時，請先讀對應 release notes 的 Breaking changes 段落。`tools/v2.9.0` 起 [Release notes 模板](https://github.com/vencil/Dynamic-Alerting-Integrations/releases/tag/tools/v2.9.0) 自動 generate（含 commit log 連結），人為 review 後才正式 publish。
