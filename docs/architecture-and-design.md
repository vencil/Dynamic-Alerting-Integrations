...
本文件針對 Platform Engineers 和 Site Reliability Engineers (SREs) 深入探討「多租戶動態警報平台」(Multi-Tenant Dynamic Alerting Platform) v1.8.0 的技術架構。
...
測試體系分為兩層：**E2E Scenario Tests**（K8s 叢集內端到端驗證）和 **Unit/Integration Tests**（pytest + go test，569+ 測試案例）。
...
v1.7.0–v1.8.0 新增大量企業功能，其測試覆蓋集中在 unit/integration 層：
...
> 完整測試套件：`make test`（Go）+ `pytest tests/`（Python, 569+ passed）。CI pipeline `.github/workflows/validate.yaml` 在每次 PR 自動執行。
...
v1.8.0 已完成企業防護網 (v1.7.0) 和生態系擴張 (v1.8.0)。以下為按優先序排列的剩餘技術方向：

```
P1 高價值/低摩擦          P2 中期演進              P3 長期藍圖
(下一版即可做)            (需客戶驗證)             (規模化需求驅動)
┌───────────────┐     ┌───────────────┐     ┌───────────────┐
│ 遷移自動化提升 │     │ Rule Pack 擴展 │     │ CRD + Operator│
│ Config Diff   │     │ Federation B  │     │ Log-to-Metric │
│ PR 回測 Bot   │     │ AM GitOps 閉環│     │               │
│               │     │ 1:N Mapping   │     │               │
└───────────────┘     └───────────────┘     └───────────────┘
```
...
#### 11.1 遷移自動化提升 (Migration Automation)

降低企業從傳統監控遷移至本平台的人工成本。

| 項目 | 說明 | 當前替代 |
|------|------|---------|
| **Onboard-to-scaffold pipeline** | `onboard` 產出直接餵入 `scaffold`，一鍵完成遷移準備 | 手動複製 `extracted-tenants.yaml` |
| **Auto-convergence detection** | `validate --watch` 自動偵測所有 metric pair 收斂，產出 cutover readiness report | 人工判讀 validation 日誌 |
| **Post-cutover health report** | 切換後自動對所有租戶執行 `diagnose`，彙整為單一報告 | 逐租戶手動 `diagnose` |
| **Rule Pack gap analysis** | 分析 `custom_` 規則，建議可替代的官方 Rule Pack | 人工比對 metric-dictionary |

#### 11.2 Config Diff Preview

`patch_config.py --diff` 模式，顯示 patch 前後的 metric 變化（類似 `terraform plan`）：新增/移除哪些 metric series、影響哪些 alert rule。降低配置變更的心理門檻。

#### 11.3 PR 回測 Bot (CI Threshold Backtesting)
...
#### 11.5 Federation 場景 B：Rule Pack 分層
...
#### 11.6 Alertmanager 配置 GitOps 閉環
...
#### 11.9 Log-to-Metric Bridge（日誌轉指標橋接）
...
以下項目原列於 future roadmap，已在 v1.7.0/v1.8.0 完成：

| 項目 | 完成版本 | 說明 |
|------|---------|------|
| Kafka / RabbitMQ Rule Pack | v1.8.0 | 標準三件式，各 5 alert + _critical 變體 |
| N:1 Tenant Mapping 自動化 | v1.8.0 | `scaffold --namespaces` + relabel snippet |
| Per-rule Routing Overrides | v1.8.0 | `_routing.overrides[]` + 子路由產出 |
| Platform Enforced Routing | v1.7.0 | `_routing_enforced` 雙軌通知 |
| Silent/Maintenance Expires | v1.7.0 | 結構化物件 + `expires` 自動失效 |
| 一站式配置驗證 | v1.7.0 | `validate_config.py` 封裝 5 類檢查 |
| PostgreSQL Rule Pack | v1.7.0 | 標準三件式，4 指標 |
| Federation 場景 A 文件 | v1.7.0 | `docs/federation-integration.md` |
| GitOps RBAC | v1.5.0 | CODEOWNERS + CI pipeline + configmap-assemble |
| Platform Dashboard | v1.5.0 | Grafana JSON — 11 面板 |
...
**文件版本：** v1.8.0 — 2026-03-07
**最後更新：** v1.8.0 — Per-rule Routing Overrides、N:1 Tenant Mapping 自動化、Kafka/RabbitMQ Rule Pack（10→13 ConfigMap）、企業級測試覆蓋矩陣重新詮釋、Future Roadmap 重新排序
