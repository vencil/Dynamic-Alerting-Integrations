...
本文件針對 Platform Engineers 和 Site Reliability Engineers (SREs) 深入探討「多租戶動態警報平台」(Multi-Tenant Dynamic Alerting Platform) v1.9.0 的技術架構。
...
#### 11.3 Alertmanager 配置 GitOps 閉環
...
#### 11.5 Blind Spot Discovery（未監控盲區掃描）

掃描叢集內的 Prometheus Targets / ServiceMonitor，交叉比對 `conf.d/` 租戶配置，找出「叢集內存在但未被本平台納管的實例」。與 `analyze_rule_pack_gaps.py`（分析 custom rule 對 Rule Pack 的覆蓋率）互補——Blind Spot 分析的是 infrastructure 對 tenant config 的覆蓋率。產出範例：「掃描發現叢集內有 5 個 PostgreSQL 實例，但只有 2 個在 tenant 配置中啟用了閾值監控」。

#### 11.6 Directory-level Config Diff（配置目錄級差異比對）

對比兩個 `conf.d/` 目錄的完整差異，產出 Markdown 報告列出所有租戶的變更爆炸半徑（blast radius）。適合 GitOps PR review 場景——審閱者一眼看出整批變更涉及哪些 tenant、哪些 metric、變嚴或變鬆。與現有 `patch_config.py --diff`（單一 metric 的 live ConfigMap preview）互補。
...
#### 11.8 Log-to-Metric Bridge（日誌轉指標橋接）
...
**文件版本：** v1.9.0 — 2026-03-07
**最後更新：** v1.9.0 — 遷移全鏈自動化（Onboard→Scaffold pipeline、Auto-Convergence、Batch Health Report、Gap Analysis）、Config Diff Preview、PR Backtest Bot、da-tools CLI 11→16 命令
