...
```bash
# 單筆 query 比對
docker run --rm --network=host ghcr.io/vencil/da-tools:v2.6.0 \
...
# 6. 租戶健康總檢
docker run --rm --network=host ghcr.io/vencil/da-tools:v2.6.0 diagnose db-a
```
...
docker run --rm -v $(pwd)/conf.d:/data/conf.d ghcr.io/vencil/da-tools:v2.6.0 \
