...
`vibe-subagent-review` 決定一輪怎麼審；本 skill 決定輪與輪之間傳什麼、什麼時候該停、什麼時候該換題目。規則的推導、量測與已打死的版本在 [`references/derivation.md`](references/derivation.md)；工具守不到的邊界在 [`references/boundaries.md`](references/boundaries.md)。
