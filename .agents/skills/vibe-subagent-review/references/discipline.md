# Review 紀律（從 SKILL.md 搬出，逐字；不自動載入）

> 主檔只留路由表、三張 checklist、收 review 的處置表與長時 agent 的入口。本檔是「怎麼報、怎麼驗、怎麼派、什麼時候該砍鷹架」，不確定時再讀。
...
以下三條是**內容判準**，留在 reviewer 端是官方明文支持的：
...
## verify-before-asserting（review finding 是一個 claim）
...
- ⚠️ **第 2 輪起，只有「自審 pass 算完整驗證」這一條不算數**——降級為 pre-check，改走〈預設檔位〉的換 context 盲審。本節其餘各條**照常適用，且對盲審一樣適用**。

## 收 review：補充

- **禁止表演性同意。** 不寫「你說得對」「好建議」再開始查。要嘛先驗完再回，要嘛直接動手讓 diff 說話。
- **範圍紀律：finding 指向既有內容時，不要在機械性 PR 裡夾帶政策變更。** 搬檔案的 PR 中 diff 顯示為「新增」是搬移的假象。這類 finding 的正解是**開自己的票、自己的受審主體**。⚠️ 但要**明說缺口是實的**，否則「範圍外」會退化成「不修的藉口」。
- 實測（#1481）：CodeRabbit 開 14 條，逐條驗證後 **11 條成立、1 條 reject（附雙模式解析實測，reviewer 自行撤回並記為 learning）、3 條交 owner 判為範圍外**。若照單全收，那 1 條會讓一支能跑的 workflow 被改壞。
- 實測（#1481）：11 條修完並推上去後，GitHub 顯示那些 thread 為 `is_outdated: true`，據此宣稱「14 條全部 resolved」——**而 API 上 `is_resolved` 全是 `false`**。分支保護的「Require conversation resolution」因此持續擋著 merge。`reject` 沒有 code fix，所以最容易被漏掉，而未 resolve 的 rejected thread 擋 merge 的力道與其他兩種**完全一樣**。只 resolve 你**真的處置過**的。
...
| 情境 | 預設 | 理由 |
|---|---|---|
| **第 1 輪**（新實作 / 例行 multi-file PR） | finder≠verifier **自審**足夠 | 便宜、涵蓋大多數情況；不為例行 review gold-plate |
| **第 2 輪起**（修法、re-fix、對同一 issue 再改） | ⛔ 自審**降為 pre-check**，預設改為**換 context 的盲審** | 實測：#1457 前三顆 commit「合計 8 位、59+ 條 finding、61 個單點變異，**作者自審 0 條**」。同一個 context 已經對自己的修法失明 |

「換 context」不必然等於多 agent harness：另開一個**不帶本輪對話**的 reviewer（新 session / 新 subagent / 另一個模型皆可），只餵**受審 diff 本身**與 [`vibe-converge`](../../vibe-converge/SKILL.md) 的跨輪交接三件組。⛔ **不要餵上一輪的完整 commit body 與修法敘事**——那正是讓下一棒繼承上一棒盲區的東西。

📋 可照抄的 prompt 骨架：[`scoped-re-review.md`](scoped-re-review.md)。⛔ **骨架裡承重的那一格在收件端不在 prompt 端**：reviewer 照報所有東西，但落在修法 diff 之外的寫進〈範圍外觀察〉，**不擋本輪、不開下一輪**，收下時記成 `kind=question`。⚠️ 這一格在本 repo **從未存在過**，效果**未量**：第一次用完把「本輪幾條 / 其中幾條落進範圍外」記進帳本。

⛔ **輪數上限 5，而且停止條件不是「審到零 finding」**——理由見 vibe-converge 與其 `references/derivation.md` §4.1。上限是**預算不是判準**。

> ⚠️ **本節整體有一個未解的前提（2026-08-26）。** 當前模型的官方 prompting 指引逐字說：`Claude Opus 5 verifies its own work without being told to. If your prompt contains explicit verification instructions … remove them: instructions like these cause over-verification`，並且 `The same applies to legacy harness scaffolding that adds separate verification steps`；其建議的 delegation 條款含 `do not use subagents to verify or double-check your own work`。
> 本節就是那種鷹架。與它相反的本 repo 實證（#1457「8 位盲審 59+ 條，作者自審 0 條」）是在**更早的模型世代**上量的。
> ⇒ **兩邊都沒有在當前模型上被量過。** 關掉它的方式是側量：同一支 PR、同一顆 SHA，自審一次並記下 finding 集合，再派**一位**盲審審同一顆 SHA，比兩個集合的差集。

## 升級到多 agent harness（大 / 高風險 review；defer-with-trigger）

改動**很多檔 / 高 blast-radius IaC / 跨 component**、或 stakes 值得時，reuse [`vibe-security-audit`](../../vibe-security-audit/SKILL.md) 的 Workflow pattern，只換 lens：
...
- **模型分層**：強模型找、便宜模型驗（見 security-audit 的 `audit-workflow.js`）。

⚠️ **不要對例行 multi-file PR 起這個**——它跟 security-audit 一樣貴。

## Go `Close()` 讀/寫不對稱（review 必查，errcheck 分不出）

`defer func(){ _ = x.Close() }()` 只對 **read-closer** 安全（`resp.Body` / `sql.Rows` / `os.Open` 唯讀檔）。**write-closer**（`os.Create` / `gzip.Writer` / 自訂 `io.WriteCloser`）的 `Close()` error **不可吞**——disk-flush 常延到 `Close()` 才發生，吞掉 = silent data loss。自訂介面（內嵌 `io.WriteCloser`）review 缺全域 context 判不出讀/寫，易把 `_ = store.Close()` 誤當資源釋放放行。**正規防禦 = named return + defer 捕捉**；`defer` 在 early return 時把 `Close()` 的 error 併進具名回傳值，panic 展開時它一樣會執行但**不會 `recover()`**。
...
1. **Workflow-first**：多階段 verify / review（≥2 個里程碑、或預估 >15 分鐘）一律用 `Workflow` 工具編排，**不用單一長時背景 `Agent`**——`phase()` / `log()` 原生串流到 `/workflows` live view，且天然把工作拆成多個短 staged agent。`vibe-security-audit` 的 `audit-workflow.js` 即此 pattern。
2. **raw `Agent` 例外 → 強制 progress ledger**：確有理由用單一背景 `Agent` 時，spawn prompt **必須**內嵌下方 ledger 契約——agent 每過一個里程碑就 append 一行到 `dev/<scope>/PROGRESS.jsonl`。parent 之後 cheap-poll 這個小檔即可（`make agent-progress`）。parent 看到 `blocked` 或連續 `fail` 時**主動介入**，不陪它燒完。
3. **單 agent 範圍上限 ~15 分鐘**：預估超過就拆成 staged agents——每段收在一個 checkpoint、前段結論以文字餵給下段（`PROGRESS.jsonl` 就是天然的交接摘要）。
...
- 觀測：`make agent-progress SCOPE=dev/<scope>`（`N=10` 調 tail 行數）——列出 SCOPE 下所有 `PROGRESS.jsonl` 尾端，並對 >15 分鐘未更新的 ledger 印 LIVENESS 警告。
- ⛔ tail agent 的 `.output` transcript（全量 JSONL 撐爆 parent context）。
