...
| 5 | **迴圈的產出打在鷹架上，不在交付物上** | 最近 60 顆 first-parent commit：self-serving（dx/lint/ci/docs/test）**47** vs product **13**（conventional-commit type/scope，0 unparsed）；最近 15 顆的 churn：出貨碼 **18.5%**／測試鷹架 **51.6%**／機具 **24.8%**；2026-08 議題開 **171** 關 **47**（淨 +124），其中仍 OPEN 的 133 張有 **≥59%** 可追溯到某支 PR 或自陳續辦（兩把結構不同的鍵取聯集，交集僅 25 ⇒ 59% 是下界） | 量在 `bf16d303`（2026-08-25） | 停止規則 **ORACLE-MISSING** ＋ **ROUND-CAP** |
...
| ROUND-CAP | **5** 輪 | obra/superpowers v6.2.0 在同一個不收斂問題上裝的 five-round circuit breaker；repair-loop 綜述把多數可得增益放在第 1–4 輪（arXiv:2607.05197） | **兩個來源都沒有精確釘住界線，本 repo 沒有量過**。5 是兩者中較寬鬆的那個——刻意選寬，因為選窄的代價（漏報）比選寬的代價（多一輪）難察覺 |
