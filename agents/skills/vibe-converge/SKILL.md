...
1. **CONVERGED** — 連續 2 輪各 0 條新 `verified` finding ⇒ 停。不要為了「再確認一次」開第三輪。
   ⚠️ **工具什麼都不印，不等於沒收斂。** 鏈最常見的結尾是：某輪修完 → 下一輪審那個修法、0 條。這時 CONVERGED 不會響（它要**連兩輪** 0 條），BLOCKING 也不會響。**這是正常的**，你自己判定收斂即可。
   **第二次**遇到同樣的沉默就開票：一次是巧合，兩次代表規則該修。（修法：`subject` 加一個 `reviews_round` 欄位，讓工具知道本輪審的是哪一輪的修法。）
